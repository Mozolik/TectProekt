...
Процедура ПеренестиДень(Команда)
	...
	ПараметрыВыбораДаты.Заголовок = НСтр("ru='Выбор даты переноса';uk='Вибір дати перенесення'");
	
	ТекстСообщения = НСтр("ru='Выберите дату, на которую будет осуществлен перенос дня %1 (%2)';uk='Виберіть дату, на яку буде здійснено перенесення дня %1 (%2)'");
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru='Данные производственного календаря еще не записаны.
                  ...
                  |
                  |Записать?'
                  |;uk='Дані виробничого календаря ще не записані.
                  |Друк можливий тільки після запису даних.
                  |
                  |Записати?'"),
...
Процедура ЗаполнитьПредставлениеПереносов(Форма)
	...
	Для Каждого КлючИЗначение Из Форма.ПереносыДней Цикл
		...
		Форма.СписокПереносов.Добавить(ДатаИсточник, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 (%3) перенесен на %2 (%4)';uk='%1 (%3) перенесений на %2 (%4)'"), Формат(ДатаИсточник, "ДФ='д ММММ'"), Формат(ДатаПриемник, "ДФ='д ММММ'"), ВидДняИсточник, ВидДняПриемник));
	КонецЦикла;
