...
Процедура Далее(Команда)
	...
	Если Элементы.Сертификаты.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выделите сертификаты, которые требуется добавить.';uk='Виділіть сертифікати, які потрібно додати.'"));
		Возврат;
	КонецЕсли;
...
Процедура ДалееПослеПоискаСертификата(Результат, Неопределен) Экспорт
	...
	Если Результат.Свойство("СертификатНеНайден") Тогда
		Контекст.Вставить("ОписаниеОшибки", НСтр("ru='Сертификат не найден на компьютере (возможно удален).';uk='Сертифікат не знайдений на комп''ютері (можливо вилучений).'"));
	Иначе
...
Процедура ПоказатьСтраницуУточнениеСвойствСертификата(Форма, СертификатКриптографии, ДанныеСертификата)
	...
	Строка = ?(ЗначениеЗаполнено(Форма.Сертификат), НСтр("ru='Обновить';uk='Оновити'"), НСтр("ru='Добавить';uk='Додати'"));
