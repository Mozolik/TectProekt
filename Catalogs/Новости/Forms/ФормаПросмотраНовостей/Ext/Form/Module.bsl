...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПолучитьТекущегоПользователя = Истина Тогда
		ЭтотОбъект.ПараметрыСеанса_ТекущийПользователь = ПараметрыСеанса.ТекущийПользователь;
	Иначе
...
Процедура НовостиПередУдалением(Элемент, Отказ)
	...
	Если (ЭтотОбъект.ТекущаяСтрокаПапки_ВариантОтбора = 0) // Все новости
			...
			ИЛИ (ЭтотОбъект.ТекущаяСтрокаПапки_ВариантОтбора = 7) // Пользовательские отборы.
			Тогда
...
Процедура КомандаПоиск(Команда)
	...
	Для каждого ТекущаяПапка Из ЭтотОбъект.Папки.ПолучитьЭлементы() Цикл
		...
		Если ТекущаяПапка.ВариантОтбора = 2 Тогда // Результаты поиска
			...
			Если ПустаяСтрока(ЭтотОбъект.СтрокаПоиска) Тогда
				ТекущаяПапка.Представление = НСтр("ru='Результаты поиска';uk='Результати пошуку'");
			Иначе
				ТекущаяПапка.Представление = НСтр("ru='Результаты поиска';uk='Результати пошуку'") + " (" + СокрЛП(ЭтотОбъект.СтрокаПоиска) + ")";
			КонецЕсли;
...
Процедура ОбновитьИнформационныеСтрокиКлиент()
	...
	лкТекущаяДата = ТекущаяДата(); // Текущая дата клиентского компьютера.

	// 1. Актуальность информации.
	лкИнфоАктуальностьИнформации = "" + Формат(лкТекущаяДата, "ДФ='dd.MM.yyyy HH:mm:ss'");
	ЭтотОбъект.ИнфоАктуальностьИнформации = лкИнфоАктуальностьИнформации; // Текст

	ТекстСообщения = НСтр("ru='Последнее обновление списка новостей было
        |%лкИнфоАктуальностьИнформации%
        |Следующее обновление списка новостей произойдет
        |%Следующее обновление списка новостей%
        |%ЭтотОбъект.ДатыПоследнихЗагрузокНовостей%'
        |;uk='Останнє оновлення списку новин було
        |%лкИнфоАктуальностьИнформации%
        |Наступне оновлення списку новин відбудеться
        |%Следующее обновление списка новостей%
        |%ЭтотОбъект.ДатыПоследнихЗагрузокНовостей%'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%лкИнфоАктуальностьИнформации%", лкИнфоАктуальностьИнформации);
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Следующее обновление списка новостей%", Формат(лкТекущаяДата + (ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления * 60), "ДФ='dd.MM.yyyy HH:mm:ss'"));
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЭтотОбъект.ДатыПоследнихЗагрузокНовостей%", ЭтотОбъект.ДатыПоследнихЗагрузокНовостей);
...
Процедура ОбновитьИнформационныеСтрокиСервер()
	...
	Запрос.Текст = "
		...
		|	ЕСТЬNULL(Рег.ДатаЗагрузкиПоследнихНовостей, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаПоследнейЗагрузки
		|ИЗ
		|	ВТ_ПоследняяНовость
		|	ЛЕВОЕ СОЕДИНЕНИЕ
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			ТекстСообщения = НСтр("ru='%Выборка.ЛентаНовостейНаименование%, последняя новость от %Выборка.ДатаПоследнейНовости%, последняя загрузка %Выборка.ДатаПоследнейЗагрузки%';uk='%Выборка.ЛентаНовостейНаименование%, остання новина від %Выборка.ДатаПоследнейНовости%, останнє завантаження %Выборка.ДатаПоследнейЗагрузки%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Выборка.ЛентаНовостейНаименование%", Выборка.ЛентаНовостейНаименование);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Выборка.ДатаПоследнейНовости%", Формат(МестноеВремя(Выборка.ДатаПоследнейНовости), "ДФ='dd.MM.yyyy HH:mm:ss'"));
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Выборка.ДатаПоследнейЗагрузки%", Формат(МестноеВремя(Выборка.ДатаПоследнейЗагрузки), "ДФ='dd.MM.yyyy HH:mm:ss'"));
	...
	Если СтрДлина(ЭтотОбъект.ДатыПоследнихЗагрузокНовостей) > 0 Тогда
		ЭтотОбъект.ДатыПоследнихЗагрузокНовостей = Символы.ПС + "Состояние загрузки новостей: " + ЭтотОбъект.ДатыПоследнихЗагрузокНовостей;
	КонецЕсли;

	// 3. Актуальность информации.
	ЭтотОбъект.ИнфоАктуальностьИнформации = "" + Формат(лкТекущаяДата, "ДФ='dd.MM.yyyy HH:mm:ss'"); // Текст
	...
	ТекстСообщения = НСтр("ru='Последнее обновление списка новостей было
        ...
        |%Следующее обновление списка новостей%
        |%ЭтотОбъект.ДатыПоследнихЗагрузокНовостей%'");
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЭтотОбъект.ИнфоАктуальностьИнформации%", ЭтотОбъект.ИнфоАктуальностьИнформации);
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Следующее обновление списка новостей%", Формат(лкТекущаяДата + (ЭтотОбъект.СписокНовостей_ИнтервалАвтообновления * 60), "ДФ='dd.MM.yyyy HH:mm:ss'"));
	ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ЭтотОбъект.ДатыПоследнихЗагрузокНовостей%", ЭтотОбъект.ДатыПоследнихЗагрузокНовостей);
	Элементы.ИнфоАктуальностьИнформации.Подсказка = ТекстСообщения;
...
Процедура ОбновитьСписокПапокОтбора()
	...
	НоваяСтрока.Наименование           = НСтр("ru='Все новости';uk='Всі новини'");
	...
	Запрос.Текст = "
		...
		|		ИЛИ Спр.ДатаЗавершения = ДАТАВРЕМЯ(1, 1, 1)
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	Если ПустаяСтрока(ЭтотОбъект.СтрокаПоиска) Тогда
		НоваяСтрока.Наименование = "Результаты поиска";
	Иначе
		НоваяСтрока.Наименование = "Результаты поиска (" + СокрЛП(ЭтотОбъект.СтрокаПоиска) + ")";
	КонецЕсли;
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	НоваяСтрока.Наименование           = НСтр("ru='Помеченные новости';uk='Позначені новини'");
	НоваяСтрока.КоличествоНепрочтенных = 0; //// ... Всегда = 0, т.к. расчет количества непрочтенных для этой ситуации будет ресурсоемким
	НоваяСтрока.ВыделеннаяСтрока       = Ложь;
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	НоваяСтрока.Наименование           = НСтр("ru='Важные новости';uk='Важливі новини'");
	НоваяСтрока.КоличествоНепрочтенных = 0; //// ... Всегда = 0, т.к. расчет количества непрочтенных для этой ситуации будет ресурсоемким
	НоваяСтрока.ВыделеннаяСтрока       = Ложь;
	...
	НоваяСтрока.ЗначениеОтбора         = Неопределено;
	НоваяСтрока.Наименование           = НСтр("ru='Корзина';uk='Кошик'");
...
Процедура УстановитьУсловноеОформление()
	...
	// 7. Представление даты: этот год.
		...
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Новости.ПредставлениеДатыПубликации");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ОтборЭлемента.ПравоеЗначение = "Этот год";
...
Процедура ПослеПоиска(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = ТипСтруктура Тогда
		...
		Для каждого ТекущаяПапка Из ЭтотОбъект.Папки.ПолучитьЭлементы() Цикл
			...
			Если ТекущаяПапка.ВариантОтбора = 2 Тогда // Результаты поиска
				// Изменить название в строке.
				ТекущаяПапка.Представление = "Результаты поиска (" + СокрЛП(ЭтотОбъект.СтрокаПоиска) + ")";
				// Должно сработать событие "ПриАктивизацииСтроки" для элемента "Папки", где и произойдет отбор.
				// Но вначале необходимо "сбросить" значение последней активной строки.
				ЭтотОбъект.ТекущаяСтрокаПапки_ВариантОтбора  = 0;
				ЭтотОбъект.ТекущаяСтрокаПапки_ЗначениеОтбора = Неопределено;
