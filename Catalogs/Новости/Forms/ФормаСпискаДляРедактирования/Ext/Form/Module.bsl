...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
			...
			лкВариантСпискаЛентНовостей = "Только для администратора";
		Иначе
	...
	Иначе
		...
		лкВариантСпискаЛентНовостей = "Для администратора или редактора новостей";
	КонецЕсли;
	...
	Если лкВариантСпискаЛентНовостей = "Только для администратора" Тогда
		Если (ПараметрыТекущегоПользователя.ЕстьРольПолныеПрава = Истина)
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли лкВариантСпискаЛентНовостей = "Для администратора или редактора новостей" Тогда
		Если (ПараметрыТекущегоПользователя.ЕстьРольПолныеПрава = Истина)
				И (ПараметрыТекущегоПользователя.ЕстьРольАдминистраторСистемы = Истина) Тогда
			Запрос = Новый Запрос;
			Запрос.Текст = "
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ЭтаФорма.РазрешенныеДляРедактированияЛентыНовостей.Количество()=0 Тогда
		ТекстСообщения = НСтр("ru='Нельзя добавлять новости, если нет ни одной локальной ленты новостей, где разрешено редактирование.
            ...
            |;uk='Не можна додавати новини, якщо немає ні однієї локальної стрічки новин, де дозволено редагування.
            |Користувачеві з ролями АдминистраторСистемы і ПолныеПрава можна редагувати всі локальні стрічки новин.
            |Користувачеві з роллю РедактированиеНовостей можна редагувати дозволені Адміністратором локальні стрічки новин.'");
...
Процедура СписокПередНачаломИзменения(Элемент, Отказ)
	...
	Если ЭтаФорма.РазрешенныеДляРедактированияЛентыНовостей.НайтиПоЗначению(Элемент.ТекущиеДанные.ЛентаНовостей) = Неопределено Тогда
		ТекстСообщения = НСтр("ru='Можно изменять новости только в локальной ленте новостей, где разрешено редактирование.
            |Пользователю с ролями АдминистраторСистемы и ПолныеПрава можно редактировать все локальные ленты новостей.
            |Пользователю с ролью РедактированиеНовостей можно редактировать разрешенные Администратором локальные ленты новостей.'
            |;uk='Можна змінювати новини тільки в локальній стрічці новин, де дозволено редагування.
            |Користувачеві з ролями АдминистраторСистемы і ПолныеПрава можна редагувати всі локальні стрічки новин.
            |Користувачеві з роллю РедактированиеНовостей можна редагувати дозволені Адміністратором локальні стрічки новин.'");
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = ТекстСообщения;
