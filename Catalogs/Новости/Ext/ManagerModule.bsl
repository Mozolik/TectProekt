...
Функция ПолучитьХТМЛТекстНовостей(лкНовости, ПараметрыОтображения = Неопределено) Экспорт
	...
	ШаблонНовостиХТМЛ = "
		...
		|				%ДатаПубликации%&nbsp;%ИнформацияОВажности%
		|			</div>
...
Процедура ПолучитьСписокНовостей(
	...
	ШаблонТекстаЗапроса = "
		...
		|				ИЛИ Спр.Ссылка.ДатаЗавершения = ДАТАВРЕМЯ(1, 1, 1)) // Актуальна
	...
	Если СтрДлина(ТекстВременныхТаблиц) > 0 Тогда
		...
		Запрос.УстановитьПараметр("ПустаяДата", '00010101000000'); // ДАТАВРЕМЯ(1, 1, 1)
...
Процедура ЗагрузитьСтандартныеЗначения() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
			...
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				...
				НСтр("ru='Новости. Сервис и регламент. Загрузка стандартных значений. Новости. Ошибка';uk='Новини. Сервіс і регламент. Завантаження стандартних значень. Новини. Помилка'") , // ИдентификаторШага
	...
	Если ПравоДоступа("Изменение", Метаданные.Справочники.Новости) = Истина
		...
		Иначе
			ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
				НСтр("ru='Новости. Сервис и регламент. Загрузка стандартных значений. Новости. Нет лент новостей';uk='Новини. Сервіс і регламент. Завантаження стандартних значень. Новини. Немає стрічок новин'"), // ИдентификаторШага
				УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
				Метаданные.Справочники.Новости, // ОбъектМетаданных
				, // Данные
				НСтр("ru='Ошибка загрузки стандартных значений новостей из макетов. Справочник лент новостей пустой.';uk='Помилка завантаження стандартних значень новин з макетів. Довідник стрічок новин порожній.'") // Комментарий
			);
				...
				Если Найти(ВРег(ТекущийМакет.Имя), ВРег("СтандартныеЗначения_")) = 1 Тогда
					...
					Иначе
						ТекстСообщения = НСтр("ru='Ошибка загрузки стандартных значений новостей из макета %ТекущийМакет.Имя%.
                            ...
                            |;uk='Помилка завантаження стандартних значень новин з макета %ТекущийМакет.Имя%.
                            |Не знайдена стрічка новин з таким ідентифікатором.'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ТекущийМакет.Имя%", ТекущийМакет.Имя);
						ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
							НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
							НСтр("ru='Новости. Сервис и регламент. Загрузка стандартных значений. Новости. Неопределенная лента новостей';uk='Новини. Сервіс і регламент. Завантаження стандартних значень. Новини. Невизначена стрічка новин'"), // ИдентификаторШага
							УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
							Метаданные.Справочники.Новости, // ОбъектМетаданных
							, // Данные
							ТекстСообщения // Комментарий
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Ошибка загрузки стандартных значений новостей из макетов.
            ...
            |;uk='Помилка завантаження стандартних значень новин з макетів.
            |У поточного користувача (ім''я ІБ: %ТекущийПользователь%) немає прав на редагування довідника новин.'");
		...
		ОбработкаНовостей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Сервис и регламент';uk='БІП:Новини.Сервіс та регламент'"), // ИмяСобытия
			НСтр("ru='Новости. Сервис и регламент. Загрузка стандартных значений. Новости. Нарушение прав доступа';uk='Новини. Сервіс і регламент. Завантаження стандартних значень. Новини. Порушення прав доступу'"), // ИдентификаторШага
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			Метаданные.Справочники.Новости, // ОбъектМетаданных
			, // Данные
			ТекстСообщения // Комментарий
