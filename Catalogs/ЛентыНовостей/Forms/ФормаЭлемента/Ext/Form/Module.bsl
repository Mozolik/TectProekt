...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Иначе
			ЭтаФорма.ТолькоПросмотр = Истина;
		КонецЕсли;
		Элементы.ЗаголовокДоступныеКатегорииНовостей.Заголовок = НСтр("ru='Настройки отборов по этой ленте новостей (действуют на все области данных):';uk='Настройки відборів по цій стрічці новин (діють на всі області даних):'");
	Иначе
		Если (ПравоДоступа("Изменение", Метаданные.Справочники.ЛентыНовостей) = Истина) Тогда
			ЭтаФорма.ТолькоПросмотр = Ложь;
		Иначе
			ЭтаФорма.ТолькоПросмотр = Истина;
		КонецЕсли;
		Элементы.ЗаголовокДоступныеКатегорииНовостей.Заголовок = НСтр("ru='Настройки отборов по этой ленте новостей:';uk='Настройки відборів по цій стрічці новин:'");
	КонецЕсли;
	...
	Иначе
		...
		Если Объект.ЗагруженоССервера = Ложь Тогда
			...
			Для Каждого ТекущийРедактор Из МассивРедакторовЛентыНовостей Цикл
				...
				НоваяСтрока.ПользовательПредставление = СтрЗаменить(НоваяСтрока.ПользовательПредставление, НСтр("ru='(полные права)';uk='(повні права)'"), НСтр("ru='(полные права, может редактировать новости во всех лентах новостей)';uk='(повні права, може редагувати новини у всіх стрічках новин)'"));
			КонецЦикла;
	...
	Если (Объект.ЗагруженоССервера = Истина) Тогда
		...
		Иначе
			Элементы.ГруппаФайлНовостей.Заголовок = НСтр("ru='Получение файла новостей (эти поля перезаполняются автоматически при изменениях на сервере 1С)';uk='Отримання файлу новин (ці поля перезаполняются автоматично при змінах на сервері 1С)'");
		КонецЕсли;
...
Процедура ИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ВРег(Объект.Протокол) = ВРег("file") Тогда
		...
		Диалог.Заголовок = НСтр("ru='Выберите файл новостей в формате Atom или RSS';uk='Виберіть файл новин у форматі Atom або RSS'");
...
Процедура ДоступныеКатегорииНовостейПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		...
		Сообщение.Текст = НСтр("ru='Нельзя добавлять категории для ленты новостей, загруженной с сервера.';uk='Не можна додавати категорії для стрічки новин, завантаженої з сервера.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура ДоступныеКатегорииНовостейПередУдалением(Элемент, Отказ)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Нельзя удалять категории для ленты новостей, загруженной с сервера.';uk='Не можна видаляти категорії для стрічки новин, завантаженої з сервера.'");
		Сообщение.Сообщить();
	КонецЕсли;
...
Процедура КомандаПодборКатегорий(Команда)
	...
	Если Объект.ЗагруженоССервера = Истина Тогда
		Отказ = Истина;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Нельзя изменять категории для ленты новостей, загруженной с сервера.';uk='Не можна змінювати категорії для стрічки новин, завантаженої з сервера.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьИнформационныеСтроки()
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок   = НСтр("ru='без исключений. В модели сервиса нельзя настроить видимость ленты новостей по пользователям.';uk='без винятків. У моделі сервісу можна налаштувати видимість стрічки новин по користувачам.'");
		Элементы.ДекорацияВидимостьПоУмолчанию.Гиперссылка = Ложь;
	Иначе
		Элементы.ДекорацияВидимостьПоУмолчанию.Гиперссылка = Истина;
		Если Объект.ИсключенияВидимости.Количество() = 0 Тогда
			Если Объект.ВидимостьПоУмолчанию = Истина Тогда
				Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок = НСтр("ru='без исключений';uk='без винятків'");
			Иначе
				Элементы.ДекорацияВидимостьПоУмолчанию.Заголовок = НСтр("ru='без исключений. Никто не сможет выбрать эту ленту новостей для получения новостей.';uk='без винятків. Ніхто не зможе вибрати цю стрічку новин для отримання новин.'");
		...
		Иначе // > 3
			...
			СписокИсключений = СтрЗаменить(СписокИсключений, "##", ", ");
			СписокИсключений = СтрЗаменить(СписокИсключений, "#", "");
			ТекстСообщения = НСтр("ru='%СписокИсключений% и еще %Объект.ИсключенияВидимости.Количество()-2% пользователей';uk='%СписокИсключений% а також %Объект.ИсключенияВидимости.Количество()-2% користувачів'");
	...
	Если Объект.Ссылка.Пустая() Тогда
		Элементы.ДекорацияСтатистика.Заголовок = НСтр("ru='Необходимо вначале записать ленту новостей, чтобы получить по ней статистику.';uk='Необхідно спочатку записати стрічку новин, щоб отримати за неї статистику.'");
	Иначе
			...
			Иначе
				...
				Если КоличествоНовостейПомеченныхНаУдаление = 0 Тогда
					ТекстСтатистики = ТекстСтатистики + " " + НСтр("ru='и среди них нет ни одной помеченной на удаление
                        |'
                        |;uk='і серед них немає жодної позначені на видалення
                        |'");
				ИначеЕсли КоличествоНовостейПомеченныхНаУдаление = КоличествоНовостейВсего Тогда // Все помечены на удаление
					ТекстСтатистики = ТекстСтатистики + ", " + НСтр("ru='все они помечены на удаление (а значит не видны в списке новостей)
                        |'
                        |;uk='всі вони позначені на видалення (а значить не видно у списку новин)
                        |'");
				Иначе
					ТекстСтатистики = СтрШаблон(
						НСтр("ru=', из них помечено на удаление %1
                            |'
                            |;uk=', з них відмічених для вилучення %1
                            |'"),
		...
		Иначе
			Если КоличествоНовостейВсего = 0 Тогда
				ТекстСтатистики = НСтр("ru='Нет ни одной загруженной новости
                    |'
                    |;uk='Немає жодної завантаженої новини
                    |'");
			Иначе
				ТекстСтатистики = СтрШаблон(
					НСтр("ru='Всего новостей в этой ленте новостей: %1';uk='Всього новин у цій стрічці новин: %1'"),
					КоличествоНовостейВсего);
				Если КоличествоНовостейПомеченныхНаУдаление = 0 Тогда
					ТекстСтатистики = ТекстСтатистики + " " + НСтр("ru='и среди них нет ни одной помеченной на удаление
                        |'
			...
			Если ДатаПоследнейУспешнойЗагрузки = '00010101' Тогда
				Если КоличествоНеудачПриЗагрузкеНовостей = 0 Тогда
					ТекстСтатистики = ТекстСтатистики + НСтр("ru='Новости по этой ленте новостей еще не загружались';uk='Новини по цій стрічці новин ще не завантажувалися'");
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтрШаблон(
							НСтр("ru='Новости по этой ленте новостей еще ни разу не были успешно загружены (было %1 неудачных попыток загрузки новостей)';uk='Новини по цій стрічці новин ще жодного разу не були успішно завантажені (%1 невдалих спроб завантаження новин)'"),
			...
			Иначе
				Если КоличествоНеудачПриЗагрузкеНовостей = 0 Тогда
					ТекстСтатистики = ТекстСтатистики
						+ СтрШаблон(
							НСтр("ru='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени)';uk='Новини по цій стрічці новин останній раз були успішно завантажені %1 (за місцевим часом)'"),
							Формат(МестноеВремя(ДатаПоследнейУспешнойЗагрузки), "ДФ='dd.MM.yyyy HH:mm:ss'"));
				Иначе
					ТекстСтатистики = ТекстСтатистики
						+ СтрШаблон(
							НСтр("ru='Новости по этой ленте новостей последний раз были успешно загружены %1 (по местному времени), после этого было %2 неудачных попыток загрузки новостей.';uk='Новини по цій стрічці новин останній раз були успішно завантажені %1 (за місцевим часом), після цього було %2 невдалих спроб завантаження новин.'"),
							Формат(МестноеВремя(ДатаПоследнейУспешнойЗагрузки), "ДФ='dd.MM.yyyy HH:mm:ss'"),
							КоличествоНеудачПриЗагрузкеНовостей);
				КонецЕсли;
			КонецЕсли;
...
Функция СформироватьПолныйАдресЛентыНовостей(лкСсылка, лкПротокол, лкСайт, лкИмяФайла)
	...
	Если Найти(ВРег(лкПротокол), ВРег("http")) = 1 Тогда // http, https
		Если ПустаяСтрока(лкСайт) Тогда
			Результат = НСтр("ru='Не заполнено поле Сайт';uk='Не заповнено поле Сайт'");
		ИначеЕсли ПустаяСтрока(лкИмяФайла) Тогда
			Результат = НСтр("ru='Не заполнено поле Имя файла';uk='Не заповнено поле Ім''я файлу'");
		...
		КонецЕсли;
	Иначе
		Результат = НСтр("ru='Адрес ленты новостей можно получить только для протоколов http и https';uk='Адресу стрічки новин можна отримати тільки для протоколів http та https'");
	КонецЕсли;
