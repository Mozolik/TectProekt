	...
	Для каждого ТекущаяСтрока Из ТаблицаПроверки Цикл
		Если ТекущаяСтрока.КоличествоСтрок > 1 Тогда
			...
			Сообщение.Текст = НСтр("ru='Категория [%КатегорияНовостей%] повторяется несколько раз.
                ...
                |;uk='Категорія [%КатегорияНовостей%] повторюється кілька разів.
                |Кожну категорію дозволено вводити тільки один раз.'");
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтотОбъект.ЗагруженоССервера = Истина Тогда
		...
		Если (ЭтотОбъект.ПометкаУдаления = Истина)
			...
			Иначе
				ТекстСообщения = НСтр("ru='Нельзя помечать на удаление ленты новостей, загруженные автоматически с сервера новостей.';uk='Не можна позначати на видалення стрічки новин, автоматично завантажені з сервера новин.'");
				Сообщение = Новый СообщениеПользователю();
