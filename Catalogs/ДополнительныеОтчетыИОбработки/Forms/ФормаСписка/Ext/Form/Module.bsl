...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокВыбора.Добавить(1, НСтр("ru='Только отчеты';uk='Тільки звіти'"));
	СписокВыбора.Добавить(2, НСтр("ru='Только обработки';uk='Тільки обробки'"));
...
Функция ВыбранЭлемент(ДанныеСтроки)
	Если ТипЗнч(ДанныеСтроки.Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
		ПоказатьПредупреждение(, НСтр("ru='Команда не может быть выполнена для указанного объекта.
            ...
            |;uk='Команда не може бути виконана для зазначеного об''єкта.
            |Виберіть додатковий звіт або обробку.'"));
		Возврат Ложь;
	КонецЕсли;
	Если ДанныеСтроки.ЭтоГруппа Тогда
		ПоказатьПредупреждение(, НСтр("ru='Команда не может быть выполнена для группы.
            |Выберите дополнительный отчет или обработку.'
            |;uk='Команда не може бути виконана для групи.
            |Виберіть додатковий звіт або обробку.'"));
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
...
Функция ИзменениеПубликации(ВариантПубликации)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(РезультатВыполнения,
			НСтр("ru='Не выбран ни один дополнительный отчет (обработка)';uk='Не обраний жоден додатковий звіт (обробка)'"));
	...
	Если ВыделенныеСтроки.Количество() = 1 Тогда
		...
		ТекстСообщения = НСтр("ru='Изменена публикация дополнительного отчета (обработки) ""%1""';uk='Змінена публікація додаткового звіту (обробки) ""%1""'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ИмяОбъекта);
	Иначе
		ТекстСообщения = НСтр("ru='Изменена публикация у %1 дополнительных отчетов (обработок)';uk='Змінена публікація у %1 додаткових звітів (обробок)'");
	...
	СтандартныеПодсистемыКлиентСервер.ВывестиОповещение(РезультатВыполнения,
		НСтр("ru='Изменена публикация';uk='Змінена публікація'"),
		ТекстСообщения);
