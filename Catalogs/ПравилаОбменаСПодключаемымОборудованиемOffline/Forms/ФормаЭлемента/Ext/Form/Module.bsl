...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    НоменклатураКлиентСервер.ПроверитьЗаполнитьПустуюУпаковку(Объект, "ЕдиницаИзмеренияВеса");
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если КоличествоТоваров > 0 Тогда
		Если Товары[КоличествоТоваров-1].Код - 99999 > ТекущийОбъект.МаксимальныйКод И ТекущийОбъект.МаксимальныйКод <> 0 Тогда
			...
			НСтр("ru='В табличной части ""Товары"" слишком много товаров. Измените максимальное количество товаров или уменьшите количество товаров к выгрузке при помощи отбора.';uk='У табличній частині ""Товари"" занадто багато товарів. Змініть максимальну кількість товарів або зменшіть кількість товарів до вивантаження за допомогою відбору.'"),,"Товары",,Отказ);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура МаксимальныйКодПриИзменении(Элемент)
	...
	Состояние(НСтр("ru='Выполняется обновление списка товаров...';uk='Виконується оновлення списку товарів...'"));
	ОбновитьСписокТоваровНаСервере();
...
Процедура ТоварыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если Не ОтменаРедактирования Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			...
			ОписаниеОшибки = НСтр("ru='Такой код уже назначен для номенклатуры %Номенклатура%';uk='Такий код вже призначений для номенклатури %Номенклатура%'");
			ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Номенклатура%", """" + НайденнаяСтрока.Номенклатура + """"
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.Характеристика), " " + НСтр("ru='с характеристикой';uk='з характеристикою'") + " """ + НайденнаяСтрока.Характеристика + """", "")
			+ ?(ЗначениеЗаполнено(НайденнаяСтрока.Упаковка), " " + НСтр("ru='в упаковке';uk='в упаковці'") + " """ + НайденнаяСтрока.Упаковка + """", ""))+НСтр("ru='. Обменять их местами?';uk='. Обміняти їх місцями?'");
	...
	Для Каждого ВыделеннаяСтрока Из Элементы.Товары.ВыделенныеСтроки Цикл
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) И ТекущиеДанные.Используется Тогда
			...
			ПоказатьВопрос(
				...
				НСтр("ru='Удаляемой номенклатуре, соответствующей заданному отбору после удаления будет заново назначены коды товаров. Продолжить?';uk='Номенклатурі, що видаляється, та яка відповідає заданому відбору після вилучення будуть заново призначені коди товарів. Продовжити?'"), 
...
Процедура УдалитьСвободныеКодыТоваровВКонцеСписка(Команда)
	...
	Если Товары.Количество() > 0 Тогда
		...
		Иначе
			ПоказатьПредупреждение(Неопределено, НСтр("ru='Нет данных для удаления';uk='Немає даних для вилучення'"));
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ru='Нет данных для удаления';uk='Немає даних для вилучення'"));
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<штрихкод будет сгенирирован при выгрузке>';uk='<штрихкод буде згенерований при вивантаженні>'"));
...
Процедура ЗагрузитьНастройкиОтбораПоУмолчанию()
	...
	Если Объект.ТипПодключаемогоОборудования = Перечисления.ТипыПодключаемогоОборудования.ККМOffline Тогда
...
Функция ТаблицаТоваров(Товары, ВидЦены)
	...
	Если Объект.ТипПодключаемогоОборудования = Перечисления.ТипыПодключаемогоОборудования.ВесыСПечатьюЭтикеток Тогда
		...
		Запрос.УстановитьПараметр("ФорматШтрихкода", РегистрыСведений.ШтрихкодыНоменклатуры.ФорматВесовогоШтрихкода(Константы.ПрефиксВесовогоШтрихкода.Получить()));
	КонецЕсли;
...
Процедура ОбновитьТаблицуТоваров(КодыТоваров, ПравилоОбмена, ВидЦены, АдресНаНастройки)
	...
	Если Параметр <> Неопределено Тогда
		Параметр.Значение = РегистрыСведений.ШтрихкодыНоменклатуры.ФорматВесовогоШтрихкода(Константы.ПрефиксВесовогоШтрихкода.Получить());
	КонецЕсли;
