...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Копирование И Группа Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Добавление новых групп в справочнике запрещено.';uk='Додавання нових груп у довіднику заборонене.'"));
	КонецЕсли;
...
Процедура ПереместитьЭлементВверх()
	...
	Если Не ВозможноПеремещениеЭлемента(Элементы.Список.ТекущиеДанные.Ссылка, "Вверх") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Перемещение данного вида контактной информации не предусмотрено';uk='Переміщення даного виду контактної інформації не передбачено'"));
		Возврат;
	КонецЕсли;
