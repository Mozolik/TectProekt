...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечататьКарточкуЭД Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"КарточкаЭД",
			НСтр("ru='Карточка электронного документа';uk='Картка електронного документа'"),
	...
	Если ПечататьЭД Тогда
		...
		СинонимМакета = НСтр("ru='Электронный документ';uk='Електронний документ'");
...
Процедура ЗаполнитьНаименованиеФайла() Экспорт
	...
	Пока ЭлементСсылка.Следующий() Цикл
		...
		Исключение
			Операция = НСтр("ru='Запись присоединенного файла';uk='Запис приєднаного файлу'");
...
Процедура СообщитьОбОшибке(Знач Описание)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Сертификаты электронной подписи. Перенос настроек в новый объект метаданных';uk='Сертифікати електронного підпису. Перенесення настройок в новий об''єкт метаданих'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаполнитьПредставлениеПодписантов(СтрокаМассива, ТребуемыеПодписи)
	...
	Если СтрокаМассива.ПроизвольныйЭД Тогда
		...
		Иначе
			...
			Если СтрокаМассива.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий Тогда
				...
				ТребуемыеПодписи.ПредставлениеОтправителя = ПредставлениеЮрФизЛицо(СтрокаМассива.ПодписьОрганизации);
				ТребуемыеПодписи.ПредставлениеПолучателя = НСтр("ru='Не требуется';uk='Не потрібно'");
	...
	Иначе
		...
		Иначе
			ТребуемыеПодписи.ПредставлениеОтправителя = НСтр("ru='Не требуется';uk='Не потрібно'");
			ТребуемыеПодписи.ПредставлениеПолучателя = НСтр("ru='Не требуется';uk='Не потрібно'");
