...
Процедура УдалитьЛишниеФайлы(Команда)
	...
	Если КоличествоЛишнихФайлов = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Нет ни одного лишнего файла на диске';uk='Немає жодного зайвого файлу на диску'"));
		Возврат;
	КонецЕсли;
	...
	Если ЧислоУдаленныхФайлов <> 0 Тогда
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Удалено файлов: %1';uk='Вилучено файлів: %1'"),
			ЧислоУдаленныхФайлов);
		ПоказатьОповещениеПользователя(
			НСтр("ru='Завершено удаление лишних файлов.';uk='Завершено видалення зайвих файлів.'"),
			,
			ТекстОповещения,
...
Процедура ЗаполнитьТаблицуЛишнихФайлов()
	...
	Для Каждого Файл Из МассивФайлов Цикл
		...
		НоваяСтрока.СтатусПроверки = НСтр("ru='Лишние файлы (есть на диске, но сведения о них отсутствуют)';uk='Зайві файли (є на диску, але відомості про них відсутні)'");
	...
	МассивЛишнихФайлов = ТаблицаФайловНаДиске.НайтиСтроки(
		Новый Структура("СтатусПроверки", НСтр("ru='Лишние файлы (есть на диске, но сведения о них отсутствуют)';uk='Зайві файли (є на диску, але відомості про них відсутні)'")));
		...
		Исключение
			...
			ШаблонОшибки =
				НСтр("ru='Путь каталога для копирования некорректен.
                    ...
                    |сервер платформы, не имеет прав доступа к указанному каталогу.
                    |
                    |%1'
                    |;uk='Шлях каталогу для копіювання некоректний.
                    |Можливо обліковий запис, від імені якої працює
                    |сервер платформи, не має прав доступу до вказаного каталогу.
                    |
                    |%1'");
...
Процедура ОбработатьСообщениеОбОшибке(ИмяФайла, ИнформацияОбОшибке)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Ошибка удаления лишних файлов';uk='Файли.Помилка вилучення зайвих файлів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При удалении файла с диска
                ...
                |возникла ошибка:
                |""%2"".'
                |;uk='При видаленні файлу з диска
                |""%1""
                |виникла помилка:
                |""%2"".'"),
			ИмяФайла,
...
Процедура СформироватьОтчетОбОшибках(ОтчетОбОшибках, МассивФайловСОшибками)
	...
	ОбластьЗаголовок.Параметры.Описание = НСтр("ru='Файлы с ошибками:';uk='Файли з помилками:'");
