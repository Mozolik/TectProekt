	...
	Иначе
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				НСтр("ru='открытие формы соглашения';uk='відкриття форми оферти'"), ТекстОшибки, ТекстСообщения);
		КонецПопытки;
	КонецЕсли;
...
Процедура ПолучательОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.ИдентификаторКонтрагента) Тогда
		Если ВыбранноеЗначение <> Объект.Контрагент Тогда
			ТекстВопроса = НСтр("ru='Была изменена организация получатель. Изменить идентификатор получателя?';uk='Була змінена організація одержувач. Змінити ідентифікатор одержувача?'");
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
		КонецЕсли;
	Иначе
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, КодВозвратаДиалога.Да);
		...
		Если ТекущийДокументСоглашения.Формировать Тогда
			Для Каждого ДокументВДругихСоглашениях Из ВидыДокументовДругихСоглашений Цикл
				Если ТекущийДокументСоглашения[?(ПроверятьВходящиеДокументы, "ВходящийДокумент", "ИсходящийДокумент")] = ДокументВДругихСоглашениях.ТипДокумента Тогда
                    ...
                    |%5.
                    |'
                    |;uk='По виду електронних документів %1 %2 
                    |вже існує діюча оферта між учасниками %3 - %4:
                    |%5.
                    |'");
...
Процедура ПроверитьЗаписатьСоглашение(ОбработчикПродолжения)
	...
	Если НЕ Объект.УдалитьЭтоТиповое Тогда
        ...
        |Записать соглашение?'
        |;uk='Зовнішні сертифікати можна вибирати тільки в записаній оферті.
        |Записати оферту?'");
...
Процедура ИзменитьНадписьНаправлениеДокументов()
	...
	ШаблонСообщения = НСтр("ru='От кого: %1, кому: %2';uk='Від кого: %1, кому: %2'");
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если Объект.УдалитьЭтоТиповое Тогда
		ТекстЗаголовка = НСтр("ru='Типовая настройка ЭДО между организациями';uk='Типова настройка ЕДО між організаціями'");
	Иначе
		ТекстЗаголовка = НСтр("ru='Настройка ЭДО между организациями';uk='Настройка ЕДО між організаціями'");
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Заголовок = ТекстЗаголовка + НСтр("ru=' (создание)';uk=' (створення)'");
	Иначе
