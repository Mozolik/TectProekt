...
Процедура ПредыдущаяСтраница(Команда)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаРезультирующаяТаблица Тогда
		...
		Элементы.КнопкаВперед.Заголовок = НСтр("ru='Далее';uk='Далі'") + ">";
...
Процедура УстановитьИнформационныеНадписи()
	...
	Если ТекущаяСтраница = Элементы.СтраницаРезультирующаяТаблица Тогда
		ИнформацияШапка                 = НСтр("ru='Результирующая таблица:';uk='Результуюча таблиця:'");
		ИнформацияПодвал                = НСтр("ru='Нажмите Готово для окончания редактирования.';uk='Натисніть Готово для закінчення редагування.'");
		Элементы.КнопкаВперед.Заголовок = НСтр("ru='Готово';uk='Готово'");
	Иначе
		Элементы.КнопкаВперед.Заголовок = НСтр("ru='Далее>>';uk='Далі>>'");
		Если ТекущаяСтраница = Элементы.СтраницаВопросы Тогда
			Если ТипТабличногоВопроса = ПредопределенноеЗначение("Перечисление.ТипыТабличныхВопросов.Составной") Тогда
				ИнформацияШапка  = НСтр("ru='Подбор вопросов. Укажите хотя бы один вопрос:';uk='Підбір запитань. Виберіть хоча б одне питання:'");
				ИнформацияПодвал = НСтр("ru='Нажмите Далее для просмотра получившейся таблицы.';uk='Натисніть Далі для перегляду отриманої таблиці.'");
			ИначеЕсли ТипТабличногоВопроса = ПредопределенноеЗначение("Перечисление.ТипыТабличныхВопросов.ПредопределенныеОтветыВСтрокахИКолонках") Тогда
				ИнформацияШапка  =НСтр("ru='Подбор вопросов. Укажите три вопроса:';uk='Підбір запитань. Вкажіть три питання:'");
				ИнформацияПодвал =НСтр("ru='Нажмите Далее для подбора предопределенных ответов.';uk='Натисніть Далі для підбору напередвизначених відповідей.'");
			Иначе
				ИнформацияШапка   =НСтр("ru='Подбор вопросов. Укажите как минимум два вопроса:';uk='Підбір запитань. Вкажіть як мінімум два питання:'");
				ИнформацияПодвал  =НСтр("ru='Нажмите Далее для подбора предопределенных ответов.';uk='Натисніть Далі для підбору напередвизначених відповідей.'");
			КонецЕсли;
		ИначеЕсли ТекущаяСтраница = Элементы.СтраницаТипТабличногоВопроса Тогда
			ИнформацияШапка       = НСтр("ru='Выбор типа табличного вопроса:';uk='Вибір типу табличного питання:'");
			ИнформацияПодвал      = НСтр("ru='Нажмите Далее для подбора вопросов:';uk='Натисніть Далі для підбору питань:'");
		Иначе
			ИнформацияШапка  = НСтр("ru='Подбор предопределенных ответов:';uk='Підбір напередвизначених відповідей:'");
			ИнформацияПодвал = НСтр("ru='Нажмите Далее для просмотра получившейся таблицы:';uk='Натисніть Далі для перегляду отриманої таблиці:'");
		КонецЕсли;
	КонецЕсли;
