...
Процедура ПрименитьПравила(ПараметрыВыгрузки, АдресХранилища) Экспорт
	...
	Если Результат[2].Пустой() Тогда
		ТекстСообщения = НСтр("ru='В выбранной папке нет писем.';uk='У вибраній папці немає листів.'");
	...
	Если ТаблицаСоответствий.Количество() > 0 Тогда
		ТекстСообщения = НСтр("ru='Перенос писем в папки выполнен.';uk='Перенесення листів в папки виконано.'");
	Иначе
		ТекстСообщения =  НСтр("ru='Ни одно письмо не было перенесено';uk='Жоден лист не був перенесений'");
	КонецЕсли;
