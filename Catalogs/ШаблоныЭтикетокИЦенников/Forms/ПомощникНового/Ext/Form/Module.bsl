...
Процедура ПриИзмененииНазначенияСервер()
	...
	Если Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляТоваров
		...
		Тогда
		Заголовок = НСтр("ru='Помощник создания этикетки';uk='Помічник створення етикетки'");
		СтрокаПоискаТипа = "Этикетки";
		Элементы.РазмерЛенты.Заголовок = НСтр("ru='Размер ленты';uk='Розмір стрічки'");
		Элементы.ОписаниеКоличестваЦенников.Видимость = Ложь;
	Иначе
		Заголовок = НСтр("ru='Помощник создания ценника';uk='Помічник створення цінника'");
		СтрокаПоискаТипа = "Ценника";
		Элементы.РазмерЛенты.Заголовок = НСтр("ru='Размер бумаги';uk='Розмір паперу'");
...
Процедура НапечататьОбразец(Команда)
	...
	Если Не ЗначениеЗаполнено(ДляЧего) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Для чего""';uk='Не заповнене поле ""Для чого""'"),,"ДляЧего");
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьВсе()
	...
	Иначе
		Элементы.ДекорацияОписание.Заголовок = НСтр("ru='Нет параметров';uk='Немає параметрів'");
	КонецЕсли;
