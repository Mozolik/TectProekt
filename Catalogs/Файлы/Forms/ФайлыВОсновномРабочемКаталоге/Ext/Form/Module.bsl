...
Процедура УдалитьИзЛокальногоКэшаФайловВыполнить()
	...
	ТекстВопроса = НСтр("ru='Удалить выбранные файлы из основного рабочего каталога?';uk='Вилучити обрані файли з основного робочого каталогу?'");
	...
	Если ПараметрыВыполнения.ЕстьЗанятыеФайлы Тогда
		ПоказатьПредупреждение(,
			НСтр("ru='Нельзя удалять из основного рабочего каталога файлы,
                       ...
                       |;uk='Не можна видаляти з основного робочого каталогу файли,
                       |зайняті для редагування.'"));
	КонецЕсли;
