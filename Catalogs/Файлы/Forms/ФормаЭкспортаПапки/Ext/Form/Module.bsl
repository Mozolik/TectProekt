...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ru='В Веб-клиенте экспорт каталогов не поддерживается.';uk='У Веб-Клієнті експорт каталогів не підтримується.'"));
		Отказ = Истина;
		Возврат;
	#КонецЕсли
	...
	Если Не ПустаяСтрока(ПапкаДляЭкспорта) Тогда
		...
		Если Не Файл.Существует() Тогда
			ТекстПредупреждения = НСтр("ru='Невозможно открыть папку выгрузки.
                                             ...
                                             |;uk='Неможливо відкрити папку вивантаження.
...
Процедура СохранитьПапку()
	...
	Если Не КаталогВыгрузки.Существует() Тогда
		...
		Исключение
			ТекстОшибки = НСтр("ru='Не удалось создать папку выгрузки:';uk='Не вдалося створити папку вивантаження:'");
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Выполняется экспорт папки ""%1""...
                   ...
                   |;uk='Виконується експорт папки ""%1""...
	...
	Если Результат.Успех = Истина Тогда
		...
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		             НСтр("ru='Успешно завершен экспорт папки ""%1""
                                ...
                                |;uk='Успішно завершений експорт папки ""%1""
                                |у каталог на диску ""%2"".'"),
...
Процедура ОбойтиДеревоФайлов4(ПараметрыВыполнения)
	...
	Исключение
		ТекстОшибки = НСтр("ru='Ошибка создания папки ""%1"":';uk='Помилка створення папки ""%1"":'");
	...
	Если ПараметрыВыполнения.ФайлНаДиске.Существует() И ПараметрыВыполнения.ФайлНаДиске.ЭтоКаталог() Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Вместо файла
                       ...
                       |;uk='Замість файлу
                       |""%1""
                       |існує папка з таким же ім''ям.
                       |
                       |Повторити експорт цього файлу?'"),
			...
			Если Не ПараметрыВыполнения.ОбщиеПараметры.ДляВсехФайлов Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='В папке ""%1""
                               ...
                               |;uk='У папці ""%1""
                               |існує файл ""%2""
                               |розмір існуючого файлу = %3 байт, дата зміни %4.
                               |розмір файлу, що зберігає, = %5 байт, дата зміни %6.
                               |
                               |Замінити існуючий файл файлом зі сховища файлів?'"),
					ПараметрыВыполнения.БазовыйКаталогСохраненияФайла,
