	...
	Если НЕ Параметры.Свойство("КаталогНаДиске") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Данная обработка вызывается из других процедур конфигурации.
                       ...
                       |;uk='Дана обробка викликається з інших процедур конфігурації.
                       |Вручну її викликати заборонено.'")); 
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ИмпортВыполнить()
	...
	Если ПустаяСтрока(Каталог) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не выбран каталог для импорта.';uk='Не обраний каталог для імпорту.'"), , "Каталог");
		Возврат;
	...
	Если ПапкаДляДобавления.Пустая() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Укажите папку.';uk='Вкажіть папку.'"), , "ПапкаДляДобавления");
		Возврат;
	КонецЕсли;
