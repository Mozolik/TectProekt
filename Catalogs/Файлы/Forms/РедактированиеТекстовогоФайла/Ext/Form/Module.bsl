...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не РежимРедактирования Тогда
		ЗаголовокСтрока = ЗаголовокСтрока + " " + НСтр("ru='(только просмотр)';uk='(тільки перегляд)'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДанныеФайла.Версия) Тогда
		...
		Если ЗначениеЗаполнено(КодировкаТекстаФайла) Тогда
			...
			КонецЕсли;
		Иначе
			КодировкаПредставление = НСтр("ru='По умолчанию';uk='По умовчанню'");
		КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = СтрЗаменить(НСтр("ru='Файл ""%1"" был изменен.';uk='Файл ""%1"" був змінений.'"), "%1", ИмяИРасширение);
...
Процедура СохранитьКак(Команда)
	...
	Если ВыборФайла.Выбрать() Тогда
		...
		Состояние(НСтр("ru='Файл успешно сохранен';uk='Файл успішно збережений'"), , ВыбранноеПолноеИмяФайла);
...
Процедура ПоказатьОтличия(Команда)
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ru='Сравнение версий в веб-клиенте не поддерживается.';uk='Порівняння версій у веб-клієнті не підтримується.'"));
	#Иначе
