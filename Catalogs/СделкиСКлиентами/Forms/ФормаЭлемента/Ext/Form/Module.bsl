...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПроцессыИспользуются И Не СделкаПроиграна И НЕ ОтветОФиксацииПроигрышаПолучен
		...
		ПоказатьВопрос(ОписаниеОповещениВопрос ,НСтр("ru='Фиксация проигрыша сделки на текущем этапе. Продолжить?';uk='Фіксація програшу угоди на поточному етапі. Продовжити?'"),
...
Процедура СоглашениеСКлиентомНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПродажиКлиент.НачалоВыбораСоглашенияСКлиентом(
		Элемент, СтандартнаяОбработка, Объект.Партнер, Объект.СоглашениеСКлиентом,,,,,Объект);
...
Процедура РассчитатьПоДокументам(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указана валюта управленческого учета. Расчет невозможен.';uk='Не вказана валюта управлінського обліку. Розрахунок неможливий.'"));
	КонецЕсли;
...
Процедура РассчитатьПоДокументамПослеОтменыЗаписи(Результат, ДополнительныеПараметры) Экспорт
    ...
    Новый Структура("Ключ", Объект.Ссылка),,,,, Новый ОписаниеОповещения("РассчитатьПоДокументамЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПерейтиНаРучноеУправление(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		НСтр("ru='Перед выполнением необходимо записать сделку.
            ...
            |;uk='Перед виконанням необхідно записати угоду.
...
Процедура ПерейтиНаРучноеУправлениеФрагмент()
    ...
    |Продолжить?'
    |;uk='Всі поточні задачі з процесу продажу будуть скасовані, бізнес-процес буде завершений.
    |Продовжити?'"),
    РежимДиалогаВопрос.ДаНет);
...
Процедура ЗафиксироватьПрогрыш(Команда)
	...
	ТекстВопроса = НСтр("ru='При записи сделки будет зафиксирован проигрыш на текущем этапе.';uk='При запису угоди буде зафіксований програш на поточному етапі.'");
	Ответ = Неопределено;
...
Процедура УстановитьПроцентУдовлетворения(Команда)
	...
	ВыбранноеЧисло = 100;
	ПоказатьВводЧисла(Новый ОписаниеОповещения("УстановитьПроцентУдовлетворенияЗавершение", ЭтотОбъект, Новый Структура("ВыбранноеЧисло", ВыбранноеЧисло)), ВыбранноеЧисло,НСтр("ru='Введите число';uk='Введіть число'"),3,0);
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Ложь);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<потребность удовлетворена>';uk='<потреба задоволена>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Сроки = НСтр("ru='Начало';uk='Початок'") + ": " + Формат(Объект.ДатаНачала,"ДЛФ=D");
	Если ЗначениеЗаполнено(Объект.ДатаОкончания) Тогда
		Сроки = Сроки + НСтр("ru='  Окончание';uk='  Закінчення'") + ": " + Формат(Объект.ДатаОкончания,"ДЛФ=D");
	КонецЕсли;
...
Процедура ОтменаЗаписиОбъекта(Знач Оповещение)
	...
	ТекстВопроса = НСтр("ru='Данные еще не записаны.
                        ...
                        |Виконання дії можливо тільки після запису.
