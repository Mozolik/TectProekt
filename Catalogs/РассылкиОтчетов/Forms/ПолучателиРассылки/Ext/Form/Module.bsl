...
Процедура ОткрытьФормаДобавленияПолучателей(ЭтоПодбор)
	...
	Если ЭтоПодбор Тогда
		...
		ПараметрыФормыВыбора.Вставить("ЗаголовокФормыПодбора", НСтр("ru='Подбор получателей рассылки';uk='Підбір одержувачів розсилки'"));
...
Процедура ДобавитьПеретащитьПолучателя(ПолучательИлиНаборПолучателей)
	...
	Если МассивНовыхСтрок.Количество() > 0 Тогда
		Если МассивНовыхСтрок.Количество() = 1 Тогда
			ЗаголовокОповещения = НСтр("ru='Получатель добавлен в рассылку';uk='Одержувач доданий у розсилку'");
		Иначе
			ЗаголовокОповещения = НСтр("ru='Получатели добавлены в рассылку';uk='Одержувачі додані в розсилку'");
		КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	Если Получатели.Количество() > 0 Тогда
		...
		Если КоличествоПолучателей = 0 Тогда 
			Отказ = Истина;
			ПоказатьПредупреждение(, НСтр("ru='Требуется указать хотя бы одного получателя';uk='Потрібно зазначити хоча б одного одержувача'"));
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
