...
Процедура СписокХозяйственныхОперацийДобавить(Команда)
	...
	ПараметрыФормы = Новый Структура("МножественныйВыбор, Отбор, ОтображениеСписком",
			Истина, Новый Структура("ХозяйственнаяОперация", Элементы.ХозяйственнаяОперация.СписокВыбора), Истина);
	...
	Для Индекс = 0 По СписокХозяйственныхОпераций.Количество() - 1 Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаСпискаХозяйственныхОпераций.ХозяйственнаяОперация) Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В строке %1 не заполнена хозяйственная операция.';uk='У рядку %1 не заповнено господарську операцію.'"),
				Индекс + 1);
