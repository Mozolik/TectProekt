...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО Тогда
		...
		Если СертификатыПодписейОрганизации.Количество() = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ЭлектронноеВзаимодействиеКлиентСервер.ТекстСообщения("Список", "Заполнение", , , "Сертификаты организации"),
				ЭтотОбъект,
...
Процедура ПередЗаписью(Отказ)
	...
	Если ПометкаУдаления Тогда
		...
		Если Не Запрос.Выполнить().Пустой() Тогда
			ТекстСообщения = НСтр("ru='Операция отменена. Текущий профиль настроек ЭДО используется в действующих настройках ЭДО.';uk='Операція скасована. Поточний профіль налаштувань ЕДО використовується в діючих настройках ЕДО.'");
...
Функция ПрофильНастроекЭДОУникален() Экспорт
	...
	Если Не ТекущийПрофильНастроекУникален Тогда
        ...
        |Способ обмена - %3;'
        |;uk='В інформаційній базі вже існує профіль настройок з реквізитами:
        |Організація - %1;
