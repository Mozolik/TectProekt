	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
	   ...
	   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса.ПользователиВМоделиСервиса") Тогда
		...
		Если НЕ ДействияСПользователемСервиса.ИзменениеАдминистративногоДоступа Тогда
			ВызватьИсключение
				НСтр("ru='Не достаточно прав доступа для изменения состава администраторов.';uk='Не достатньо прав доступу для зміни складу адміністраторів.'");
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли Объект.Ссылка = ПредопределенноеЗначение("Справочник.ГруппыДоступа.Администраторы")
		...
		ПоказатьВопрос(
			...
			НСтр("ru='У группы доступа Администраторы должен быть профиль Администратор.
                       ...
                       |;uk='У групи доступу Адміністратори повинен бути профіль Адміністратор.
                       |
                       |Установити профіль у групі доступу (ні - відкрити тільки для перегляду)?'"),
		...
		ИначеЕсли НЕ ТолькоПросмотр И ИзменилсяСоставВидовДоступаПриЧтении Тогда
			...
			ПоказатьВопрос(
				...
				НСтр("ru='Изменился состав видов доступа профиля этой группы доступа.
                           ...
                           |;uk='Змінився склад видів доступу профілю цієї групи доступу.
                           |
                           |Оновити види доступу в групі доступу (ні - відкрити тільки для перегляду)?'"),
				РежимДиалогаВопрос.ДаНет,
				,
				КодВозвратаДиалога.Нет);
	...
	Если ПараметрыЗаписи.Свойство("ПредупредитьЧтоПрофильПомеченНаУдаление") Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("ru='Группа доступа не влияет на права участников
                       ...
                       |;uk='Група доступу не впливає на права учасників
                       |так як її профіль відмічений для вилучення.'"));
	КонецЕсли;
	...
	Для Каждого ТекущаяСтрока Из ДеревоПользователейСтроки Цикл
		...
		Если НЕ ЗначениеЗаполнено(Участник) Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				УточнитьСообщение(НСтр("ru='Пользователь не выбран.';uk='Користувач не обраний.'"), Участник),
				"ПользователиГруппы",
				НомерСтроки,
				УточнитьСообщение(НСтр("ru='Пользователь в строке %1 не выбран.';uk='Користувач у рядку %1 не обраний.'"), Участник));
			Продолжить;
		КонецЕсли;
		...
		Если НайденныеЗначения.Количество() > 1 Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Пользователь ""%2"" повторяется.';uk='Користувач ""%2"" повторюється.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Пользователь ""%2"" в строке %1 повторяется.';uk='Користувач ""%2"" в рядку %1 повторюється.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Внешний пользователь ""%2"" повторяется.';uk='Зовнішній користувач ""%2"" повторюється.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Внешний пользователь ""%2"" в строке %1 повторяется.';uk='Зовнішній користувач ""%2"" в рядку %1 повторюється.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Группа пользователей ""%2"" повторяется.';uk='Група користувачів ""%2"" повторюється.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Группа пользователей ""%2"" в строке %1 повторяется.';uk='Група користувачів ""%2"" в рядку %1 повторюється.'");
			Иначе
				ТекстДляОднойОшибки      = НСтр("ru='Группа внешних пользователей ""%2"" повторяется.';uk='Група зовнішніх користувачів ""%2"" повторюється.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Группа внешних пользователей ""%2"" в строке %1 повторяется.';uk='Група зовнішніх користувачів ""%2"" в рядку %1 повторюється.'");
			КонецЕсли;
		...
		Если Объект.Ссылка = Справочники.ГруппыДоступа.Администраторы
		   И ТипЗнч(ТекущаяСтрока.Пользователь) <> Тип("СправочникСсылка.Пользователи") Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Внешний пользователь ""%2"" недопустим в предопределенной группе доступа Администраторы.';uk='Зовнішній користувач ""%2"" неприпустимий у визначеної групи доступу Адміністратори.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Внешний пользователь ""%2"" в строке %1 недопустим в предопределенной группе доступа Администраторы.';uk='Зовнішній користувач ""%2"" в рядку %1 неприпустимий у визначеної групи доступу Адміністратори.'");
				
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Группа пользователей ""%2"" недопустима в предопределенной группе доступа Администраторы.';uk='Група користувачів ""%2"" неприпустима в наперед визначеній групі доступу Адміністратори.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Группа пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.';uk='Група користувачів ""%2"" в рядку %1 неприпустима в напередвизначеній групі доступу Адміністратори.'");
			Иначе
				ТекстДляОднойОшибки      = НСтр("ru='Группа внешних пользователей ""%2"" недопустима в предопределенной группе доступа Администраторы.';uk='Група зовнішніх користувачів ""%2"" неприпустима в наперед визначеній групі доступу Адміністратори.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Группа внешних пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.';uk='Група зовнішніх користувачів ""%2"" в рядку %1 неприпустима в напередвизначеній групі доступу Адміністратори.'");
			КонецЕсли;
		...
		Если НедопустимыеПользователи.Найти(ТекущаяСтрока.Пользователь) <> Неопределено Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Пользователь ""%2"" недопустим для указанного типа участников.';uk='Користувач ""%2"" неприпустимий для зазначеного типу учасників.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Пользователь ""%2"" в строке %1 недопустим для указанного типа участников.';uk='Користувач ""%2"" в рядку %1 неприпустимий для зазначеного типу учасників.'");
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Группа пользователей ""%2"" недопустима для указанного типа участников.';uk='Група користувачів ""%2"" неприпустима для зазначеного типу учасників.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Группа пользователей ""%2"" в строке %1 недопустима для указанного типа участников.';uk='Група користувачів ""%2"" в рядку %1 неприпустима для зазначеного типу учасників.'");
			ИначеЕсли ТипЗнч(ТекущаяСтрока.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстДляОднойОшибки      = НСтр("ru='Внешний пользователь ""%2"" недопустим для указанного типа участников.';uk='Зовнішній користувач ""%2"" неприпустимий для зазначеного типу учасників.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Внешний пользователь ""%2"" в строке %1 недопустим для указанного типа участников.';uk='Зовнішній користувач ""%2"" в рядку %1 неприпустимий для зазначеного типу учасників.'");
			Иначе // Группа внешних пользователей.
				ТекстДляОднойОшибки      = НСтр("ru='Группа внешних пользователей ""%2"" недопустима для указанного типа участников.';uk='Група зовнішніх користувачів ""%2"" неприпустима для зазначеного типу учасників.'");
				ТекстДляНесколькихОшибок = НСтр("ru='Группа внешних пользователей ""%2"" в строке %1 недопустима для указанного типа участников.';uk='Група зовнішніх користувачів ""%2"" в рядку %1 неприпустима для зазначеного типу учасників.'");
			КонецЕсли;
		...
		Если ТипЗнч(СтрокаРодителя.Пользователь) =
			...
			ПоказатьПредупреждение(,
				НСтр("ru='Пользователи групп отображаются для сведения,
                           ...
                           |;uk='Користувачі груп відображаються для відомості,
                           |що вони одержують доступ груп користувачів.
                           |Їх не можна вилучити в цьому списку.'"));
		Иначе
			ПоказатьПредупреждение(,
				НСтр("ru='Внешние пользователи групп отображаются для сведения,
                           |что они получают доступ групп внешних пользователей.
                           |Их нельзя удалить в этом списке.'
                           |;uk='Зовнішні користувачі груп відображаються для відомості,
                           |що вони одержують доступ груп зовнішніх користувачів.
                           |Їх не можна вилучити в цьому списку.'"));
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		Если ЗначениеЗаполнено(ТекущийОбъект.Пользователь) Тогда
			...
			Заголовок
				...
				+ " "
				+ НСтр("ru='(Группа доступа)';uk='(Група доступу)'");
...
Процедура ВыбратьПодобратьПользователейЗавершение(ВыборИПодборВнешнихПользователей, Подобрать) Экспорт
	...
	Если ВыборИПодборВнешнихПользователей Тогда
		...
		Если Не ИспользоватьВнешнихПользователей Тогда
			ПоказатьПредупреждение(, НСтр("ru='Ведение внешних пользователей отключено в настройках программы.';uk='Ведення зовнішніх користувачів відключене в настройках програми.'"));
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Недостаточно прав для выбора внешних пользователей.';uk='Недостатньо прав для вибору зовнішніх користувачів.'"));
		КонецЕсли;
	Иначе
...
Функция ПараметрыРасширеннойФормыПодбора()
	...
	ЗаголовокФормыПодбора = НСтр("ru='Подбор участников группы доступа';uk='Підбір учасників групи доступу'");
