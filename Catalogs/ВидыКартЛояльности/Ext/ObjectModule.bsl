...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаТЧ Из ШаблоныКодовКартЛояльности Цикл
		...
		Если ТипКарты = Перечисления.ТипыКарт.Магнитная
			...
			Если СтрДлина(СтрокаТЧ.НачалоДиапазонаМагнитногоКода) > СтрокаТЧ.ДлинаМагнитногоКода Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке %1 списка ""Шаблоны кодов карт лояльности"" количество символов нижней границы диапазона магнитного кода превышает длину магнитного кода.';uk='У рядку %1 списку ""Шаблони кодів карт лояльності"" кількість символів нижньої межі діапазону магнітного коду перевищує довжину магнітного коду.'"), СтрокаТЧ.НомерСтроки),
					ЭтотОбъект,
			...
			Если СтрДлина(СтрокаТЧ.КонецДиапазонаМагнитногоКода) > СтрокаТЧ.ДлинаМагнитногоКода Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке %1 списка ""Шаблоны кодов карт лояльности"" количество символов верхней границы диапазона магнитного кода превышает длину магнитного кода.';uk='У рядку %1 списку ""Шаблони кодів карт лояльності"" кількість символів верхньої межі діапазону магнітного коду перевищує довжину магнітного коду.'"), СтрокаТЧ.НомерСтроки),
					ЭтотОбъект,
			...
			Если СтрокаТЧ.КонецДиапазонаМагнитногоКода < СтрокаТЧ.НачалоДиапазонаМагнитногоКода Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке %1 списка ""Шаблоны кодов карт лояльности"" нижняя граница диапазона магнитного кода превышает верхнюю границу дипазона магнитного кода.';uk='У рядку %1 списку ""Шаблони кодів карт лояльності"" нижня межа діапазону магнітного коду перевищує верхню межу дипазона магнітного коду.'"), СтрокаТЧ.НомерСтроки),
					ЭтотОбъект,
		...
		Если ТипКарты = Перечисления.ТипыКарт.Штриховая
			...
			Если СтрДлина(СтрокаТЧ.НачалоДиапазонаШтрихкода) > СтрокаТЧ.ДлинаШтрихкода Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке %1 списка ""Шаблоны кодов карт лояльности"" количество символов нижней границы диапазона штрихкода превышает длину штрихкода.';uk='У рядку %1 списку ""Шаблони кодів карт лояльності"" кількість символів нижньої межі діапазону штрихкоду перевищує довжину штрих-коду.'"), СтрокаТЧ.НомерСтроки),
					ЭтотОбъект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ШаблоныКодовКартЛояльности", СтрокаТЧ.НомерСтроки, "НачалоДиапазонаШтрихкода"),
					,
					Отказ);
			...
			Если СтрДлина(СтрокаТЧ.КонецДиапазонаШтрихкода) > СтрокаТЧ.ДлинаШтрихкода Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке %1 списка ""Шаблоны кодов карт лояльности"" количество символов верхней границы диапазона штрихкода превышает длину штрихкода.';uk='У рядку %1 списку ""Шаблони кодів карт лояльності"" кількість символів верхньої межі діапазону штрихкоду перевищує довжину штрих-коду.'"), СтрокаТЧ.НомерСтроки),
					ЭтотОбъект,
			...
			Если СтрокаТЧ.КонецДиапазонаШтрихкода < СтрокаТЧ.НачалоДиапазонаШтрихкода Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В строке %1 списка ""Шаблоны кодов карт лояльности"" нижняя граница диапазона штрихкода превышает верхнюю границу дипазона штрихкода.';uk='У рядку %1 списку ""Шаблони кодів карт лояльності"" нижня межа діапазону штрихкоду перевищує верхню межу дипазона штрих-коду.'"), СтрокаТЧ.НомерСтроки),
					ЭтотОбъект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ШаблоныКодовКартЛояльности", СтрокаТЧ.НомерСтроки, "НачалоДиапазонаШтрихкода"),
					,
					Отказ);
	...
	Если ЗначениеЗаполнено(ДатаНачалаДействия) И ЗначениеЗаполнено(ДатаОкончанияДействия) И ДатаНачалаДействия > ДатаОкончанияДействия Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Дата окончания действия вида карт лояльности должна быть не меньше даты начала действия.';uk='Дата закінчення дії виду карт лояльності повинна бути не менше дати початку дії.'"),
			ЭтотОбъект,
