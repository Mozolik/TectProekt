...
Процедура НеИспользоватьИсключенияПоЦеновымГруппам(Команда)
	...
	Если Объект.ЦеновыеГруппы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("НеИспользоватьИсключенияПоЦеновымГруппамЗавершение", ЭтотОбъект), НСтр("ru='Таблица уточнений будет очищена, продолжить?';uk='Таблиця уточнень буде очищена, продовжити?'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
