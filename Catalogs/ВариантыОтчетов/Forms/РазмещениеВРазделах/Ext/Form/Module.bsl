...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СмешаннаяВажность = НСтр("ru='Различная';uk='Різна'");
...
Процедура Разместить(Команда)
	Если ПроверитьКоличествоВариантов() Тогда
		ЗаписатьНаСервере();
		ТекстОповещения = НСтр("ru='Изменены настройки вариантов отчетов (%1 шт.).';uk='Змінено настройки варіантів звітів (%1 шт.).'");
...
Функция ПроверитьКоличествоВариантов()
	...
	Если КоличествоВариантов = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Необходимо заполнить список ""Варианты отчетов""';uk='Необхідно заповнити список ""Варіанти звітів""'"),
			,
			"ИзменяемыеВарианты");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПерезаполнитьДерево(ТолькоСнятьФлажки)
	...
	Если КоличествоВариантов = 0 Тогда
		ТекстСообщения = НСтр("ru='Необходимо выбрать варианты отчетов';uk='Необхідно обрати варіанти звітів'");
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из ОтфильтрованныеВарианты Цикл
			Если ТекущаяПричина <> СтрокаТаблицы.Причина Тогда
				...
				Если ТекущаяПричина = 1 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ru='Помеченные на удаление:';uk='Відмічені для вилучення:'");
				ИначеЕсли ТекущаяПричина = 2 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ru='Недостаточно прав для изменения:';uk='Недостатньо прав для зміни:'");
				ИначеЕсли ТекущаяПричина = 3 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ru='Отчет отключен или недоступен по правам:';uk='Звіт відключений або недоступний по правах:'");
				ИначеЕсли ТекущаяПричина = 4 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + НСтр("ru='Вариант отчета отключен по функциональной опции:';uk='Варіант звіту відключено по функціональній опції:'");
				КонецЕсли;
			КонецЕсли;
		...
		Если КоличествоВариантов = 0 Тогда
			РезультатВыполнения.Текст = НСтр("ru='Недостаточно прав для размещения в разделах выбранных вариантов отчетов.';uk='Недостатньо прав для розміщення в розділах обраних варіантів звітів.'");
		...
		Иначе
			РезультатВыполнения.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недостаточно прав для размещения в разделах некоторых вариантов отчетов (%1).';uk='Недостатньо прав для розміщення в розділах деяких варіантів звітів (%1).'"),
