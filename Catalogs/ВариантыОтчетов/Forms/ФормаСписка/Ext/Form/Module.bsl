...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокВыбора.Добавить(1, НСтр("ru='Все, кроме внешних';uk='Усі, крім зовнішніх'"));
	СписокВыбора.Добавить(Перечисления.ТипыОтчетов.Внутренний,     НСтр("ru='Внутренние';uk='Внутрішні'"));
	СписокВыбора.Добавить(Перечисления.ТипыОтчетов.Расширение,     НСтр("ru='Расширения';uk='Розширення'"));
...
Функция РазместитьВариантыВПодсистеме(ПараметрыРазмещения)
		...
		Для Каждого ВариантСсылка Из ПараметрыРазмещения.Варианты.Массив Цикл
			...
			ИначеЕсли ВариантСсылка.ПометкаУдаления Тогда
				НеМогутРазмещаться = ?(НеМогутРазмещаться = "", "", НеМогутРазмещаться + Символы.ПС)
					...
					+ Строка(ВариантСсылка)
					+ " ("
					+ НСтр("ru='помечен на удаление';uk='відмічений для вилучення'")
					+ ")";
				Продолжить;
			КонецЕсли;
	...
	Если ПараметрыРазмещения.Варианты.Всего = Размещено Тогда
		Если ПараметрыРазмещения.Варианты.Всего = 1 Тогда
			Если ПараметрыРазмещения.Действие = "Перемещение" Тогда
				Шаблон = НСтр("ru='Успешно перемещен в ""%1"".';uk='Успішно перенесено у ""%1"".'");
			Иначе
				Шаблон = НСтр("ru='Успешно размещен в ""%1"".';uk='Успішно розміщений у ""%1"".'");
			КонецЕсли;
			Текст = ПараметрыРазмещения.Варианты.Представление;
			Ссылка = ПолучитьНавигационнуюСсылку(ПараметрыРазмещения.Варианты.Массив[0]);
		Иначе
			Если ПараметрыРазмещения.Действие = "Перемещение" Тогда
				Шаблон = НСтр("ru='Успешно перемещены в ""%1"".';uk='Успішно переміщені в ""%1"".'");
			Иначе
				Шаблон = НСтр("ru='Успешно размещены в ""%1"".';uk='Успішно розміщені у ""%1"".'");
			КонецЕсли;
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Варианты отчетов (%1).';uk='Варіанти звітів (%1).'"), Формат(ПараметрыРазмещения.Варианты.Всего, "ЧН=0; ЧГ=0"));
			Ссылка = Неопределено;
		КонецЕсли;
	...
	Иначе
		...
		Если Не ПустаяСтрока(НеМогутРазмещаться) Тогда
			ТекстОшибок = ?(ТекстОшибок = "", "", ТекстОшибок + Символы.ПС + Символы.ПС)
				+ НСтр("ru='Не могут размещаться в командном интерфейсе:';uk='Не можуть розміщуватися в командному інтерфейсі:'")
				+ Символы.ПС
				+ НеМогутРазмещаться;
		КонецЕсли;
		Если Не ПустаяСтрока(УжеРазмещены) Тогда
			ТекстОшибок = ?(ТекстОшибок = "", "", ТекстОшибок + Символы.ПС + Символы.ПС)
				+ НСтр("ru='Уже размещены в этом разделе:';uk='Вже розміщені в цьому розділі:'")
				+ Символы.ПС
				+ УжеРазмещены;
		КонецЕсли;
		
		Если ПараметрыРазмещения.Действие = "Перемещение" Тогда
			Шаблон = НСтр("ru='Перемещено вариантов отчетов: %1 из %2.';uk='Переміщено варіантів звітів: %1 з %2.'");
		Иначе
			Шаблон = НСтр("ru='Размещено вариантов отчетов: %1 из %2.';uk='Розміщено варіантів звітів: %1 з %2.'");
		КонецЕсли;
...
Функция НачатьЗамер(Событие)
	...
	Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
		Комментарий = Комментарий
			...
			+ "; " + НСтр("ru='Включая подчиненные:';uk='Включаючи підпорядковані:'") + " " + Строка(ВключаяПодчиненные);
	Иначе
