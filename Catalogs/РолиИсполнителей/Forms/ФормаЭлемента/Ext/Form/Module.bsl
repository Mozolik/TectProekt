		...
		Для каждого СтрокаТаблицы Из Объект.Назначение Цикл
			Если ТипЗнч(СтрокаТаблицы.ТипПользователей) <> ТипЗнч(Справочники.Пользователи.ПустаяСсылка()) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю( 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль не может использоваться с обязательным уточнением для назначения: %1.';uk='Роль не може використовуватися з обов''язковим уточненням для призначення: %1.'"), НаименованиеНазначения ),,,
...
Процедура ВыбратьНазначение(Команда)
	...
	ПользователиСлужебныйКлиент.ВыбратьНазначение(ЭтотОбъект, НСтр("ru='Выбор назначения роли';uk='Вибір ролі призначення'"),,, ОписаниеОповещения);
КонецПроцедуры
