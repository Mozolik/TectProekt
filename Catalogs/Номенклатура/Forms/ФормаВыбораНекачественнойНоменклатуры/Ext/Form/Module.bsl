...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга") Тогда
		ТекстСообщения = НСтр("ru='Качество услуг изменять нельзя.';uk='Якість послуг змінювати не можна.'");
		ВызватьИсключение ТекстСообщения;
	ИначеЕсли ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа") Тогда
		ТекстСообщения = НСтр("ru='Качество работ изменять нельзя.';uk='Якість робіт змінювати не можна.'");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	...
	Иначе
		ШаблонТекстаСообщения = НСтр("ru='Для товара %Товар% не найден товар исходного качества. Выбор товаров другого качества невозможен.';uk='Для товару %Товар% не знайдений товар вихідної якості. Вибір товарів іншої якості неможливий.'");
	...
	Если Элементы.СтраницаОбщегоСписка.Видимость Тогда
		...
		Если РеквизитыНоменклатуры.ИспользоватьСерии
			...
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ОбщийСписок,
				...
				Истина);
			Элементы.СтраницаОбщегоСписка.Заголовок = НСтр("ru='Товары другого качества с видом номенклатуры товара исходного качества';uk='Товари іншої якості з видом номенклатури товару вихідної якості'");
		КонецЕсли;
	КонецЕсли;
