...
Процедура ОбновитьСтраныМираПоКлассификатору(Знач Добавлять = Ложь) Экспорт
	...
	Для Каждого СтрокаКлассификатора Из ТаблицаКлассификатора() Цикл
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка записи страны мира %1 (код %2) при обновлении классификатора, %3';uk='Помилка запису країни світу %1 (код %2) при оновленні класифікатора, %3'"),
...
Функция СсылкаПоДаннымКлассификатора(Знач Отбор, Знач ДополнительныеДанные = Неопределено) Экспорт
	...
	Если ДанныеПоиска=Неопределено Тогда
		ВызватьИсключение НСтр("ru='Некорректный код страны мира для поиска в классификаторе';uk='Некоректний код країни світу для пошуку в класифікаторі'");
	КонецЕсли;
...
Функция ПолучитьТаблицуПоКодуЯзыкаИБ(Данные)
	...
	ТребуетсяУкраинскийСиноним = (Локализация.КодЯзыкаИнформационнойБазы() = "uk");
