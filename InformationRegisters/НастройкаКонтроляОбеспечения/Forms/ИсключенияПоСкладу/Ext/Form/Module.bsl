...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Контролировать Тогда
		Заколовок = НСтр("ru='Номенклатура без контроля обеспечения (Склад: %1)';uk='Номенклатура без контролю забезпечення (Склад: %1)'");
	Иначе
		Заколовок = НСтр("ru='Номенклатура с контролем обеспечения (Склад: %1)';uk='Номенклатура з контролем забезпечення (Склад: %1)'");
	КонецЕсли;
...
Процедура СформироватьНадпись(Строка)
	...
	Если Строка.ТипЗаписи = 0 Тогда
		Строка.Характеристика = НСтр("ru='<не используются>';uk='<не використовуються>'");
	ИначеЕсли Строка.ТипЗаписи = 1 И Строка.КоличествоИсключений = 0 Тогда
		Строка.Характеристика = НСтр("ru='для всех характеристик';uk='для всіх характеристик'");
	ИначеЕсли Строка.ТипЗаписи = 2 И Строка.КоличествоИсключений = 1 Тогда
		Строка.Характеристика =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='для %1 характеристики';uk='для %1 характеристики'"), Строка.КоличествоИсключений);
	Иначе
		Строка.Характеристика =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='для %1 характеристик';uk='для %1 характеристик'"), Строка.КоличествоИсключений);
	КонецЕсли;
