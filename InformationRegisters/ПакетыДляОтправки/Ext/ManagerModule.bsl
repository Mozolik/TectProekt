...
Процедура ЗаписатьНовыйПакет(ДатаЗаписи, JSONСтруктура, СледующийНомерПакета) Экспорт
	...
	НаборЗаписей = РегистрыСведений.ПакетыДляОтправки.СоздатьНаборЗаписей();
...
Процедура УдалитьСтарыеПакеты() Экспорт
	...
	|	РегистрСведений.ПакетыДляОтправки КАК ПакетыДляОтправки
	|";
	ПакетовДляОтправки = ЦентрМониторингаВызовСервераПовтИсп.ПолучитьПараметрыЦентраМониторинга("ПакетовДляОтправки");
	...
	Если ВсегоПакетов > ПакетовДляОтправки Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='УдалитьСтарыеПакеты';uk='УдалитьСтарыеПакеты'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
...
Процедура УдалитьПакет(НомерПакета) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ru='УдалитьПакеты';uk='УдалитьПакеты'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
КонецПроцедуры
