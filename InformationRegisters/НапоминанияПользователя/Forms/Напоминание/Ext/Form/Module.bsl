...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.СпособУстановкиВремениНапоминания = ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ОтносительноТекущегоВремени") Тогда
		СпособУстановкиВремениНапоминания = НСтр("ru='через';uk='через'") + " " + НапоминанияПользователяКлиентСервер.ПредставлениеВремени(Объект.ИнтервалВремениНапоминания);
	Иначе
...
Процедура СпособУстановкиВремениНапоминанияПриИзменении(Элемент)
	...
	Если ИнтервалВремени > 0 Тогда
		...
		СпособУстановкиВремениНапоминания = НСтр("ru='через';uk='через'") + " " + ИнтервалВремениСтрокой;
	Иначе
		Если Элементы.СпособУстановкиВремениНапоминания.СписокВыбора.НайтиПоЗначению(СпособУстановкиВремениНапоминания) = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Интервал времени не определен.';uk='Інтервал часу не визначений.'"), , "СпособУстановкиВремениНапоминания");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииИнтервалаВремени(Элемент)
	...
	Если Объект.ИнтервалВремениНапоминания > 0 Тогда
		ИнтервалВремениСтрокой = НапоминанияПользователяКлиентСервер.ПредставлениеВремени(Объект.ИнтервалВремениНапоминания);
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Интервал времени не определен';uk='Інтервал часу не визначений'"), , "ИнтервалВремениСтрокой");
	КонецЕсли;
КонецПроцедуры
...
Процедура Удалить(Команда)
	...
	КнопкиДиалога.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Удалить';uk='Вилучити'"));
	КнопкиДиалога.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru='Не удалять';uk='Не видаляти'"));
	
	ОписаниеОповещения = Новый ОписаниеОповещения("УдалитьНапоминание", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Удалить напоминание?';uk='Вилучити нагадування?'"), КнопкиДиалога);
...
Функция ПолучитьПредопределенныеСпособыОповещения()
	...
	Результат.Вставить(НСтр("ru='относительно предмета';uk='відносно предмета'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ОтносительноВремениПредмета"));
	Результат.Вставить(НСтр("ru='в указанное время';uk='у зазначений час'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.ВУказанноеВремя"));
	Результат.Вставить(НСтр("ru='периодически';uk='періодично'"), ПредопределенноеЗначение("Перечисление.СпособыУстановкиВремениНапоминания.Периодически"));
	Возврат Результат;
КонецФункции
...
Процедура ЗаполнитьСпособыОповещения()
	...
	Для Каждого Интервал Из ИнтервалыВремени Цикл
		СпособыОповещения.Добавить(НСтр("ru='через';uk='через'") + " " + Интервал);
...
Процедура ЗаполнитьВариантыПериодичности()
	...
	КонецЦикла;
	Элементы.ВариантПериодичности.СписокВыбора.Добавить("", НСтр("ru='по заданному расписанию...';uk='за заданим розкладом...'"));	
КонецПроцедуры
...
Процедура ОткрытьДиалогНастройкиРасписанияЗавершение(ВыбранноеРасписание, ДополнительныеПараметры) Экспорт
	...
	Если Не РасписаниеСоответствуетТребованиям(Расписание) Тогда 
		ПоказатьПредупреждение(, НСтр("ru='Периодичность в течение дня не поддерживается, соответствующие настройки очищены.';uk='Періодичність протягом дня не підтримується, відповідні настройки очищені.'"));
	КонецЕсли;
