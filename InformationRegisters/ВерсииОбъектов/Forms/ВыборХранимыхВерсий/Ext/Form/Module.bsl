...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВерсионированиеОбъектов.НомерПоследнейВерсии(Ссылка) = 0 Тогда
		...
		Элементы.НетВерсий.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Предыдущие версии отсутствуют: ""%1"".';uk='Попередні версії відсутні: ""%1"".'"), Строка(Ссылка));
	КонецЕсли;
...
Процедура ВыполнитьПереходНаВыбраннуюВерсию(ОтменятьПроведение = Ложь)
	...
	ИначеЕсли Результат = "ОшибкаПроведения" Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Переход на версию не был выполнен по причине:
                ...
                |;uk='Перехід на версію не був виконаний через:
                | %1
	...
	Иначе //Результат = "ВосстановлениеВыполнено"
		...
		ПоказатьОповещениеПользователя(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Восстановлена версия № %1.';uk='Відновлена версія № %1.'"), ПредставлениеНомераВерсии),
...
Процедура ЗаполнитьИерархиюВерсий(ИерархияВерсий, СписокВерсий)
	...
	Если ПропущенныеВерсии.Количество() > 0 Тогда
		Если СписокВерсий.Количество() = ПропущенныеВерсии.Количество() Тогда
			ВызватьИсключение НСтр("ru='Не удалось сформировать полный список версий. Часть вложенных версий пропущена.';uk='Не вдалося сформувати повний список версій. Частина вкладених версій пропущена.'");
		КонецЕсли;
...
Функция ИмяЭтойИнформационнойБазы()
	Возврат НСтр("ru='Эта программа';uk='Дана програма'");
КонецФункции
