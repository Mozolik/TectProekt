...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СравниваемыеВерсии.Количество() > 1 Тогда
		...
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сравнение версий ""%1"" (№№ %2)';uk='Порівняння версій ""%1"" (№№ %2)'"),
			ОбщегоНазначения.ПредметСтрокой(СсылкаНаОбъект), СтрокаНомераВерсий);
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Версия объекта ""%1"" №%2';uk='Версія об''єкта ""%1"" №%2'"),
	...
	Для Каждого текСтрока Из ТаблицаИзмененийТабличныхДокументов Цикл
		...
		Для Индекс = 0 По ВГраница Цикл
			...
			Если ТекСтрока[ИмяКолонки] = "И" Тогда
				...
				ШаблонТекста = НСтр("ru='сравнить версию №%1 с версией №%2';uk='порівняти версію №%1 з версією №%2'");
			...
			ИначеЕсли текСтрока[ИмяКолонки] = "У" Тогда
				...
				Область.Текст = НСтр("ru='Сохранение изменений табличного документа отключено';uk='Збереження змін табличного документа вимкнено'");
				
			ИначеЕсли текСтрока[ИмяКолонки] = "Д" Тогда
				Область = ТЧОтчета.Присоединить(ШаблонЯчейкаДобавлена);
				Область.Текст = НСтр("ru='открыть';uk='відкрити'");
...
Функция ВывестиШапку(ТЧОтчета, СписокВерсий, ЧислоВерсий)
	...
	ОбластьШапка.Параметры.НаименованиеОтчета = НСтр("ru='Отчет по изменениям версий объекта';uk='Звіт по змінах версій об''єкта'");
...
Функция ПолучитьОписаниеПоВерсии(НомерВерсии)
	...
	Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='№ %1 / (%2) / %3';uk='№ %1 / (%2) / %3'"), НомерВерсии.Представление, 
...
Процедура ДобавитьНомераСтрокВТабличныеЧасти(ТабличныеЧасти)
	...
	Для Каждого Соответствие Из ТабличныеЧасти Цикл
		...
		Если Таблица.Колонки.Найти("НомерСтроки") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Таблица.Колонки.Вставить(0, "НомерСтроки",,НСтр("ru='№ строки';uk='№ рядка'"));
...
Процедура ОткрытьФормуСравненияТабличныхДокументов(ИмяТабличногоДокумента, Версия0, Версия1)
	...
	ЗаголовокШаблон = НСтр("ru='Версия №%1';uk='Версія №%1'");
