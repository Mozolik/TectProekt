...
Процедура ПриОткрытии(Отказ)
	...
	ШаблонПодсказки = НСтр("ru='Комплект правил можно скачать с %1
        |или найти в %2'
        |;uk='Комплект правил можна скачати з %1
        |або знайти в %2'");
	
	ШаблонКаталогОбновлений = НСтр("ru='каталоге поставки программы ""%1""';uk='каталозі поставки програми ""%1""'");
	ШаблонКаталогОбновлений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКаталогОбновлений, ИмяПрограммы);
	
	ШаблонПользовательскийСайт = НСтр("ru='сайта поддержки пользователей платформы';uk='сайту підтримки користувачів платформи'");
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Иначе
		Если ИсточникПравилКонвертации = ПредопределенноеЗначение("Перечисление.ИсточникиПравилДляОбменаДанными.МакетКонфигурации") Тогда
            ...
            |;uk='Правила з файлу не завантажені. Закриття призведе до використання типових правил конвертації.
            |Використовувати типові правила конвертації?'");
			
			Оповещение = Новый ОписаниеОповещения("ЗакрытьФормуЗагрузкиПравил", ЭтотОбъект);
			
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("Использовать", НСтр("ru='Использовать';uk='Використовувати'"));
...
Процедура ЗагрузитьПравилаЗавершение(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если НРег(ЧастиИмени.Расширение) <> ".zip" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректный формат файла комплекта правил. Ожидается zip архив, содержащий три файла:
            ...
            |;uk='Некоректний формат файлу комплекту правил. Очікується zip архів, що містить три файли:
...
Процедура ВыполнитьЗагрузкуПравил(Знач АдресПомещенногоФайла, Знач ИмяФайла, ОписаниеОшибки = Неопределено)
	...
	Если ТипЗнч(ОписаниеОшибки) <> Тип("Булево") И ОписаниеОшибки <> Неопределено Тогда
		...
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			Кнопки.Добавить("Отмена", НСтр("ru='Закрыть';uk='Закрити'"));
		Иначе
		...
		ПараметрыФормы.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			ПараметрыФормы.Заголовок = НСтр("ru='Правила не могут быть загружены';uk='Правила не можуть бути завантажені'");
		Иначе
			ПараметрыФормы.Заголовок = НСтр("ru='Синхронизация данных может работать некорректно';uk='Синхронізація даних може працювати некоректно'");
		КонецЕсли;
...
Процедура ОбновитьИнформациюОПравилах()
	...
	Если ИсточникПравил = "ЗагруженныеИзФайла" Тогда
		ИнформацияОбИспользовании = НСтр("ru='Используются правила загруженные из файла.';uk='Використовуються правила, які завантажені з файлу.'");
	Иначе
		ИнформацияОбИспользовании = НСтр("ru='Используются типовые правила из состава конфигурации.';uk='Використовуються типові правила зі складу конфігурації.'");
	КонецЕсли;
