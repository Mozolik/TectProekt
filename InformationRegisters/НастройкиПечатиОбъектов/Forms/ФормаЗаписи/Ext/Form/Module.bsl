...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не ПраваПользователяПовтИсп.ДобавлениеИзменениеНастройкиПечатиОбъектов()
	 И ЗначениеЗаполнено(ТекущийОбъект.Партнер) Тогда
		...
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Нет прав на запись настройки печати по партнеру';uk='Немає прав на запис настройки друку за партнером'"));
	...
	Если Не ПраваПользователяПовтИсп.СохранениеНастроекПечатиОбъектовПоУмолчанию()
	 И ЗначениеЗаполнено(ТекущийОбъект.Организация) Тогда
		...
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Нет прав на запись настройки печати по организации';uk='Немає прав на запис настройки друку за організацією'"));
	...
	Если КомплектПечатныхФорм.НайтиСтроки(Новый Структура("Печатать", Истина)).Количество() = 0 Тогда
		...
		Отказ = Истина;
		ТекстСообщения = НСтр("ru='Выберите хотя бы одну печатную форму.';uk='Виберіть хоча б одну друковану форму.'");
