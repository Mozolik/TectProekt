...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ru='Настройка синхронизации файлов:';uk='Настройка синхронізації файлів:'")
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ОбъектСинхронизации = "ТолькоФайлыЭлемента" И Не ЗначениеЗаполнено(ЭлементСправочника) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не заполнен объект с присоединенными файлами.';uk='Не заповнений об''єкт з приєднаними файлами.'"),
			,
			"ЭлементСправочника");
	КонецЕсли;
...
Процедура ДобавитьВОтборИнтервалИсключение(ВыбранноеЗначение)
	...
	ТекстПредставления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Очищать спустя %1 дней относительно даты (%2)';uk='Очищати через %1 днів відносно дати (%2)'"), 
