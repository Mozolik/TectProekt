...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(ИсточникДействия) = Тип("СправочникСсылка.УсловияПредоставленияСкидокНаценок") Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Номенклатура, по которой проверяется условие ""%1""';uk='Номенклатура, по якій перевіряється умова ""%1""'"), ИсточникДействия);
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Номенклатура, на которую предоставляется скидка (наценка) ""%1""';uk='Номенклатура, на яку надається знижка (націнка) ""%1""'"), ИсточникДействия);
	КонецЕсли;
