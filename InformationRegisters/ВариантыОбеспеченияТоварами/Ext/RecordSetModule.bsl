...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ТекстСообщения = НСтр("ru='Источник обеспечения, указанный в способе обеспечения потребностей, для типа обеспечения ""Перемещение"" должен отличаться от склада, для которого опеределяется вариант обеспечения товарами.';uk='Джерело забезпечення, зазначене у способі забезпечення потреб, для типу забезпечення ""Переміщення"" повинно відрізнятися від складу, для якого визначається варіант забезпечення товарами.'");
