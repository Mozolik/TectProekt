	...
	Если ОписаниеПрав = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка в процедуре РегистрыСведений.НастройкиПравОбъектов.Прочитать()
                       ...
                       |;uk='Помилка в процедурі РегистрыСведений.НастройкиПравОбъектов.Прочитать()
                       |
                       |Невірне значення параметра СсылкаНаОбъект ""%1"".
                       |Для об''єктів таблиці ""%2"" права не настроюються.'"),
		...
		Если Настройки.Колонки.Найти(Строка.Право) = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка в процедуре РегистрыСведений.НастройкиПравОбъектов.Прочитать()
                           ...
                           |;uk='Помилка в процедурі РегистрыСведений.НастройкиПравОбъектов.Прочитать()
                           |
                           |Для об''єктів таблиці ""%1""
                           |право ""%2"" не настроюється, однак воно записане
                           |у регістрі відомостей НастройкиПравОбъектов для
                           |об''єкта ""%3"".
                           |
                           |Можливо, оновлення інформаційної бази
                           |не виконане або виконане з помилкою.
                           |Потрібно виправити дані регістру.'"),
				СсылкаНаОбъект.Метаданные().ПолноеИмя(),
				Строка.Право,
				Строка(СсылкаНаОбъект));
		КонецЕсли;
		...
		Если ПустыеСсылкиВладелецПрав.Получить(ТипВладельцаПрав) = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка в процедуре ОбновитьВспомогательныеДанныеРегистра
                           ...
                           |;uk='Помилка в процедурі ОбновитьВспомогательныеДанныеРегистра
                           |модуля менеджера регістра відомостей НастройкиПравОбъектов.
                           |
                           |Тип власників прав ""%1"" не вказаний у вимірюванні Об''єкт.'"),
				ТипВладельцаПрав);
		КонецЕсли;
...
Функция ВозможныеПрава()
	...
	ЗаголовокОшибки =
		НСтр("ru='Ошибка в процедуре ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов
                   ...
                   |;uk='Помилка в процедурі ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов
	...
	Для каждого ВозможноеПраво Из ВозможныеПрава Цикл
		...
		Если ОбъектМетаданныхВладельца = Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не найден владелец прав ""%1"".';uk='Не знайдено власник прав ""%1"".'"),
			...
			Если ОпределяемыйТипВладельцевПрав.Получить(ТипСсылки) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Тип владельца прав ""%1""
                               ...
                               |;uk='Тип власника прав ""%1""
                               |не вказано в типі ""Власник настройок прав"", що визначається.'"),
					Строка(ТипСсылки));
			КонецЕсли;
			...
			Если (ТипыПодпискиЗаписатьЗависимыеНаборыЗначенийДоступа.Получить(ТипОбъекта) <> Неопределено
				...
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Тип владельца прав ""%1""
                               ...
                               |;uk='Тип власника прав ""%1""
                               |не вказано у типі ""Значення доступу"", що визначається,
                               |але використовується для заповнення наборів значень доступу,
                               |так як вказаний в одній з підписок на подію:
                               |- ЗаписатьЗависимыеНаборыЗначенийДоступа*,
                               |- ЗаписатьНаборыЗначенийДоступа*.
                               |Потрібно вказати тип у типі ""Значення доступу"", що визначається
                               |для коректного заповнення регістра НаборыЗначенийДоступа.'"),
					Строка(ТипСсылки));
			КонецЕсли;
			...
			Если СвойстваВидовДоступа.ПоТипамЗначений.Получить(ТипСсылки) <> Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Тип владельца прав ""%1""
                               ...
                               |;uk='Тип власника прав ""%1""
                               |не може використовуватися, як тип значень доступу,
                               |але виявлений в описі виду доступу ""%2"".'"),
					Строка(ТипСсылки),
			...
			Если СвойстваВидовДоступа.ПоТипамГруппИЗначений.Получить(ТипСсылки) <> Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Тип владельца прав ""%1""
                               ...
                               |но обнаружен в описании вида доступа ""%2"".'
                               |;uk='Тип власника прав ""%1""
                               |не може використовуватися, як тип груп значень доступу,
                               |але виявлений в описі виду доступу ""%2"".'"),
					Строка(ТипСсылки),
					СвойстваВидовДоступа.ПоТипамЗначений.Получить(ТипСсылки).Имя);
			КонецЕсли;
			...
			Если ТипыПодпискиОбновитьГруппыВладельцевНастроекПрав.Получить(ТипОбъекта) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Тип владельца прав ""%1""
                               ...
                               |;uk='Тип власника прав ""%1""
                               |не вказано в аналізованому типі ""Власник настройок прав об''єкт"".'"),
					Строка(ТипОбъекта));
			КонецЕсли;
		...
		Если ПраваВладельца.Получить(ВозможноеПраво.Имя) <> Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для владельца прав ""%1""
                           ...
                           |;uk='Для власника прав ""%1""
                           |повторно визначено право ""%2"".'"),
				ВозможноеПраво.ВладелецПрав,
				ВозможноеПраво.Имя);
		КонецЕсли;
				...
				Если Свойство = "ЧтениеВТаблицах" Тогда
					ОписаниеОшибки =
						НСтр("ru='Для владельца прав ""%1""
                                   ...
                                   |;uk='Для власника прав ""%1""
                                   |для права ""%2"" в таблицях для читання вказаний символ ""*"".
                                   |У цьому випадку окремих таблиць вказувати не потрібно.'")
				Иначе
					ОписаниеОшибки =
						НСтр("ru='Для владельца прав ""%1""
                                   ...
                                   |В этом случае отдельных таблиц указывать не нужно.'
                                   |;uk='Для власника прав ""%1""
                                   |для права ""%2"" в таблицях для зміни вказаний символ ""*"".
                                   |У цьому випадку окремих таблиць вказувати не потрібно.'")
				КонецЕсли;
				...
				Если Свойство = "ЧтениеВТаблицах" Тогда
					ОписаниеОшибки =
						НСтр("ru='Для владельца прав ""%1""
                                   ...
                                   |;uk='Для власника прав ""%1""
                                   |для права ""%2"" в таблицях для читання вказаний символ ""*"".
                                   |Однак символ ""*"" вже зазначено в таблицях для читання для права ""%3"".'")
				Иначе
					ОписаниеОшибки =
						НСтр("ru='Для владельца прав ""%1""
                                   |для права ""%2"" в таблицах для изменения указан символ ""*"".
                                   ...
                                   |;uk='Для власника прав ""%1""
                                   |для права ""%2"" в таблицях, для зміни вказаний символ ""*"".
                                   |Однак символ ""*"" вже зазначено в таблицях, для зміни права ""%3"".'")
				КонецЕсли;
		...
		ИначеЕсли Свойство = "ЧтениеВТаблицах" Тогда
			ОписаниеОшибки =
				НСтр("ru='Для владельца прав ""%1""
                           ...
                           |;uk='Для власника прав ""%1""
                           |для права ""%2"" вказана конкретна таблиця для читання ""%3"".
                           |Однак це не має сенсу, оскільки право Читання може залежати тільки від права Читання.
                           |Має сенс використовувати тільки символ ""*"".'");
		...
		ИначеЕсли Метаданные.НайтиПоПолномуИмени(Значение) = Неопределено Тогда
			...
			Если Свойство = "ЧтениеВТаблицах" Тогда
				ОписаниеОшибки = НСтр("ru='Для владельца прав ""%1""
                                            ...
                                            |;uk='Для власника прав ""%1""
                                            |для права ""%2"" не знайдена таблиця для читання ""%3"".'")
			Иначе
				ОписаниеОшибки = НСтр("ru='Для владельца прав ""%1""
                                            |для права ""%2"" не найдена таблица для изменения ""%3"".'
                                            |;uk='Для власника прав ""%1""
                                            |для права ""%2"" не знайдена таблиця для зміни ""%3"".'")
			КонецЕсли;
