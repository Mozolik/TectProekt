...
Процедура ОбновитьНадписьПояснение()
	...
	Если Запись.НазначеноПользователем Тогда
		Если Запись.НормативноеКоличествоЗапаса = 0 Тогда
			ТекстНадписи = НСтр("ru='Упаковка не участвует в подпитке, т.к. указано нулевое нормативное значение.';uk='Упаковка не бере участь у підживленні, бо зазначено нульове нормативне значення.'");
		Иначе
			ТекстНадписи = НСтр("ru='Упаковка участвует в подпитке, т.к. нормативное значение указано вручную.';uk='Упаковка бере участь у підживленні, бо нормативне значення зазначене вручну.'");
		КонецЕсли;
	ИначеЕсли Не ЗначениеЗаполнено(Запись.Класс) Тогда
		ТекстНадписи = НСтр("ru='Упаковка не участвует в подпитке, т.к. не рассчитаны параметры прогноза.';uk='Упаковка не бере участь у підживленні, бо не розраховані параметри прогнозу.'");
	ИначеЕсли Запись.ВероятностьОтгрузкиВТечениеДня < МинимальнаяВероятностьОтгрузки Тогда
		ТекстНадписи = НСтр("ru='Упаковка не участвует в подпитке, т.к. вероятность отгрузки в течение дня меньше минимальной (%МинимальнаяВероятность%%), заданной в настройках подпитки.';uk='Упаковка не бере участь у підживленні, бо ймовірність відвантаження протягом дня менше мінімальної (%МинимальнаяВероятность%%), заданої в настройках підживлення.'");
		ТекстНадписи = СтрЗаменить(ТекстНадписи,"%МинимальнаяВероятность%", Строка(МинимальнаяВероятностьОтгрузки));
	Иначе
		ТекстНадписи = НСтр("ru='Упаковка участвует в подпитке, т.к. вероятность отгрузки в течение дня больше минимальной (%МинимальнаяВероятность%%), заданной в настройках подпитки.';uk='Упаковка бере участь у підживленні, бо ймовірність відвантаження протягом дня більше мінімальної (%МинимальнаяВероятность%%), заданої в настройках підживлення.'");
		ТекстНадписи = СтрЗаменить(ТекстНадписи,"%МинимальнаяВероятность%", Строка(МинимальнаяВероятностьОтгрузки));
	КонецЕсли;		
