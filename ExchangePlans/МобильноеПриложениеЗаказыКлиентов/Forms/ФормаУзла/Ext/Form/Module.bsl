...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Булево(РегистрироватьОплатыПоПлатежнымКартам)
		...
		ТекстСообщения =
			НСтр("ru='Укажите эквайринговый терминал для создания документов ""Эквайринговая операция"".';uk='Вкажіть еквайринговий термінал для створення документів ""Еквайрингова операція"".'");
	...
	Если ИспользоватьЗаданияДляУправленияТорговымиПредставителями
		...
		ТекстСообщения =
			НСтр("ru='Укажите куратора для торгового представителя.';uk='Вкажіть куратора для торгового представника.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
	...
	Если Булево(ИспользоватьПКОДляРегистрацииОплаты) И ИспользоватьНесколькоКасс
		...
		ТекстСообщения =
			НСтр("ru='Укажите кассу для создания документов ""Приходный кассовый ордер"".';uk='Вкажіть касу для створення документів ""Прибутковий касовий ордер"".'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура ПриЧтенииСозданииНаСервере(НастройкиОбмена = Неопределено)
	...
	Заголовок = НСтр("ru='Настройка синхронизации с устройством';uk='Настройка синхронізації з пристроєм'");
	Если НЕ ЗначениеЗаполнено(Объект.ВидНастройкиОбмена) Тогда
		ИспользованиеОбщихНастроек = 0;
	Иначе
		ИспользованиеОбщихНастроек = 1;
...
Процедура ПроверитьПользователяМобильногоПриложения(Отказ = Ложь, ТекстСообщения = "")
	...
	Если НЕ ПроверкаПользователя.ПользовательМобильногоПриложения Тогда
		...
		+ НСтр("ru='Пользователь не входит в группу пользователей мобильного приложения.';uk='Користувач не входить в групу користувачів мобільного додатку.'") + Символы.ПС;
		Отказ = Истина;
	КонецЕсли;
	Если ИспользоватьПКОДляРегистрацииОплаты И Не ПроверкаПользователя.Касса Тогда
		ТекстСообщения = ТекстСообщения
		+ НСтр("ru='У пользователя недостаточно прав для создания документов ""Приходный кассовый ордер"".';uk='У вас недостатньо прав для створення документів ""Прибутковий касовий ордер"".'") + Символы.ПС;
		Отказ = Истина;
	КонецЕсли;
	Если ЗначениеЗаполнено(ЭквайринговыйТерминал) И Не ПроверкаПользователя.ПлатежнаяКарта Тогда
		ТекстСообщения = ТекстСообщения
		+ НСтр("ru='У пользователя недостаточно прав для создания документов ""Эквайринговая операция"".';uk='У вас недостатньо прав для створення документів ""Еквайрингова операція"".'");
		Отказ = Истина;
	КонецЕсли;
	Если Отказ Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
...
Процедура УстановитьВидимостьДоступность()
	...
	Элементы.ФормаОтвязатьУстройство.Видимость = ЗначениеЗаполнено(Объект.Ссылка) И НЕ СтатусОбмена = 0;
	
	ДоступностьЗаполненияОбщихНастроек();
	
	Если СтатусОбмена = 0 Тогда
		СтатусОбменаНадпись = НСтр("ru='Ожидается первая синхронизация данных';uk='Очікується перша синхронізація даних'");
	ИначеЕсли СтатусОбмена = 1 Тогда
		СтатусОбменаНадпись = НСтр("ru='Синхронизация успешно завершена в %ДатаОбмена%';uk='Синхронізація успішно завершена в %ДатаОбмена%'");
	ИначеЕсли СтатусОбмена = 2 Тогда
		СтатусОбменаНадпись = НСтр("ru='Синхронизация завершена с ошибками в %ДатаОбмена%';uk='Синхронізація завершена з помилками в %ДатаОбмена%'");
	КонецЕсли;
	СтатусОбменаНадпись = СтрЗаменить(СтатусОбменаНадпись, "%ДатаОбмена%", Формат(ДатаОбмена, "ДЛФ=DT"));
	Элементы.СтатусОбмена.Видимость = ЗначениеЗаполнено(Объект.Ссылка);
	Элементы.СтатусОбменаНадпись.Видимость = ЗначениеЗаполнено(Объект.Ссылка);
