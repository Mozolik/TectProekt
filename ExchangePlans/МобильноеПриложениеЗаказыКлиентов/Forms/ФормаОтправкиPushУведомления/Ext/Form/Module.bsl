...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если УказаныПолучатели Тогда
		Элементы.ТекстPushУведомления.Заголовок = НСтр("ru='Введите текст push-уведомления';uk='Введіть текст push-повідомлення'");
	Иначе
		Элементы.ТекстPushУведомления.Заголовок = НСтр("ru='Шаг 1 из 2. Введите текст push-уведомления';uk='Крок 1 з 2. Введіть текст push-повідомлення'");
	...
	Заголовок = НСтр("ru='Push - уведомление';uk='Push - повідомлення'");
...
Процедура АдресатыПолучательВыбранПриИзменении(Элемент)
	...
	Если НЕ Элемент.Родитель.ТекущиеДанные.ВозможнаОтправка И Элемент.Родитель.ТекущиеДанные.ПолучательВыбран Тогда
		...
		ЗаголовокВопроса = НСтр("ru='Получатель push-уведомления';uk='Одержувач push-повідомлення'");
		ТекстВопроса = НСтр("ru='Отправка push-уведомления на данное устройство не возможна. Отсутствует информация об идентификаторе устройства.
            ...
            |;uk='Відправка push-повідомлення на ваш пристрій не можлива. Відсутня інформація про ідентифікатор пристрою.
            |Все одно включити цей пристрій в список одержувачів повідомлення?'");
		СписокКнопок = Новый СписокЗначений;
		СписокКнопок.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru='Не включать';uk='Не включати'"));
		СписокКнопок.Добавить(КодВозвратаДиалога.ОК, НСтр("ru='Включить';uk='Включити'"));
...
Процедура ЗаголовкиКомандФормы()
	...
	Если Элементы.СтраницыОсновные.ТекущаяСтраница = Элементы.СтраницаТекстУведомления Тогда
		Если УказаныПолучатели Тогда
			Элементы.Далее.Заголовок = НСтр("ru='Отправить';uk='Відправити'");
		Иначе
...
Процедура ОтправитьУведомление(Успешно)
	...
	Если Получатели.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите получателей push-уведомления';uk='Вкажіть одержувачів push-повідомлення'"));
		Возврат;
	КонецЕсли;
	...
	Если ПолучателиБезДоставки.Количество() > 0 Тогда
		...
		Для каждого ПолучательБезДоставки Из ПолучателиБезДоставки Цикл
			СтрокаСообщения = НСтр("ru='Уведомление не было отправлено на устройство: %Устройство%';uk='Повідомлення не надіслано на пристрій: %Устройство%'");
...
Процедура ТекстPushУведомленияИзменениеТекста(Форма, ТекстРедактирования)
	...
	Если НЕ Форма.КоличествоСимволов = СтрДлина(ТекстРедактирования) ИЛИ Форма.КоличествоСимволов = 0 Тогда
		...
		ТекстУведомления = "";
		ТекстЗаголовкаДекорации = НСтр("ru='%Символов% из 127 символов';uk='%Символов% з 127 символів'");
...
Функция УсловияПереходаПоСтраницамНеВыполнены(Команда)
	...
	Если Элементы.СтраницыОсновные.ТекущаяСтраница = Элементы.СтраницаТекстУведомления Тогда
		Если НЕ ЗначениеЗаполнено(ТекстPushУведомления) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Введите текст push-уведомления';uk='Введіть текст push-повідомлення'"),, "ТекстPushУведомления", "ТекстPushУведомления");
			Возврат Истина;
		КонецЕсли;
	ИначеЕсли Элементы.СтраницыОсновные.ТекущаяСтраница = Элементы.СтраницаАдресатыУведомления Тогда
		Если НЕ Команда.Имя = "Назад" Тогда
			КОтправке = АдресатыPush.НайтиСтроки(Новый Структура("ПолучательВыбран", Истина));
			Если КОтправке.Количество() = 0 Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Укажите получателей push-уведомления';uk='Вкажіть одержувачів push-повідомлення'"),, "Адресаты", "АдресатыPush");
				Возврат Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
