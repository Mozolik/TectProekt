...
Процедура ПеренестиБезналичныеДенежныеСредства(Параметры) Экспорт
	...
	Пока ВыборкаПоРегистраторам.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось обработать движения документа %Документ% по регистру ""Денежные средства (безналичные)"". Причина: %Причина%';uk='Не вдалося обробити руху документа %Документ% регістру ""Грошові кошти (безготівкові)"". Причина: %Причина%'");
