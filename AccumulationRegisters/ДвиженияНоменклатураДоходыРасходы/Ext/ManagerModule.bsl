...
Процедура КорректироватьДвиженияОтчетаКомиcсионераОСписании(Параметры) Экспорт
    ...
    Пока ВыборкаПоДокументу.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Не удалось обработать движения документа %Документ% по регистру ""Движения Номенклатура - Доходы/Расходы"". Причина: %Причина%';uk='Не вдалося обробити руху документа %Документ% регістру ""Руху Номенклатура - Доходи/Витрати"". Причина: %Причина%'");
