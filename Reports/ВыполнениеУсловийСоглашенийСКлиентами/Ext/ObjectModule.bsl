...
Процедура ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Свойство("ПараметрКоманды")
		...
		Если Параметры.ПараметрыОтчет.ДополнительныеПараметры.ИмяКоманды = "ВыполнениеУсловийСоглашенийСКлиентами" Тогда
			ЭтаФорма.ФормаПараметры.Отбор.Вставить("Соглашение", Параметры.ПараметрКоманды);
		ИначеЕсли Параметры.ПараметрыОтчет.ДополнительныеПараметры.ИмяКоманды = "ВыполнениеУсловийСоглашенийСКлиентамиПоПартнеру" Тогда
	...
	Если ПараметрСоглашение <> Неопределено И ПараметрПартнер <> Неопределено И ПараметрВалюта <> Неопределено Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ЕстьОшибкиСтатус Тогда
				...
				ТекстОшибки = НСтр("ru='Соглашение с клиентом ""%Соглашение%"" должно быть в статусе ""Действует"".';uk='Оферта клієнту ""%Соглашение%"" має бути в статусі ""Діє"".'");
			...
			Если Выборка.ЕстьОшибкиРегулярное Тогда
				...
				ТекстОшибки = НСтр("ru='Соглашение с клиентом ""%Соглашение%"" должно быть регулярным.';uk='Оферта клієнту ""%Соглашение%"" має бути регулярною.'");
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Соглашение%", Выборка.Соглашение);
			...
			Если Выборка.ЕстьОшибкиТиповое Тогда
				...
				ТекстОшибки = НСтр("ru='Соглашение с клиентом %Соглашение% должно быть индивидуальным.';uk='Оферта клієнту %Соглашение% має бути індивідуальною.'");
					ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Соглашение%", Выборка.Соглашение);
