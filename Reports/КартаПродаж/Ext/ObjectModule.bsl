...
Процедура ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка) Экспорт
	...
	МасштабГеографическойСхемы = Новый РеквизитФормы("МасштабСхемы", ОписаниеТипов, "", НСтр("ru='Масштаб';uk='Масштаб'"));
	ШиринаГеографическойСхемы = Новый РеквизитФормы("ШиринаСхемы", ОписаниеТипов, "", НСтр("ru='Ширина';uk='Ширина'"));
	ВысотаГеографическойСхемы = Новый РеквизитФормы("ВысотаСхемы", ОписаниеТипов, "", НСтр("ru='Высота';uk='Висота'"));
...
Процедура ВывестиСхему(ДокументРезультат, ТаблицаДанных)
	...
	Если СтруктураГеоСхемы = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Для формирования отчета необходимо выбрать карту в разделе администрирования ""CRM и продажи"".';uk='Для формування звіту необхідно вибрати карту в розділі адміністрування ""CRM і продажі"".'");
	Иначе
...
Процедура ЗаполнитьСхему(Схема, ТаблицаДанных)
	...
	СерияВыручкаЗаПериодСравнения.Текст = НСтр("ru='Выручка за период сравнения';uk='Виторг за період порівняння'");
	...
	СерияИзменениеВыручки.Текст = НСтр("ru='Изменение выручки, %';uk='Зміна виторгу, %'");
	...
	Если ПараметрВариантАнализа.Значение = 0 Тогда // Выручка за период отчета
		...
		ЭлементЛегенды.ТекстПодписи = НСтр("ru='Выручка, грн.';uk='Виторг, грн.'");
		...
		Если ТипЗнч(ТаблицаГраницИнтервалов) = Тип("ТаблицаЗначений") Тогда
			...
			ЭлементЛегенды.ТекстПодписи = НСтр("ru='менее';uk='менше'") + " " + Строка(ТаблицаГраницИнтервалов[0].Значение);
			...
			Для инд = 2 по КоличествоОтсечекИнтервалов Цикл
				...
				ЭлементЛегенды.ТекстПодписи = НСтр("ru='от';uk='від'") + " " + Строка(ТаблицаГраницИнтервалов[инд-2].Значение)
					 + " " + НСтр("ru='до';uk='до'") + " " + Строка(ТаблицаГраницИнтервалов[инд-1].Значение);
			КонецЦикла;
			...
			Если КоличествоОтсечекИнтервалов >= 1 Тогда
				...
				ЭлементЛегенды.Картинка = БиблиотекаКартинок[НазваниеКартинки];
				ЭлементЛегенды.ТекстПодписи = НСтр("ru='более';uk='більш'") + " " + Строка(ТаблицаГраницИнтервалов[КоличествоОтсечекИнтервалов-1].Значение);
			КонецЕсли;
		...
		ИначеЕсли ТипЗнч(ТаблицаГраницИнтервалов) = Тип("Число") Тогда // Есть всего одни регион
			...
			ЭлементЛегенды.Серия = СерияВыручка;
			ЭлементЛегенды.Картинка = БиблиотекаКартинок.ГрадацияВыручки1;
			ЭлементЛегенды.ТекстПодписи = НСтр("ru='равна';uk='дорівнює'") + " " + Строка(ТаблицаГраницИнтервалов);
	...
	ИначеЕсли ПараметрВариантАнализа.Значение = 1 Тогда // Изменение выручки относительно предыдущего периода
		...
		ЭлементЛегенды.ТекстПодписи = НСтр("ru='Изменение выручки, %';uk='Зміна виторгу, %'");
		...
		Если ЗначениеЗаполнено(ПараметрВыделениеЛидеровАутсайдеров.Значение) Тогда
			...
			ЭлементЛегенды.ТекстПодписи = НСтр("ru='лидеры роста';uk='лідери зростання'");
		КонецЕсли;
		
		ЭлементЛегенды = Схема.ОбластьЛегенды.Элементы.Добавить();
		ЭлементЛегенды.Серия = СерияИзменениеВыручки;
		ЭлементЛегенды.Картинка = БиблиотекаКартинок.ГрадацияИзмененияВыручки5;
		ЭлементЛегенды.ТекстПодписи = НСтр("ru='рост выше среднего (более';uk='зростання вище середнього (більше'") + " " + СтрокаСреднийРост + " %)";
		
		ЭлементЛегенды = Схема.ОбластьЛегенды.Элементы.Добавить();
		ЭлементЛегенды.Серия = СерияИзменениеВыручки;
		ЭлементЛегенды.Картинка = БиблиотекаКартинок.ГрадацияИзмененияВыручки3;
		ЭлементЛегенды.ТекстПодписи = НСтр("ru='рост ниже среднего (до';uk='зріст нижче середнього (до'") + " " + СтрокаСреднийРост + " %)";
		
		ЭлементЛегенды = Схема.ОбластьЛегенды.Элементы.Добавить();
		ЭлементЛегенды.Серия = СерияИзменениеВыручки;
		ЭлементЛегенды.Картинка = БиблиотекаКартинок.ГрадацияИзмененияВыручки1;
		ЭлементЛегенды.ТекстПодписи = НСтр("ru='падение';uk='падіння'");
		
		Если ЗначениеЗаполнено(ПараметрВыделениеЛидеровАутсайдеров.Значение) Тогда
			ЭлементЛегенды = Схема.ОбластьЛегенды.Элементы.Добавить();
			ЭлементЛегенды.Серия = СерияИзменениеВыручки;
			ЭлементЛегенды.Картинка = БиблиотекаКартинок.ГрадацияИзмененияВыручки2;
			ЭлементЛегенды.ТекстПодписи = НСтр("ru='аутсайдеры по падению';uk='аутсайдери з падіння'");
		КонецЕсли;
			...
			Если Регион <> Неопределено Тогда
				...
				Если ПараметрВариантАнализа.Значение = 0 Тогда
					...
					Если ИтогПоРегиону.КоэффициентНасыщенности > 0 Тогда
						...
						Регион.Цвет = Новый Цвет(R, G, B);
					КонецЕсли;
					
				ИначеЕсли ПараметрВариантАнализа.Значение = 1 Тогда // Изменение выручки относительно предыдущего периода
					
					Если ИтогПоРегиону.ЕстьПадение Тогда
						МониторингЦелевыхПоказателей.ПолучитьRGBпоHSV(0, ИтогПоРегиону.КоэффициентНасыщенности, 100, R, G, B);
						Регион.Цвет = Новый Цвет(R, G, B);
					ИначеЕсли ИтогПоРегиону.ЕстьРост Тогда
						МониторингЦелевыхПоказателей.ПолучитьRGBпоHSV(120, ИтогПоРегиону.КоэффициентНасыщенности, 100, R, G, B);
						Регион.Цвет = Новый Цвет(R, G, B);
					ИначеЕсли ИтогПоРегиону.ЕстьРостНижеСреднего Тогда
						МониторингЦелевыхПоказателей.ПолучитьRGBпоHSV(60, ИтогПоРегиону.КоэффициентНасыщенности, 100, R, G, B);
						Регион.Цвет = Новый Цвет(R, G, B);
					КонецЕсли;
