...
Процедура ВывестиДанныеВТаблицу(Результат, СтруктураФормированияОтчета, ТаблицаОтчета)
	...
	Если СтруктураФормированияОтчета.ГруппироватьПоПартнеру Тогда
		...
		ОбластьЗаголовок.Параметры.ТекстЗаголовка = НСтр("ru='Контактная информация контактных лиц';uk='Контактна інформація контактних осіб'");
	...
	Иначе
		...
		ОбластьЗаголовок.Параметры.ТекстЗаголовка = НСтр("ru='Контактная информация по партнерам';uk='Контактна інформація за партнерами'");
		ТаблицаОтчета.Вывести(ОбластьЗаголовок);
		ТаблицаОтчета.Вывести(ОбластьПустаяСтрока);
		ТаблицаОтчета.НачатьГруппуСтрок("Отбор");
		ТаблицаОтчета.Вывести(ОбластьОтбор,2);
...
Процедура ВывестиШапкуОтчета(СтруктураФормированияОтчета, ТаблицаОтчета, Макет)
	...
	ОбластьГруппировкаШапкаРеквизитПартнера.Параметры.ЗаголовокДанныеПартнера =НСтр("ru='Данные партнера';uk='Дані партнера'");
	...
	ОбластьГруппировкаШапкаРеквизитПартнера.Области.ЗаголовокДанныеПартнера.ГраницаСправа = ГраницаСплошнаяЛиния;
	ОбластьГруппировкаШапкаРеквизитПартнера.Параметры.ЗаголовокДанныеПартнера =НСтр("ru='Рабочее наименование';uk='Робоче найменування'");
	ТаблицаОтчета.Вывести(ОбластьГруппировкаШапкаРеквизитПартнера);
		...
		Для каждого Элемент Из МассивВыбранныхПолейПартнера Цикл
			...
			Если Элемент <> "Классификация" Тогда
				Если Элемент = "ДатаРегистрации" Тогда
					ОбластьСтрокаРеквизитПартнера.Параметры.Значение = Формат(ВыборкаПартнеры[Элемент],"ДФ=dd.MM.yyyy");
				Иначе
	...
	Если КоличествоКолонок > 0 Тогда
		Пока НомерВыводимойКолонки < КоличествоКолонок + 1 Цикл
			...
			Если НомерВыводимойКолонки = 1 Тогда 
				ОбластьШапкаКЛ.Параметры.Значение = НСтр("ru='Контактная информация';uk='Контактна інформація'");
			Иначе
...
Процедура ДобавитьВЗапросДанныеКлассификации(Запрос,СтруктураФормированияОтчета)
	...
	Если СтруктураФормированияОтчета.ГруппироватьПоПартнеру ИЛИ 
		НЕ (ПравоДоступа("Просмотр", Метаданные.РегистрыСведений.ABCXYZКлассификацияКлиентов)) Тогда 
	...
	Если ЕстьВыводКлассификации Тогда
		...
		Запрос.УстановитьПараметр("НеКлассифицирован",НСтр("ru='Не классифицирован';uk='Не класифікований'"));
...
Функция ТекстЗапросаВыбранныеПоляПартнера(ВыбранныеПоляПартнера)
	...
	Для каждого Строка Из ВыбранныеПоляПартнера Цикл
		...
		Если ТипЗнч(Строка.Значение) = Тип("СправочникСсылка.ВидыКонтактнойИнформации") ИЛИ 
			Строка.Значение = "Классификация" Тогда
			Прервать;
		КонецЕсли;
