...
Процедура ЗаполнитьТаблицыВыбранныхПолей()
	...
	НоваяСтрокаТаблицы(ВыбранныеПоляПартнера,НСтр("ru='Публичное наименование';uk='Публічне найменування'"),НСтр("ru='НаименованиеПолное';uk='НаименованиеПолное'"));
	Если ПолучитьФункциональнуюОпцию("ИспользоватьБизнесРегионы") Тогда
		 НоваяСтрокаТаблицы(ВыбранныеПоляПартнера,НСтр("ru='Бизнес-регион';uk='Бізнес-регіон'"),НСтр("ru='БизнесРегион';uk='БизнесРегион'"));
	КонецЕсли;
	НоваяСтрокаТаблицы(ВыбранныеПоляПартнера,НСтр("ru='Основной менеджер';uk='Основний менеджер'"),НСтр("ru='ОсновнойМенеджер';uk='ОсновнойМенеджер'"));
	НоваяСтрокаТаблицы(ВыбранныеПоляПартнера,НСтр("ru='Дата регистрации';uk='Дата реєстрації'"),НСтр("ru='ДатаРегистрации';uk='ДатаРегистрации'"));
	Если ПравоДоступа("Просмотр", Метаданные.РегистрыСведений.ABCXYZКлассификацияКлиентов) Тогда
		НоваяСтрокаТаблицы(ВыбранныеПоляПартнера,НСтр("ru='ABC\XYZ классификация';uk='ABC\XYZ класифікація'"),НСтр("ru='Классификация';uk='Класифікація'"));
	КонецЕсли;
