...
Процедура СформироватьОтчет(ТаблицаОтчета,Опрос,ВидОтчета) Экспорт
	...
	Если ВидОтчета = НСтр("ru='АнализОтветов';uk='АнализОтветов'") Тогда
		
		СформироватьОтчетАнализОтветов(ТаблицаОтчета,Опрос);
		
	ИначеЕсли ВидОтчета = НСтр("ru='АнализОтветивших';uk='АнализОтветивших'") Тогда
...
Процедура ВывестиОтветВариантыОтвета(ТаблицаОтчета,СтрокаДерева,Макет,ПолныйКод)
	...
	Если СтрокаДерева.Строки[0].ТипОтвета = Перечисления.ТипыОтветовНаВопрос.Булево Тогда
		...
		Область.Параметры.ВариантОтвета = НСтр("ru='Да';uk='Так'");
		...
		Область = Макет.ПолучитьОбласть("ВариантыОтветов");
		Область.Параметры.ВариантОтвета = НСтр("ru='Нет';uk='Ні'");
...
Функция ВыполнитьЗапросПоОтветившимОпросСОпределеннымСоставомРеспондентов(Опрос)
	...
	Запрос.УстановитьПараметр("Опрос",Опрос);
	Запрос.УстановитьПараметр("Заполнил",НСтр("ru='Заполнил';uk='Заповнив'"));
	Запрос.УстановитьПараметр("НачалЗаполнять",НСтр("ru='Начал заполнять';uk='Почав заповнювати'"));
	Запрос.УстановитьПараметр("НеЗаполнял",НСтр("ru='Не заполнял';uk='Не заповнював'"));
...
Функция ПолучитьПредставлениеОпросаДляЗаголовка(РеквизитыОпрос)
	...
	ПредставлениеОпросаДляЗаголовка =  НСтр("ru='Опрос';uk='Опитування'") + " ";
	Если РеквизитыОпрос.ДатаНачала <> Дата(1,1,1) ИЛИ РеквизитыОпрос.ДатаОкончания <> Дата(1,1,1) Тогда
		ПредставлениеОпросаДляЗаголовка = ПредставлениеОпросаДляЗаголовка + НСтр("ru='проводился в период';uk='проводився в період'") + " ";
	КонецЕсли;
	
	Если РеквизитыОпрос.ДатаНачала <> Дата(1,1,1) Тогда
		ПредставлениеОпросаДляЗаголовка = ПредставлениеОпросаДляЗаголовка + НСтр("ru='с';uk='з'") + " " + Формат(РеквизитыОпрос.ДатаНачала,"ДФ=dd.MM.yy") + " ";
	КонецЕсли;
	
	Если РеквизитыОпрос.ДатаОкончания <> Дата(1,1,1) Тогда
		ПредставлениеОпросаДляЗаголовка = ПредставлениеОпросаДляЗаголовка + НСтр("ru='по';uk='по'") + " " + Формат(РеквизитыОпрос.ДатаОкончания,"ДФ=dd.MM.yy")+ " ";
	КонецЕсли; 
	
	ПредставлениеОпросаДляЗаголовка = ПредставлениеОпросаДляЗаголовка + НСтр("ru='на основании документа';uk='на підставі документа'") + " " + РеквизитыОпрос.Представление + ".";
