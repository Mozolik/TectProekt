...
Функция ТекстЗапросаОПредоставленныхРучныхСкидках(СоответствиеЗапросыДанные,НомерПоследнегоПакета, СтруктураПериодовФормированияОтчета)
	...
	Если СтруктураПериодовФормированияОтчета.ЕстьДополнительныйПериод Тогда
		...
		|			0,
		|			&ОкончаниеПериода,
		|			СУММА(ВыручкаИСебестоимостьПродажОбороты.СуммаВыручкиОборот - ВыручкаИСебестоимостьПродажОбороты.СебестоимостьОборот)
		|		ИЗ
		|			РегистрНакопления.ВыручкаИСебестоимостьПродаж.Обороты(&НачалоПериода, &ОкончаниеПериода, , АналитикаУчетаПоПартнерам.Партнер <> ЗНАЧЕНИЕ(Справочник.Партнеры.РозничныйПокупатель) И ЗаказКлиента.Менеджер В (ВЫБРАТЬ ОтборПоМенеджерам.Ссылка ИЗ ОтборПоМенеджерам КАК ОтборПоМенеджерам)) КАК ВыручкаИСебестоимостьПродажОбороты,
		|			Константы КАК Константы
		|		
...
Функция ТекстЗапросаОЭффективностиВзаимодействий(СоответствиеЗапросыДанные,НомерПоследнегоПакета, СтруктураПериодовФормированияОтчета)
	...
	Если СтруктураПериодовФормированияОтчета.ЕстьДополнительныйПериод Тогда
		...
		|	ГДЕ
		|		(НЕ ЭлектронноеПисьмоИсходящее.Предмет ССЫЛКА Справочник.СделкиСКлиентами)
		...
		|	ГДЕ
		|		(НЕ ЭлектронноеПисьмоВходящее.Предмет ССЫЛКА Справочник.СделкиСКлиентами)
		...
		|	ГДЕ
		|		(НЕ ТелефонныйЗвонок.Предмет ССЫЛКА Справочник.СделкиСКлиентами)
		...
		|	ГДЕ
		|		(НЕ Встреча.Предмет ССЫЛКА Справочник.СделкиСКлиентами)
		...
		|			ПО ЭлектронноеПисьмоИсходящее.Предмет = СделкиСКлиентами.Ссылка
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ТелефонныйЗвонок КАК ТелефонныйЗвонок
		|			ПО ЭлектронноеПисьмоИсходящее.Предмет = ТелефонныйЗвонок.Ссылка
		|	ГДЕ
		|		ЭлектронноеПисьмоИсходящее.Предмет ССЫЛКА Справочник.СделкиСКлиентами
		|		И (НЕ ЭлектронноеПисьмоИсходящее.ПометкаУдаления)
		|		И ЭлектронноеПисьмоИсходящее.Дата МЕЖДУ &НачалоПериода И &ОкончаниеПериода
		|		И ЭлектронноеПисьмоИсходящее.СтатусПисьма = ЗНАЧЕНИЕ(Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Отправлено)
		...
		|		Документ.ЭлектронноеПисьмоВходящее КАК ЭлектронноеПисьмоВходящее
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СделкиСКлиентами КАК СделкиСКлиентами
		|			ПО ЭлектронноеПисьмоВходящее.Предмет = СделкиСКлиентами.Ссылка
		|	ГДЕ
		|		ЭлектронноеПисьмоВходящее.Предмет ССЫЛКА Справочник.СделкиСКлиентами
		|		И (НЕ ЭлектронноеПисьмоВходящее.ПометкаУдаления)
		|		И ЭлектронноеПисьмоВходящее.Дата МЕЖДУ &НачалоПериода И &ОкончаниеПериода
		|		И (НЕ СделкиСКлиентами.ПометкаУдаления)
		|		И СделкиСКлиентами.Закрыта
		...
		|		Документ.ТелефонныйЗвонок КАК ТелефонныйЗвонок
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СделкиСКлиентами КАК СделкиСКлиентами
		|			ПО ТелефонныйЗвонок.Предмет = СделкиСКлиентами.Ссылка
		|	ГДЕ
		|		ТелефонныйЗвонок.Предмет ССЫЛКА Справочник.СделкиСКлиентами
		|		И (НЕ ТелефонныйЗвонок.ПометкаУдаления)
		|		И ТелефонныйЗвонок.Дата МЕЖДУ &НачалоПериода И &ОкончаниеПериода
		|		И (НЕ СделкиСКлиентами.ПометкаУдаления)
		|		И СделкиСКлиентами.Закрыта
		...
		|		Документ.Встреча КАК Встреча
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СделкиСКлиентами КАК СделкиСКлиентами
		|			ПО Встреча.Предмет = СделкиСКлиентами.Ссылка
		|	ГДЕ
		|		Встреча.Предмет ССЫЛКА Справочник.СделкиСКлиентами
		|		И (НЕ Встреча.ПометкаУдаления)
		|		И Встреча.Дата МЕЖДУ &НачалоПериода И &ОкончаниеПериода
		|		И (НЕ СделкиСКлиентами.ПометкаУдаления)
		|		И СделкиСКлиентами.Закрыта
...
Функция ТекстЗапросаОПродажахНаОдноВзаимодействие(СоответствиеЗапросыДанные, НомерПоследнегоПакета, СтруктураПериодовФормированияОтчета)
	...
	Если СтруктураПериодовФормированияОтчета.ЕстьДополнительныйПериод Тогда
		...
		|			ВыручкаИСебестоимостьПродажОбороты.СуммаВыручкиОборот
		|		ИЗ
		|			РегистрНакопления.ВыручкаИСебестоимостьПродаж.Обороты(&НачалоПериода, &ОкончаниеПериода, ,ЗаказКлиента.Менеджер В (ВЫБРАТЬ ОтборПоМенеджерам.Ссылка ИЗ ОтборПоМенеджерам КАК ОтборПоМенеджерам)) КАК ВыручкаИСебестоимостьПродажОбороты) КАК ВложенныйЗапрос
		|	
		|	СГРУППИРОВАТЬ ПО
		|		ВложенныйЗапрос.Ответственный
		|	
		|	ИМЕЮЩИЕ
		|		СУММА(ВложенныйЗапрос.Ссылка) > 0 И СУММА(ВложенныйЗапрос.Продажи) > 0) КАК ВложенныйЗапрос
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Процедура ВывестиЛегенду(ТаблицаОтчета,ИмяНабораДанных,ЕстьДополнительныйПериод,ПараметрыФормирования, ВалютаУправленческогоУчета, СтруктураПериодовФормированияОтчета, Макет)
	...
	Если ЕстьДополнительныйПериод И (НЕ ПараметрыФормирования.ВыводитьДиаграммы) Тогда
		...
		ТекстЛегенды = ТекстЛегенды +
		               СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполняется сравнение с данными за период с %1 по %2';uk='Виконується порівняння з даними за період з %1 з %2'") + " ",
		               Формат(СтруктураПериодовФормированияОтчета.ДатаНачалаДополнительныйПериод,"ДФ=dd.MM.yyyy"),
		               Формат(СтруктураПериодовФормированияОтчета.ДатаОкончанияДополнительныйПериод,"ДФ=dd.MM.yyyy"));
...
Процедура ВывестиДиаграмму(ТаблицаДанных, ТаблицаОтчета, ИмяНабора, ВыводитсяОсновнойПериод, СтруктураПериодовФормированияОтчета, Макет)
	...
	ДиаграммаОбъект.ФорматЗначенийВПодписях = "ЧДЦ=2; ДФ=dd.MM.yyyy";
	...
	Если ИмяНабора = "Продажи" Тогда
		...
		Для Каждого Серия Из Серии Цикл
			Если Серия.Текст = "Продажи" ИЛИ Серия.Текст = "Текущий" Тогда
				Серия.Текст = НСтр("ru='Продажи';uk='Продажі'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Предыдущий" Тогда
				Серия.Текст = НСтр("ru='Продажи предыдущего периода';uk='Продажі попереднього періоду'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Прирост" Тогда
				Серия.Текст = НСтр("ru='Прирост продаж в текущем периоде';uk='Приріст продажів в поточному періоді'");
				Серия.Цвет = ЦветПриростПрибыли;
			ИначеЕсли Серия.Текст = "Спад" Тогда
				Серия.Текст = НСтр("ru='Уменьшение продаж в текущем периоде';uk='Зменшення продажів в поточному періоді'");
	...
	ИначеЕсли ИмяНабора = "Прибыль" Тогда
		...
		Для Каждого Серия Из Серии Цикл
			Если Серия.Текст = "Прибыль" ИЛИ Серия.Текст = "Текущий" Тогда
				Серия.Текст = НСтр("ru='Прибыль';uk='Прибуток'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Предыдущий" Тогда
				Серия.Текст = НСтр("ru='Прибыль в предыдущем периоде';uk='Прибуток в попередньому періоді'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Прирост" Тогда
				Серия.Текст = НСтр("ru='Прирост прибыли в текущем периоде';uk='Приріст прибутку в поточному періоді'");
				Серия.Цвет = ЦветПриростПрибыли;
			ИначеЕсли Серия.Текст = "Спад" Тогда
				Серия.Текст = НСтр("ru='Уменьшение прибыли в текущем периоде';uk='Зменшення прибутку в поточному періоді'");
				Серия.Цвет = ЦветПотери;
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли ИмяНабора = "ПоступлениеДС" Тогда
		...
		Для Каждого Серия Из Серии Цикл
			Если Серия.Текст = "ПоступлениеДС" ИЛИ Серия.Текст = "Текущий" Тогда
				Серия.Текст = НСтр("ru='Поступление денежных средств';uk='Надходження грошових коштів'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Предыдущий" Тогда
				Серия.Текст = НСтр("ru='Поступление денежных средств в предыдущем периоде';uk='Надходження грошових коштів у попередньому періоді'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Прирост" Тогда
				Серия.Текст = НСтр("ru='Прирост поступления денежных средств в текущем периоде';uk='Приріст надходження грошових коштів у поточному періоді'");
				Серия.Цвет = ЦветПриростПрибыли;
			ИначеЕсли Серия.Текст = "Спад" Тогда
				Серия.Текст = НСтр("ru='Уменьшение поступления денежных средств в текущем периоде';uk='Зменшення надходження коштів у поточному періоді'");
				Серия.Цвет = ЦветПотери;
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли ИмяНабора = "ПоступлениеДС_МинусСебестоимость" Тогда
		...
		Для Каждого Серия Из Серии Цикл
			Если Серия.Текст = "ПоступлениеДС_МинусСебестоимость" ИЛИ Серия.Текст = "Текущий" Тогда
				Серия.Текст = НСтр("ru='Выручка (по оплате) минус себестоимость (по отгрузке)';uk='Виторг (по оплаті) мінус собівартість (по відвантаженню)'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Предыдущий" Тогда
				Серия.Текст = НСтр("ru='Значение показателя в предыдущем периоде';uk='Значення показника у попередньому періоді'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Прирост" Тогда
				Серия.Текст = НСтр("ru='Прирост показателя в текущем периоде';uk='Приріст показника в поточному періоді'");
				Серия.Цвет = ЦветПриростПрибыли;
			ИначеЕсли Серия.Текст = "Спад" Тогда
				Серия.Текст = НСтр("ru='Уменьшение показателя в текущем периоде';uk='Зменшення показника в поточному періоді'");
				Серия.Цвет = ЦветПотери;
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли ИмяНабора = "ПотериПросроченнаяДебиторка" Тогда
		...
		Для Каждого Серия Из Серии Цикл
			Если Серия.Текст = "ПотериДебиторка" ИЛИ Серия.Текст = "Текущий" Тогда
				Серия.Текст = НСтр("ru='Потери';uk='Втрати'");
				Серия.Цвет = ЦветПотери;
			ИначеЕсли Серия.Текст = "Предыдущий" Тогда
				Серия.Текст = НСтр("ru='Значение показателя в предыдущем периоде';uk='Значення показника у попередньому періоді'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
	...
	ИначеЕсли ИмяНабора = "ПредоставленныеРучныеСкидки" Тогда
		...
		Для Каждого Серия Из Серии Цикл
			...
			ИначеЕсли Серия.Текст = "СуммаРучнойСкидки" ИЛИ Серия.Текст = "СуммаРучнойСкидкиТекущий" Тогда
				Серия.Текст = НСтр("ru='Сумма ручной скидки';uk='Сума ручної знижки'");
				Серия.Цвет = ЦветПотери;
			ИначеЕсли Серия.Текст = "ПрибыльПредыдущий" Тогда
				Серия.Текст = НСтр("ru='Прибыль в предыдущем периоде';uk='Прибуток в попередньому періоді'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "СуммаРучнойСкидкиПредыдущий" Тогда
				Серия.Текст = НСтр("ru='Сумма ручной скидки в предыдущем периоде';uk='Сума ручної знижки в попередньому періоді'");
				Серия.Цвет = ЦветПриростПрибыли;
				Серия.Цвет = ЦветПотери;
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли ИмяНабора = "ЭффективностьВеденияСделок" Тогда
		...
		Для Каждого Серия Из Серии Цикл
			Если Серия.Текст = "ВыиграноСделок" ИЛИ Серия.Текст = "ВыиграноСделокТекущий"Тогда
				Серия.Текст = НСтр("ru='Выиграно сделок';uk='Виграно угод'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "ПроиграноСделок" ИЛИ Серия.Текст = "ПроиграноСделокТекущий" Тогда
				Серия.Текст = НСтр("ru='Проиграно сделок';uk='Програно угод'");
				Серия.Цвет = ЦветПотери;
			ИначеЕсли Серия.Текст = "ВыиграноСделокПредыдущий" Тогда
				Серия.Текст = НСтр("ru='Выиграно сделок в предыдущем периоде';uk='Виграно угод в попередньому періоді'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "ПроиграноСделокПредыдущий" Тогда
				Серия.Текст = НСтр("ru='Проиграно сделок в предыдущем периоде';uk='Програно угод в попередньому періоді'");
				Серия.Цвет = ЦветПриростПрибыли;
				Серия.Цвет = ЦветПотери;
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли ИмяНабора = "ЭффективностьВзаимодействий" Тогда
		...
		Для Каждого Серия Из Серии Цикл
			Если Серия.Текст = "ВзаимодействийПоВыиграннымСделкам" ИЛИ Серия.Текст = "ВзаимодействийПоВыиграннымСделкамТекущий"Тогда
				Серия.Текст = НСтр("ru='Взаимодействий по выигранным сделкам';uk='Взаємодій за виграними угодами'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "ВзаимодействийПоПроиграннымСделкам" ИЛИ Серия.Текст = "ВзаимодействийПоПроиграннымСделкамТекущий" Тогда
				Серия.Текст = НСтр("ru='Взаимодействий по проигранным сделкам';uk='Взаємодій за програними угодами'");
				Серия.Цвет = ЦветПотери;
			ИначеЕсли Серия.Текст = "ПрочиеВзаимодействия" ИЛИ Серия.Текст = "ПрочиеВзаимодействияТекущий" Тогда
				Серия.Текст = НСтр("ru='Прочие взаимодействия';uk='Інші взаємодії'");
				Серия.Цвет = ЦветПрочиеВзаимодействия;
			ИначеЕсли Серия.Текст = "ВзаимодействийПоВыиграннымСделкамПредыдущий" Тогда
				Серия.Текст = НСтр("ru='Взаимодействий по выигранным сделкам в предыдущем периоде';uk='Взаємодій за виграними угодами в попередньому періоді'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "ВзаимодействийПоПроиграннымСделкамПредыдущий" Тогда
				Серия.Текст = НСтр("ru='Взаимодействий по проигранным сделкам в предыдущем периоде';uk='Взаємодій за програними угодами в попередньому періоді'");
				Серия.Цвет = ЦветПриростПрибыли;
				Серия.Цвет = ЦветПотери;
			ИначеЕсли Серия.Текст = "ПрочиеВзаимодействияПредыдущий" Тогда
				Серия.Текст = НСтр("ru='Прочие взаимодействия в предыдущем периоде';uk='Інші взаємодії в попередньому періоді'");
				Серия.Цвет = ЦветПрочиеВзаимодействия;
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли ИмяНабора = "ПродажиНаОднуСделку" Тогда
		...
		Для Каждого Серия Из Серии Цикл
			Если Серия.Текст = "ПродажиНаСделку" ИЛИ Серия.Текст = "Текущий" Тогда
				Серия.Текст = НСтр("ru='Продажи на одну сделку';uk='Продажі на одну угоду'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Предыдущий" Тогда
				Серия.Текст = НСтр("ru='Продажи на одну сделку в предыдущем периоде';uk='Продажі на одну угоду в попередньому періоді'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Прирост" Тогда
				Серия.Текст = НСтр("ru='Прирост продаж на одну сделку в текущем периоде';uk='Приріст продажів на одну угоду в поточному періоді'");
				Серия.Цвет = ЦветПриростПрибыли;
			ИначеЕсли Серия.Текст = "Спад" Тогда
				Серия.Текст = НСтр("ru='Уменьшение продаж на одну сделку в текущем периоде';uk='Зменшення продажів на одну угоду в поточному періоді'");
				Серия.Цвет = ЦветПотери;
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли ИмяНабора = "ПродажиНаОдноВзаимодействие" Тогда
		...
		Для Каждого Серия Из Серии Цикл
			Если Серия.Текст = "ПродажиНаВзаимодействие" ИЛИ Серия.Текст = "Текущий" Тогда
				Серия.Текст = НСтр("ru='Продажи на одно взаимодействие';uk='Продажі на одне взаємодія'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Предыдущий" Тогда
				Серия.Текст = НСтр("ru='Продажи на одно взаимодействие в предыдущем периоде';uk='Продажі на одну взаємодію в попередньому періоді'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Прирост" Тогда
				Серия.Текст = НСтр("ru='Прирост продаж на одно взаимодействие в текущем периоде';uk='Приріст продажів на одну взаємодію в поточному періоді'");
				Серия.Цвет = ЦветПриростПрибыли;
			ИначеЕсли Серия.Текст = "Спад" Тогда
				Серия.Текст = НСтр("ru='Уменьшение продаж на одно взаимодействие в текущем периоде';uk='Зменшення продажів на одну взаємодію в поточному періоді'");
				Серия.Цвет = ЦветПотери;
			КонецЕсли;
		КонецЦикла;
	...
	ИначеЕсли ИмяНабора = "ПродажиНовымКлиентам" Тогда
		...
		Для Каждого Серия Из Серии Цикл
			Если Серия.Текст = "Продажи" ИЛИ Серия.Текст = "Текущий" Тогда
				...
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Предыдущий" Тогда
				Серия.Текст = НСтр("ru='Продажи новым клиентам в предыдущем периоде';uk='Продажі новим клієнтам у попередньому періоді'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Прирост" Тогда
				Серия.Текст = НСтр("ru='Прирост продаж новым клиентам в текущем периоде';uk='Приріст продажів новим клієнтам у поточному періоді'");
				Серия.Цвет = ЦветПриростПрибыли;
			ИначеЕсли Серия.Текст = "Спад" Тогда
				Серия.Текст = НСтр("ru='Уменьшение продаж новым клиентам в текущем периоде';uk='Зменшення продажів новим клієнтам у поточному періоді'");
				Серия.Цвет = ЦветПотери;
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого Серия Из Серии Цикл
			Если Серия.Текст = "Количество" ИЛИ Серия.Текст = "Текущий" Тогда
				Серия.Текст = НСтр("ru='Количество претензий';uk='Кількість претензій'");
				Серия.Цвет = ЦветПотери;
			ИначеЕсли Серия.Текст = "Предыдущий" Тогда
				Серия.Текст = НСтр("ru='Количество претензий в предыдущем периоде';uk='Кількість претензій в попередньому періоді'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "Прирост" Тогда
				Серия.Текст = НСтр("ru='Прирост зарегистрированных претензий в текущем периоде';uk='Приріст зареєстрованих претензій у поточному періоді'");
				Серия.Цвет = ЦветПотери;
			ИначеЕсли Серия.Текст = "Спад" Тогда
				Серия.Текст = НСтр("ru='Уменьшение зарегистрированных претензий в текущем периоде';uk='Зменшення зареєстрованих претензій у поточному періоді'");
				Серия.Цвет = ЦветПриростПрибыли;
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого Серия Из Серии Цикл
			Если Серия.Текст = "Новые" ИЛИ Серия.Текст = "НовыеТекущий" Тогда
				Серия.Текст = НСтр("ru='Новые клиенты';uk='Нові клієнти'");
				Серия.Цвет = ЦветПриростПрибыли;
			ИначеЕсли Серия.Текст = "Потерянные" ИЛИ Серия.Текст = "ПотерянныеТекущий" Тогда
				Серия.Текст = НСтр("ru='Потерянные клиенты';uk='Втрачені клієнти'");
				Серия.Цвет = ЦветПотери;
			ИначеЕсли Серия.Текст = "Вернувшиеся" ИЛИ Серия.Текст = "ВернувшиесяТекущий"Тогда
				Серия.Текст = НСтр("ru='Вернувшиеся клиенты';uk='Клієнти, що повернулись'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "НовыеПредыдущий" Тогда
				Серия.Текст = НСтр("ru='Новые клиенты в предыдущем периоде';uk='Нові клієнти в попередньому періоді'");
				Серия.Цвет = ЦветПриростПрибыли;
			ИначеЕсли Серия.Текст = "ВернувшиесяПредыдущий" Тогда
				Серия.Текст = НСтр("ru='Вернувшиеся клиенты в предыдущем периоде';uk='Клієнти, що повернулись в попередньому періоді'");
				Серия.Цвет = ЦветСинийСоСтальнымОттенком;
			ИначеЕсли Серия.Текст = "ПотерянныеПредыдущий" Тогда
				Серия.Текст = НСтр("ru='Потерянные клиенты в предыдущем периоде';uk='Втрачені клієнти в попередньому періоді'");
				Серия.Цвет = ЦветПотери;
			КонецЕсли;
		КонецЦикла;
...
Функция ТекстЛегенды(ИмяНабораДанных, ДляДополнительногоПериода, ВалютаУправленческогоУчета, СтруктураПериодовФормированияОтчета)
	...
	Если НЕ ДляДополнительногоПериода Тогда
		...
		ДатаНачала = Формат(СтруктураПериодовФормированияОтчета.ДатаНачала,"ДФ=dd.MM.yyyy");
		ДатаОкончания = Формат(СтруктураПериодовФормированияОтчета.ДатаОкончания,"ДФ=dd.MM.yyyy");
		
	Иначе
		
		ДатаНачала = Формат(СтруктураПериодовФормированияОтчета.ДатаНачалаДополнительныйПериод,"ДФ=dd.MM.yyyy");
		ДатаОкончания = Формат(СтруктураПериодовФормированияОтчета.ДатаОкончанияДополнительныйПериод,"ДФ=dd.MM.yyyy");
	...
	Если ИмяНабораДанных = "Продажи" Тогда
		...
		Если НЕ ДляДополнительногоПериода Тогда
			...
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о продажах за период с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   ...
                                                                                   |;uk='Інформація про продажі за період з %1 з %2 у валюті управлінського обліку (%3). 
                                                                                   |Сортування проводиться в порядку убування продажів.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
		...
		Иначе
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сравнение с продажами за период с %1 по %2 в валюте управленческого учета (%3).
                                                                                   ...
                                                                                   |;uk='Порівняння з продажами за період з %1 по %2 у валюті управлінського обліку (%3).
                                                                                   |Виводиться інформація про продажі попереднього періоду. Якщо в поточному періоді суми продажів збільшилися 
                                                                                   |порівняно з попереднім періодом, то додатково (зеленим кольором) відображається сума, на яку
                                                                                   |збільшилися продажі в порівнянні з попереднім періодом. Якщо суми продажу в поточному періоді зменшилися
                                                                                   |порівняно з попереднім періодом, то суми, на яку зменшилися продажі, відображаються сірим кольором.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
	...
	ИначеЕсли ИмяНабораДанных = "Прибыль" Тогда
		...
		Если НЕ ДляДополнительногоПериода Тогда
			...
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о прибыли полученной за период с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   ...
                                                                                   |;uk='Інформація про прибуток, отриманий за період з %1 з %2 у валюті управлінського обліку (%3). 
                                                                                   |Сортування проводиться в порядку убування прибутку.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
		...
		Иначе
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сравнение с прибылью полученной за период с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   ...
                                                                                   |;uk='Порівняння з прибутком отриманим за період з %1 по %2 у валюті управлінського обліку (%3). 
                                                                                   |Виводиться інформація про отриманий прибуток в попередньому періоді. Якщо в поточному періоді прибуток збільшився 
                                                                                   |порівняно з попереднім періодом, то додатково (зеленим кольором) відображається сума, на яку
                                                                                   |збільшився прибуток у порівнянні з попереднім періодом. Якщо прибуток у поточному періоді зменшилася
                                                                                   |порівняно з попереднім періодом, то сума, на яку зменшився прибуток відображається сірим кольором.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
	...
	ИначеЕсли ИмяНабораДанных = "ПоступлениеДС" Тогда
		...
		Если НЕ ДляДополнительногоПериода Тогда
			...
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о поступлении денежных средств за период с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   ...
                                                                                   |;uk='Інформація про надходження коштів за період з %1 по %2 у валюті управлінського обліку (%3). 
                                                                                   |Сортування проводиться в порядку зменшення суми грошових коштів, що надійшли.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
		...
		Иначе
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сравнение с поступившими денежными средствами за период с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   ...
                                                                                   |;uk='Порівняння грошовими коштами, що надійшли, за період з %1 по %2 у валюті управлінського обліку (%3). 
                                                                                   |Виводиться інформація про грошові кошти, що надійшли у попередньому періоді.Якщо в поточному періоді сума грошових 
                                                                                   |коштів, що надійшли, збільшилася в порівнянні з попереднім періодом, то додатково (зеленим кольором) відображається сума, 
                                                                                   |на яку збільшилося надходження грошових коштів в порівнянні з попереднім періодом. Якщо сума надходження грошових 
                                                                                   |коштів у поточному періоді зменшилася порівняно з попереднім періодом, то сума, на яку зменшилось надходження
                                                                                   |грошових коштів, відображається сірим кольором.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
	...
	ИначеЕсли ИмяНабораДанных = "ПоступлениеДС_МинусСебестоимость" Тогда
		...
		Если НЕ ДляДополнительногоПериода Тогда
			...
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о поступивших денежных средствах за вычетом себестоимости продаж в период с %1 по %2
                                                                                   ...
                                                                                   |;uk='Інформація про кошти, що надійшли, за вирахуванням собівартості продажів в період з %1 по %2
                                                                                   |у валюті управлінського обліку (%3).Сортування проводиться в порядку убування показника.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
		...
		Иначе
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сравнение со значением показателя за период с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   ...
                                                                                   |;uk='Порівняння зі значенням показника за період з %1 по %2 у валюті управлінського обліку (%3). 
                                                                                   |Виводиться інформація про значення показника у попередньому періоді. Якщо в поточному періоді значення показника виросло
                                                                                   |порівняно з попереднім періодом,то додатково (зеленим кольором) відображається значення, на яке збільшився показник
                                                                                   |порівняно з попереднім періодом. Якщо значення показника в поточному періоді зменшилося порівняно з попереднім періодом,
                                                                                   |те значення, на яке зменшився показник відображається сірим кольором.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
	...
	ИначеЕсли ИмяНабораДанных = "ПотериПросроченнаяДебиторка" Тогда
		...
		Если НЕ ДляДополнительногоПериода Тогда
			...
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о потерях, понесенных компанией в результате несоблюдения договорных сроков оплаты клиентами
                                                                                   ...
                                                                                   |;uk='Інформація про втрати, понесених компанією в результаті недотримання договірних строків оплати клієнтами
                                                                                   |у період з %1 з %2 у валюті управлінського обліку (%3). Показник розраховується за формулою: 
                                                                                   |Втрати = Сума простроченої заборгованості * Дні простроченої заборгованості * Річна ставка для розрахунку фінансових
                                                                                   |втрат / (100*365).
                                                                                   |Сортування проводиться в порядку убування показника.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
		...
		Иначе
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сравнение со значением показателя за период с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   ...
                                                                                   |;uk='Порівняння зі значенням показника за період з %1 по %2 у валюті управлінського обліку (%3). 
                                                                                   |Виводиться інформація про значення показника у попередньому періоді.Якщо в поточному періоді значення показника виросло
                                                                                   |порівняно з попереднім періодом,то додатково (сірим кольором) відображається значення, на яке збільшився показник
                                                                                   |порівняно з попереднім періодом. Якщо значення показника в поточному періоді зменшилося порівняно з попереднім періодом,
                                                                                   |то значення, на яке зменшився показник, відображається зеленим кольором.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
	...
	ИначеЕсли ИмяНабораДанных = "ПредоставленныеРучныеСкидки" Тогда
		...
		Если НЕ ДляДополнительногоПериода Тогда
			...
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о полученной прибыли и предоставленных ручных скидках в период с %1 по %2 в валюте управленческого учета (%3).
                                                                                   ...
                                                                                   |;uk='Інформація про отриманий прибуток і наданих ручних знижки в період з %1 з %2 у валюті управлінського обліку (%3).
                                                                                   |Сортування проводиться в порядку убування частки наданих ручних знижок щодо отриманого прибутку.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
			
		Иначе
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о полученной прибыли и предоставленных ручных скидках в период с %1 по %2 в валюте управленческого учета (%3).';uk='Інформація про отриманий прибуток і наданих ручних знижках в період з %1 з %2 у валюті управлінського обліку (%3).'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
	...
	ИначеЕсли ИмяНабораДанных = "ЭффективностьВеденияСделок" Тогда
		...
		Если НЕ ДляДополнительногоПериода Тогда
			...
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о выигранных и проигранных сделках, завершившихся в период с %1 по %2.
                                                                                   ...
                                                                                   |;uk='Інформація про виграні і програні угоди, що завершилися в період з %1 з %2.
                                                                                   |Сортування проводиться в порядку убування частки виграних угод.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
			
		Иначе
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о выигранных и проигранных сделках, завершившихся в период с %1 по %2.';uk='Інформація про виграні і програні угоди, що завершилися в період з %1 з %2.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
	...
	ИначеЕсли ИмяНабораДанных = "ЭффективностьВзаимодействий" Тогда
		...
		ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о взаимодействиях, посвященных выигранным, проигранным сделкам, а также 
                                                                                   ...
                                                                                   |;uk='Інформація про взаємодії, присвячені виграним, програним операціях, а також 
                                                                                   |іншим взаємодії, зареєстрованими у період з %1 з %2.
                                                                                   |Сортування проводиться в порядку убування частки взаємодій, присвяченим виграним операціях.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
	...
	ИначеЕсли ИмяНабораДанных = "ПродажиНаОднуСделку" Тогда
		...
		Если НЕ ДляДополнительногоПериода Тогда
			...
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о сумме продаж на одну завершенную сделку в период с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   |Сортировка производится в порядке убывания продаж.'
                                                                                   |;uk='Інформація про суму продажів на одну завершену угоду в період з %1 з %2 у валюті управлінського обліку (%3). 
                                                                                   |Сортування проводиться в порядку убування продажів.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
		...
		Иначе
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сравнение с продажами на одну завершенную сделку в с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   ...
                                                                                   |по сравнению с предыдущим периодом. Если значение показателя в текущем периоде уменьшилось по сравнению с предыдущим периодом,
                                                                                   |то значение, на которое уменьшился показатель отображается серым цветом.'
                                                                                   |;uk='Порівняння з продажами на одну завершену операцію в з %1 по %2 у валюті управлінського обліку (%3). 
                                                                                   |Виводиться інформація про значення показника у попередньому періоді. Якщо в поточному періоді значення показника виросло
                                                                                   |порівняно з попереднім періодом,то додатково(зеленим кольором) відображається значення, на яке збільшився показник
                                                                                   |порівняно з попереднім періодом. Якщо значення показника в поточному періоді зменшилося порівняно з попереднім періодом,
                                                                                   |те значення, на яке зменшився показник, відображається сірим кольором.'"),
	...
	ИначеЕсли ИмяНабораДанных = "ПродажиНаОдноВзаимодействие" Тогда
		...
		Если НЕ ДляДополнительногоПериода Тогда
			...
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о сумме продаж на одно зарегистрированное взаимодействие в период с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   |Сортировка производится в порядке убывания продаж.'
                                                                                   |;uk='Інформація про суму продажів на одну зареєстровану взаємодію в період з %1 з %2 у валюті управлінського обліку (%3). 
                                                                                   |Сортування проводиться в порядку убування продажів.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
		...
		Иначе
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сравнение с продажами одно зарегистрированное взаимодействие в с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   ...
                                                                                   |по сравнению с предыдущим периодом. Если значение показателя в текущем периоде уменьшилось по сравнению с предыдущим периодом,
                                                                                   |то значение, на которое уменьшился показатель, отображается серым цветом.'
                                                                                   |;uk='Порівняння з продажами на одну зареєстровану взаємодію з %1 по %2 у валюті управлінського обліку (%3). 
                                                                                   |Виводиться інформація про значення показника у попередньому періоді. Якщо в поточному періоді значення показника виросло
                                                                                   |порівняно з попереднім періодом,то додатково (зеленим кольором) відображається значення, на яке збільшився показник
                                                                                   |порівняно з попереднім періодом. Якщо значення показника в поточному періоді зменшилося порівняно з попереднім періодом,
                                                                                   |те значення, на яке зменшився показник, відображається сірим кольором.'"),
	...
	ИначеЕсли ИмяНабораДанных = "ОсновныеЭтапыПотерь" Тогда
		...
		ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о 5 наиболее часто встречающихся при проигрыше сделок в период с %1 по %2
                                                                              ...
                                                                              |;uk='Інформація про 5 таких, що найбільш часто зустрічаються при програші угод в період з %1 з %2
                                                                              |поєднаннях ""менеджер - етап операції"".'"),
			                                                                  ДатаНачала,
			                                                                  ДатаОкончания,
			                                                                  ВалютаУправленческогоУчета);
	...
	ИначеЕсли ИмяНабораДанных = "ПричиныПроигрышаСделок" Тогда
		...
		ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о 5 наиболее часто встречающихся при проигрыше сделок в период с %1 по %2
                                                                              ...
                                                                              |;uk='Інформація про 5 таких, що найбільш часто зустрічаються при програші угод в період з %1 з %2
                                                                              |поєднаннях ""менеджер - причина програшу угоди"".'"),
			                                                                  ДатаНачала,
			                                                                  ДатаОкончания);
	...
	ИначеЕсли ИмяНабораДанных = "НеудовлетворениеПервичногоСпроса" Тогда
		...
		ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о 5 наиболее часто встречающихся для закрытых в период с %1 по %2 сделок
                                                                              ...
                                                                              |;uk='Інформація про 5 таких, що найбільш часто зустрічаються для закритих в період з %1 з %2 угод
                                                                              |поєднаннях ""менеджер - причина незадоволення первинного попиту"".
                                                                              |Інформація виводиться у валюті управлінського обліку (%3)'"),
			                                                                  ДатаНачала,
			                                                                  ДатаОкончания);
	...
	ИначеЕсли ИмяНабораДанных = "ДинамикаКлиентскойБазы" Тогда
		...
		ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о новых, вернувшихся и потерянных в период с %1 по %2 клиентах';uk='Інформація про нові, таких, що повернулися і втрачених в період з %1 з %2 клієнтів'"),
			                                                                  ДатаНачала,
			                                                                  ДатаОкончания);
		Иначе
	...
	ИначеЕсли ИмяНабораДанных = "ПродажиНовымКлиентам" Тогда
		...
		Если НЕ ДляДополнительногоПериода Тогда
			...
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о продажах новым клиентам за период с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   ...
                                                                                   |;uk='Інформація про продажі новим клієнтам за період з %1 з %2 у валюті управлінського обліку (%3). 
                                                                                   |Сортування проводиться в порядку убування продажів новим клієнтам.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
		...
		Иначе
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сравнение с продажами новым клиентам за период с %1 по %2 в валюте управленческого учета (%3). 
                                                                                   ...
                                                                                   |по сравнению с предыдущим периодом. Если значение показателя в текущем периоде уменьшилось по сравнению с предыдущим периодом,
                                                                                   |то значение, на которое уменьшился показатель, отображается серым цветом.'
                                                                                   |;uk='Порівняння з продажами новим клієнтам за період з %1 по %2 у валюті управлінського обліку (%3). 
                                                                                   |Виводиться інформація про продажі новим клієнтам попереднього періоду. Якщо в поточному періоді значення показника виросло
                                                                                   |порівняно з попереднім періодом,то додатково (зеленим кольором) відображається значення, на яке збільшився показник
                                                                                   |порівняно з попереднім періодом. Якщо значення показника в поточному періоді зменшилося порівняно з попереднім періодом,
                                                                                   |те значення, на яке зменшився показник, відображається сірим кольором.'"),
			                                                                       ДатаНачала,
	...
	ИначеЕсли ИмяНабораДанных = "ВозникшиеПретензииКлиентов" Тогда
		...
		Если НЕ ДляДополнительногоПериода Тогда
			...
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о количестве возникших в период с %1 по %2 претензий клиентах. 
                                                                                   ...
                                                                                   |;uk='Інформація про кількість претензій клієнтів, які виникли в період з %1 з %2. 
                                                                                   |Сортування проводиться в порядку убування кількості претензій клієнтів.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
		...
		Иначе
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сравнение с количеством возникших в период с %1 по %2 претензий клиентов. 
                                                                                   ...
                                                                                   |по сравнению с предыдущим периодом. Если значение показателя в текущем периоде уменьшилось по сравнению с предыдущим периодом,
                                                                                   |то значение, на которое уменьшился показатель отображается зеленым цветом.'
                                                                                   |;uk='Порівняння з кількістю претензій клієнтів, що виникли в період з %1 по %2 . 
                                                                                   |Виводиться інформація про кількість претензій клієнтів, що виникли у попередній період. Якщо в поточному періоді значення показника виросло
                                                                                   |порівняно з попереднім періодом, то додатково (сірим кольором) відображається значення, на яке збільшився показник
                                                                                   |порівняно з попереднім періодом. Якщо значення показника в поточному періоді зменшилося порівняно з попереднім періодом,
                                                                                   |те значення, на яке зменшився показник відображається зеленим кольором.'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания,
			                                                                       ВалютаУправленческогоУчета);
	...
	ИначеЕсли ИмяНабораДанных = "ПричиныВозникновенияПретензий" Тогда
		...
		Если НЕ ДляДополнительногоПериода Тогда
			...
			ТекстЛегенды = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Информация о 5 наиболее часто встречающихся для зарегистрированных в период с %1 по %2 претензий клиентов
                                                                                   ...
                                                                                   |;uk='Інформація про 5 таких, що найбільш часто зустрічаються для зареєстрованих в період з %1 з %2 претензій клієнтів
                                                                                   |поєднаннях ""менеджер - причина виникнення претензії"".'"),
			                                                                       ДатаНачала,
			                                                                       ДатаОкончания);
...
Функция ТаблицаДанныхДляТабличногоВывода(ИмяНабора,ТаблицаДанных,ЕстьДополнительныйПериод)
	...
	Иначе
		...
		Если ИмяНабора = "Прибыль" ИЛИ ИмяНабора = "Продажи" 
			...
			ТаблицаКВозврату.Колонки[1].Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 в текущем периоде';uk='%1 у поточному періоді'"),ЗаголовокПоказателя(ИмяНабора));
			ТаблицаКВозврату.Колонки[2].Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 в предыдущем периоде';uk='%1 у попередньому періоді'"),ЗаголовокПоказателя(ИмяНабора));
			ТаблицаКВозврату.Колонки[3].Заголовок = НСтр("ru='Абсолютное изменение';uk='Абсолютна зміна'");
			ТаблицаКВозврату.Колонки[4].Заголовок = НСтр("ru='Относительное изменение(%)';uk='Відносна зміна(%)'");
...
Функция ЗаголовокПоказателя(ИмяПоказателя)
	...
	Если ЗаголовокПоказателя = "ПоступлениеДС" Тогда
		...
		ЗаголовокПоказателя = НСтр("ru='Поступление денежных средств';uk='Надходження грошових коштів'");
		
	ИначеЕсли  ЗаголовокПоказателя = "ПоступлениеДС_МинусСебестоимость" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Выручка (по оплате) минус себестоимость (по отгрузке)';uk='Виторг (по оплаті) мінус собівартість (по відвантаженню)'");
		
	ИначеЕсли  ЗаголовокПоказателя = "ПотериПросроченнаяДебиторка" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Потери';uk='Втрати'");
		
	ИначеЕсли  ЗаголовокПоказателя = "ПродажиНаОднуСделку" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Продажи на одну сделку';uk='Продажі на одну угоду'");
		
	ИначеЕсли ЗаголовокПоказателя = "ПродажиНаОдноВзаимодействие" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Продажи на одно взаимодействие';uk='Продажі на одне взаємодія'");
		
	ИначеЕсли ЗаголовокПоказателя = "ПродажиНовымКлиентам" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Продажи новым клиентам';uk='Продажі новим клієнтам'");
		
	ИначеЕсли ЗаголовокПоказателя = "СуммаРучнойСкидки" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Сумма ручной скидки';uk='Сума ручної знижки'");
		
	ИначеЕсли ЗаголовокПоказателя = "ДоляСкидкиОтПрибыли" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Доля предоставленной ручной скидки от прибыли (%)';uk='Частка наданої ручної знижки від прибутку (%)'");
		
	ИначеЕсли ЗаголовокПоказателя = "ВыиграноСделок" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Выиграно сделок';uk='Виграно угод'");
		
	ИначеЕсли ЗаголовокПоказателя = "ПроиграноСделок" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Проиграно сделок';uk='Програно угод'");
		
	ИначеЕсли ЗаголовокПоказателя = "ДоляВыигранныхСделок" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Доля выигранных сделок (%)';uk='Частка виграних угод (%)'");
		
	ИначеЕсли ЗаголовокПоказателя = "ВзаимодействийПоВыиграннымСделкам" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Взаимодействий по выигранным сделкам';uk='Взаємодій за виграними угодами'");
		
	ИначеЕсли ЗаголовокПоказателя = "ВзаимодействийПоПроиграннымСделкам" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Взаимодействий по проигранным сделкам';uk='Взаємодій за програними угодами'");
		
	ИначеЕсли ЗаголовокПоказателя = "ПрочиеВзаимодействия" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Прочие взаимодействия';uk='Інші взаємодії'");
		
	ИначеЕсли ЗаголовокПоказателя = "ДоляВзаимодействийПоВыиграннымСделкам" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Доля взаимодействий по выигранным сделкам(%)';uk='Частка взаємодій за виграними угодами(%)'");
		
	ИначеЕсли ЗаголовокПоказателя = "Новые" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Новые клиенты';uk='Нові клієнти'");
		
	ИначеЕсли ЗаголовокПоказателя = "Потерянные" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Потерянные клиенты';uk='Втрачені клієнти'");
		
	ИначеЕсли ЗаголовокПоказателя = "Вернувшиеся" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Вернувшиеся клиенты';uk='Клієнти, що повернулись'");
		
	ИначеЕсли ЗаголовокПоказателя = "ДоляНовыхКлиентов" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Доля новых клиентов (%)';uk='Частка нових клієнтів (%)'");
		
	ИначеЕсли ЗаголовокПоказателя = "ВозникшиеПретензииКлиентов" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='Возникшие претензии';uk='Претензії, що виникли'");
		
	ИначеЕсли ЗаголовокПоказателя = "ПричиныПроигрышаСделок" ИЛИ ЗаголовокПоказателя = "ПричиныПроигрышаСделокИмяПоказателя" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='""Менеджер - причина проигрыша""';uk='""Менеджер - причина програшу""'");
		
	ИначеЕсли ЗаголовокПоказателя = "ОсновныеЭтапыПотерь" ИЛИ ЗаголовокПоказателя = "ОсновныеЭтапыПотерьИмяПоказателя" Тогда
		
		ЗаголовокПоказателя = НСтр("ru='""Менеджер - этап сделки""';uk='""Менеджер - етап угоди""'");
	...
	ИначеЕсли ЗаголовокПоказателя = "ОсновныеЭтапыПотерьЗначениеПоказателя" ИЛИ ЗаголовокПоказателя = "ПричиныПроигрышаСделокЗначениеПоказателя"
		...
		ЗаголовокПоказателя = НСтр("ru='Количество';uk='Кількість'");
	...
	ИначеЕсли ЗаголовокПоказателя = "НеудовлетворениеПервичногоСпроса" ИЛИ ЗаголовокПоказателя = "НеудовлетворениеПервичногоСпросаИмяПоказателя" Тогда
		...
		ЗаголовокПоказателя = НСтр("ru='""Менеджер - причина неудовлетворения""';uk='""Менеджер - причина незадоволення""'");
	...
	ИначеЕсли ЗаголовокПоказателя = "СуммаПервичногоСпроса" ИЛИ ЗаголовокПоказателя = "НеудовлетворениеПервичногоСпросаЗначениеПоказателя" Тогда
		...
		ЗаголовокПоказателя = НСтр("ru='Сумма неудовлетворенного спроса';uk='Сума незадоволеного попиту'");
	...
	ИначеЕсли ЗаголовокПоказателя = "ПричиныВозникновенияПретензий" ИЛИ ЗаголовокПоказателя = "ПричиныВозникновенияПретензийИмяПоказателя" Тогда
		...
		ЗаголовокПоказателя = НСтр("ru='""Менеджер - причина возникновения""';uk='""Менеджер - причина виникнення""'");
		
	КонецЕсли;
	
	Если ЭтоВыводЗаголовкаТекущегоПериода Тогда
	
		ЗаголовокПоказателя = ЗаголовокПоказателя + НСтр("ru=' в текущем периоде';uk=' у поточному періоді'");
	
	ИначеЕсли ЭтоВыводЗаголовкаПредыдущегоПериода Тогда
	
		ЗаголовокПоказателя = ЗаголовокПоказателя + НСтр("ru=' в предыдущем периоде';uk=' у попередньому періоді'");
...
Функция СтруктураПериодовФормированияОтчетов(ПараметрыФормирования)
	...
	СтруктураКВозврату = Новый Структура("ДатаНачала,ДатаОкончания,ЕстьДополнительныйПериод,ДатаНачалаДополнительныйПериод,ДатаОкончанияДополнительныйПериод",
	                                     ПериодФормированияОтчета.ДатаНачала,
	                                     ПериодФормированияОтчета.ДатаОкончания,
	                                     Ложь,
	                                     Дата(1,1,1),
	                                     Дата(1,1,1));
