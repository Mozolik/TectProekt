...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	НастройкиОтчета.Описание = НСтр("ru='Внешние ресурсы, используемые программой и дополнительными модулями';uk='Зовнішні ресурси, які використовуються програмою та додатковими модулями'");
	...
	НастройкиОтчета.НастройкиДляПоиска.НаименованияПолей = 
        ...
        |Порт'
        |;uk='Ім''я та ідентифікатор COM-класу
        |Ім''я комп''ютера
        |Адреса
        |Читання даних
        |Запис даних
        |Ім''я макета або файлу компоненти
        |Контрольна сума
        |Шаблон командного рядка
        |Протокол
...
Функция СловарьМодуляКонфигурации() Экспорт
	...
	Результат.Вставить("Именительный", НСтр("ru='Программа';uk='Програма'"));
	Результат.Вставить("Родительный", НСтр("ru='Программы';uk='Програми'"));
...
Функция ПредставлениеЗапросовРазрешенийНаИспользованиеВнешнихРесурсов(Знач ОперацииАдминистрирования, Знач ОписаниеДобавляемыхРазрешений, Знач ОписаниеУдаляемыхРазрешений, Знач КакТребуемые = Ложь) Экспорт
	...
	Для Каждого СтрокаТаблицыМодулей Из ТаблицаМодулей Цикл
		...
		Если Добавляемые.Количество() > 0 Тогда
			...
			Если КакТребуемые Тогда
				...
				ТекстШапки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Для %1 ""%2"" требуется использование следующих внешних ресурсов:';uk='Для %1 ""%2"" потрібно використання наступних зовнішніх ресурсів:'"),
			...
			Иначе
				...
				ТекстШапки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Для %1 ""%2"" будут предоставлены следующие разрешения на использование внешних ресурсов:';uk='Для %1 ""%2"" будуть надані наступні дозволи на використання зовнішніх ресурсів:'"),
					НРег(Словарь.Родительный),
					НаименованиеМодуля);
		...
		Если Удаляемые.Количество() > 0 Тогда
			...
			Если КакТребуемые Тогда
				ВызватьИсключение НСтр("ru='Некорректный запрос разрешений';uk='Некоректний запит дозволів'");
			КонецЕсли;
			
			ТекстШапки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Будут удалены следующие ранее предоставленные для %1 ""%2"" разрешения на использование внешних ресурсов:';uk='Будуть вилучені наступні раніше надані для %1 ""%2"" дозволи на використання зовнішніх ресурсів:'"),
					НРег(Словарь.Родительный),
					НаименованиеМодуля);
...
Процедура СформироватьПредставлениеОпераций(ТабличныйДокумент, Знач Макет, Знач ОперацииАдминистрирования)
	...
	Для Каждого Описание Из ОперацииАдминистрирования Цикл
		...
		Если Описание.Операция = Перечисления.ОперацииАдминистрированияПрофилейБезопасности.Удаление Тогда
			...
			ТекстШапки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Будет удален профиль безопасности для %1 ""%2"".';uk='Буде вилучений профіль безпеки для %1 ""%2"".'"),
					НРег(Словарь.Родительный),
					НаименованиеМодуля);
					...
					Иначе
						...
						Если ТипРазрешения = "FileSystemAccess" Тогда
							...
							Если Разрешение.Path = "/temp" Тогда
								ДополнениеРазрешения.Вставить("Path", НСтр("ru='Каталог временных файлов';uk='Каталог тимчасових файлів'"));
							КонецЕсли;
							
							Если Разрешение.Path = "/bin" Тогда
								ДополнениеРазрешения.Вставить("Path", НСтр("ru='Каталог, в который установлен сервер платформы';uk='Каталог, в який встановлений сервер платформи'"));
							КонецЕсли;
