...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВариантОтчета = "ДиаграммаГанта" Тогда
		...
		Если Не ЗначениеЗаполнено(ПериодДень.Дата) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не заполнено значение поля День!';uk='Не заповнене значення поля День!'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		И КонецВыборки.Использование Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Значение начала периода не может быть больше значения конца!';uk='Значення початку періоду не може бути більше значення кінця!'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ВариантОтчета = "АктивностьПользователя" Тогда
		...
		Если Не ЗначениеЗаполнено(Пользователь) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не заполнено значение поля Пользователь!';uk='Не заповнене значення поля Користувач!'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		Если Отчеты.АнализЖурналаРегистрации.ИмяПользователяИБ(Пользователь) = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Формирование отчета возможно только для пользователя, которому указано имя для входа в программу.';uk='Формування звіту можливо тільки для користувача, якому зазначено ім''я для входу в програму.'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ВариантОтчета = "АнализАктивностиПользователей" Тогда
		...
		Если Не ЗначениеЗаполнено(ПользователиИГруппы) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не заполнено значение поля Пользователи!';uk='Не заповнене значення поля Користувачі!'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
