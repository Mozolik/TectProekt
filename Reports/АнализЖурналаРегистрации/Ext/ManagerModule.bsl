...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта.Описание = 
        ...
        |;uk='Дозволяє виконувати моніторинг активності користувачів
        |в програмі (наскільки інтенсивно і з якими об''єктами працюють користувачі).'");
	...
	НастройкиВарианта.Описание = 
        ...
        |;uk='Докладна інформація про те,
        |з якими об''єктами працював користувач в програмі.'");
	
	НастройкиВарианта = МодульВариантыОтчетов.ОписаниеВарианта(Настройки, НастройкиОтчета, "КонтрольЖурналаРегистрации");
	НастройкиВарианта.Описание = НСтр("ru='Список критичных записей журнала регистрации.';uk='Список критичних записів журналу реєстрації.'");
	...
	НастройкиВарианта.Описание = НСтр("ru='Выводит график выполнения регламентных заданий в программе.';uk='Виводить графік виконання регламентних завдань у програмі.'");
...
Функция СтрокаНеделяГода(ДатаГода)
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неделя %1';uk='Тиждень %1'"), НеделяГода(ДатаГода));
КонецФункции
	...
	Для Каждого Элемент Из МаксМассивПользователей Цикл
		...
		Для Каждого ИмяПользователя Из ВременныйМассив Цикл
			Если ИмяПользователя = Элемент Тогда
				...
				ПользовательИНомер = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 (%2)';uk='%1 (%2)'"),
					Элемент,
...
Функция СформироватьОтчетПоПродолжительностиРаботыРегламентныхЗаданий(ПараметрыЗаполнения) Экспорт
	...
	Если ВыводитьОтбор.Значение = ТипВыводаТекстаКомпоновкиДанных.Выводить
		...
		Если МинимальнаяПродолжительностьСеансовРегламентныхЗаданий > 0 Тогда
			РежимОтображенияИнтервалов = НСтр("ru='Отключено';uk='Відключено'");
		Иначе
			РежимОтображенияИнтервалов = НСтр("ru='Включено';uk='Включене'");
		КонецЕсли;
		...
		Для Каждого СтрокаОдновременноСессий Из ОдновременноСессий Цикл
			...
			Для Каждого Элемент Из МассивРегламентныхЗаданий Цикл
				Если Не ТипЗнч(Элемент) = Тип("Число")
					...
					Область.Параметры.СписокРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 (сеанс %2)';uk='%1 (сеанс %2)'"),
						Элемент,
		...
		Если ДлительностьИнтервалаРегламентногоЗадания >= МинимальнаяПродолжительностьСеансовРегламентныхЗаданий Тогда
			Если ТекущееСобытие <> СтрокаРегламентныеЗадания.НазваниеСобытия Тогда
				Если ТекущееСобытие <> Неопределено
					...
					ТочкаНазвание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 (%2 из %3)';uk='%1 (%2 з %3)'"),
						Точка.Значение,
				...
				ИначеЕсли Не ЗначениеЗаполнено(СтрокаРегламентныеЗадания.СобытиеМетаданные) Тогда
					ТочкаНазвание = НСтр("ru='Фоновые задания';uk='Фонові завдання'");
					Точка = ДиаграммаГанта.УстановитьТочку(ТочкаНазвание);
					ОбщаяПродолжительностьРЗ = 0;
				КонецЕсли;
			КонецЕсли;
			...
			Интервал.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 - %2';uk='%1 - %2'"),
	...
	Для Каждого СтрокаДанныхЖурналаРегистрации Из ДанныеЖурналаРегистрации Цикл 
		...
		Если ТекущаяДата <> ДатаОдновременноРегламентныхЗаданий Тогда
			Если СтрокаТаблицы <> Неопределено Тогда
				...
				СтрокаТаблицы.ДатаОдновременноРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 - %2';uk='%1 - %2'"),
...
Функция РасшифровкаРегламентногоЗадания(Расшифровка) Экспорт
	...
	Область = Макет.ПолучитьОбласть("Таблица");
	Область.Параметры.ТипЗадания = НСтр("ru='Регламентное';uk='Регламентне'");
...
Функция ПродолжительностьРегламентногоЗадания(ПродолжительностьРЗ)
	...
	ИначеЕсли ПродолжительностьРЗ <= 60 Тогда
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 сек';uk=' %1 сек'"), ПродолжительностьРЗ);
	ИначеЕсли 60 < ПродолжительностьРЗ <= 3600 Тогда
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 мин %2 сек';uk='%1 хв %2 сек'"), ПродолжительностьМинуты, ПродолжительностьСекунды);
	ИначеЕсли ПродолжительностьРЗ > 3600 Тогда
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 ч %2 мин';uk='%1 год %2 хв'"), ПродолжительностьЧасы, ПродолжительностьМинуты);
	КонецЕсли;
