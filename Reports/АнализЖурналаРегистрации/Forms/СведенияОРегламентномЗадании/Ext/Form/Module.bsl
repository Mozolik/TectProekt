...
Процедура НастроитьРасписаниеРегламентногоЗадания(Команда)
	...
	Иначе
		ПоказатьПредупреждение(,НСтр("ru='Невозможно получить расписание регламентного задания: регламентное задание было удалено или не указано его наименование.';uk='Неможливо отримати розклад регламентного завдання: регламентне завдання було вилучено або не зазначене його найменування.'"));
	КонецЕсли;
...
Процедура ПерейтиКЖурналуРегистрации(Команда)
	...
	Для Каждого Область Из Отчет.ВыделенныеОбласти Цикл
		...
		Если Расшифровка = Неопределено
			...
			ПоказатьПредупреждение(,НСтр("ru='Выберите строку или ячейку нужного сеанса задания';uk='Виберіть рядок або комірку потрібного сеансу завдання'"));
			Возврат;
		КонецЕсли;
		ДатаНачала = Расшифровка.Получить(0);
		ДатаОкончания = Расшифровка.Получить(1);
