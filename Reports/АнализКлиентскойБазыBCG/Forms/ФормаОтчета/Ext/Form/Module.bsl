...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Список.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не включено использование ни одного типа ABC и XYZ классификации партнеров.';uk='Не включено використання жодного типу ABC і XYZ класифікації партнерів.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ДатаПредыдущегоСрезаПриИзменении(Элемент)
	...
	Если ДатаПредыдущегоСреза >= ДатаТекущегоСреза Тогда
		ТекстСообщения = НСтр("ru='Дата предыдущего среза должна быть меньше даты текущего среза.';uk='Дата попереднього зрізу повинна бути менша дати поточного зрізу.'");
		ОчиститьСообщения();
	...
	Иначе
		...
		Иначе
			ТекстСообщения = НСтр("ru='Среза на %Дата% не существует.';uk='Зрізу на %Дата% не існує.'");
...
Процедура СформироватьОтчет(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ТипПараметраКлассификации) Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Необходимо указать тип параметра классификации';uk='Необхідно вказати тип параметра класифікації'"),,"ТипПараметраКлассификации");
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДатаТекущегоСреза) Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнена дата текущего среза. Данное значение заполняется автоматически при выборе даты анализа.';uk='Не заповнена дата поточного зрізу. Дане значення заповнюється автоматично при виборі дати аналізу.'"),,"ДатаТекущегоСреза");
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДатаПредыдущегоСреза) Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнена дата преыдыдущего среза.';uk='Не заповнена дата попереднього зрізу.'"),,"ДатаПредыдущегоСреза");
	КонецЕсли;
...
Процедура ЗаполнитьПараметры(Параметр1, Параметр2, КоличествоКлиентов, Область)
	...
	Если Изменения Тогда
		...
		Если ТипПараметра1 = Тип("ПеречислениеСсылка.ABCКлассификация") Тогда
			...
			ЗаголовокСписка = НСтр("ru='Перешли из %Параметр1% в %Параметр2%';uk='Перейшли з %Параметр1% в %Параметр2%'");
		Иначе
	...
	Иначе
		...
		ЗаголовокСписка = НСтр("ru='Отнесены к %Параметр1%, %Параметр2%';uk='Віднесені до %Параметр1%, %Параметр2%'");
	КонецЕсли;
...
Процедура УстановитьПериоды(ТекстОшибки)
	...
	Если Не Выборка.Пустой() Тогда
		...
		Иначе
			ТекстОшибки = НСтр("ru='На %Дата% отсутствует предыдущий срез.';uk='На %Дата% відсутній попередній зріз.'");
		...
		КонецЕсли;
	Иначе
		ТекстОшибки = НСтр("ru='На %Дата% классификация отсутствует.';uk='На %Дата% класифікація відсутня.'");
		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Дата%", Формат(АнализНаДату, "ДЛФ=ДД"));
	КонецЕсли;
...
Процедура ВывестиМатрицы()
	...
	Пока ВыборкаТекущийXYZ.Следующий() Цикл
		...
		Если ВыборкаТекущийXYZ.ТекущийXYZ = Перечисления.XYZКлассификация.НеКлассифицирован Тогда
			...
			ИмяРасшифровки = "Отчет.ABCXYZРаспределениеКлиентов.Форма";
			ЗаголовокСписка = НСтр("ru='Потерянные клиенты';uk='Втрачені клієнти'");
		...
		ИначеЕсли ВыборкаТекущийXYZ.ТекущийXYZ = Перечисления.XYZКлассификация.ПустаяСсылка() Тогда
			...
			ИмяРасшифровки = "Отчет.ABCXYZРаспределениеКлиентов.Форма";
			ЗаголовокСписка = НСтр("ru='Потенциальные клиенты';uk='Потенційні клієнти'");
		...
		ОбластьМатриц.Параметры.dВопросы = Новый Структура(
			...
			ИмяРасшифровки, НСтр("ru='Отнесены к категории BCG ""вопросы""';uk='Віднесені до категорії BCG ""питання""'"), Истина);
