...
Функция СформироватьОтчет()
	...
	Если НЕ ЗначениеЗаполнено(ПараметрПериод) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ПараметрПериод) Тогда
			СтрокаСообщения = НСтр("ru='Не установлено значение параметра ""Дата отчета"".';uk='Не встановлене значення параметру ""Дата звіту"".'");
	...
	ОбластьЗаголовок.Параметры.ЗаголовокОтчета = НСтр("ru='Анализ исполнения ассортимента';uk='Аналіз виконання асортименту'")
	...
	ТекстПараметров = НСтр("ru='Дата отчета :';uk='Дата звіту :'") + " " + ПараметрПериод;
	...
	Если ЭлементыОтбора.Количество() > 0 Тогда
		...
		Для Каждого ЭлементОтбора ИЗ ЭлементыОтбора Цикл
			...
			Если ЛевоеЗначение = НСтр("ru='Ассортимент исполняется';uk='Асортимент виконується'") Тогда
	...
	Если ЭтоРасшифровка Тогда
		ОбластьШапка.Параметры.ЗаголовокГруппировок = НСтр("ru='Товарная категория / Марка / Номенклатура';uk='Товарна категорія / Марка / Номенклатура'");
	...
	Иначе
		ОбластьШапка.Параметры.ЗаголовокГруппировок= НСтр("ru='Вид номенклатуры / Товарная категория';uk='Вид номенклатури / Товарна категорія'");
		ОбластьУровень3 = Макет.ПолучитьОбласть("ОбластьУровень3|Группировки");
		ОбластьУровень3_Показатели = Макет.ПолучитьОбласть("ОбластьУровень3|Показатели");
...
Процедура ЗаполнитьПараметрыИВывестиОбласть(СтруктураОбластей, Выборка, СтруктураРасшифровки, СоответствиеПоказателей)
	...
	Для н = 1 по 6 Цикл
		ПодОбласть = ОбластьПоказатели.Область(1,н,1,н);
