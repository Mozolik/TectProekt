...
Процедура ВыполнитьЗагрузкуДанныхИзСообщенияОбменаВКэш(Отказ, ИндексыСтрок)
	...
	Если ТаблицыДляЗагрузки.Количество() > 0 Тогда
		...
		Если ОбработкаОбменаДанными.ФлагОшибки() Тогда
			...
			НСтрока = НСтр("ru='При загрузке сообщения обмена возникли ошибки: %1';uk='При завантаженні повідомлення обміну виникли помилки: %1'");
...
Процедура УдалитьВременныйКаталогСообщенийОбмена(ИмяВременногоКаталога)
	...
	Если Не ПустаяСтрока(ИмяВременногоКаталога) Тогда
		...
		Исключение
			СтрокаСообщения = НСтр("ru='Ошибка при попытке удаления файлов из временного каталога сообщений обмена %1. Описание ошибки: %2';uk='Помилка при спробі вилучення файлів з тимчасового каталогу повідомлень обміну %1. Опис помилки: %2'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ДополнитьТаблицуСтатистики(Отказ)
	...
	Для Каждого СтрокаТаблицы Из ИнформацияСтатистики Цикл
		...
		Исключение
			...
			СтрокаСообщения = НСтр("ru='Ошибка: тип ""%1"" не определен.';uk='Помилка: тип ""%1"" не визначений.'");
