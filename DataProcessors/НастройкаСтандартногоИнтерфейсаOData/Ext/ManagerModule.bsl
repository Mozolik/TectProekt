...
//                                         к станартному интерфейсу OData.
//
...
//                                         в составе структуры только в тех слуаях, когда требуется
//                                         изменить пароль.
//
	...
	Если НастройкиАвторизации.Используется Тогда
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(
				ИмяСобытияЖурналаРегистрации(НСтр("ru='ЗаписьПользователя';uk='ЗаписьПользователя'")),
		...
		Исключение
			...
			Комментарий = СтрШаблон(
				НСтр("ru='При записи пользователя для стандартного интерфейса OData произошла ошибка.
                      ...
                      |%2
                      |-------------------------------------------'
                      |;uk='Підчас запису користувача для стандартного інтерфейсу OData сталася помилка.
                      |
                      |Опис користувача ІБ:
                      |-------------------------------------------
                      |%1
...
// логин и пароль которого будет исопльзоваться при подключении к стандартному интерфейсу
// OData (в модели сервиса).
//
...
Процедура ПроверитьСоставРолиOData() Экспорт
	...
	Если ИзбыточныеПрава.Количество() > 0 Тогда
		ТекстОшбки = Символы.НПП + НСтр("ru='Следующие права избыточно включены в состав роли:';uk='Такі права надлишково включені до складу ролі:'") + Символы.ПС + Символы.ВК + 
			ПредставлениеИзбыточныхИлиНедоставющихПрав(ИзбыточныеПрава, 2);
		Ошибки.Добавить(ТекстОшбки);
	КонецЕсли;
	
	Если НедостающиеПрава.Количество() > 0 Тогда
		ТекстОшбки = Символы.НПП + НСтр("ru='Следующие права должны быть включены в состав роли:';uk='Такі права повинні бути включені до складу ролі:'") + Символы.ПС + Символы.ВК + 
			ПредставлениеИзбыточныхИлиНедоставющихПрав(НедостающиеПрава, 2);
		Ошибки.Добавить(ТекстОшбки);
	КонецЕсли;
	
	Если Ошибки.Количество() > 0 Тогда
		
		ТекстИсключения = СтрШаблон(НСтр("ru='Обнаружены ошибки в составе прав роли %1:';uk='Виявлені помилки у складі прав ролі %1:'"),
			РольДляСтандартногоИнтерфейсаOData().Имя);
...
Функция ПараметрыНастройкиСоставаСтандартногоИнтерейсаOData() Экспорт
...
Процедура ПроверитьСоставРолиODataПоОбъектуМетаданных(ОбъектМетаданных, ЭталонныйСостав, ИзбыточныеПрава, НедоставющиеПрава)
	...
	Если НедостающиеПраваПоОбъекту.Количество() > 0 Тогда
		НедоставющиеПрава.Вставить(ОбъектМетаданных, НедостающиеПраваПоОбъекту);
	КонецЕсли;
...
Функция ЭтоДопустимыйОбъектМетаданныхOData(Знач ОбъектМетаданных)
	...
	Если ОбщегоНазначенияБТС.ЭтоСправочник(ОбъектМетаданных)
		...
		Или ОбщегоНазначенияБТС.ЭтоРегистрБухгалетрии(ОбъектМетаданных)
...
Функция ПредставлениеИзбыточныхИлиНедоставющихПрав(Знач ОписанияПрав, Знач Отступ)
...
Функция СвойстваПользователяСтандартногоИнтерфейсаOData()
	...
	Если Не ПравоДоступа("АдминистрированиеДанных", Метаданные) Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав доступа для настройки автоматического REST-сервиса';uk='Недостатньо прав доступу для настройки автоматичного REST-сервісу'");
	КонецЕсли;
...
Функция ИмяСобытияЖурналаРегистрации(Знач Суффикс)
	...
	Возврат НСтр("ru='НастройкаСтандартногоИнтерфейсаOData.';uk='НастройкаСтандартногоИнтерфейсаOdata.'") + СокрЛП(Суффикс);
...
Процедура ПроверитьСоставРолиODataПриОбновлении() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Обработчик не должен использоваться при выключенном разделении по областям данных';uk='Обробник не повинен використовуватися при вимкненому поділі по областях даних'");
	КонецЕсли;
...
Процедура ПроверитьВозможностьСозданияПользователяДляВызововСтандартногоИнтерфейсаOData()
	...
	Если КоличествоПользователей = 0 Тогда
		...
		ВызватьИсключение НСтр("ru='Нельзя создать отдельные логин и пароль для использования автоматического REST-сервиса, т.к. в программе отсутствуют другие пользователи.';uk='Не можна створити окремі логін та пароль для використання автоматичного REST-сервісу, оскільки в програмі відсутні інші користувачі.'");
