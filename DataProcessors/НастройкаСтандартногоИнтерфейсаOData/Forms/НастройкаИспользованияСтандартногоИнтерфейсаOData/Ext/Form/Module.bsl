...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДанныеИнициализации = Обработки.НастройкаСтандартногоИнтерфейсаOData.ПараметрыНастройкиСоставаСтандартногоИнтерейсаOData();
...
Процедура ОбъектыМетаданныхИспользованиеПриИзменении(Элемент)
	...
	Если Зависимости.Количество() > 0 Тогда
		...
		Колбек = Новый ОписаниеОповещения("ОбъектыМетаданныхИспользованиеПриИзмененииПродолжение", ЭтотОбъект, Контекст);
...
&НаСервере
Процедура УстановитьИспользованиеЗависимостей(Знач Зависимости, Знач Использовнаие)
	...
	Для Каждого КорневойЭлемент Из КорневыеЭлементы Цикл
		...
		Для Каждого ЭлементДерева Из ЭлементыДерева Цикл
			...
			Если Зависимости.Найти(ЭлементДерева.ПолноеИмя) <> Неопределено Тогда
				...
				ЭлементДерева.Использование = Использовнаие;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		Колбек = Новый ОписаниеОповещения("ПродолжитьЗакрытиеПослеВопроса", ЭтотОбъект);
		ПоказатьВопрос(Колбек, НСтр("ru='Данные были изменены. Сохранить изменения?';uk='Дані були змінені. Зберегти зміни?'"), РежимДиалогаВопрос.ДаНетОтмена);
