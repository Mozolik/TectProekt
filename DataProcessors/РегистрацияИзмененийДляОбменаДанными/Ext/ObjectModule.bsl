...
// Параметры:
	...
	ТекНомерСтроки = 1;
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 1,  2,  Ложь,   "Константы",               НСтр("ru='Константы';uk='Константи'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 3,  4,  Истина, "Справочники",             НСтр("ru='Справочники';uk='Довідники'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 5,  6,  Истина, "Последовательности",      НСтр("ru='Последовательности';uk='Послідовності'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 7,  8,  Истина, "Документы",               НСтр("ru='Документы';uk='Документи'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 9,  10, Истина, "ПланыВидовХарактеристик", НСтр("ru='Планы видов характеристик';uk='Плани видів характеристик'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 11, 12, Истина, "ПланыСчетов",             НСтр("ru='Планы счетов';uk='Плани рахунків'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 13, 14, Истина, "ПланыВидовРасчета",       НСтр("ru='Планы видов расчета';uk='Плани видів розрахунку'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 15, 16, Истина, "РегистрыСведений",        НСтр("ru='Регистры сведений';uk='Регістри відомостей'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 17, 18, Истина, "РегистрыНакопления",      НСтр("ru='Регистры накопления';uk='Регістри накопичення'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 19, 20, Истина, "РегистрыБухгалтерии",     НСтр("ru='Регистры бухгалтерии';uk='Регістри бухгалтерії'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 21, 22, Истина, "РегистрыРасчета",         НСтр("ru='Регистры расчета';uk='Регістри розрахунку'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 23, 24, Истина, "БизнесПроцессы",          НСтр("ru='Бизнес-процессы';uk='Бізнес-процеси'"));
	СформироватьУровеньМетаданных(ТекНомерСтроки, ТекПараметры, 25, 26, Истина, "Задачи",                  НСтр("ru='Задачи';uk='Задачі'"));
...
Функция ИзмеренияНабораЗаписей(ИмяТаблицы, ВсеИзмерения = Ложь) Экспорт
	...
	Если Метаданные.РегистрыНакопления.Содержит(Мета)
	 ...
	 Или Метаданные.Последовательности.Содержит(Мета) Тогда
		...
		Строка.Заголовок   = НСтр("ru='Регистратор';uk='Реєстратор'");
	КонецЕсли;
	...
	Если ЭтоРегистрСведений И Мета.ОсновнойОтборПоПериоду Тогда
		...
		Строка.Заголовок   = НСтр("ru='Период';uk='Період'");
	КонецЕсли;
	...
	Если Метаданные.РегистрыРасчета.Содержит(Мета.Родитель()) Тогда
		...
		Строка.ТипЗначения = Документы.ТипВсеСсылки();
		Строка.Заголовок   = НСтр("ru='Объект перерасчета';uk='Об''єкт перерахунку'");
	КонецЕсли;
...
Функция ПредставлениеСсылки(ОбъектПредставления) Экспорт
	...
	Если ПустаяСтрока(Результат) Тогда
		Результат = НСтр("ru='не задано';uk='не задано'");
	КонецЕсли;
...
Функция ПроверитьКорректностьНастроек(КлючНастройки = "") Экспорт
		...
		Попытка
			...
			Иначе
				Если ЭтоФайловаяБаза() Тогда
					Текст = НСтр("ru='Файл ""%1"" не доступен';uk='Файл ""%1"" не доступний'");
				Иначе
					Текст = НСтр("ru='Файл ""%1"" не доступен на сервере';uk='Файл ""%1"" не доступний на сервері'");
				КонецЕсли;
	...
	Иначе
		...
		Если Метаданные.Обработки.Найти(НастройкаАдресВнешнейОбработкиЗапросов) = Неопределено Тогда
			Текст = НСтр("ru='Обработка ""%1"" не найдена в составе конфигурации';uk='Обробка ""%1"" не знайдена в складі конфігурації'");
			Результат.НастройкаАдресВнешнейОбработкиЗапросов = СтрЗаменить(Текст, "%1", НастройкаАдресВнешнейОбработкиЗапросов);
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	Инфо.Вставить("Наименование", НСтр("ru='Регистрация изменений для обмена данными';uk='Реєстрація змін для обміну даними'"));
	Инфо.Вставить("Версия",       "1.0");
	Инфо.Вставить("ВерсияБСП",    "1.2.1.4");
	Инфо.Вставить("Информация",    НСтр("ru='Обработка для управления регистрацией объектов на узлах обмена до формирования выгрузки. При работе в составе конфигурации с БСП версии 2.1.2.0 и старше производит контроль ограничений миграции данных для узлов обмена.';uk='Обробка для управління реєстрацією об''єктів на вузлах обміну до формування вивантаження. При роботі в складі конфігурації із БСП версії 2.1. 2.0 і старше робить контроль обмежень міграції даних для вузлів обміну.'"));
	...
	Команда.Представление = НСтр("ru='Редактирование регистрации изменений объекта';uk='Редагування реєстрації змін об''єкта'");
