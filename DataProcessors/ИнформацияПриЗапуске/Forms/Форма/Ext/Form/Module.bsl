...
Функция ПодготовитьДанныеФормы()
	...
	Для Каждого ПакетСтраниц Из ПодготовленныеПакеты Цикл
		ПакетСтраниц.ЗаголовокФормы = НСтр("ru='Информация';uk='Інформація'");
		...
		Если СтрНачинаетсяС(ПакетСтраниц.Раздел, "_") Тогда
			...
			Иначе
				...
				Если ТекущееПодменю = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найдена группа ""%1""';uk='Не знайдена група ""%1""'"), ИмяПодменю);
				КонецЕсли;
		...
		ИначеЕсли НаименованиеТекущегоРаздела <> ПакетСтраниц.Раздел Тогда
			...
			ЭтоГлавная = (ПакетСтраниц.Раздел = НСтр("ru='Главная';uk='Головна'"));
