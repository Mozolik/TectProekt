...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЭтоУправлениеТорговлей Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Свертка выполняется только для Управления торговлей';uk='Згортка виконується тільки для Управління торгівлею'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Не ВыполняетсяЗакрытие Тогда
		...
		НСтрока = НСтр("ru='Завершить работу с помощником?';uk='Завершити роботу з помічником?'");
...
Процедура ДекорацияЖурналРегистрацииНажатие(Элемент)
	ОткрытьФорму("Обработка.ЖурналРегистрации.Форма", Новый Структура("СобытиеЖурналаРегистрации", НСтр("ru='СверткаИБ';uk='СверткаИБ'")));
КонецПроцедуры
...
Процедура Подключаемый_СтраницаПриветствие_ПриПереходеДалее(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаСверткиИБ) ТОгда
		ТекстСообщения = НСтр("ru='Поле ""Дата свертки"" не заполнено';uk='Поле ""Дата згортки"" не заповнено'");
