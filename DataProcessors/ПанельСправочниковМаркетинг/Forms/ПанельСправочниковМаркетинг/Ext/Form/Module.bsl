...
Процедура УправлениеЭлементами()
	...
	Элементы.ОткрытьГруппыРассылокИОповещений.Видимость  = ПравоДоступаГруппыРассылокИОповещений И ИспользоватьПочтовыйКлиент;
	Элементы.ОткрытьВидыОповещений.Видимость             = ПравоДоступаВидыОповещений И ИспользоватьПочтовыйКлиент;
	Элементы.ОткрытьСправочникШаблоныСообщений.Видимость = ПравоДоступаШаблоныСообщений И ИспользоватьПочтовыйКлиент;
	...
	Если НЕ ИспользоватьСделки Тогда
		...
		Элементы.ГруппаНастроекСделки.Заголовок = НСтр("ru='Проекты';uk='Проекти'");
	...
	Если ИспользоватьПатнеровКакКонтрагентов Тогда
		...
		Если КомандаРолиПартнеровВСделкахИПроектах <> Неопределено Тогда
			КомандаРолиПартнеровВСделкахИПроектах.Заголовок = НСтр("ru='Роли контрагентов в сделках и проектах';uk='Ролі контрагентів в угодах та проектах'");
			КомандаРолиПартнеровВСделкахИПроектах.Подсказка = 
			       НСтр("ru='Классификатор, используемый для понимания, какую роль выполняют косвенно или непосредственно участвующие в сделке или проекте контрагенты.';uk='Класифікатор, що використовується для розуміння, яку роль виконують контрагенти, які побічно або безпосередньо беруть участь в угоді або проекті.'");
		КонецЕсли;
	КонецЕсли;
	...
	Если Не Элементы.ГруппаРолиКонтактныхЛиц.Видимость Тогда
		Если ИспользоватьПартнеровКакКонтрагентов Тогда
			Элементы.ГруппаПартнерыКонтактныеЛица.Заголовок = НСтр("ru='Контрагенты';uk='Контрагенти'");
		Иначе
			Элементы.ГруппаПартнерыКонтактныеЛица.Заголовок = НСтр("ru='Партнеры';uk='Партнери'");
		КонецЕсли;
	Иначе
		Если ИспользоватьПартнеровКакКонтрагентов Тогда
			Элементы.ГруппаПартнерыКонтактныеЛица.Заголовок = НСтр("ru='Контрагенты и контактные лица';uk='Контрагенти та контактні особи'");
		КонецЕсли;
	КонецЕсли;
	...
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	           ...
	           НСтр("ru='Виды ролей контактных лиц, описывающих деятельность контактного лица на предприятии контрагента. Например, директор, руководитель отдела, менеджер.';uk='Види ролей контактних осіб, що описують діяльність контактної особи на підприємстві контрагента. Наприклад, директор, керівник відділу, менеджер.'"),
	...
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	           ...
	           НСтр("ru='Классификатор деления контрагентав на группы, по которым можно ограничивать доступ на просмотр, добавление (изменение) контрагентов и доступ к документам по контрагентам.';uk='Класифікатор розподілу контрагентів на групи, за якими можна обмежувати доступ на перегляд, додавання (зміну) контрагентів і доступ до документів по контрагентам.'"),
	           ИспользоватьПартнеровКакКонтрагентов);
	
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	           ЭтотОбъект, "СправочникВидыСвязейПартнеров",
	           НСтр("ru='Виды связей контрагентов.';uk='Види зв''язків контрагентів.'"),
	           ИспользоватьПартнеровКакКонтрагентов);
	
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	           ЭтотОбъект, "ДекорацияВидыСвязейПартнеров",
	           НСтр("ru='Классификатор видов деловых связей , которые могут быть настроены между контрагентами торгового предприятия.';uk='Класифікатор видів ділових зв''язків , які можуть бути налаштовані між контрагентами торговельного підприємства.'"),
	           ИспользоватьПартнеровКакКонтрагентов);
