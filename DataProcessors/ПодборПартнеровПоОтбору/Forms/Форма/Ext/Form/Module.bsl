...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                       ЭтотОбъект, "ПодобранныеПартнеры", НСтр("ru='Подобранные контрагенты';uk='Підібрані контрагенти'"), ИспользоватьПартнеровКакКонтрагентов);
	ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
	                       ЭтотОбъект, "ПартнерыПартнеры", НСтр("ru='Контрагент';uk='Контрагент'"), ИспользоватьПартнеровКакКонтрагентов);
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ВыполняетсяЗакрытие И Не ПеренестиВДокумент И Партнеры.Количество() > 0 Тогда
		
		Если ИспользоватьПартнеровКакКонтрагентов Тогда
			ТекстВопроса = НСтр("ru='Подобранные контрагенты не перенесены в документ. Перенести?';uk='Підібрані контрагенти не перенесені в документ. Перенести?'");
		Иначе
			ТекстВопроса = НСтр("ru='Подобранные партнеры не перенесены в документ. Перенести?';uk='Підібрані партнери не перенесені в документ. Перенести?'");
		КонецЕсли;
...
Процедура ОбработатьПереданныеПараметры(Параметры)
	...
	Иначе
		ПартнерыИКонтрагенты.ЗаголовокРеквизитаВЗависимостиОтФОИспользоватьПартнеровКакКонтрагентов(
		                          ЭтотОбъект, "", НСтр("ru='Подбор контрагентов по отбору';uk='Підбір контрагентів за відбором'"));
	КонецЕсли;
