...
Процедура ПостроитьФормуПоЧастямАдреса()
	...
	Для Каждого КлючЗначение Из ЧастиАдреса Цикл
		...
		Если Часть.Уровень < 7 И НЕ (ФорматАдреса = "КЛАДР" И (Часть.Уровень = 2 ИЛИ Часть.Уровень = 5))Тогда
			...
			Если ФорматАдреса = "КОАТУУ" Тогда
...
Процедура Подключаемый_ПолеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипВыбранного = Тип("Структура") Тогда
		...
		ОчиститьДочерниеПоляАдреса(Элемент.Имя);
		ЗаполнитьРодительскиеПоляАдреса(Элемент.Имя, ВыбранноеЗначение);
...
&НаКлиенте
Процедура ЗаполнитьРодительскиеПоляАдреса(ИмяЭлемента, ВыбранноеЗначение)
	...
	Если УровеньВыбранногоПоля = 0 ИЛИ УровеньВыбранногоПоля = 1 Тогда
		Возврат;
	КонецЕсли;
	
	Если УровеньВыбранногоПоля > 1 Тогда
		Элемент = ЭлементыФормыПоУровням.НайтиПоЗначению(1);
		Если Элемент <> Неопределено Тогда 
			Если ЭтотОбъект[Элемент.Представление] <> "" Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	ТекстВопроса = НСтр("ru='Заполнить предыдущие уровни?';uk='Заповнити попередні рівні?'");
	
	ДополнительныеПараметры = Новый Структура();
	ДополнительныеПараметры.Вставить("УровеньПоля", УровеньВыбранногоПоля);
	ДополнительныеПараметры.Вставить("Родители", ВыбранноеЗначение.Родители);
	
	Оповещение = Новый ОписаниеОповещения("ПослеВопросаОЗаполненииРодителя", ЭтотОбъект, ДополнительныеПараметры);
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеВопросаОЗаполненииРодителя(Результат, Параметр) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		
		УровеньВыбранногоПоля = Параметр.УровеньПоля;
		Родители              = Параметр.Родители;
		
		ЧастиАдресаВышеПоУровню = Новый СписокЗначений;
		Для Каждого Часть Из ЭлементыФормыПоУровням Цикл
			Если Часть.Значение < УровеньВыбранногоПоля Тогда
				ЧастиАдресаВышеПоУровню.Добавить(Часть.Значение, Часть.Представление);
			КонецЕсли;
		КонецЦикла;
		
		ЧастиАдресаВышеПоУровню.СортироватьПоЗначению(НаправлениеСортировки.Убыв);
		
		ТекущийРодитель = Неопределено;
		
		Для Индекс = 1 ПО ЧастиАдресаВышеПоУровню.Количество() Цикл
			
			Родители.Свойство("ВышеНа"+Индекс, ТекущийРодитель);
			Если ТекущийРодитель <> Неопределено Тогда
				УстановитьЗначениеЧастиАдреса(Элементы[ЧастиАдресаВышеПоУровню[Индекс-1].Представление], ТекущийРодитель);
			КонецЕсли;
...
Процедура Подключаемый_ПолеАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если (НЕ ФорматАдреса = "КОАТУУ") Или СтрДлина(Текст) < 3 Тогда
