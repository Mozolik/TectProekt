...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.СписокКонтактнойИнформации) = Тип("Массив") Тогда
		...
		КонецЦикла;
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='История изменений (%1)';uk='Історія змін (%1)'"), ПредставлениеКонтактнойИнформации);
...
Процедура ИсторияПередУдалением(Элемент, Отказ)
	...
	Если НЕ Отказ Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru='Вы хотите удалить адрес действующий с';uk='Ви хочете вилучити адреса чинний з'") + " " + Формат(ДействуетС, "ДЛФ=DD")+ "?", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
	Отказ = Истина;
КонецПроцедуры
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Первоначальное значение';uk='Первісне значення'"));
...
Функция КонтактнаяИнформацияНеКорректная()
	...
	Для Индекс = 0 По История.Количество() - 1 Цикл
		...
		Если ПустаяСтрока(СтрокаИстории.ЗначенияПолей) ИЛИ ПустаяСтрока(СтрокаИстории.Представление)Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Поле ""%1"" не может быть пустым.';uk='Поле ""%1"" не може бути порожнім.'"), ПредставлениеКонтактнойИнформации),, "История[" + Строка(Индекс) + "].Представление");
