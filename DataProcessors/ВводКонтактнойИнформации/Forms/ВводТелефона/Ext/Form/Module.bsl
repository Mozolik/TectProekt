...
Функция РезультатВыбора()
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("Обработка.ВводКонтактнойИнформации.Форма.ВводТелефона", "КодыСтраныИГорода", Коды, НСтр("ru='Коды страны и города';uk='Коди країни і міста'"));
...
Функция ОшибкиЗаполненияТелефона()
	...
	Если СтрДлина(НомерТелефонаТолькоЦифры) > 15 Тогда
		СписокОшибок.Добавить("НомерТелефона", НСтр("ru='Номере телефона слишком длинный';uk='Номер телефону занадто довгий'"));
	КонецЕсли;
	Если НомерТелефонаСодержитНедопустимыеСимволы(ПолныйНомерТелефона) Тогда
		СписокОшибок.Добавить("НомерТелефона", НСтр("ru='Номер телефона содержит недопустимые символы';uk='Номер телефону містить заборонені символи'"));
	КонецЕсли;
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок)
	...
	Если СписокОшибок.Количество()=0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Телефон введен корректно.';uk='Телефон введений коректно.'"));
		Возврат;
	КонецЕсли;
