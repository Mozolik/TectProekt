...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АвтоЦвет                 = Новый Цвет;
	СтранаРоссия = Справочники.СтраныМира.ПустаяСсылка();
	...
	ВидКонтактнойИнформации.ПроверятьКорректность = Ложь;
	...
	Если АдресТолькоРоссийский  Тогда
		...
		Если Страна <> СтранаУкраина Тогда
			...
			КонецЕсли;
			ТекстПредупрежденияПриОткрытии = НСтр("ru='Адрес введен некорректно: допускается ввод только украинских адресов. Значение поля ""Страна"" было изменено на ""Украина"", необходимо проверить остальные поля.';uk='Адреса введено некоректно: дозволяється введення лише українських адрес. Значення поля ""Країна"" було змінено на ""Україна"", необхідно перевірити інші поля.'"); 
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
					...
					Если СведенияОРегионе.Загружен = Ложь Тогда
						...
						ДоступностьКлассификатора.КраткоеПредставлениеОшибки = 
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Необходимо загрузить адресные сведения по региону ""%1"" (в меню ""%2"").';uk='Необхідно завантажити адресні відомості по регіону ""%1"" (в меню ""%2"").'"),
							XDTOКонтактная.Состав.Состав.СубъектРФ,
							?(ЭтоТакси, НСтр("ru='Еще';uk='Ще'"), НСтр("ru='Все действия';uk='Всі дії'")));
	...
	ИначеЕсли ВыбранноеЗначение.Отказ Тогда
		...
		Иначе
			Если НЕ ПодборПоИндексуДоступен Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Индекс %1 не найден в адресном классификаторе';uk='Індекс %1 не знайдено в адресному класифікаторі'"),
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		Если МожноЗагружатьКлассификатор И РегионНеЗагружен Тогда
			...
			УправлениеКонтактнойИнформациейКлиент.ПредложениеЗагрузкиКлассификатора(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Данные для ""%1"" не загружены.';uk='Дані для ""%1"" не завантажені.'"), ВыбранноеЗначение.Представление),
...
Процедура УлицаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		ПараметрыФормы.Вставить("Улица", Улица);
		ПараметрыФормы.Вставить("Заголовок", НаселенныйПункт + " " + НСтр("ru='(не найден)';uk='(не знайдений)'"));
	КонецЕсли;
...
Процедура НаселенныйПунктИностранныйАдресаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СоответствиеСтранаКлассификатор = УправлениеКонтактнойИнформациейКлиентСервер.ПолучитьСоответствиеСтранаКлассификатор();
	КлассификаторДляСтраны = ?(ЗначениеЗаполнено(СоответствиеСтранаКлассификатор[ВРег(Страна)]), СоответствиеСтранаКлассификатор[ВРег(Страна)], "КЛАДР");
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("НаселенныйПунктДетально", НаселенныйПунктДетально);
	ПараметрыФормы.Вставить("ФорматАдреса", КлассификаторДляСтраны);
...
Процедура ПроверитьЗаполнениеАдреса(Команда)
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		ПоказатьПредупреждение(, НСтр("ru='Адрес не может быть проверен, так как он введен в свободной форме.';uk='Адреса не може бути перевірена, тому що вона введена у вільній формі.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВвестиАдресВСвободнойФорме(Команда)
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		ТекстВопроса = НСтр("ru='Изменения, введенные вручную, будут потеряны.
                                                        ...
                                                        |Продолжить?'
                                                        |;uk='Зміни, введені вручну, будуть втрачені.
                                                        |Вихідний адресу буде переміщений в коментар.
                                                        |Продовжити?'");
	Иначе
		ТекстВопроса = НСтр("ru='Вы действительно хотите ввести адрес в свободной форме?';uk='Ви дійсно хочете ввести адресу у вільній формі?'");
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ВвестиАдресВСвободнойФормеЗавершение", ЭтотОбъект);
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , , НСтр("ru='Подтверждение';uk='Підтвердження'"));
КонецПроцедуры
...
Процедура ВвестиАдресВСвободнойФормеЗавершение(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		Комментарий = ?(НЕ ПустаяСтрока(Комментарий), Комментарий + Символы.ПС, "")
			+ НСтр("ru='Исходный адрес:';uk='Вихідна адреса:'") + " " + ПредставлениеАдреса;
		ПодключитьОбработчикОжидания("УстановитьПиктограммуКомментария", 0.1, Истина);
	КонецЕсли;
...
Функция РезультатаВыбораПоОбновлениюФлагов(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	Контекст.ВидКонтактнойИнформации.ПроверятьКорректность      = Ложь;
...
Функция СписокОшибокЗаполнения(Контекст, ПредупреждатьОбОтсутствии)
	...
	Если Результат.Количество() = 0 // Нет ошибок
		...
		Результат.Добавить("/", НСтр("ru='Адрес пуст.';uk='Адреса порожня.'"));
	КонецЕсли;
...
Процедура ЗначениеРеквизитовПоКонтактнойИнформации(Контекст, РедактируемаяИнформация)
	...
	Иначе
		СсылкаНаУкраину = Справочники.СтраныМира.Украина;
		Если ВРЕГ(НаименованиеСтраны) = "УКРАИНА" 
			ИЛИ ВРЕГ(НаименованиеСтраны) = "УКРАЇНА" 
			ИЛИ ВРЕГ(НаименованиеСтраны) = ВРЕГ(СокрЛП(СсылкаНаУкраину.Наименование)) Тогда
			Контекст.Страна    = СсылкаНаУкраину;
			Контекст.КодСтраны = СсылкаНаУкраину.Код;
		Иначе
	...
	Иначе
		...
		СоответствиеСтранаКлассификатор = УправлениеКонтактнойИнформациейКлиентСервер.ПолучитьСоответствиеСтранаКлассификатор();
		КлассификаторДляСтраны = ?(ЗначениеЗаполнено(СоответствиеСтранаКлассификатор[ВРег(Контекст.Страна)]), СоответствиеСтранаКлассификатор[ВРег(Контекст.Страна)], "КЛАДР");
		Контекст.НаселенныйПунктДетально = УправлениеКонтактнойИнформациейКлиентСервер.СтруктураЧастейАдресаНаселенногоПункта(КлассификаторДляСтраны);
...
Процедура ЗначениеРеквизитовПоКонтактнойИнформацииАдресИностранныйАдрес(Адрес, Контекст)
	...
	Контекст.НаселенныйПунктДетально.Регион.Наименование = СтруктураАдрес.Регион;
	Контекст.НаселенныйПунктДетально.Район.Наименование = СтруктураАдрес.Район;
	Контекст.НаселенныйПунктДетально.Город.Наименование = СтруктураАдрес.Город;
	Контекст.НаселенныйПунктДетально.НаселенныйПункт.Наименование = СтруктураАдрес.НаселенныйПункт;
...
Процедура ОтобразитьИнформациюОДатахДействияАдреса(ДействуетС, ДействуетДо)
	...
	Если ЗначениеЗаполнено(ДействуетДо) И ЗначениеЗаполнено(ДействуетС)Тогда
		Если НачалоДня(ДействуетДо -10) = НачалоДня(ДействуетС) Тогда
			ТекстИсторическийАдрес = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Адрес действовал только %1';uk='Адреса діяла тільки %1'"), Формат(ДействуетС, "ДЛФ=DD"));
		Иначе
			ТекстИсторическийАдрес = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Адрес действовал с %1 по %2 (включительно)';uk='Адреса діяла з %1 з %2 (включно)'"), Формат(ДействуетС, "ДЛФ=DD"), Формат(ДействуетДо - 10, "ДЛФ=DD"));
		КонецЕсли;
	ИначеЕсли НЕ ЗначениеЗаполнено(ДействуетС) И ЗначениеЗаполнено(ДействуетДо) Тогда
		ТекстИсторическийАдрес = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Адрес действовал до %1 (включительно)';uk='Адреса діяла до %1 (включно)'"), Формат(ДействуетДо - 10, "ДЛФ=DD"));
	Иначе
		ТекстИсторическийАдрес = НСтр("ru='Адрес действует в настоящие время';uk='Адреса діє у теперішній час'");
	КонецЕсли;
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок, ПредупреждатьОбОтсутствии)
	...
	Если КоличествоОшибок = 0 И ПредупреждатьОбОтсутствии Тогда
		// Нет ошибок
		ПоказатьПредупреждение(, НСтр("ru='Адрес введен корректно.';uk='Адреса введена коректно.'"));
...
Процедура СостояниеВводаПредставленияАдреса(РедактироватьПредставлениеАдреса, ФормироватьПредставление = Истина)
	...
	Если РедактироватьПредставлениеАдреса Тогда
		Элементы.ПредставлениеИностранногоАдреса.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Верх;
		Элементы.ПредставлениеИностранногоАдреса.Заголовок          = НСтр("ru='Адрес в свободной форме';uk='Адреса у вільній формі'");
		...
		КонецЕсли;
	Иначе 
		Элементы.ПредставлениеИностранногоАдреса.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
		Элементы.ПредставлениеИностранногоАдреса.Заголовок          = "";
...
Процедура ПроверитьДоступностьКлассификатора(Знач РезультатПроверки = Неопределено)
	...
	Если РезультатПроверки = Неопределено Тогда
		...
		Иначе
			...
			НаименованиеЗадания = НСтр("ru='Проверка доступности сервиса адресного классификатора';uk='Перевірка доступності сервісу адресного класифікатора'");
...
Процедура ОбновитьДоступностьКлассификатора(Контекст)
	...
	Если ДоступностьКлассификатора.Отказ Тогда
		Контекст.ТекстСообщенияСервиса = НСтр("ru='Автоподбор и проверка адреса недоступны:';uk='Автоматичний вибір та перевірка адреси недоступні:'") + Символы.ПС 
			+ ДоступностьКлассификатора.КраткоеПредставлениеОшибки;
	КонецЕсли;
