...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		ТекстЗаголовка = НСтр("ru='В последний раз резервное копирование проводилось: %1';uk='Востаннє резервне копіювання проводилося: %1'");
	...
	Если РучнойЗапуск Тогда
		...
		Элементы.Далее.Заголовок = НСтр("ru='Сохранить резервную копию';uk='Зберегти резервну копію'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ru='Резервное копирование недоступно в клиенте под управлением ОС Linux.';uk='Резервне копіювання недоступно в клієнті під управлінням ОС Linux.'");
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ТекущаяСтраница = Элементы.СтраницыПомощника.ПодчиненныеЭлементы.СтраницаИнформацииИВыполненияРезервногоКопирования Тогда
		...
		ТекстПредупреждения = НСтр("ru='Прервать подготовку к резервному копированию данных?';uk='Перервати підготовку до резервного копіювання даних?'");
...
Процедура ПерейтиНаСтраницуИнформацииИВыполненияРезервногоКопирования(ПерейтиДалее)
	...
	Если КоличествоСеансовИнформационнойБазы() = 1 Тогда
		...
		СоединенияИБВызовСервера.УстановитьБлокировкуСоединений(НСтр("ru='Для выполнения резервного копирования.';uk='Для виконання резервного копіювання.'"), "РезервноеКопирование");
...
Процедура ПерейтиНаСтраницуРезультатовРезервногоКопирования()
	...
	Элементы.Отмена.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
...
Процедура ОтменитьПодготовку()
	...
	Элементы.НадписьНеУдалось.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1.
        ...
        |;uk='%1.
        |Підготовка до резервному копіюванню скасована. Інформаційна база розблокована.'"),
...
Процедура УстановитьЗаголовокКнопкиДалее(ЭтоКнопкаДалее)
	...
	Элементы.Далее.Заголовок = ?(ЭтоКнопкаДалее, НСтр("ru='Далее >';uk='Далі >'"), НСтр("ru='Готово';uk='Готово'"));
...
Процедура НачатьРезервноеКопирование()
	...
	ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(РезервноеКопированиеИБКлиент.СобытиеЖурналаРегистрации(),
		"Информация",  НСтр("ru='Выполняется резервное копирование информационной базы:';uk='Виконується резервне копіювання інформаційної бази:'") + " " + ИмяГлавногоФайлаСкрипта);
