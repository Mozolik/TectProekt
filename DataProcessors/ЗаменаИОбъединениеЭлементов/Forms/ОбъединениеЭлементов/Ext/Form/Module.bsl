...
Процедура МестаИспользованияПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Удалить из списка для объединения элемент ""%1""?';uk='Вилучити зі списку об''єднання елемент ""%1""?'"),
...
Функция ПроверитьОбъединяемыеСсылки(Знач НаборСсылок)
	...
	Если КоличествоСсылок < 2 Тогда
		Результат.Ошибка = НСтр("ru='Для объединения необходимо указать несколько элементов.';uk='Для об''єднання необхідно зазначити кілька елементів.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если Контроль.ЕстьГруппы Тогда
		Результат.Ошибка = НСтр("ru='Один из объединяемых элементов является группой.
                                      ...
                                      |;uk='Один з об''єднуваних елементів є групою.
                                      |Групи не можуть бути об''єднані.'");
	ИначеЕсли Контроль.КоличествоВладельцев > 1 Тогда 
		Результат.Ошибка = НСтр("ru='У объединяемых элементов различные владельцы.
                                      ...
                                      |;uk='У поєднуваних елементів різні власники.
                                      |Такі елементи не можуть бути об''єднані.'");
	ИначеЕсли Контроль.КоличествоСсылок <> КоличествоСсылок Тогда
		Результат.Ошибка = НСтр("ru='Все объединяемые элементы должны быть одного типа.';uk='Усі елементи, що об''єднуються, повинні бути одного типу.'");
	Иначе 
		// Все нормально
	...
	Если ЕстьПравоБезвозвратногоУдаления Тогда
		Если ТекущийВариантУдаления = "Пометка" Тогда
			ТекстПодсказки = НСтр("ru='Элементы (%1) будут <a href = ""ПереключениеРежимаУдаления"">помечены на удаление</a> и заменены во всех местах
                ...
                |;uk='Елементи (%1) <a href = ""ПереключениеРежимаУдаления"">позначені на вилучення</a> і замінені у всіх місцях
                |використання на ""%2"" (відзначений стрілкою).'");
		Иначе
			ТекстПодсказки = НСтр("ru='Элементы (%1) будут <a href = ""ПереключениеРежимаУдаления"">удалены безвозвратно</a> и заменены во всех местах
                |использования на ""%2"" (отмечен стрелкой).'
                |;uk='Елементи (%1) <a href = ""ПереключениеРежимаУдаления"">вилучені безповоротно</a> і замінені у всіх місцях
                |використання на ""%2"" (відзначений стрілкою).'");
		КонецЕсли;
	Иначе
		ТекстПодсказки = НСтр("ru='Элементы (%1) будут помечены на удаление и заменены во всех местах
            |использования на ""%2"" (отмечен стрелкой).'
            |;uk='Елементи (%1) будуть позначені на вилучення та замінено у всіх місцях
            |використання на ""%2"" (відзначений стрілкою).'");
	КонецЕсли;
...
Функция СообщениеОЗавершении()
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Элементы (%1) объединены в ""%2""';uk='Елементи (%1) об''єднані в ""%2""'"),
...
Процедура СформироватьНадписьНеудачныеЗамены()
	...
	Элементы.РезультатНеудачныеЗамены.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Объединение элементов не выполнено. В некоторых местах использования не может быть произведена
                   ...
                   |;uk='Об''єднання елементів не виконано. В деяких місцях використання не може бути проведена
                   |автоматична заміна на ""%1""'"),
		ОсновнойЭлемент);
...
Процедура ЗаполнитьМестаИспользования(Знач ТаблицаИспользования)
	...
	Для Каждого Строка Из ТаблицаИспользования Цикл
		...
		СтрокаИспользования.НеИспользуется = ?(Мест = 0, НСтр("ru='Не используется';uk='Не використовується'"), "");
	КонецЦикла;
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Объединение элементов %1 в один';uk='Об''єднання елементів %1 в один'"), Представление);
КонецПроцедуры
...
Процедура ДобавитьСтрокиМестИспользования(Знач МассивСсылок)
	...
	Для Каждого Ссылка Из МассивСсылок Цикл
		...
		Если СуществующиеСтроки.Количество() = 0 Тогда
			...
			Строка.НеИспользуется    = НСтр("ru='Не рассчитано';uk='Не розраховано'");
		Иначе
...
Процедура ИнициализироватьСценарийМастера()
	...
	КнопкиМастера.Отмена.Подсказка = НСтр("ru='Отказаться от объединения элементов';uk='Відмовитися від об''єднання елементів'");
	...
	КнопкиМастера.Далее.Заголовок = НСтр("ru='Объединить >';uk='Об''єднати >'");
	КнопкиМастера.Далее.Подсказка = НСтр("ru='Начать объединение элементов';uk='Почати об''єднання елементів'");
	...
	КнопкиМастера.Далее.Видимость = Ложь;
	КнопкиМастера.Отмена.Заголовок = НСтр("ru='Прервать';uk='Перервати'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ru='Прервать объединение элементов';uk='Перервати об''єднання елементів'");
	...
	КнопкиМастера.Отмена.Подсказка = НСтр("ru='Закрыть результаты объединения';uk='Закрити результати об''єднання'");
	...
	КнопкиМастера = КнопкиМастера();
	КнопкиМастера.Назад.Заголовок = НСтр("ru='< В начало';uk='< На початок'");
	КнопкиМастера.Назад.Подсказка = НСтр("ru='Вернутся к выбору основного элемента';uk='Повернутися до вибору основного елемента'");
	КнопкиМастера.Далее.КнопкаПоУмолчанию = Истина;
	КнопкиМастера.Далее.Заголовок = НСтр("ru='Повторить';uk='Повторити'");
	КнопкиМастера.Далее.Подсказка = НСтр("ru='Повторить объединение';uk='Повторити об''єднання'");
	КнопкиМастера.Отмена.Заголовок = НСтр("ru='Отмена';uk='Відмінити'");
	КнопкиМастера.Отмена.Подсказка = НСтр("ru='Закрыть результаты объединения';uk='Закрити результати об''єднання'");
...
Процедура ШагВыборОсновногоЭлементаПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	НастройкиДиалога.Вставить("Заголовок", НСтр("ru='Невозможно объединить элементы';uk='Неможливо об''єднати елементи'"));
	...
	Если Не ИнформацияОЗадании.Завершено Тогда
		Если ПоказатьДиалогПередЗакрытием Тогда
			...
			Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ОпределитьМестаИспользования" Тогда
				ТекстВопроса = НСтр("ru='Прервать поиск мест использования и закрыть форму?';uk='Перервати пошук місць використання і закрити форму?'");
			ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ЗаменитьСсылки" Тогда
				ТекстВопроса = НСтр("ru='Прервать объединение элементов и закрыть форму?';uk='Перервати об''єднання елементів і закрити форму?'");
			КонецЕсли;
...
Функция ФоновоеЗаданиеЗапустить()
	...
	Если НастройкиПошаговогоМастера.ИмяПроцедуры = "ОпределитьМестаИспользования" Тогда
		...
		НаименованиеМетода = НСтр("ru='Поиск и удаление дублей: Определение мест использования';uk='Пошук і вилучення дублів: Визначення місць використання'");
		ПараметрыМетода = МассивСсылокИзНабора(МестаИспользования);
		
	ИначеЕсли НастройкиПошаговогоМастера.ИмяПроцедуры = "ЗаменитьСсылки" Тогда
		
		НаименованиеМетода = НСтр("ru='Поиск и удаление дублей: Объединение элементов';uk='Пошук і вилучення дублів: Об''єднання елементів'");
		ПарыЗамен = Новый Соответствие;
		Для Каждого Строка Из МестаИспользования Цикл
			ПарыЗамен.Вставить(Строка.Ссылка, ОсновнойЭлемент);
		КонецЦикла;
