...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.Заголовок = НСтр("ru='Подбор по категориям';uk='Підбір за категоріями'");
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ВыполняетсяЗакрытие и НЕ ПеренестиВДокумент И Модифицированность Тогда
		...
		ТекстВопроса = НСтр("ru='Состав выбранных товаров был изменен.
                                ...
                                |;uk='Склад обраних товарів був змінений.
                                |Перенести вибрані позиції в документ?'");
		Отказ = Истина;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Прочие марки';uk='Інші марки'"));
...
Процедура СформироватьДеревоКатегорийСервер()
	...
	Запрос.УстановитьПараметр("ТекстПрочиеМарки", НСтр("ru='Прочие марки';uk='Інші марки'"));
...
Процедура ОбновитьИнформационнуюНадписьКоличестваТоваров()
	...
	ПозицииПрописью = НРег(ЧислоПрописью(Объект.ТаблицаВыбранныхТоваров.Количество(), "Л = ru_RU; ДП = Ложь; НП = Истина; НД = Ложь;", НСтр("ru='позиция,позиции,позиций,ж,,,,,0';uk='позиція,позиції,позицій,ж,,,,,0'")));
	ИнформационнаяНадпись = НСтр("ru='Всего подобрано %Позиций%';uk='Усоьго підібрано %Позиций%'")+"(" + ?(ПоказыватьПодобранныеТовары, НСтр("ru='Скрыть';uk='Приховати'"), НСтр("ru='Показать';uk='Показати'")) + ")";
	...
	Если СтрокаДерева <> Неопределено Тогда
		...
		Для Каждого ПодчиненнаяСтрока ИЗ СтрокаДереваСтроки  Цикл
			Если ПодчиненнаяСтрока.ВидКатегорияМарка = Марка
				ИЛИ (ПодчиненнаяСтрока.ВидКатегорияМарка = НСтр("ru='Прочие марки';uk='Інші марки'") И НЕ ЗначениеЗаполнено(Марка)) Тогда
...
Процедура ПроверитьИДобавитьТоварыВКорзину(СписокВыбранныхТоваров)
	...
	Если РезультатПроверкиПриДобавленииВКорзину.Количество() = 1 Тогда
			...
			Если СписокВыбранныхТоваровДляДобавления.Количество() = 0 Тогда
				...
				Если ПроблемаДобавленияТовара.КоличествоТоваров = 1 Тогда
					ТекстВопроса = НСтр("ru='Данный товар уже выведен из ассортимента.
                                                  ...
                                                  |;uk='Даний товар вже виведений з асортименту.
                                                  |Додавання його в документ не потрібно.'");
				Иначе
					ТекстВопроса = НСтр("ru='Выбранные товары уже выведены из ассортимента.
                                                  |Добавление их в документ не требуется.'
                                                  |;uk='Вибрані товари вже виведені з асортименту.
                                                  |Додавання в документ не потрібно.'");
				КонецЕсли; 
			Иначе
				// Некоторые товары можно добавить без вопроса
				ТекстВопроса = НСтр("ru='Часть выбранного товара уже выведена из ассортимента.
                                              |Добавление их в документ не требуется.'
                                              |;uk='Частина обраного товару вже виведена з асортименту.
                                              |Додавання в документ не потрібне.'");
			КонецЕсли;
		...
		ИначеЕсли ПроблемаДобавленияТовара.ИмяПроблемы = "ТоварУжеВСтадии" Тогда	
            ...
            |									  |Добавление его в документ не требуется.'
            |;uk='Даний товар вже перебуває в стадії ""%1"".
            |									|Додавання його в документ не потрібно. '");
				Иначе
					ТекстВопроса = НСтр("ru='Выбранные товары уже находится в стадии ""%1"".
                                                  |Добавление их в документ не требуется.'
                                                  |;uk='Вибрані товари вже знаходиться в стадії ""%1"".
                                                  |Додавання в документ не потрібно.'");
				КонецЕсли; 
			Иначе
				// Некоторые товары можно добавить без вопроса
				ТекстВопроса = НСтр("ru='Часть выбранного товара уже находится в стадии ""%1"".
                                              |Добавление их в документ не требуется.'
                                              |;uk='Частина обраного товару вже знаходиться в стадії ""%1"".
                                              |Додавання в документ не потрібне.'");
			КонецЕсли;
			...
			Если СписокВыбранныхТоваровДляДобавления.Количество() = 0 Тогда
					...
					Если ЗначениеЗаполнено(Марка) Тогда
						ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru='По товарной категории ""%1"" и марке ""%2"" не установлена квота.
                                                            ...
                                                            |;uk='По товарній категорії ""%1"" і марки ""%2"" не встановлена квота.
                                                            |Присутність даного товару в асортименті
                                                            ||суперечить асортиментної політики.'"),
												ТоварнаяКатегория,
												Марка);
					Иначе
						ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru='По товарной категории ""%1"" не установлена квота.
                                                            |Присутствие данного товара в ассортименте
                                                            |														  |противоречит ассортиментной политике.'
                                                            |;uk='По товарній категорії ""%1"" не встановлена квота.
                                                            |Присутність даного товару в асортименті
                                                            ||суперечить асортиментної політики.'"),
												ТоварнаяКатегория);
					КонецЕсли; 
				Иначе
					ТекстВопроса = НСтр("ru='По товарным категориям выбранных товаров не установлена квота.
                                                ...
                                                |									  |противоречит ассортиментной политике.'
                                                |;uk='По товарним категоріям вибраних товарів не встановлена квота.
                                                |Присутність обраних товарів в асортименті
                                                ||суперечить асортиментної політики.'");
				КонецЕсли; 
			Иначе
				// Некоторые товары можно добавить без вопроса
				ТекстВопроса = НСтр("ru='По товарным категориям части выбранных товаров не установлена квота.
                                            ...
                                            |;uk='По товарних категоріях частини вибраних товарів не встановлена квота.
                                            |Присутність цих товарів в асортименті суперечить асортиментній політиці.'");
			КонецЕсли;
	...
	ИначеЕсли РезультатПроверкиПриДобавленииВКорзину.Количество() > 1 Тогда
		...
		Если СписокВыбранныхТоваровДляДобавления.Количество() = 0 Тогда
			...
			Если СписокВыбранныхТоваров.Количество() = 1 Тогда
				ТекстВопроса = НСтр("ru='Добавление товара в документ не требуется:';uk='Додавання товару в документ не потрібно:'");
			Иначе
				ТекстВопроса = НСтр("ru='Добавление выбранных товаров в документ не требуется:';uk='Додавання вибраних товарів в документ не потрібно:'");
			КонецЕсли;
		Иначе
			// Некоторые товары можно добавить без вопроса
			ТекстВопроса = НСтр("ru='Часть выбранного товара не требуется добавлять в документ:';uk='Частину обраного товару не потрібно додавати в документ:'");
		КонецЕсли;
		...
		Для каждого ПроблемаДобавленияТовара Из РезультатПроверкиПриДобавленииВКорзину Цикл
			...
			Если ПроблемаДобавленияТовара.ИмяПроблемы = "ТоварУжеВыведенИзАссортимента" Тогда
				...
				ПредставлениеПроблемы = НСтр("ru='- товар уже выведен из ассортимента';uk='- товар вже виведений з асортименту'");
				
			ИначеЕсли ПроблемаДобавленияТовара.ИмяПроблемы = "ТоварУжеВСтадии" Тогда
				
				ПредставлениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru='- товар уже находится в стадии ""%1""';uk='- товар уже перебуває у стадії ""%1""'"), 
											Стадия);
											
			ИначеЕсли ПроблемаДобавленияТовара.ИмяПроблемы = "НеУстановленаКвота" Тогда

				ПредставлениеПроблемы = НСтр("ru='- не установлена квота';uk='- не встановлена квота'");
	...
	Если ТекстВопроса <> Неопределено Тогда
		...
		Если СписокВыбранныхТоваров.Количество() = 1 Тогда
			СписокКнопок.Добавить("ДобавитьВсе", НСтр("ru='Добавить';uk='Додати'"));
			СписокКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Не добавлять';uk='Не додавати'"));
			
		ИначеЕсли СписокВыбранныхТоваровДляДобавления.Количество() <> 0 Тогда
			СписокКнопок.Добавить("ДобавитьПодходящие", НСтр("ru='Добавить только подходящие';uk='Додати тільки відповідні'"));
			СписокКнопок.Добавить("ДобавитьВсе", НСтр("ru='Добавить все';uk='Додати всі'"));
			СписокКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Не добавлять';uk='Не додавати'"));
		Иначе
			СписокКнопок.Добавить("ДобавитьВсе", НСтр("ru='Добавить все';uk='Додати всі'"));
			СписокКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Не добавлять';uk='Не додавати'"));
...
Процедура ДобавитьТоварыВКорзинуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если Не ПоказыватьПодобранныеТовары Тогда
		...
		Если СписокВыбранныхТоваров.Количество() = 1 Тогда
			...
			ТекстСобытия = НСтр("ru='Товар добавлен в таблицу подбора';uk='Товар додано в таблицю підбору'");
			ТекстОповещения = НСтр("ru='Товар %%Товар%% добавлен в таблицу подбора';uk='Товар %%Товар%% доданий до таблиці підбору'");
			ТекстОповещения = СтрЗаменить(ТекстОповещения, "%%Товар%%", Строка(ДанныеТовара.Номенклатура));
		Иначе
			ТекстСобытия = НСтр("ru='Товары добавлены в таблицу подбора';uk='Товари додані в таблицю підбору'");
			ТекстОповещения = НСтр("ru='Выбранные товары добавлены в таблицу подбора';uk='Вибрані товари додані в таблицю підбору'");
		КонецЕсли;
