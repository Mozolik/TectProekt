...
Процедура ДобавитьВершину(Знач ИмяОбъектаМетаданных, Знач ТолькоПриОтсутствии = Истина) Экспорт
	...
	Если УжеСуществует Тогда
		...
		Если ТолькоПриОтсутствии Тогда
			Возврат;
		Иначе
			ВызватьИсключение НСтр("ru='Попытка дублирования!';uk='Спроба дублювання!'");
		КонецЕсли;
...
//    которые были добавлены в качестьве ребер к другим вершинам.
//
...
Функция ОбъектМетаданных(Знач ПолноеИмя)
	...
	Если ОбъектМетаданных = Неопределено Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='В текущей конфигурации отсутствует объект метаданных %1, присутствующих в файле данных!';uk='У поточній конфігурації відсутній об''єкт метаданих %1, присутній у файлі даних!'"),
			ПолноеИмя
		);
...
//
Функция Вершина(Знач ОбъектМетаданых, Знач ИсключениеПриОтсутствии = Истина)
	
	ПараметрыОтбора = Новый Структура();
	ПараметрыОтбора.Вставить("ОбъектМетаданных", ОбъектМетаданых);
	...
	ИначеЕсли Строки.Количество() = 0 Тогда
		...
		Если ИсключениеПриОтсутствии Тогда
			...
			ВызватьИсключение СтрШаблон(НСтр("ru='В графе отсутствует вершина для объекта метаданных %1!';uk='У графі відсутня вершина для об''єкта метаданих %1!'"),
				ОбъектМетаданых.ПолноеИмя());
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Нарушение уникальности граф для объекта метаданных %1!';uk='Порушення унікальності граф для об''єкта метаданих %1!'"),
			ОбъектМетаданых.ПолноеИмя());
...
// Выполняеь поиск в глубину при топологической сортировке.
//
// Параметры:
//  Вершина - СтрокаТаблицыЗначений - строка таблицы значений ТекущийГраф,
//  РезультатСортировки - Массив(ОбъектМетаданных) - результат топлогической сортировки.
//
Процедура ПоискВГлубину(Вершина, РезультатСортировки)
	
	// Если вошли в "серую" вершину - найден цикл, топологическая сортирвка невозможна
	Если Вершина.Цвет = Серый Тогда
		
		ВызватьИсключение НСтр("ru='Рекурсивная зависимость!';uk='Рекурсивна залежність!'");
