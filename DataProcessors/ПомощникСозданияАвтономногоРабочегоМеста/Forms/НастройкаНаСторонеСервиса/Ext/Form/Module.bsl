...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбработчикиСобытия = ОбщегоНазначения.ОбработчикиСлужебногоСобытия(
		"ТехнологияСервиса.РаботаВМоделиСервиса.ОбменДаннымиВМоделиСервиса\ПриСозданииАвтономногоРабочегоМеста");
	...
	ЗаголовокЭлемента = НСтр("ru='Для автономной работы на Вашем компьютере должна быть установлена
                            |платформа версии [ВерсияПлатформы]'
                            |;uk='Для автономної роботи на Вашому комп''ютері повинна бути встановлена
                            |платформа версії [ВерсияПлатформы]'");
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ru='Отменить создание автономного рабочего места?';uk='Скасувати створення автономного робочого місця?'");
...
Процедура КакУстановитьИлиОбновитьВерсиюПлатформы1СПредприятие(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Как установить или обновить версию платформы';uk='Як встановити або оновити версію платформи'"));
...
Процедура СохранитьИнструкциюКак(Команда)
	...
	ПолучитьФайл(АдресВоВременномХранилище, НСтр("ru='Инструкция по настройке автономного рабочего места.html';uk='Інструкція по настройці автономного робочого місця.html'"));
...
Процедура СоздатьНачальныйОбразАвтономногоРабочегоМестаНаСервере(Отказ)
	...
	Попытка
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
						...
						КонтекстПомощника,
						НСтр("ru='Создание начального образа автономного рабочего места';uk='Створення початкового образу автономного робочого місця'"),
						Истина);
...
Функция АдресПриложенияВИнтернете()
	...
	Если Не ПараметрыСоединения.Свойство("ws") Тогда
		ВызватьИсключение НСтр("ru='Создать автономное рабочее место можно только в режиме веб-клиента.';uk='Створити автономне робоче місце можна тільки в режимі веб-клієнта.'");
	КонецЕсли;
...
Функция Подключаемый_НастройкаВыгрузки_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеАвтономногоРабочегоМеста) Тогда
		...
		НСтрока = НСтр("ru='Не задано наименование автономного рабочего места.';uk='Не задане найменування автономного робочого місця.'");
...
Функция Подключаемый_Окончание_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	ЗаголовокЭлемента = СтрЗаменить(ЗаголовокЭлемента, "[ЕдиницаИзмерения]", НСтр("ru='Мб';uk='Мб'"));
