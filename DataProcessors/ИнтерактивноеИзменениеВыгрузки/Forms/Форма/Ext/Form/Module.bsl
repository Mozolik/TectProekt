...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.УзелИнформационнойБазы) Тогда
		Текст = НСтр("ru='Настройка обмена данными не найдена.';uk='Настройка обміну даними не знайдена.'");
...
Процедура ДополнительнаяРегистрацияПередУдалением(Элемент, Отказ)
	...
	Если Количество>1 Тогда
		ТекстПредставления = НСтр("ru='выбранные строки';uk='обрані рядки'");
	...
	ТекстВопроса = НСтр("ru='Удалить из дополнительных данных %1 ?';uk='Вилучити з додаткових даних %1 ?'");    
	...
	ИначеЕсли ТипВыбранного= Тип("Структура") Тогда
		Если ВыбранноеЗначение.ДействиеВыбора=3 Тогда
			...
			Если Не ПустаяСтрока(ПредставлениеТекущейНастройки) И ПредставлениеНастройки<>ПредставлениеТекущейНастройки Тогда
				ТекстВопроса  = НСтр("ru='Восстановить настройки ""%1""?';uk='Відновити настройки ""%1""?'");
...
Процедура ОбновитьКоличество(Команда)
	...
	Если Не КоличествоОбновлено() Тогда
...
Функция РедактированиеОтбораСтрокиДополнительныйСоставСервер(СтруктураВыбора)
	...
	ТекущиеДанные.Количество   = НСтр("ru='Не рассчитано';uk='Не розраховано'");
...
Функция ДобавитьВДополнительныйСоставВыгрузки(Элемент)
	...
	Иначе
		...
		Строка.Количество = НСтр("ru='Не рассчитано';uk='Не розраховано'");
		ОбновитьНадписьОбщегоКоличества();
	КонецЕсли;
...
Функция КоличествоОбновлено()
	...
	РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор,
		...
		ПараметрыЗадания, НСтр("ru='Расчет количества объектов для отправки при синхронизации';uk='Розрахунок кількості об''єктів для відправлення при синхронізації'"));
...
&НаСервере
Процедура ЗагрузитьЗначенияКоличеств()
...
Процедура ОбновитьНадписьОбщегоКоличества(Количество = Неопределено) 
	...
	Если Количество = Неопределено Тогда
		ТекстКоличества = НСтр("ru='<не рассчитано>';uk='<не розраховано>'");
	Иначе
		ТекстКоличества = НСтр("ru='Объектов: %1';uk='Об''єктів: %1'");
...
Процедура СброситьНадписьКоличествТаблицы()
	ТекстКоличеств = НСтр("ru='Не рассчитано';uk='Не розраховано'");
	Для Каждого Строка Из Объект.ДополнительнаяРегистрация Цикл
