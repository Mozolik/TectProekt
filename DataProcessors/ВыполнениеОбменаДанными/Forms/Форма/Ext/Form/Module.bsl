...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		...
		Иначе
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ru='Не заданы параметры формы. Форма не может быть открыта.';uk='Не задані параметри форми. Форма не може бути відкрита.'"), Отказ);
			Возврат;
		КонецЕсли;
	...
	Заголовок = НСтр("ru='Синхронизация данных с ""%1""';uk='Синхронізація даних з ""%1""'");
...
Функция ФоновоеЗаданиеЗапустить(Знач ПараметрыЗадания, ОшибкаРасхожденияВерсийПриПолученииДанных, ПроверятьРасхождениеВерсий)
	Если ПараметрыЗадания.ВыполнятьЗагрузку Тогда
		...
		НаименованиеЗадания = НСтр("ru='Загрузка данных';uk='Завантаження даних'");
	Иначе
		НаименованиеЗадания = НСтр("ru='Выгрузка данных';uk='Вивантаження даних'");
	КонецЕсли;
