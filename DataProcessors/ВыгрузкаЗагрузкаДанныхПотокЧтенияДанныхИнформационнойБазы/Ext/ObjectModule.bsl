...
Процедура ОткрытьФайл(Знач ИмяФайла) Экспорт
	...
	Если ТекущаяИнициализация Тогда
		...
		ВызватьИсключение НСтр("ru='Объект уже был инициализирован ранее!';uk='Об''єкт вже був ініціалізований раніше!'");
...
Функция ПрочитатьОбъектДанныхИнформационнойБазы() Экспорт
	...
	Если ПотокЧтения.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
		...
		Если ПотокЧтения.Имя <> "DumpElement" Тогда
			ВызватьИсключение НСтр("ru='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента DumpElement.';uk='Помилка читання XML. Неправильний формат файлу. Очікується початок елемента DumpElement.'");
		КонецЕсли;
...
//
// Параметры:
//	ПотокЧтения - ЧтнеиеXML - поток чтения выгрузки.
//
// Возвращаемое значение:
//	Строка - текст врагмента XML.
//
Функция ПрочитатьФрагментПотока()
	...
	Исключение
		ТекстЖР = СтрШаблон(НСтр("ru='Ошибка копирования фрагмента исходного файла. Частично скопированный фрагмент:
                  |%1'
                  |;uk='Помилка копіювання фрагмента вихідного файлу. Частково скопійований фрагмент:
                  |%1'"),
				ЗаписьФрагмента.Закрыть());
		ЗаписьЖурналаРегистрации(НСтр("ru='Выгрузка/загрузка данных.Ошибка чтения XML';uk='Вивантаження/завантаження даних.Помилка читання XML'",ТехнологияСервисаИнтеграцияСБСП.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , ТекстЖР);
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ВызватьИсключениеЧтенияXML(Знач Фрагмент, Знач ТекстОшибки)
	...
	ВызватьИсключение СтрШаблон(НСтр("ru='Ошибка при чтении данных из файла %1: при чтении фрагмента
              ...
              |
              |%3.'
              |;uk='Помилка при читанні даних з файлу %1: при читанні фрагменту
              |
              |%2
              |
              |выдбулася помилка:
