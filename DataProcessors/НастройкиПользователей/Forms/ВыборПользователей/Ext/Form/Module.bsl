...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОчисткаНастроек Тогда
		Заголовок = НСтр("ru='Выбор пользователей для очистки настроек';uk='Вибір користувачів для очищення настройок'");
		Элементы.Надпись.Заголовок = НСтр("ru='Выберите пользователей, которым необходимо очистить настройки';uk='Виберіть користувачів, яким необхідно очистити настройки'");
	КонецЕсли;
...
Процедура Выбрать(Команда)
	...
	Если ПользователиПриемник.Количество() = 0 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Необходимо отметить одного или несколько пользователей.';uk='Необхідно позначити одного або кілька користувачів.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьЗаголовокГруппы(Форма, ГруппаПользователей, СтрокаСпискаПользователей, ЗначениеПометки)
	...
	Если ПомеченныйПользователь <> Неопределено И ЗначениеПометки <> СтрокаСпискаПользователей.Пометка Тогда
		...
		ГруппаПользователей.НаименованиеГруппыИПомеченоПользователей = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 (%2)';uk='%1 (%2)'"), Строка(ГруппаПользователей.Группа), ГруппаПользователей.ПомеченоПользователей);
	КонецЕсли;
