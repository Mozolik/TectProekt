...
Процедура ОбновитьЗаголовокСтраницы(КоличествоНастроек)
	...
	Если ВыбраннаяСтраницаНастроек = "НастройкиОтчетовСтраница" Тогда
		...
		Если КоличествоНастроекОтчетов = 0 Тогда
			ТекстЗаголовка = НСтр("ru='Настройки отчетов';uk='Настройки звітів'");
		Иначе
			ТекстЗаголовка = НСтр("ru='Настройки отчетов (%1)';uk='Настройки звітів (%1)'");
	...
	ИначеЕсли ВыбраннаяСтраницаНастроек = "ВнешнийВидСтраница" Тогда
		...
		Если КоличествоНастроекВнешнегоВида = 0 Тогда
			ТекстЗаголовка = НСтр("ru='Внешний вид';uk=' Зовнішній вигляд'");
		Иначе
			ТекстЗаголовка = НСтр("ru='Внешний вид (%1)';uk=' Зовнішній вигляд (%1)'");
	...
	ИначеЕсли ВыбраннаяСтраницаНастроек = "ПрочиеНастройкиСтраница" Тогда
		...
		Если КоличествоПрочихНастроек = 0 Тогда
			ТекстЗаголовка = НСтр("ru='Прочие настройки';uk='Інші настройки'");
		Иначе
			ТекстЗаголовка = НСтр("ru='Прочие настройки (%1)';uk='Інші настройки (%1)'");
...
Процедура ЗагрузитьЗначенияПометок(ДеревоЗначений, ПомеченныеНастройки, ВидНастроек)
	...
	Если КоличествоПомеченных > 0 Тогда
		...
		Если ВидНастроек = "НастройкиОтчетов" Тогда
			...
			Элементы.НастройкиОтчетовСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Настройки отчетов (%1)';uk='Настройки звітів (%1)'"), КоличествоПомеченных);
		ИначеЕсли ВидНастроек = "ВнешнийВид" Тогда
			КоличествоНастроекВнешнегоВида = КоличествоПомеченных;
			Элементы.ВнешнийВидСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Внешний вид (%1)';uk=' Зовнішній вигляд (%1)'"), КоличествоПомеченных);
		ИначеЕсли ВидНастроек = "ПрочиеНастройки" Тогда
			КоличествоПрочихНастроек = КоличествоПомеченных;
			Элементы.ПрочиеНастройкиСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Прочие настройки (%1)';uk='Інші настройки (%1)'"), КоличествоПомеченных);
		КонецЕсли;
