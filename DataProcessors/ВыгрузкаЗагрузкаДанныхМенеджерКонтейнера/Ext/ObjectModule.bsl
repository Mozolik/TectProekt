...
Функция ПараметрыВыгрузки() Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Контейнер не инициализирован для выгрузки данных!';uk='Контейнер не ініціалізований для вивантаження даних!'");
	КонецЕсли;
...
Процедура УстановитьКоличествоОбъектов(Знач ПолныйПутьКФайлу, Знач ЧислоОбъектов = Неопределено) Экспорт
	...
	Если ФайлыВСоставе.Количество() = 0 Или ФайлыВСоставе.Количество() > 1 Тогда 
		ВызватьИсключение НСтр("ru='Файл не найден';uk='Файл не знайдено'");
	КонецЕсли;
...
Процедура ИсключитьФайл(Знач ПолныйПутьКФайлу) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru='Файл %1 не найден в составе контейнера!';uk='Файл %1 не знайдений в складі контейнера!'"), ПолныйПутьКФайлу);
	Иначе
...
Процедура ЗаменитьФайл(Знач ИмяВконтейнере, Знач ПолныйПутьКФайлу, Знач УдалятьФайлЗаменыСДиска = Ложь) Экспорт
	...
	Если СтрокаИсходногоФайла = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru='Файл с идентификатором %1 не найден в составе контейнера!';uk='Файл з ідентифікатором %1 не знайдений в складі контейнера!'"), ИмяВконтейнере);
	Иначе
...
Процедура ОбновитьСоставВыгружаемыхФайлов()
	...
	Для Каждого ТекущийФайл Из Состав Цикл 
		...
		Если Не Файл.Существует() Тогда 
			ВызватьИсключение НСтр("ru='Файл был удален';uk='Файл був вилучений'");
		КонецЕсли;
...
//
// Параметры:
//	КаталогЗагрузки - Строка - путь к каталоу загрузки.
//
Процедура ИнициализироватьЗагрузку(Знач КаталогЗагрузки, Знач ПараметрыЗагрузки) Экспорт
	...
	Если Не ФайлСодержимого.Существует() Тогда
		...
		ВызватьИсключение НСтр("ru='Ошибка загрузки данных. Неверный формат файла. В архиве не обнаржен файл PackageContents.xml.
                                |Возможно, файл был получен из предыдущих версий программы или поврежден!'
                                |;uk='Помилка завантаження даних. Невірний формат файлу. В архіві не знайдений файл PackageContents.xml.
                                |Можливо, файл був отриманий з попередніх версій програми або пошкоджений!'");
	...
	Если ПотокЧтения.ТипУзла <> ТипУзлаXML.НачалоЭлемента
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента %1.';uk='Помилка читання XML. Неправильний формат файлу. Очікується початок елементу %1.'"),
			"Data"
		);
...
Функция ПараметрыЗагрузки() Экспорт
	...
	Если ДляЗагрузки Тогда
		Возврат Новый ФиксированнаяСтруктура(Параметры);
	Иначе
		ВызватьИсключение НСтр("ru='Контейнер не инициализирован для загрузки данных!';uk='Контейнер не ініціалізований для завантаження даних!'");
	КонецЕсли;
...
Функция ПолучитьФайлИзКаталога(Знач ВидФайла, Знач ТипДанных = Неопределено) Экспорт
	...
	ИначеЕсли Файлы.Количество() > 1 Тогда
		ВызватьИсключение НСтр("ru='В выгрузке содержится дублирующаяся информация';uk='У вивантаженні міститься інформація, що дублюється'");
	КонецЕсли;
...
Функция ПолучитьПроизвольныйФайл(Знач ТипДанных = Неопределено) Экспорт
	...
	Если Файлы.Количество() = 0 Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru='В выгрузке отсутствует произвольный файл с типом данным %1!';uk='У вивантаженні відсутній довільний файл з типом даних %1!'"),
			ТипДанных
		);
	ИначеЕсли Файлы.Количество() > 1 Тогда
		ВызватьИсключение НСтр("ru='В выгрузке содержится дублирующаяся информация';uk='У вивантаженні міститься інформація, що дублюється'");
...
Функция ПолучитьОписанияФайловИзКаталога(Знач ВидФайла, Знач ТипДанных = Неопределено) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Неизвестный вид файла';uk='Невідомий вид файлу'");
...
Функция ПолучитьПолноеИмяФайла(Знач ОтносительноеИмяФайла) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru='В контейнере не обнаружен файл с относительным именем %1!';uk='У контейнері не виявлений файл з відносним ім''ям %1!'"),
...
Функция ПолучитьОтносительноеИмяФайла(Знач ПолноеИмяФайла) Экспорт
	...
	Если СтрокаСостава = Неопределено Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru='В контейнере не обнаружен файл %1!';uk='У контейнері не виявлений файл %1!'"),
			ПолноеИмяФайла
		);
	Иначе
...
Процедура ПроверкаИнициализацииКонтейнера(Знач ПриИнициализации = Ложь)
	...
	Если ДляВыгрузки И ДляЗагрузки Тогда
		ВызватьИсключение НСтр("ru='Некорректная инициализация контейнера!';uk='Некоректна ініціалізація контейнера!'");
	КонецЕсли;
	
	Если ПриИнициализации Тогда
		
		Если КонтейнерИнициализирован <> Неопределено И КонтейнерИнициализирован Тогда
			ВызватьИсключение НСтр("ru='Контейнер выгрузки уже был инициализирован ранее!';uk='Контейнер вивантаження вже був ініціалізований раніше!'");
		КонецЕсли;
		
	Иначе
		
		Если Не КонтейнерИнициализирован Тогда
			ВызватьИсключение НСтр("ru='Контейнер выгрузки не инициализирован!';uk='Контейнер вивантаження не ініціалізований!'");
		КонецЕсли;
...
Функция ПолучитьКаталогДляРазмещенияФайла(Знач ВидФайла)
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru='Вид файла %1 не поддерживается!';uk='Вид файлу %1 не підтримується!'"), ВидФайла);
	КонецЕсли;
...
// Работа с описанием содердимого контейнера

Процедура ЗаписатьСодержимоеКонтейнераВФайл(ИмяФайла)
	
	Правила = ПравилаСериализацииСодерижимогоКонтейнера();
...
Функция ПравилаСериализацииСодерижимогоКонтейнера()
...
// Иниицализация дефолтного состояния контейнера
