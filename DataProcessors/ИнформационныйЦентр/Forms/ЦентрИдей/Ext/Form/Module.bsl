...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "НоваяИдея" Или ИмяСобытия = "ПроголосовалиЗаИдею"
		Или ИмяСобытия = "ДобавленКомментарийКИдее" Или ИмяСобытия = "УдаленКомментрийКИдее" Тогда 
...
Процедура ПроизвестиПоиск()
	...
	Если ПустаяСтрока(ТекстПоиска) Тогда 
		НСтр("ru='Текстовый запрос поиска не должен быть пустым';uk='Текстовий запит пошуку не повинен бути порожнім'");
	КонецЕсли;
