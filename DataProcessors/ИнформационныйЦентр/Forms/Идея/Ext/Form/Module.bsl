...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ДобавленКомментарийКИдее" Или ИмяСобытия = "УдаленКомментрийКИдее" Тогда 
...
Процедура УдалитьНажатие(Элемент)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Удалить Ваш комментарий?';uk='Вилучити Ваш коментар?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура УдалитьКомментарийОповещение(Ответ, ИмяЭлемента) Экспорт
	...
	Оповестить("УдаленКомментрийКИдее");
...
Процедура ЗаполнитьИдею()
	...
	ДаннеПоИдее = ПолучитьДаннеПоИдее();
	СформироватьСтраницу(ДаннеПоИдее);
	
КонецПроцедуры

&НаСервере
Функция ПолучитьДаннеПоИдее()
...
&НаСервере
Процедура СформироватьСтраницу(Знач ДаннеПоИдее)
	
	ПредставлениеИдеи = ДаннеПоИдее.Idea;
	...
	СписокКомментариев = ДаннеПоИдее.CommentsList;
...
Процедура ОтобразитьСодержание(Знач ПредставлениеИдеи)
	...
	// Поместиь во временное хранилище картинки
...
Процедура ДобавитьКомментарийСервер()
	...
	Попытка
		WSПрокси = ИнформационныйЦентрСервер.ПолучитьПроксиЦентраИдей();
		WSПрокси.addIdeaComment(ИдентификаторИдеи, Строка(ИдентификаторПользователя), "", ТекстКомментария);
	Исключение
