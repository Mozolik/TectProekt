...
Процедура ЗаполнитьОбращения(ПредставлениеСпискаОбращений)
	...
	Для Каждого ОбращениеОбъект Из ПредставлениеСпискаОбращений.Incidents Цикл 
		...
		НовоеОбращение.Наименование = ?(ПустаяСтрока(ОбращениеОбъект.Name), НСтр("ru='<Без темы>';uk='<Без теми>'"), ОбращениеОбъект.Name);
		...
		Если НовоеОбращение.КоличествоНепросмотренныхВзаимодействий <> 0 Тогда 
			...
			ЕстьОтвет = ?(НовоеОбращение.КоличествоНепросмотренныхВзаимодействий = 1, НСтр("ru='Непрочитанное';uk='Непрочитане'"), НСтр("ru='Непрочитанные';uk='Непрочитані'"));
