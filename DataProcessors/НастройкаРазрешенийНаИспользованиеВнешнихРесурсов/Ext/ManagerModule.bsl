...
Функция ВыполнитьОбработкуЗапросовПроверкиПрименения() Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		ВызватьИсключение НСтр("ru='Транзакция активна';uk='Транзакція активна'");
	КонецЕсли;
...
Функция СтрокаТаблицыРазрешений(Знач ТаблицаРазрешений, Знач Отбор, Знач ДобавитьПриОтсутствии = Истина) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Нарушение уникальности строк в таблице разрешений по отбору %1';uk='Порушення унікальності рядків у таблиці дозволів за відбором %1'"),
