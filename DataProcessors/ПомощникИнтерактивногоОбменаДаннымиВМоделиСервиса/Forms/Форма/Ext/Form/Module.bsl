...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбменДаннымиВМоделиСервисаПовтИсп.СинхронизацияДанныхПоддерживается() Тогда
		ТекстОшибки = НСтр("ru='Синхронизация данных для конфигурации не поддерживается!';uk='Синхронізація даних для конфігурації не підтримується!'");
		
	ИначеЕсли Не Параметры.Свойство("УзелИнформационнойБазы", Объект.УзелИнформационнойБазы) Тогда
		ТекстОшибки = НСтр("ru='Обработка не предназначена для непосредственного использования.';uk='Обробка не призначена для безпосереднього використання.'");
		
	ИначеЕсли Объект.УзелИнформационнойБазы.Пустая() Тогда
		ТекстОшибки = НСтр("ru='Настройка обмена данными не найдена.';uk='Настройка обміну даними не знайдена.'");
...
Процедура ДополнениеВыгрузкиИсторияОтборовВыборИзМеню(Знач ВыбранныйЭлемент, Знач ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ПредставлениеНастройки)=Тип("Строка") Тогда
		...
		ТекстЗаголовка = НСтр("ru='Подтверждение';uk='Підтвердження'");
		ТекстВопроса = СтрШаблон(НСтр("ru='Восстановить настройки ""%1""?';uk='Відновити настройки ""%1""?'"), ПредставлениеНастройки
		);
...
Процедура ОбработчикОжиданияКорреспондента()
	...
	Иначе
		СостояниеДлительнойОперации.ИнформацияОбОшибке = НСтр("ru='Ошибка сообщения корреспонденту';uk='Помилка повідомлення кореспонденту'");
...
Функция ФоновоеЗаданиеВыгрузкиНаСервере()
	...
	Задание = ФоновыеЗадания.Выполнить("ОбменДаннымиВМоделиСервиса.ОбменПоТребованию", 
		ПараметрыФоновогоВыполнения, , НСтр("ru='Интерактивный обмен по требованию.';uk='Інтерактивний обмін на вимогу.'"));
