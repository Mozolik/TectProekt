...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИспользоватьАдресноеХранение Тогда
		Элементы.Вариант.СписокВыбора.Добавить("По складской группе",
											НСтр("ru='По складской группе упаковок';uk='За складською групою упаковок'"));
	КонецЕсли;
...
Процедура ЗаполнитьУпаковки(Команда)
	...
	Если Вариант = "По складской группе" Тогда
		
		Если Не ЗначениеЗаполнено(СкладскаяГруппа) Тогда
			ТекстСообщения = СтрЗаменить(ШаблонСообщения,"%ИмяПоля%",НСтр("ru='Складская группа';uk='Складська група'"));
...
Процедура УстановитьДоступностьВидимостьРеквизитов()
	...
	Элементы.СкладскаяГруппа.Доступность = (Вариант = "По складской группе");
