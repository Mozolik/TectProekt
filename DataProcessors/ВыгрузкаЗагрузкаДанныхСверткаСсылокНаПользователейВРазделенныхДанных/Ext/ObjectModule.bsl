...
Процедура ПередВыгрузкойДанных(Контейнер) Экспорт
	...
	ВыгрузкаЗагрузкаДанныхСлужебный.ЗаписатьОбъектВФайл(ТекущийИдентификаторНеуказанногоПользователя, ИмяФайла);
...
Процедура ПередВыгрузкойОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект, Артефакты, Отказ) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("СправочникОбъект.Пользователи") Тогда
		...
		ИначеЕсли ПользователиСлужебныйВМоделиСервисаБТС.ПользовательЗарегистрированКакНеразделенный(Объект.ИдентификаторПользователяИБ) Тогда
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Объект метаданных %1 не может быть обработан обработчиком ПользователиСлужебныйВМоделиСервисаБТС.ПередВыгрузкойОбъекта()!';uk='Об''єкт метаданих %1 не може бути оброблений обробником ПользователиСлужебныйВМоделиСервисаБТС.ПередВыгрузкойОбъекта()!'"),
...
Процедура ПослеВыгрузкиОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект, Артефакты) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("СправочникОбъект.Пользователи") Тогда
		...
		Если Объект.Ссылка.УникальныйИдентификатор() <> ТекущийИдентификаторНеуказанногоПользователя Тогда
			...
			ЭтоСсылкаНеразделенногоПользователя = ПользователиСлужебныйВМоделиСервисаБТС.ПользовательЗарегистрированКакНеразделенный(
...
Процедура ПередЗагрузкойДанных(Контейнер) Экспорт
	...
	ИсходныйИдентификаторНеуказанногоПользователя = ВыгрузкаЗагрузкаДанныхСлужебный.ПрочитатьОбъектИзФайла(ИмяФайла);
...
Процедура ПередСопоставлениемСсылок(Контейнер, ОбъектМетаданных, ТаблицаИсходныхСсылок, СтандартнаяОбработка, Отказ) Экспорт
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Тип данных указан неверно';uk='Тип даних вказано невірно'");
...
Процедура ПередЗагрузкойТипа(Контейнер, ОбъектМетаданных, Отказ) Экспорт
	...
	Если ПользователиСлужебныйВМоделиСервисаБТСПовтИсп.СписокНаборовЗаписейСоСсылкамиНаПользователей().Получить(ОбъектМетаданных) <> Неопределено Тогда
...
Процедура ПередЗагрузкойОбъекта(Контейнер, Объект, Артефакты, Отказ) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("СправочникОбъект.Пользователи") Тогда
		...
		Для Каждого Артефакт Из Артефакты Цикл
			...
			Если Артефакт.Тип() = ТипАртефактНеразделенногоПользователя() Тогда
				...
				Если ПользователиСлужебныйВМоделиСервисаБТС.ПользовательЗарегистрированКакНеразделенный(Идентификатор) Тогда
					
					Объект.ИдентификаторПользователяИБ = Идентификатор;
					Объект.Наименование = ПользователиСлужебныйВМоделиСервисаБТС.ПолноеИмяСлужебногоПользователя(Идентификатор);
	...
	ИначеЕсли ПользователиСлужебныйВМоделиСервисаБТСПовтИсп.СписокНаборовЗаписейСоСсылкамиНаПользователей().Получить(Объект.Метаданные()) <> Неопределено Тогда
		
		// Набор записей, содержащий измерение с типом СправочникСсылка.Пользователи
		
		СернутьСсылкиНаПользователейВНаборе(Объект);
		
	Иначе
		
		ВызватьИсключение СтрШаблон(НСтр("ru='Объект метаданных %1 не может быть обработан обработчиком ПользователиСлужебныйВМоделиСервисаБТС.ПередЗагрузкойОбъекта()!';uk='Об''єкт метаданих %1 не може бути оброблений обробником ПользователиСлужебныйВМоделиСервисаБТС.ПередЗагрузкойОбъекта()!'"),
			Объект.Метаданные().ПолноеИмя());
...
Функция ТипАртефактНеразделенногоПользователя() Экспорт
...
Процедура СернутьСсылкиНаПользователейВНаборе(НаборЗаписей)
