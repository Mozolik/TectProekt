...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ЗначениеЗаполнено(ТипСоздаваемогоОбъекта) Тогда
		...
		Если ТипСоздаваемогоОбъекта = "DMIncomingDocument" Тогда 
			Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("ru='Входящий документ:';uk='Вхідний документ:'");
		ИначеЕсли ТипСоздаваемогоОбъекта = "DMOutgoingDocument" Тогда 
			Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("ru='Исходящий документ:';uk='Вихідний документ:'");
		ИначеЕсли ТипСоздаваемогоОбъекта = "DMInternalDocument" Тогда
			Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("ru='Внутренний документ:';uk='Внутрішній документ:'");
		ИначеЕсли ТипСоздаваемогоОбъекта = "DMCorrespondent" Тогда 
			Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("ru='Корреспондент:';uk='Кореспондент:'");
		КонецЕсли;
		Элементы.ГруппаИнфо.Видимость = Ложь;
		
	Иначе // нет подходящих правил
		
		Элементы.ДекорацияОбъектЗаголовок.Заголовок = НСтр("ru='Соответствие в Документообороте';uk='Відповідність у Документообігу'");
		Элементы.ДекорацияИнфо.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не настроено заполнение документа Документооборота из ""%1""';uk='Не настроєно заповнення документа Документообігу з ""%1""'"), ИмяПотребителя);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если (ИмяСобытия = "Запись_ДокументооборотДокумент" 
		ИЛИ ИмяСобытия = "Запись_ДокументооборотОбъект") И  Источник = Элементы.ДекорацияОбъект Тогда 
...
Процедура ДекорацияОбъектВыбратьНажатие(Элемент)
	...
	Иначе // тип неизвестен, предложим выбор пользователю
		...
		ПараметрыФормы.Вставить("Заголовок", НСтр("ru='Выбрать в Документообороте';uk='Вибрати в Документообігу'"));
		ПараметрыФормы.Вставить("ЗаголовокКоманды", НСтр("ru='Выбрать';uk='Обрати'"));
...
Процедура ДекорацияОбъектОчиститьНажатие(Элемент)
	...
	ИнтеграцияС1СДокументооборотКлиент.ПоказатьВопросДаНет(Оповещение, ТекстВопроса, 
		НСтр("ru='Очистить';uk='Очистити'"), НСтр("ru='Не очищать';uk='Не очищати'"), КодВозвратаДиалога.Нет);
...
Процедура ОбработатьФормуСогласноВерсииСервиса()
	...
	Иначе // версия известна
		...
		Иначе
			...
			ДоступенФункционалКорреспонденты = ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.3.2.3");
...
Процедура ДекорацияОбъектСоздатьНажатиеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ДокументID = Результат.id;
	ДокументТип = Результат.type;
	Документ = Результат.name;
...
Процедура ОбновитьДекорацииДокумента()
	...
	Элементы.ДеревоСвязейДобавитьСвязьВнутреннийДокумент.Доступность = ДокументЗаполнен;
	Элементы.ДеревоСвязейДобавитьСвязьВходящийДокумент.Доступность = ДокументЗаполнен;
	Элементы.ДеревоСвязейДобавитьСвязьИсходящийДокумент.Доступность =  ДокументЗаполнен;
	Элементы.ДеревоСвязейОткрыть.Доступность =  ДокументЗаполнен;
	Элементы.ДеревоСвязейУдалить.Доступность =  ДокументЗаполнен;
	
	Элементы.ДеревоСвязейДобавитьСвязьВнутреннийДокументКонтекст.Доступность = ДокументЗаполнен;
	Элементы.ДеревоСвязейДобавитьСвязьВходящийДокументКонтекст.Доступность = ДокументЗаполнен;
	Элементы.ДеревоСвязейДобавитьСвязьИсходящийДокументКонтекст.Доступность =  ДокументЗаполнен;
	...
	Если ДокументТип = "DMCorrespondent" Тогда
		...
		Элементы.ДеревоПисемПисьмо.Заголовок = НСтр("ru='Документ';uk='Документ'");
	Иначе
		...
		Элементы.ДеревоПисемПисьмо.Заголовок = НСтр("ru='Письмо';uk='Лист'");
	КонецЕсли;
		...
		Иначе
			...
			Иначе // нет подходящих правил
				Элементы.ДекорацияОбъектЗаголовок.Заголовок = 
					НСтр("ru='Соответствие в Документообороте:';uk='Відповідність у Документообігу:'");
			КонецЕсли;
		КонецЕсли;
...
Процедура ВыбратьСуществующийФайлВ1СДокументооборот()
	...
	Оповещение = Новый ОписаниеОповещения(
		"ВыбратьСуществующийФайлВ1СДокументооборотЗавершение", ЭтаФорма);
...
Процедура СоздатьФайлСДиска()
	...
	Если Не ЗначениеЗаполнено(ИдентификаторКорневойПапки1СДокументооборот) Тогда
		ПоказатьПредупреждение(, 
			НСтр("ru='Не указана корневая папка для хранения файлов Документооборота.';uk='Не вказана коренева папка для зберігання файлів Документообігу.'"));
		Возврат;
	КонецЕсли;
...
//
Процедура СоздатьФайлСДискаПеретаскиванием(Файл) Экспорт
	...
	Если Размер > МаксРазмерФайла Тогда
		ВызватьИсключение
			   СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				 НСтр("ru='Размер файла ""%1"" (%2 Мб) превышает максимально допустимый (%3 Мб).';uk='Розмір файлу ""%1"" (%2 Мб) перевищує максимально допустимий (%3 Мб).'"),
				 ПараметрыОповещения.ПолноеИмяФайла, 
				 ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(Размер / (1024 * 1024)),           
				 ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьСтрокуСРазмеромФайла(МаксРазмерФайла / (1024 * 1024)));
	КонецЕсли;
...
Процедура СоздатьФайлСДискаПослеПомещенияФайла(ПомещенныеФайлы, ПараметрыОповещения) Экспорт
	...
	Если НастройкиДокументооборот.НужноИзвлечьТекст Тогда
		ПараметрыОповещения.Вставить("Текст", ФайловыеФункцииСлужебныйКлиентСервер.ИзвлечьТекст(
			ПараметрыОповещения.ПолноеИмяФайла));
	Иначе
...
Процедура НачатьДобавлениеСвязи(ТипДобавляемогоДокумента)
	...
	ИнтеграцияС1СДокументооборотКлиент.НачатьДобавлениеСвязи(
		ДокументID, ДокументТип, Документ, ТипДобавляемогоДокумента, ОписаниеОповещения, Отбор);
