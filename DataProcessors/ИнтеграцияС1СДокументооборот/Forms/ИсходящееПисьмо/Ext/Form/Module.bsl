...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.2.8.1.CORP") Тогда
		ВызватьИсключение(НСтр("ru='Функционал не поддерживается в данной версии Документооборота.';uk='Функціонал не підтримується в даній версії Документообігу.'"));
	КонецЕсли;
...
Процедура АдресатыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Не ЗначениеЗаполнено(СтрокаДанных.ТипАдреса) Тогда
		Если КоличествоАдресатовДоДобавления = 1 Тогда
			СтрокаДанных.ТипАдреса = НСтр("ru='Кому:';uk='Кому:'");
		Иначе
			СтрокаДанных.ТипАдреса = НСтр("ru='Копия:';uk='Копія:'");
		КонецЕсли;
	КонецЕсли;
...
Процедура Отправить(Команда)
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Письмо ""%1"" отправлено.';uk='Лист ""%1"" відправлено.'"), Тема));
...
Процедура ЗаполнитьФормуИзОбъектаXDTO(ПисьмоXDTO, НовоеПисьмо = Ложь)
	...
	Если ЗначениеЗаполнено(ПисьмоXDTO.objectId.id) Тогда
		ДобавитьАдресатов(ПисьмоXDTO.senderAddress, НСтр("ru='От:';uk='Від:'"));
	Иначе
		Отправитель = ПисьмоXDTO.senderAddress;
	КонецЕсли;
	
	ДобавитьАдресатов(ПисьмоXDTO.recipients, НСтр("ru='Кому:';uk='Кому:'"));
	ДобавитьАдресатов(ПисьмоXDTO.courtesyCopyRecipients, НСтр("ru='Копия:';uk='Копія:'"));
	ДобавитьАдресатов(ПисьмоXDTO.blindCourtesyCopyRecipients, НСтр("ru='СК:';uk='СК:'"));
	...
	Если ЗначениеЗаполнено(ПисьмоXDTO.subject) Тогда
		Заголовок = ПисьмоXDTO.subject + НСтр("ru=' (Исходящее письмо)';uk=' (Вихідний лист)'");
	Иначе
...
Процедура ОтобразитьКоличествоФайловСервер()
	...
	Иначе
		Элементы.ДеревоПриложенийСсылка.Заголовок = НСтр("ru='Файлы';uk='Файли'");
	КонецЕсли;
...
Процедура НачатьДобавлениеФайлаСДискаПослеПодключенияРасширения(Подключено, ПараметрыОповещения) Экспорт	
	...
	Иначе 
		...
		ПоказатьПредупреждение(, НСтр("ru='Необходимо подключить расширение для работы файлами.';uk='Необхідно підключити розширення для роботи файлами.'"));
...
Процедура НачатьДобавлениеФайлаСДискаПоМассивуИмен(ПолныеИменаФайлов)	
	...
	Для Каждого ПолноеИмяФайла Из ПолныеИменаФайлов Цикл
		...
		ОписаниеФайла.Вставить("КартинкаТипаОбъекта",
			ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьИндексПиктограммыФайла(ОписаниеФайла.Расширение));
...
Процедура НачатьДобавлениеФайлаСДискаПослеПомещенияФайлов(ПомещенныеФайлы, ПараметрыОповещения) Экспорт
	...
	Для Н = 0 по ПомещенныеФайлы.Количество() - 1 Цикл
		ПомещенныйФайл = ПомещенныеФайлы[Н];
		ОписаниеФайла = ПараметрыОповещения.ОписанияФайлов[Н];
...
&НаКлиенте
Процедура НачатьДобавлениеФайлаСДискаПослеИнициализацииФайла6(Файл, ПараметрыОповещения) Экспорт
КонецПроцедуры
...
Функция ПроверитьПисьмоПередОтправкой()
	...
	Если СписокАдресов.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Нельзя отправить письмо. Необходимо указать хотя бы одного получателя.';uk='Не можна відправити лист. Необхідно вказати хоча б одного одержувача.'"),,
				"Адресаты");
	...
	Если ПустаяСтрока(Тема) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Нельзя отправить письмо. Не заполнена тема.';uk='Не можна відправити лист. Не заповнена тема.'"),,
			"Тема");
