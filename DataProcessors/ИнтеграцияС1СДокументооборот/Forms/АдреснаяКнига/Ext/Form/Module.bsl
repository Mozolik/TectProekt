...
&НаКлиенте
Процедура ОткрытьКорреспондентыНажатие(Элемент)
...
Процедура ОбновитьКомандыПереходаКСтраницамНаСервере(ТекущийЭлементПредставления)
	...
	УстановитьСтандартноеОформлениеНаСервере(Элементы.ОткрытьКорреспонденты);
...
Процедура ЗаполнитьАдреснуюКнигу(КомандаЗаполнения, ИндексЭлемента = Неопределено)
	...
	Если ТипАдресатов = "СотрудникиПоПодразделениям" Тогда
		Элементы.ЗаголовокАдресатов.Заголовок = НСтр("ru='Сотрудники по подразделениям:';uk='Співробітники по підрозділах:'");
	...
	ИначеЕсли ТипАдресатов = "СотрудникиСписком" Тогда
		Элементы.ЗаголовокАдресатов.Заголовок = НСтр("ru='Сотрудники списком:';uk='Співробітники списком:'");
	...
	ИначеЕсли ТипАдресатов = "СотрудникиПоГруппам" Тогда
		Элементы.ЗаголовокАдресатов.Заголовок = НСтр("ru='Сотрудники по группам:';uk='Співробітники по групах:'");
		Запрос.addressBookType = "usersInFolders";
		Элементы.Адресаты.Отображение = ОтображениеТаблицы.Дерево;
		Элементы.Адресаты.НачальноеОтображениеДерева = НачальноеОтображениеДерева.НеРаскрывать;
	ИначеЕсли ТипАдресатов = "ЛичныеАдресаты" Тогда
		Элементы.ЗаголовокАдресатов.Заголовок = НСтр("ru='Личные адресаты:';uk='Особисті адресати:'");
		Запрос.addressBookType = "personalRecipients";
		Элементы.Адресаты.Отображение = ОтображениеТаблицы.Дерево;
		Элементы.Адресаты.НачальноеОтображениеДерева = НачальноеОтображениеДерева.НеРаскрывать;
	ИначеЕсли ТипАдресатов = "Корреспонденты" Тогда
		Элементы.ЗаголовокАдресатов.Заголовок = НСтр("ru='Корреспонденты:';uk='Кореспонденти:'");
...
Процедура ЗаполнитьДеревоАдресатов(Ответ, КореньДерева)
	...
	Для каждого Элемент из Ответ.items Цикл
		...
		Иначе
			Если ТипАдресатов <> "Корреспонденты" Тогда
