...
Процедура НачалоДняПриИзменении(Элемент)
	...
	Если НачалоДня > ОкончаниеДня И ЗначениеЗаполнено(ОкончаниеДня) Тогда 
		ПоказатьПредупреждение(, НСтр("ru='Указанное время начала дня больше времени окончания!';uk='Зазначений час початку дня більше часу закінчення!'"));
	КонецЕсли;
...
Процедура ОкончаниеДняПриИзменении(Элемент)
	...
	Если ОкончаниеДня < НачалоДня И ЗначениеЗаполнено(ОкончаниеДня) Тогда 
		ПоказатьПредупреждение(, НСтр("ru='Указанное время окончания дня меньше времени начала';uk='Зазначений час закінчення дня менше часу початку'"));	
	КонецЕсли;
	...
	Если (СпособУказанияВремениID = ЗначениеПеречисленияВремяНачала) И (Работы.Количество() > 0) И ЗначениеЗаполнено(ОкончаниеДня) Тогда 
		Если ОкончаниеДня < Работы[Работы.Количество()-1].Начало Тогда
			ПоказатьПредупреждение(, НСтр("ru='Указанное время окончания дня меньше времени начала одной из работ!';uk='Зазначений час закінчення дня менше часу початку однієї з робіт!'"));
...
Процедура ЗаполнитьСписокТиповИсточника(СписокТипов) 
	...
	ТипыИсточника.Вставить("DMInternalDocument",НСтр("ru='Внутренний документ';uk='Внутрішній документ'"));
	ТипыИсточника.Вставить("DMIncomingEMail", НСтр("ru='Входящее письмо';uk='Вхідний лист'"));
	ТипыИсточника.Вставить("DMIncomingDocument", НСтр("ru='Входящий документ';uk='Вхідний документ'"));
	ТипыИсточника.Вставить("DMDailyReport", НСтр("ru='Ежедневный отчет';uk='Щоденний звіт'"));
	ТипыИсточника.Вставить("DMBusinessProcessTask", НСтр("ru='Задача';uk='Задача'"));
	ТипыИсточника.Вставить("DMOutgoingEMail", НСтр("ru='Исходящее письмо';uk='Вихідний лист'"));
	ТипыИсточника.Вставить("DMOutgoingDocument", НСтр("ru='Исходящий документ';uk='Вихідний документ'"));
	ТипыИсточника.Вставить("DMActivity", НСтр("ru='Мероприятие';uk='Захід'"));
	ТипыИсточника.Вставить("DMProject", НСтр("ru='Проект';uk='Проект'"));
	ТипыИсточника.Вставить("DMProjectTask", НСтр("ru='Проектная задача';uk='Проектна задача'"));
	ТипыИсточника.Вставить("DMFile", НСтр("ru='Файл';uk='Файл'"));
...
Процедура СформироватьЗаголовокФормы()
    ...
    Если Не ЗначениеЗаполнено(ID) Тогда
		ЭтаФорма.Заголовок = НСтр("ru='Ежедневный отчет (создание)';uk='Щоденний звіт (створення)'");
	Иначе
		ЭтаФорма.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ежедневный отчет за %1';uk='Щоденний звіт за %1'"), Формат(Дата, "ДФ=dd.MM.yyyy"));
	КонецЕсли;
...
Процедура ОбновитьПодвал()
	...
	ДлительностьРабот = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Всего за день: %1';uk='Всього за день: %1'"), ИнтеграцияС1СДокументооборотКлиентСервер.ЧислоВСтроку(ДлительностьРаботСек, "0"));
	...
	Для Каждого Строка Из Работы Цикл
		...
		Если Не ЗначениеЗаполнено(Строка.Работа) Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не заполнена колонка ""Содержание работ"" в строке %1 списка ""Работы"".';uk='Не заповнена колонка ""Зміст робіт"" у рядку %1 списку ""Роботи"".'"),
				ИндексСтроки+1);
		...
		Если Не ЗначениеЗаполнено(Строка.ВидРабот) Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не заполнена колонка ""Вид работ"" в строке %1 списка ""Работы"".';uk='Не заповнена колонка ""Вид робіт"" у рядку %1 списку ""Роботи"".'"),
				ИндексСтроки+1);
		...
		Если СпособУказанияВремениID = ЗначениеПеречисленияДлительность Тогда
			...
			Если Не ЗначениеЗаполнено(Длительность) Тогда 
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не заполнена колонка ""Время"" в строке %1 списка ""Работы"".';uk='Не заповнена колонка ""Час"" у рядку %1 списку ""Роботи"".'"),
					ИндексСтроки+1);
		...
		Иначе
			Если Не ЗначениеЗаполнено(Строка.Начало) Тогда 
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не заполнена колонка ""Начало работ"" в строке %1 списка ""Работы"".';uk='Не заповнена колонка ""Початок робіт"" у рядку %1 списку ""Роботи"".'"),
					ИндексСтроки+1);
			...
			Если ДоступенФункционалХронометраж Тогда
				Если Не ЗначениеЗаполнено(Строка.Окончание) Тогда 
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не заполнена колонка ""Окончание работ"" в строке %1 списка ""Работы"".';uk='Не заповнена колонка ""Закінчення робіт"" у рядку %1 списку ""Роботи"".'"),
						ИндексСтроки+1);
				...
				Если Строка.Начало > Строка.Окончание Тогда 
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Время начала работ больше, чем время окончания работ в строке %1 списка ""Работы"".';uk='Час початку робіт більше, ніж час закінчення робіт у рядку %1 списку ""Роботи"".'"),
						ИндексСтроки+1);
			...
			Для Инд1 = 0 По Работы.Количество()-2 Цикл
				...
				Для Инд2 = Инд1+1 По Работы.Количество()-1 Цикл
					...
					Если (Строка2.Начало >= Строка1.Начало И Строка2.Начало < Строка1.Окончание)
						...
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Найдены пересекающиеся интервалы времени в строках %1 и %2 списка ""Работы"".';uk='Знайдені пересічні інтервали часу в рядках %1 і %2 списку ""Роботи"".'"),
							Инд1+1,
							Инд2+1);
