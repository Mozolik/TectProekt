...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1 (Файл)';uk='%1 (Файл)'"), Наименование);
	
	Если Параметры.Свойство("РазрешеноРедактирование", РазрешеноРедактирование) = Неопределено Тогда
		РазрешеноРедактирование = Истина;
	КонецЕсли;
	
	РедактируетсяДругимПользователем = Редактируется И НЕ РедактируетсяТекущимПользователем;
	
	Элементы.ПанельОткрытьДляРедактирования.Доступность = РазрешеноРедактирование
		И ДоступенЗахватИРедактирование И НЕ РедактируетсяДругимПользователем;
	Элементы.ОткрытьДляРедактирования.Доступность = РазрешеноРедактирование
		И ДоступенЗахватИРедактирование И НЕ РедактируетсяДругимПользователем;
		
	Элементы.ПанельЗакончитьРедактирование.Доступность = РазрешеноРедактирование
		И РедактируетсяТекущимПользователем;
	Элементы.ЗакончитьРедактирование.Доступность = РазрешеноРедактирование
		И РедактируетсяТекущимПользователем;
		
	Элементы.ПанельОбновитьИзФайлаНаДиске.Доступность = РазрешеноРедактирование
		И Не РедактируетсяДругимПользователем;
	Элементы.ПанельСохранитьИзменения.Доступность = РазрешеноРедактирование
		И РедактируетсяТекущимПользователем;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ИнтеграцияС1СДокументооборотКлиент.ПоказатьВопросДаНет(Оповещение, ТекстВопроса,
			НСтр("ru='Сохранить';uk='Зберегти'"), НСтр("ru='Не сохранять';uk='Не зберігати'"), КодВозвратаДиалога.Да);
...
Процедура ФайловыеОперацииЕще(Команда)
	...
	Если ДоступенЗахватИРедактирование Тогда
		Подменю.Добавить("ОткрытьКаталогФайла", НСтр("ru='Открыть каталог файла';uk='Відкрити каталог файлу'"),, 
			БиблиотекаКартинок.Папка);
	КонецЕсли;
	Если (Не Редактируется Или РедактируетсяТекущимПользователем)
		И РазрешеноРедактирование Тогда
		Подменю.Добавить("ОбновитьИзФайлаНаДиске", НСтр("ru='Обновить из файла на диске';uk='Оновити з файлу на диску'"),, 
	...
	Если ДоступенЗахватИРедактирование И РедактируетсяТекущимПользователем Тогда
		Подменю.Добавить("ОтменитьРедактирование", НСтр("ru='Отменить редактирование';uk='Скасувати редагування'"),,
			БиблиотекаКартинок.ОсвободитьФайл);
		Если РазрешеноРедактирование Тогда
			Подменю.Добавить("СохранитьИзменения", НСтр("ru='Сохранить изменения';uk='Зберегти зміни'"),,
...
Процедура УдалитьПодпись(Команда)
	...
	ТекстВопроса = НСтр("ru='Удалить выделенные подписи?';uk='Вилучити виділені підписи?'");
	ИнтеграцияС1СДокументооборотКлиент.ПоказатьВопросДаНет(Оповещение, ТекстВопроса,
		НСтр("ru='Удалить';uk='Вилучити'"), НСтр("ru='Не удалять';uk='Не видаляти'"));
...
Процедура ЗаполнитьФорму(ОписаниеФайла)
	...
	Иначе
		...
		Элементы.ДекорацияРедактируется.Заголовок = ?(Редактируется, НСтр("ru='Редактируется';uk='Редагується'"), "");
	КонецЕсли;
	РедактируетсяДругимПользователем = Редактируется И НЕ РедактируетсяТекущимПользователем;
	
	Элементы.ОткрытьДляРедактирования.Доступность = 
		ДоступенЗахватИРедактирование И НЕ РедактируетсяДругимПользователем;
	Элементы.ПанельОткрытьДляРедактирования.Доступность = 
		ДоступенЗахватИРедактирование И НЕ РедактируетсяДругимПользователем;
	Элементы.ЗакончитьРедактирование.Доступность = РедактируетсяТекущимПользователем;
	Элементы.ПанельЗакончитьРедактирование.Доступность = РедактируетсяТекущимПользователем;
	Элементы.ПанельСохранитьИзменения.Доступность = РедактируетсяТекущимПользователем;
	Элементы.ПанельОтменитьРедактирование.Доступность = РедактируетсяТекущимПользователем;
		
	Элементы.ПанельОбновитьИзФайлаНаДиске.Доступность = 
		Не РедактируетсяДругимПользователем;
...
Процедура ЗаписатьВыполнитьКлиент(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Наименование) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("ru='Наименование';uk='Найменування'")),, 
			"Наименование");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
