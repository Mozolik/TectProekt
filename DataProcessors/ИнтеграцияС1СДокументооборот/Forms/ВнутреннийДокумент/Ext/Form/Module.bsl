...
Процедура СвязьОчиститьНажатие(Элемент)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Вы действительно хотите очистить соответствие для
                    |%1?'
                    |;uk='Ви дійсно хочете очистити відповідність для
                    |%1?'"), Строка(ВнешнийОбъект));
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Очистить';uk='Очистити'"));
	Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Не очищать';uk='Не очищати'"));
...
&НаКлиенте
Процедура КорреспондентОткрытие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	Если ЗначениеЗаполнено(КорреспондентID) Тогда
		ИнтеграцияС1СДокументооборотКлиент.ОткрытьОбъект("DMCorrespondent", КорреспондентID, Элемент);
	КонецЕсли;
...
Процедура КорреспондентНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборотКлиент.ВыбратьЗначениеИзСписка("DMCorrespondent", "Корреспондент", ЭтаФорма);
...
&НаКлиенте
Процедура КорреспондентАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание,
	СтандартнаяОбработка)
...
&НаКлиенте
Процедура КорреспондентОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных,
	СтандартнаяОбработка)
...
Процедура КонтактноеЛицоНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе	
		ТекстСообщения = НСтр("ru='Не заполнено поле ""Корреспондент""';uk='Не заповнене поле ""Кореспондент""'");
...
Процедура ФайлыПередУдалением(Элемент, Отказ)
	...
	Если Элементы.Файлы.ВыделенныеСтроки.Количество() = 1 Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Пометить ""%1"" на удаление?';uk='Відмітити ""%1"" для вилучення?'"), Элементы.Файлы.ТекущиеДанные.Наименование);
	Иначе
...
Процедура Зарегистрировать(Команда)
	...
	Если ОграничиватьДоступностьПолей Тогда 
		...
		ТекстВопроса = НСтр("ru='После регистрации документ станет недоступным для изменения.
            |Продолжить?'
            |;uk='Після реєстрації документ стане недоступним для зміни.
            |Продовжити?'");
			...
			Иначе
				...
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Включен хронометраж по другим объектам:
                        ...
                        |;uk='Включений хронометраж по іншим об''єктам:
                        |%1
                        |Відключити хронометраж і зафіксувати трудовитрати
                        |перед включенням нового хронометражу?'"),
						СтрокаОбъектовХронометража);
...
Процедура СоздатьБизнесПроцесс(Команда)
	...
	Иначе
		...
		ТекстВопроса = НСтр("ru='Документ еще не записан.
            |Создать бизнес-процесс можно только после записи. Записать документ?'
            |;uk='Документ ще не записаний.
            |Створити бізнес-процес можна тільки після запису. Записати документ?'");
...
Процедура ДобавитьПечатнуюФорму(Команда)
	...
	Если ФормаДобавления = Неопределено Тогда
		ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 не имеет печатных форм.';uk='%1 не має друкованих форм.'"), Строка(ВнешнийОбъект));
		ПоказатьПредупреждение(, ТекстПредупреждения);
	КонецЕсли;
...
Процедура ЗаполнитьФайлыКопированием(Команда)
	...
	Режим.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Заполнить';uk='Заповнити'"));
	Режим.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Не заполнять';uk='Не заповнювати'"));
	
	Оповещение = Новый ОписаниеОповещения("ЗаполнитьФайлыКопированиемЗавершение", ЭтаФорма);
	ТекстВопроса =  НСтр("ru='Заполнить файлы копированием из присоединенных файлов?';uk='Заповнити файли копіюванням з приєднаних файлів?'");
...
Процедура СохранитьФайлНаДиск(Команда)
	...
	ТекущиеДанные = Элементы.Файлы.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда
		ИнтеграцияС1СДокументооборотКлиент.СохранитьФайлКак(
			ТекущиеДанные.ID,
			ТекущиеДанные.Наименование,
			ТекущиеДанные.Расширение,
			УникальныйИдентификатор);
...
&НаКлиенте
Процедура ДобавитьСвязьВнутреннийДокумент(Команда)
	
	НачатьДобавлениеСвязи("DMInternalDocument");
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьСвязьВходящийДокумент(Команда)
	
	НачатьДобавлениеСвязи("DMIncomingDocument");
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьСвязьИсходящийДокумент(Команда)
	
	НачатьДобавлениеСвязи("DMOutgoingDocument");
...
Процедура ПодписатьДокумент(Команда)
	...
	Если ПустаяСтрока(ID) Тогда 
		...
		ТекстВопроса = НСтр("ru='Документ еще не записан.
            |Подписать его можно только после записи. Записать документ?'
            |;uk='Документ ще не записаний.
            |Підписати його можна тільки після запису. Записати документ?'");
...
Процедура УдалитьПодпись(Команда)
	...
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Удалить';uk='Вилучити'"));
	Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Не удалять';uk='Не видаляти'"));
	
	Оповещение = Новый ОписаниеОповещения("УдалитьПодписьЗавершение", ЭтаФорма);
	ПоказатьВопрос(Оповещение, НСтр("ru='Удалить выделенные подписи?';uk='Вилучити виділені підписи?'"), Кнопки);
...
//
&НаКлиенте
Процедура ЗаписатьИВозможноЗакрыть(ЗакрытьПослеЗаписи)
	
	Оповещение = Новый ОписаниеОповещения(
		"ЗаписатьИВозможноЗакрытьЗавершение", ЭтаФорма, ЗакрытьПослеЗаписи);
	...
	Если ОграничиватьДоступностьПолей 
		...
		ТекстВопроса = НСтр("ru='При записи документ будет зарегистрирован и станет недоступным для изменения.
            |Продолжить?'
            |;uk='При запису документ буде зареєстрований і стане недоступним для зміни.
            |Продовжити?'");
		ИнтеграцияС1СДокументооборотКлиент.ПоказатьВопросДаНет(
			Оповещение, ТекстВопроса,,, КодВозвратаДиалога.Да);
...
//
&НаКлиенте
Процедура ЗаписатьИВозможноЗакрытьЗавершение(Ответ, ЗакрытьПослеЗаписи) Экспорт
	...
	Если Не ДокументЗаполнен Тогда
		...
		Если ПравилаЗаполнения.Количество() = 1 Тогда
			ТекстЗаголовкаСоздать = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='создать %1';uk='створити %1'"), НРег(ПравилаЗаполнения[0].Представление));
		ИначеЕсли ПравилаЗаполнения.Количество() = 0 Тогда
			Элементы.ГруппаСвязь.Видимость = Ложь;
			Возврат;
		Иначе
			ТекстЗаголовкаСоздать = НСтр("ru='создать...';uk='створити...'");
		КонецЕсли;
...
Процедура ЗаполнитьФайлыКопированиемЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Состояние(НСтр("ru='Идет заполнение файлов копированием из присоединенных файлов...';uk='Йде заповнення файлів копіюванням з приєднаних файлів...'"));
...
//
&НаКлиенте
Процедура СписокФайловПослеДобавленияФайла(ОписаниеФайла, Параметры) Экспорт
	
	ОбновитьСписокФайловКлиент(ОписаниеФайла.Идентификатор);
...
// Начинает добавление связанного документа указанного типа.
&НаКлиенте
Процедура НачатьДобавлениеСвязи(ТипДобавляемогоДокумента)
	...
	Если ЗначениеЗаполнено(КорреспондентID) Тогда
		...
		Условие.Вставить("Значение", Корреспондент);
		Условие.Вставить("ЗначениеID", КорреспондентID);
	...
	ИнтеграцияС1СДокументооборотКлиент.НачатьДобавлениеСвязи(
		ID, Тип, Представление, ТипДобавляемогоДокумента, ОписаниеОповещения, Отбор);
...
Процедура УстановитьДоступностьПоСостоянию(ОбъектXDTO)
	...
	Если ДоступныеПоля.Количество() = 0 Или ДоступныеПоля.Найти("register") <> Неопределено Тогда 
