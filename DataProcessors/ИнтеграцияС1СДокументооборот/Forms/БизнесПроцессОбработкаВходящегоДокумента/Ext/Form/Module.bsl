...
Процедура ШаблонИсполненияОзнакомленияПредставлениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СписокДоступныхТиповШаблоновБизнесПроцессов.Добавить(
		Новый Структура("XDTOClassName, Presentation", 
				...
				НСтр("ru='Шаблон исполнения';uk='Шаблон виконання'")));
		
	СписокДоступныхТиповШаблоновБизнесПроцессов.Добавить(
		Новый Структура("XDTOClassName, Presentation", 
				"DMBusinessProcessAcquaintanceTemplate", 
				НСтр("ru='Шаблон ознакомления';uk='Шаблон ознайомлення'")));
		
	ЗаголовокФормы = НСтр("ru='Тип шаблона бизнес-процесса';uk='Тип шаблону бізнес-процесу'");
...
Процедура Записать(Команда)
	...
	Если РезультатЗаписи Тогда
		...
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Бизнес-процесс ""%1"" сохранен.';uk='Бізнес-процес ""%1"" збережено.'"), Представление));
	КонецЕсли;
...
Процедура СтартоватьИЗакрыть(Команда)
	...
	Если РезультатЗапуска Тогда 
		...
		ТекстСостояния = НСтр("ru='Бизнес-процесс ""%Наименование%"" успешно запущен.';uk='Бізнес-процес ""%Наименование%"" успішно запущений.'");
...
Функция ПодготовитьБизнесПроцесс(Прокси)
	...
	ОбъектXDTO = Обработки.ИнтеграцияС1СДокументооборот.ПодготовитьШапкуБизнесПроцесса(
		Прокси, "DMBusinessProcessIncomingDocumentProcessing", ЭтаФорма,"Важность,Стартован,Завершен,Описание,Срок");
