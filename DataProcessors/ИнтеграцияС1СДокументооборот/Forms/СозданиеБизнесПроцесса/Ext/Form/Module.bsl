...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.ГруппаНетПредмета;
...
Процедура ПриОткрытии(Отказ)
	...
	Если СозданНовыйДокумент Тогда 
		ПараметрыОповещения = Новый Структура("name, id, type, documentType", Предмет, ПредметID, ПредметТип, ВидДокумента);
...
&НаКлиенте
Процедура ВыбратьДокументНажатиеЗавершение(ВыбранныйЭлемент, ПараметрыОповещения) Экспорт
...
&НаКлиенте
Процедура СоздатьДокументНажатиеЗавершение(ВыбранныйЭлемент, ПараметрыФормы) Экспорт
...
Процедура УстановитьЗаголовокПредметаПоТипуОбъекта()
	...
	Если ПредметТип = "DMInternalDocument" Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Внутренний документ';uk='Внутрішній документ'");
		
	ИначеЕсли ПредметТип = "DMIncomingDocument" Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Входящий документ';uk='Вхідний документ'");
		
	ИначеЕсли ПредметТип = "DMOutgoingDocument" Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Исходящий документ';uk='Вихідний документ'");
		
	ИначеЕсли ПредметТип = "DMFile" Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Файл';uk='Файл'");
		
	ИначеЕсли ПредметТип = "DMActivity" Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Мероприятие';uk='Захід'");
		
	ИначеЕсли ПредметТип = "DMIncomingEMail" Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Входящее письмо';uk='Вхідний лист'");
		
	ИначеЕсли ПредметТип = "DMOutgoingEMail" Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Исходящее письмо';uk='Вихідний лист'");
		
	ИначеЕсли ПредметТип = "DMProject" Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Проект';uk='Проект'");
		
	ИначеЕсли ПредметТип = "DMProjectTask" Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Проектная задача';uk='Проектна задача'");
		
	ИначеЕсли ПредметТип = "DMDiscussionMessage" Тогда
		Элементы.Предмет.Заголовок = НСтр("ru='Сообщение';uk='Повідомлення'");
		...
		Если ЗначениеЗаполнено(Данные.ШаблонID) Тогда 
			Если ЗаполнитьСтартоватьПроцесс(Данные.ТипПроцесса, ПараметрыФормы) Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Стартован бизнес-процесс по шаблону ""%1"".';uk='Стартован бізнес-процес за шаблоном ""%1"".'"), Данные.Наименование);
				ТекстЗаголовка = НСтр("ru='Стартован процесс';uk='Стартован процес'");
