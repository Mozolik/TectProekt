	...
	Если Источник <> Неопределено Тогда
		...
		Попытка
			Если ЗаполнятьПредставление Тогда
				Если Источник.objectId.Свойства().Получить("presentation") <> Неопределено И Источник.objectId.Установлено("presentation") Тогда
...
//Параметры: 
//	Форма - УправляемаяФорма - форма документа Документооборота
...
Процедура УстановитьЗаголовокДокумента(Форма) Экспорт
	...
	Если Не ЗначениеЗаполнено(Форма.ID) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (создание)';uk='%1 (створення)'"), ПредставлениеТипа);
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (%2)';uk='%1 (%2)'"), ПредставлениеДокумента, ПредставлениеТипа);
	КонецЕсли;
...
Процедура УстановитьСвойстваЭлементовХронометража(
	...
	Если ВключенХронометраж Тогда // хронометраж включен
		КомандаПереключитьХронометраж.Подсказка = НСтр("ru='Закончить хронометраж';uk='Закінчити хронометраж'");
		ЭлементПереключитьХронометраж.Пометка = Истина;
	Иначе
		КомандаПереключитьХронометраж.Подсказка = НСтр("ru='Включить хронометраж';uk='Включити хронометраж'");
...
//Параметры:
//	Форма - УправляемаяФорма - управляемая форма документа Документооборота
...
Функция ЗаполнитьДеревоСвязейИНачатьПолучениеФайлов(Форма, ОбъектXDTO, 
	...
	Для Каждого Связь Из ОбъектXDTO.relations Цикл
		...
		Если ЗначениеЗаполнено(Связь.relatedDocument.regNumber)
			...
			СтрокаДокумент.Заголовок = СтроковыеФункцииКлиентСервер.
				ПодставитьПараметрыВСтроку(НСтр("ru='%1 (№ %2 от %3)';uk='%1 (№ %2 від %3)'"),
	...
	Если ЭлементДляОбновленияЗаголовка <> Неопределено Тогда
		Если ОбъектXDTO.relations.Количество() = 0 Тогда 
			ТекстЗаголовка = НСтр("ru='Связи';uk='Зв''язки'");
		Иначе
			ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Связи (%1)';uk='Зв''язки (%1)'"),
	...
	Если СвязанныеДокументы.Количество() > 0 Тогда
		Возврат ИнтеграцияС1СДокументооборотВызовСервера.НачатьАсинхронныйВызов(
			"Обработки.ИнтеграцияС1СДокументооборот.ПолучитьФайлыСвязанныхДокументов",
			СвязанныеДокументы);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ПриИзмененииВидаНаФормеДокумента(Форма) Экспорт 
	...
	Если ЗначениеЗаполнено(ВидДокументаID) И ЗначениеЗаполнено(ВидДокументаТип) Тогда 
		...
		Исключение
			ВызватьИсключение СтрШаблон(
				НСтр("ru='Вид документа ""%1"" не найден в Документообороте';uk='Вид документа ""%1"" не знайдено у Документообігу'"),
				Форма.ВидДокумента);
		КонецПопытки;
	...
	ИначеЕсли ИмяФормы = "Обработка.ИнтеграцияС1СДокументооборот.Форма.ВнутреннийДокумент" Тогда 
		...
		Элементы.СрокИсполнения.Видимость = (DocumentType <> Неопределено И DocumentType.performanceDateEnabled = Истина); 
		Элементы.ГруппаКорреспондент.ОтображатьЗаголовок = (DocumentType <> Неопределено И DocumentType.correspondentEnabled = Истина); 
...
//Параметры:
//	Форма - УправляемаяФорма - форма объекта или списка Документооборота
Процедура ОбработатьФормуПриНедоступностиФункционалаВерсииСервиса(Форма) Экспорт
	...
	Форма.Элементы.ДекорацияФункционалНеПоддерживается.Заголовок = 
		НСтр("ru='Функционал не поддерживается в данной версии Документооборота.';uk='Функціонал не підтримується в даній версії Документообігу.'");
...
Процедура ЗаполнитьСтандартнуюШапкуБизнесПроцесса(Форма, ОбъектXDTO) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Форма.ID) Тогда
		Форма.Заголовок = Форма.Заголовок + НСтр("ru=' (Создание)';uk=' (Створення)'");
...
Процедура ЗаполнитьПредметыВФорме(Форма, ОбъектXDTO) Экспорт
	...
	Если Форма.ДоступнаМультипредметность Тогда
		...
		Для Каждого target из ОбъектXDTO.targets.items Цикл
			...
			СтрокаПредмета.ИмяПредмета = target.name;
			СтрокаПредмета.Предмет = target.target.name;
			СтрокаПредмета.ПредметID = target.target.objectId.Id;
			СтрокаПредмета.ПредметТип = target.target.objectId.type;
			СтрокаПредмета.РольПредмета = target.role.objectId.id;
			СтрокаПредмета.Картинка = ИнтеграцияС1СДокументооборотКлиентСервер.
				НомерКартинкиПоРолиПредмета(СтрокаПредмета.РольПредмета);
				
			Если target.target.Установлено("externalObject") Тогда
				СтрокаПредмета.Ссылка = ИнтеграцияС1СДокументооборот.СсылкаПоВнешнемуОбъекту(
					target.target.externalObject);
				КонецЕсли;
	...
	Иначе // мультипредметность не поддерживается
		...
		ЗаполнитьОбъектныйРеквизит(Форма, ОбъектXDTO.target, "Предмет");
...
//	Форма - УправляемаяФорма, Структура - управляемая форма или структура реквизитов документа Документооборота
...
Процедура ВыполнитьПоискПоРеквизитам(ТипОбъекта, РеквизитыПоиска, АдресВоВременномХранилище, 
	...
	Если Представление = "" Тогда
		Представление = НСтр("ru='Условия не заданы.';uk='Умови не задані.'");
	Иначе
		Представление = Представление + ".";
	КонецЕсли;
	...
	Если ?(Результат.Установлено("tooManyObjects"), Результат.tooManyObjects, Ложь) Тогда
		Представление = Представление
			+ " " + НСтр("ru='Перетащите сюда любой реквизит, чтобы уточнить условия.';uk='Перетягніть сюди будь-який реквізит, щоб уточнити умови.'");
		;
	Иначе
...
Функция ПодготовитьЛистСогласования(ПредметыСогласования)
	...
	Если Не ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.4.8.1") Тогда
		ВызватьИсключение НСтр("ru='Для вывода листа согласования нужен Документооборот версии не ниже 1.4.8.1';uk='Для виведення листа погодження потрібен Документообіг версії не нижче 1.4.8.1'");
	КонецЕсли;
