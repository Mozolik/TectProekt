...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Не ВыполняетсяЗакрытие
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Перенести и закрыть';uk='Перенести й закрити'"));
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Не переносить и закрыть';uk='Не переносити та закрити'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		ПоказатьВопрос(Новый ОписаниеОповещения("ПослеВопросаОЗакрытии", ЭтотОбъект),
			НСтр("ru='Выбранные транспортные средства не перенесены в список формируемых заданий. Перенести?';uk='Вибрані транспортні засоби не перенесені в список завдань, що формуються. Перенести?'"), Кнопки);
	КонецЕсли;
...
Процедура Выбрать(Команда)
	...
	Иначе
		Если Не ЗначениеЗаполнено(Перевозчик) Тогда
			ТекстСообщения = НСтр("ru='Выберите перевозчика';uk='Виберіть перевізника'");
			ПоказатьПредупреждение(, ТекстСообщения);
			Возврат;
		КонецЕсли;
		ПеренестиСтроки(Элементы.ТипыТранспортныхСредствПеревозчика.ВыделенныеСтроки, Ложь)
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<нет заданий на перевозку>';uk='<немає завдань на перевезення>'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<наша транспортная служба>';uk='<наша транспортна служба>'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ЦветТекстаОтмененнойСтрокиДокумента);
...
Процедура УстановитьЗаголовокТранспортныеСредстваЗаданияНаПеревозку()
	...
	Иначе
		...
		Элементы.ТранспортныеСредстваЗаданияНаПеревозку.Заголовок = НСтр("ru='Задания на перевозку на ';uk='Завдання на перевезення на '") + Формат(ДатаОтбор, "ДФ=""д МММ""");
	КонецЕсли;
КонецПроцедуры
	...
	Для Каждого Стр из ТранспортныеСредства Цикл
		...
		Для Каждого ВлСтр из МассивЗаданийНаПеревозку Цикл
			Если НачалоДня(ВлСтр.ВремяС) = НачалоДня(ВлСтр.ВремяПо) Тогда
				СтрЗаданияНаПеревозку = СтрЗаданияНаПеревозку + ?(СтрЗаданияНаПеревозку = "","",Символы.ПС) + НСтр("ru='С';uk='З'")
										+ Символы.НПП + Формат(ВлСтр.ВремяС, "ДФ=""ЧЧ:мм""") + Символы.НПП + НСтр("ru='до';uk='до'")
										+ Символы.НПП + Формат(ВлСтр.ВремяПо, "ДФ=""ЧЧ:мм""") + " (" + ВлСтр.Статус + ")"
			Иначе
				СтрЗаданияНаПеревозку = СтрЗаданияНаПеревозку + ?(СтрЗаданияНаПеревозку = "","",Символы.ПС) + НСтр("ru='С';uk='З'")
										+ Символы.НПП + Формат(ВлСтр.ВремяС, "ДФ=""ЧЧ:мм""") + Символы.НПП + НСтр("ru='весь день (';uk='весь день ('") + ВлСтр.Статус + ")"
			КонецЕсли;
		КонецЦикла;
...
Процедура ПеренестиСтроки(Знач МассивСтрок, ЭтоТранспортныеСредства = Истина)
	Если МассивСтрок.Количество() = 0 тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбран транспорт';uk='Не вибрано транспорт'"));
		Возврат;
	КонецЕсли;
