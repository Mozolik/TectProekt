...
Процедура ОК(Команда)
	...
	Если (СпособНумерацииСерий = 1 ИЛИ ОграничениеГенерацииНомеров = 1)
	 И (КонечныйНомер < НачальныйНомер ИЛИ НачальныйНомер = 0 ИЛИ КонечныйНомер = 0) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Неверно задан диапазон номеров серий.';uk='Невірно заданий діапазон номерів серій.'"),, "НачальныйНомер");
		Возврат;
	КонецЕсли;
	
	Если СпособНумерацииСерий = 0 И НЕ ЗначениеЗаполнено(КоличествоСерий) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указано количество серий.';uk='Не зазначена кількість серій.'"),, "КоличествоСерий");
		Возврат;
	КонецЕсли;
	
	Если СпособНумерацииСерий = 0 И ОграничениеГенерацииНомеров = 1
	 И КоличествоСерий > (КонечныйНомер - НачальныйНомер + 1) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Количество номеров серий превышает размер диапазона.';uk='Кількість номерів серій перевищує розмір діапазону.'"),, "КоличествоСерий");
		Возврат;
	КонецЕсли;
	
	Если ИспользоватьКоличество И ВКаждойСерии = 1 И НЕ ЗначениеЗаполнено(ВКаждойСерииКоличество) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указано количество номенклатуры в серии.';uk='Не зазначена кількість номенклатури в серії.'"),, "ВКаждойСерииКоличество");
		Возврат;
	КонецЕсли;
