...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтрНайти(", Добавить, Скопировать, Изменить,", ", " + Действие + ",") = 0 Тогда
		...
		ВызватьИсключение НСтр("ru='Неверные параметры открытия формы ""Регламентное задание"".';uk='Невірні параметри відкриття форми ""Регламентне завдання"".'");
	КонецЕсли;
	...
	Иначе
		...
		Если Задание.Метаданные = Неопределено Тогда
			ИмяМетаданных        = НСтр("ru='<нет метаданных>';uk='<немає метаданних>'");
			СинонимМетаданных    = НСтр("ru='<нет метаданных>';uk='<немає метаданних>'");
			ИмяМетодаМетаданных  = НСтр("ru='<нет метаданных>';uk='<немає метаданних>'");
		Иначе
	...
	Если Действие <> "Изменить" Тогда
		Идентификатор = НСтр("ru='<будет создан при записи>';uk='<буде створено при записі>'");
		Использование = Ложь;
...
Процедура ВыборШаблонаНовогоРегламентногоЗадания()
	...
	ОписанияМетаданныхРегламентныхЗаданий.ПоказатьВыборЭлемента(
		...
		НСтр("ru='Выберите шаблон регламентного задания';uk='Виберіть шаблон регламентного завдання'"));
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если Действие = "Изменить" Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 (Регламентное задание)';uk='%1 (Регламентне завдання)'"), Представление);
	Иначе
		Заголовок = НСтр("ru='Регламентное задание (создание)';uk='Регламентне завдання (створення)'");
	КонецЕсли;
