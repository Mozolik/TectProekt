...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НадписьЛогина.Заголовок = НСтр("ru='Логин:';uk='Логін:'") + " " + Параметры.login;
...
Процедура АдресНажатие(Элемент, СтандартнаяОбработка)
	...
	Если РегионыСтраны = Неопределено Тогда
		...
		РегионыСтраны.Добавить("-1", НСтр("ru='<не выбран>';uk='<не вибрано>'"));
	КонецЕсли;
	...
	Если НавигационнаяСсылка = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("ru='Не получается ввести информацию об организации при регистрации программного продукта.
                ...
                |Факс: %8'
                |;uk='Не виходить ввести інформацію про організації при реєстрації програмного продукту.
                |
                |Назва організації: %1
                |Тип діяльності: %2
                |ІПН: %3
		...
		ТекстСообщенияПродолжение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Формат(ДатаПриобретенияПрограммы, "Л=ru_RU; ДФ=dd.MM.yyyy; ДЛФ=D"),
			Строка(ЧислоРабочихМест),
			Ответственный);
		
		ИнтернетПоддержкаПользователейКлиент.СлужебнаяОтправитьСообщениеВТехПоддержку(
			НСтр("ru='Интернет-поддержка. Ввод информации об организации.';uk='Інтернет-підтримка. Введення інформації про організацію.'"),
...
Процедура КомандаОтмена(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		НСтр("ru='Вы уверены, что хотите отказаться от регистрации программного продукта?';uk='Ви впевнені, що хочете відмовитися від реєстрації програмного продукту?'"),
		РежимДиалогаВопрос.ДаНет,
		,
		,
		НСтр("ru='Регистрация программного продукта';uk='Реєстрація програмного продукту'"));
...
Процедура НастроитьОтображениеФормы()
	...
	ПредставлениеАдреса   = НСтр("ru='<введите адрес>';uk='<введіть адресу>'");
...
Функция ЗаполнениеПолейКорректно()
	...
	Если Организация = "-1" Тогда
		...
		Если ПустаяСтрока(НазваниеОрганизации) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru='Поле ""Название организации"" не заполнено.';uk='Поле ""Назва організації"" не заповнено.'"),
				"НазваниеОрганизации",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(ТипДеятельности) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru='Поле ""Тип деятельности"" не заполнено.';uk='Поле ""Тип діяльності"" не заповнено.'"),
				"ТипДеятельности",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(ИНН) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru='Поле ""ИНН"" не заполнено.';uk='Поле ""ІПН"" не заповнено.'"),
				"ИНН",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(Руководитель) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru='Поле ""Руководитель"" не заполнено.';uk='Поле ""Керівник"" не заповнено.'"),
				"Руководитель",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(Телефон) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru='Поле ""Телефон"" не заполнено.';uk='Поле ""Телефон"" не заповнено.'"),
				"Телефон",
				Отказ);
		КонецЕсли;
		
		Если ПустаяСтрока(АдресЭлектроннойПочты) Тогда
			СообщитьОбОшибкеЗаполненияПоля(НСтр("ru='Поле ""Адрес электронной почты"" не заполнено.';uk='Поля ""Адреса електронної пошти"" не заповнено.'"),
				"АдресЭлектроннойПочты",
				Отказ);
		КонецЕсли;
	...
	Если ПустаяСтрока(ГдеПриобретенаПрограмма) Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru='Поле ""Место покупки"" не заполнено.';uk='Поле ""Місце покупки"" не заповнено.'"),
			"ГдеПриобретенаПрограмма",
			Отказ);
	КонецЕсли;
	
	Если ДатаПриобретенияПрограммы = '00010101' Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru='Поле ""Дата покупки"" не заполнено.';uk='Поле ""Дата покупки"" не заповнено.'"),
			"ДатаПриобретенияПрограммы",
			Отказ);
	КонецЕсли;
	
	Если ЧислоРабочихМест = 0 Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru='Поле ""Число рабочих мест"" не заполнено.';uk='Поле ""Кількість робочих місць"" не заповнено.'"),
			"ЧислоРабочихМест",
			Отказ);
	КонецЕсли;
	
	Если ПустаяСтрока(Ответственный) Тогда
		СообщитьОбОшибкеЗаполненияПоля(НСтр("ru='Поле ""Ответственный сотрудник"" не заполнено.';uk='Поле ""Відповідальний співробітник"" не заповнено.'"),
			"Ответственный",
			Отказ);
	КонецЕсли;
...
Процедура СформироватьПредставлениеАдреса()
	...
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Район, НСтр("ru='район';uk='район'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Город, НСтр("ru='г.';uk='р.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Улица, НСтр("ru='ул.';uk='вул.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Дом, НСтр("ru='д.';uk='д.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Строение, НСтр("ru='стр.';uk='стор.'") + " ");
	ДобавитьПодстроку(ПредставлениеАдреса, ДанныеПочтовогоАдреса.Квартира, НСтр("ru='кв.';uk='кв.'") + " ");
	
	Если ПустаяСтрока(ПредставлениеАдреса) Тогда
		Если Организация = "-1" Тогда
			ПредставлениеАдреса = НСтр("ru='<введите адрес>';uk='<введіть адресу>'");
		Иначе
			ПредставлениеАдреса = НСтр("ru='<не заполнен>';uk='<не заповнений>'");
		КонецЕсли;
	КонецЕсли;
