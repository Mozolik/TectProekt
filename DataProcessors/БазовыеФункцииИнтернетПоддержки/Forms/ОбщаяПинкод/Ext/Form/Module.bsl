	...
	Если НавигационнаяСсылка = "TechSupport" Тогда
		...
		ТекстСообщения =
			НСтр("ru='Не получается зарегистрировать программный продукт.
                |
                |Пинкод: %1.'
                |;uk='Не вдається зареєструвати програмний продукт.
                |
                |Пінкод: %1.'");
		...
		ИнтернетПоддержкаПользователейКлиент.СлужебнаяОтправитьСообщениеВТехПоддержку(
			НСтр("ru='Интернет-поддержка. Регистрация продукта.';uk='Інтернет-підтримка. Реєстрація продукту.'"),
			ТекстСообщения,
			,
			,
			ЛогинПользователя,
...
Функция ЗаполнениеПолейКорректно()
	...
	Если ПустаяСтрока(ПинКод) Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не заполнено поле ""Пинкод""';uk='Не заповнено поле ""Пінкод""'");
	...
	Если СтрДлина(ПинКодСтр) <> 16 Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Пин-код должен состоять из 16 цифр.';uk='Пін-код повинен складатися з 16 цифр.'");
		Сообщение.Поле  = "ПинКод";
		Сообщение.Сообщить();
