...
Функция ПроверкаПодключения(СообщениеОбОшибке)
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		СообщениеОбОшибке = НСтр("ru='Подключаемая информационная база является файловой,
            ...
            |у зв''язку з чим не підтримує роботу методів веб-сервісу.'");
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗагрузитьДанныеВИнформационнуюБазу(ИдентификаторФайла, ИдентификаторДлительнойОперации, СообщениеОбОшибке)
	...
	ФоновоеЗаданиеЗагрузкиДанных = ФоновыеЗадания.Выполнить("ОбменДаннымиСлужебный.ЗагрузитьДанныеXDTOВИнформационнуюБазу",
		...
		НСтр("ru='Загрузка данных в информационную базу через web-сервис ""Enterprise Data Upload""';uk='Завантаження даних в інформаційну базу через web-сервіс ""Enterprise Data Upload""'"));
