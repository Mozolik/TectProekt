...
Функция ДоставитьСообщения(КодОтправителя, ПотокХранилище)
	...
	Если Отправитель.Пустая() Тогда
		...
		ВызватьИсключение НСтр("ru='Заданы неправильные настройки подключения к конечной точке.';uk='Задані неправильні настройки підключення до кінцевої точки.'");
	...
	Если ДанныеПрочитаныЧастично Тогда
		...
		ВызватьИсключение НСтр("ru='Произошла ошибка при доставке быстрых сообщений - некоторые сообщения
                                ...
                                |;uk='Сталася помилка при доставці швидких повідомлень - деякі повідомлення
                                |не були доставлені з-за встановлених блокувань областей даних!
                                |
                                |Ці повідомлення будуть оброблені в рамках черги обробки повідомлень системи.'");
...
Функция ПроверитьПодключениеНаСторонеОтправителя(КодОтправителя)
	...
	Если ОбменСообщениямиВнутренний.КодЭтогоУзла() <> КодОтправителя Тогда
		...
		ВызватьИсключение НСтр("ru='Настройки подключения базы получателя указывают на другого отправителя.';uk='Настройки підключення бази одержувача вказують на іншого відправника.'");
