...
Функция ПолучитьПланыОбменаКонфигурации()
	...
	Для Каждого ИмяПланаОбмена Из ОбменДаннымиВМоделиСервисаПовтИсп.ПланыОбменаСинхронизацииДанных() Цикл
		...
		Если ПустаяСтрока(ExchangeFeature.ExchangeRole) Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru='Не задано возвращаемое значение функции ИмяКонфигурацииИсточника() в модуле менеджера плана обмена %1';uk='Не задане значення що повертається для функції ИмяКонфигурацииИсточника() у модулі менеджера плану обміну %1'"),
				ИмяПланаОбмена
			);
		КонецЕсли;
