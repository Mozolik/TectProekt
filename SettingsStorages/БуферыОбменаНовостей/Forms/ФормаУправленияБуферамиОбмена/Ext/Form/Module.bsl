...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВРег(ЭтаФорма.КлючОбъекта) = ВРег("Документы.Новости.КатегорииПростые") 
			...
			ИЛИ ВРег(ЭтаФорма.КлючОбъекта) = ВРег("Справочники.Пользователи.ПраваДоступаКТематическимПодборкам")
			Тогда
		...
		Если ЭтаФорма.ТекущийБуферОбмена.Количество() > 0 Тогда
			Если НЕ ПустаяСтрока(ЭтаФорма.КлючНастройки) Тогда
				Если ЭтаФорма.СписокКлючейНастроек.НайтиПоЗначению(ЭтаФорма.КлючНастройки) = Неопределено Тогда
					...
					СписокКнопок.Добавить("ЗаписатьИЗакрыть", "Записать и закрыть");
					СписокКнопок.Добавить("Закрыть", "Закрыть без записи");
					СписокКнопок.Добавить("Отмена", "Не закрывать");
					ТекстВопроса = НСтр("ru='Вы ввели наименование настройки (%ЭтаФорма.КлючНастройки%),
                        |но не записали ее.
                        |
                        |Записать?'
                        |;uk='Ви ввели найменування настройки (%ЭтаФорма.КлючНастройки%),
                        |але не записали її.
                        |
                        |Записати?'");
					ТекстВопроса = СтрЗаменить(ТекстВопроса, "%ЭтаФорма.КлючНастройки%", ЭтаФорма.КлючНастройки);
					ПоказатьВопрос(ОписаниеОповещенияОЗакрытии, ТекстВопроса, СписокКнопок, 0, "ЗаписатьИЗакрыть", "Текущее значение буфера обмена не сохранено");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура КомандаВыгрузитьСписокБуферовОбмена(Команда)
	...
	Диалог.Заголовок = "Выберите файл, куда записать список буферов обмена";
...
Процедура КомандаЗагрузитьСписокБуферовОбмена(Команда)
	...
	Диалог.Заголовок = "Выберите файл с сохраненными ранее буферами обмена";
	Диалог.МножественныйВыбор = Ложь;
	Диалог.Расширение = "xml";
	Диалог.Фильтр = "Файл xml (*.xml)|*.xml";
	Если Диалог.Выбрать() Тогда
			...
			Для каждого ТекущаяСтрока Из ЭтаФорма.ТекущийБуферОбмена Цикл
				ЭтаФорма.ТекущийБуферОбмена_Представление = 
					...
					+ СокрЛП(ТекущаяСтрока.Значение.Метаданные) + ", "
						...
						//+ СокрЛП(ТекущаяСтрока.Значение.ПоказыватьВФормеОбъекта) + ", "
						+ ПолучитьОписаниеВажности(ТекущаяСтрока.Значение.Важность)
						//+ СокрЛП(ТекущаяСтрока.Значение.ДатаСбросаВажности) + ", "
						//+ СокрЛП(ТекущаяСтрока.Значение.ДатаСбросаВажностиМестная) + ", "
						//+ СокрЛП(ТекущаяСтрока.Значение.ЭтоПостояннаяНовость) + ", "
					+ Символы.ПС;
			КонецЦикла;
...
Процедура КомандаВыгрузитьСписокБуферовОбменаНаСервере(ИмяФайла)
	...
	ЗаписьХМЛ.ОткрытьФайл(
		...
		ЗаписьХМЛ.ЗаписатьКомментарий("Выгрузка данных буферов обмена для " + ЭтаФорма.КлючОбъекта + ", " + Формат(ТекущаяДатаСеанса(),"ДФ='dd.MM.yyyy HH:mm:ss'"));
