...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяПользователяИБ = "";
	Если ЭтаФорма.ТекущийПользователь <> ПараметрыСеанса.ТекущийПользователь Тогда
	...
	Если ЭтаФорма.СписокНовостей_ИнтервалАвтообновления % (60*24*7) = 0 Тогда // 1 раз в неделю
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = "Список новостей обновляется еженедельно";
	ИначеЕсли ЭтаФорма.СписокНовостей_ИнтервалАвтообновления % (60*24) = 0 Тогда // 1 раз в день
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = "Список новостей обновляется ежедневно";
	ИначеЕсли ЭтаФорма.СписокНовостей_ИнтервалАвтообновления % 60 = 0 Тогда // 1 раз в час
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = "Список новостей обновляется ежечасно";
	Иначе
		ЭтаФорма.СписокНовостей_ИнтервалАвтообновления_Представление = "Список новостей обновляется каждые " + ЭтаФорма.СписокНовостей_ИнтервалАвтообновления + " минут";
	КонецЕсли;
	...
	Если ЭтаФорма.ТекущийПользователь <> ПараметрыСеанса.ТекущийПользователь Тогда
		ЭтаФорма.Заголовок = "Настройки отображения новостей (" + ЭтаФорма.ТекущийПользователь + ")";
	КонецЕсли;
	...
	Если ТипЗнч(ПараметрыФормы) = ТипСтруктура Тогда
		...
		Иначе
			ПоказатьПредупреждение(
				...
				НСтр("ru='Вначале включите получение новостей по этой ленте.';uk='Спочатку ввімкніть отримання новин по цій стрічці.'"),
				0,
				НСтр("ru='Предупреждение';uk='Попередження'"));
		КонецЕсли;
	КонецЕсли;
	...
	Для каждого ТекущаяСтрока Из ЭтаФорма.ЛентыНовостей Цикл
		Если (ТекущаяСтрока.ЛентаНовостей = лкЛентаНовостей) ИЛИ (лкЛентаНовостей = Неопределено) Тогда
			...
			Если (ТекущаяСтрока.КоличествоВсехОтборов = 0) Тогда
				...
				ПредставлениеВсехОтборовПодробно = "По этой ленте отборы не настраиваются";
			Иначе
				ПредставлениеВсехОтборовКратко               = "";
				...
				ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно
					...
					+ ?(ПустаяСтрока(ПредставлениеПользовательскихОтборов),
							"Нет отборов, разрешенных для настройки пользователем" + Символы.ПС,
							"Отборы, настроенные пользователем (можно изменить):" + Символы.ПС + ПредставлениеПользовательскихОтборов + Символы.ПС);

				ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно
					+ ?(ПустаяСтрока(ПредставлениеАдминистративныхОтборов),
							"",
							"Отборы, настроенные администратором (нельзя изменить):" + Символы.ПС + ПредставлениеАдминистративныхОтборов + Символы.ПС);

				ПредставлениеВсехОтборовПодробно = ПредставлениеВсехОтборовПодробно
					+ ?(ПустаяСтрока(ПредставлениеАвтоматическиЗаполняемыхОтборов),
							"",
							"Автоматически заполняемые отборы (нельзя изменить):" + Символы.ПС + ПредставлениеАвтоматическиЗаполняемыхОтборов + Символы.ПС);

				Если ПустаяСтрока(ПредставлениеВсехОтборовПодробно) Тогда
					ПредставлениеВсехОтборовКратко   = НСтр("ru='Отборы не настроены';uk='Відбори не настроєний'");
					ПредставлениеВсехОтборовПодробно = НСтр("ru='Отборы не настроены';uk='Відбори не настроєний'");
				Иначе
					ПредставлениеВсехОтборовКратко   = НСтр("ru='Настроен детальный отбор';uk='Налаштований детальний відбір'");
				КонецЕсли;
			...
			Если ТекущаяСтрока.ОбязательныйКанал = Истина Тогда
				ПредставлениеВсехОтборовПодробно = 
					"По этой ленте нельзя отключить получение новостей (так настроено администратором)" + Символы.ПС
					+ ПредставлениеВсехОтборовПодробно;
			КонецЕсли;
