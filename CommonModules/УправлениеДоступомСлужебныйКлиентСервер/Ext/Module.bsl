...
Процедура ЗаполнитьВсеРазрешеныПредставление(Форма, ОписаниеВидаДоступа, ДобавитьКоличествоЗначений = Истина) Экспорт
		...
		Иначе
			ЧислоИПредмет = Формат(КоличествоЗначений, "ЧГ=") + " "
				+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоЗначений,
					"Л = ru_RU", НСтр("ru='значение,значения,значений,,,,,,0';uk='значення,значення,значень,,,,,,0'"));
		КонецЕсли;
	...
	Если КоличествоЗначений = 0 Тогда
		Представление = ?(ОписаниеВидаДоступа.ВсеРазрешены,
			НСтр("ru='Все разрешены, без исключений';uk='Всі дозволені, без виключень'"),
			НСтр("ru='Все запрещены, без исключений';uk='Всі заборонені, без виключень'"));
	Иначе
		ЧислоИПредмет = Формат(КоличествоЗначений, "ЧГ=")
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоЗначений,
				"Л = ru_RU", НСтр("ru='-го значения,-х значений,-и значений,,,,,,0';uk='-го значення,-х значень,-і значень,,,,,,0'"));
		
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			?(ОписаниеВидаДоступа.ВсеРазрешены,
				НСтр("ru='Все разрешены, кроме %1';uk='Всі дозволені, крім %1'"),
				НСтр("ru='Все запрещены, кроме %1';uk='Всі заборонені, крім %1'")),
			ЧислоИПредмет);
	КонецЕсли;
...
Процедура ПриИзмененииТекущегоВидаДоступа(Форма) Экспорт
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТекущиеДанные.ВидДоступа = Форма.ВидДоступаПользователи Тогда
			ШаблонНадписи = ?(ТекущиеДанные.ВсеРазрешены,
				НСтр("ru='Запрещенные значения (%1) - текущий пользователь всегда разрешен';uk='Заборонені значення (%1) - поточний користувач завжди дозволений'"),
		...
		ИначеЕсли ТекущиеДанные.ВидДоступа = Форма.ВидДоступаВнешниеПользователи Тогда
			ШаблонНадписи = ?(ТекущиеДанные.ВсеРазрешены,
				...
				НСтр("ru='Разрешенные значения (%1) - текущий внешний пользователь всегда разрешен';uk='Дозволені значення (%1) - поточний зовнішній користувач завжди дозволений'") );
		Иначе
			ШаблонНадписи = ?(ТекущиеДанные.ВсеРазрешены,
	...
	Если Форма.ТекущиеТипыВыбираемыхЗначений.Количество() = 0 Тогда
		Форма.ТекущиеТипыВыбираемыхЗначений.Добавить(Неопределено, НСтр("ru='Неопределено';uk='Невизначено'"));
	КонецЕсли;
	...
	Пока НЕ Отказ И ИндексВидаДоступа >= 0 Цикл
		...
		Если СтрокаВидаДоступа.ВидДоступа = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				...
				НСтр("ru='Вид доступа не выбран.';uk='Вид доступу не вибраний.'"),
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				НСтр("ru='Вид доступа в строке %1 не выбран.';uk='Вид доступу в рядку %1 не вибраний.'"),
		...
		Если Параметры.Назначение <> Неопределено
		  И Не ВидДоступаСоответствуетНазначениюПрофиля(СтрокаВидаДоступа.ВидДоступа, НазначениеПрофиля) Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				Параметры.ПутьКТаблицам + "ВидыДоступа[%1].ВидДоступа",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вид доступа ""%1"" не соответствует назначению профиля.';uk='Вид доступу ""%1"" не відповідає призначенню профілю.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вид доступа ""%1"" в строке %%1 не соответствует назначению профиля.';uk='Вид доступу ""%1"" в рядку %%1 не відповідає призначенню профіля.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				Параметры.ВидыДоступа.Индекс(СтрокаВидаДоступа));
			Отказ = Истина;
			Прервать;
		...
		Если НайденныеВидыДоступа.Количество() > 1 Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
				Параметры.ПутьКТаблицам + "ВидыДоступа[%1].ВидДоступа",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вид доступа ""%1"" повторяется.';uk='Вид доступу ""%1"" повторюється.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				"ВидыДоступа",
				ВидыДоступа.Найти(СтрокаВидаДоступа),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вид доступа ""%1"" в строке %%1 повторяется.';uk='Вид доступу ""%1"" в рядку %%1 повторюється.'"),
					СтрокаВидаДоступа.ВидДоступаПредставление),
				Параметры.ВидыДоступа.Индекс(СтрокаВидаДоступа));
			Отказ = Истина;
			Прервать;
		...
		Пока НЕ Отказ И ИндексЗначенияДоступа >= 0 Цикл
			...
			Если СтрокаЗначенияДоступа.ЗначениеДоступа = Неопределено Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					НСтр("ru='Значение не выбрано.';uk='Значення не вибране.'"),
					"ЗначенияДоступа",
					ЗначенияДоступа.Найти(СтрокаЗначенияДоступа),
					НСтр("ru='Значение в строке %1 не выбрано.';uk='Значення в рядку %1 не вибрано.'"),
