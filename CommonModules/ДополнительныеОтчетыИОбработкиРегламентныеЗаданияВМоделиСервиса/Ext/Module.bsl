...
Процедура УстановитьПараметрыЗадания(Задание, Использование, Параметры, Расписание) Экспорт
	...
	Если Не Константы.РазрешитьВыполнениеДополнительныхОтчетовИОбработокРегламентнымиЗаданиямиВМоделиСервиса.Получить() Тогда
		ВызватьИсключение НСтр("ru='Периодическое выполнение команд дополнительных обработок в качестве заданий запрещено администрацией сервиса!';uk='Періодичне виконання команд додаткових обробок в якості завдань заборонено адміністрацією сервісу!'");
	КонецЕсли;
	...
	Если Расписание.ТребуетсяВыполнение(ПроверяемаяДата, ИсходнаяДата) Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Расписание, задаваемое для выполнения команд дополнительного отчета или обработки в качестве заданий, должно быть не чаще, чем 1 раз в %1 секунд!';uk='Розклад, що задається для виконання команд додаткового звіту чи обробки в якості завдань, повинно бути не частіше, ніж 1 раз у %1 секунд!'"), МинимальныйИнтервал);
