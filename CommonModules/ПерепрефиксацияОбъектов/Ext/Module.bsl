...
Процедура СлужебнаяУстановитьПрефиксИБИПерепрефиксоватьОбъектыСозданныеВЭтойИБ(
	...
	Если ТранзакцияАктивна() Тогда
		...
		ВызватьИсключение НСтр("ru='Изменение префикса информационной базы не может выполняться в транзакции.';uk='Зміна префікса інформаційної бази не може виконуватися в транзакції.'");
		
	ИначеЕсли Не Пользователи.ЭтоПолноправныйПользователь() Тогда
		
		ВызватьИсключение НСтр("ru='Недостаточно прав для изменения префикса информационной базы.';uk='Недостатньо прав для зміни префікса інформаційної бази.'");
...
Функция НовыйКодОбъекта(Знач ФорматНовогоКода, Знач Код, Знач ЭтоДокумент, Объект)
	...
	Если КоличествоЛидирующихНулей < 0 Тогда
		...
		СтрокаСообщения = НСтр("ru='Преобразование %1 объекта %2 не может быть выполнено.
            ...
            |;uk='Перетворення %1 об''єкта %2 не може бути виконано.
            |Недостатня довжина %1. Мінімальна довжина %1 об''єкта винна становити %3 символів.'");
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, ?(ЭтоДокумент, НСтр("ru='номера';uk='номера'"), НСтр("ru='кода';uk='коду'")),
...
Процедура ПроверитьВозможностьУстановкиПрефикса()
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
		...
		ВызватьИсключение НСтр("ru='Перепрефиксация объектов недоступна.';uk='Перепрефіксація об''єктів недоступна.'");
...
Функция СобытиеЖурналаРегистрацииПерепрефиксацияОбъектов()
	...
	Возврат НСтр("ru='Префиксация объектов.Изменение префикса информационной базы';uk='Префіксація об''єктів.Зміна префікса інформаційної бази'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
