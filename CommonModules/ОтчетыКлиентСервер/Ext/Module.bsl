...
Функция ПредставлениеЭлементаУсловногоОформления(ЭлементКД) Экспорт
	...
	Если ПредставлениеОформления = "" Тогда
		ПредставлениеОформления = НСтр("ru='Не оформлять';uk='Не оформлювати'");
	КонецЕсли;
	
	ПредставлениеПолей = Строка(ЭлементКД.Поля);
	Если ПредставлениеПолей = "" Тогда
		ПредставлениеПолей = НСтр("ru='Все поля';uk='Всі поля'");
	Иначе
		ПредставлениеПолей = НСтр("ru='Поля:';uk='Поля:'") + " " + ПредставлениеПолей;
	КонецЕсли;
	...
	Иначе
		Разделитель = "; ";
		ПредставлениеОтбора = НСтр("ru='Условие:';uk='Умова:'") + " " + ПредставлениеОтбора;
	КонецЕсли;
...
Функция ПредставлениеЦвета(Цвет)
	...
	ИначеЕсли Цвет.Вид = ВидЦвета.Абсолютный Тогда
		...
		Если Представление = "0, 0, 0" Тогда
			Представление = НСтр("ru='Черный';uk='Чорний'");
		ИначеЕсли Представление = "255, 255, 255" Тогда
			Представление = НСтр("ru='Белый';uk='Білий'");
		КонецЕсли;
	ИначеЕсли Цвет.Вид = ВидЦвета.АвтоЦвет Тогда
		Представление = НСтр("ru='Авто';uk='Авто'");
	Иначе
		Представление = "";
	КонецЕсли;
	Возврат Представление;
...
Функция ПараметрыКопирования(ТипЭлемента, Коллекция)
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Копирование элементов ""%1"" не поддерживается';uk='Копіювання елементів ""%1"" не підтримується'"),
			ТипЭлемента);
...
Функция СписокВыбораВидовСравнения(ОписаниеТипов) Экспорт
	...
	Если ИнформацияОТипах.ОграниченнойДлины Тогда
		...
		Если ИнформацияОТипах.СодержитОбъектныеТипы Тогда
			...
			Список.Добавить(ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии); // НСтр("ru='В списке включая подчиненные';uk='У списку включаючи підпорядковані'")
			Список.Добавить(ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии); // НСтр("ru='Не в списке включая подчиненные';uk='Не в списку включаючи підпорядковані'").
			
			Список.Добавить(ВидСравненияКомпоновкиДанных.ВИерархии); // НСтр("ru='В группе';uk='В групі'")
			Список.Добавить(ВидСравненияКомпоновкиДанных.НеВИерархии); // НСтр("ru='Не в группе';uk='Не в групі'")
