...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Актуализирует данные первого показа.';uk='Актуалізує дані першого показу.'");
...
Функция ПакетыСтраниц(НосительМакетов) Экспорт
	...
	Для НомерСтроки = 3 По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		Если НРег(СтрокаТаблицы.Раздел) = НРег(НСтр("ru='Реклама';uk='Реклама'")) Тогда
...
Функция ИзвлечьФайлыПакета(НосительМакетов, ИмяМакета) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Информация при запуске';uk='Інформація при запуску'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
