...
Функция ПолучитьНаименованиеЗаданияВыполненияОтчета(Форма) Экспорт
	...
	НаименованиеЗадания = НСтр("ru='Выполнение отчета: %1';uk='Виконання звіту: %1'");
...
Процедура ИзменитьЗаголовокКнопкиПанельНастроек(Кнопка, ВидимостьПанелиНастроек) Экспорт
	...
	Если ВидимостьПанелиНастроек Тогда
		Кнопка.Заголовок = НСтр("ru='Скрыть настройки';uk='Приховати настройки'");
	Иначе
		Кнопка.Заголовок = НСтр("ru='Показать настройки';uk='Показати настройки'");
	КонецЕсли;
...
Процедура ОрганизацияПриИзменении(Форма) Экспорт 
	...
	Отчет = Форма.Отчет;
	ТипыСвязанныеСОрганизацией = БухгалтерскиеОтчетыВызовСервераПовтИсп.ТипыСвязанныеСОрганизацией();
