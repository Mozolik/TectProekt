			...
			Если ВРег(Строка.ВидДоступа) = ВРег("ПравоИзменения") Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка в функции ЕстьРоль модуля УправлениеДоступом.
                               ...
                               |;uk='Помилка у функції ЕстьРоль модуля УправлениеДоступом.
                               |У наборі значень доступу вказаний вид доступу ПравоИзменения
                               |таблиці з ідентифікатором ""%1"".
                               |Обмеження перевірки ролі (як додаткового права)
                               |може бути залежність тільки від права Читання.'"),
		...
		ИначеЕсли ИменаВидовДоступа.Получить(Строка.ВидДоступа) <> Неопределено
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка в функции ЕстьРоль модуля УправлениеДоступом.
                           ...
                           |;uk='Помилка у функції ЕстьРоль модуля УправлениеДоступом.
                           |Набір значень доступу містить відомий вид доступу ""%1"",
                           |який не потрібно вказувати.
                           |
                           |Вказувати потрібно тільки спеціальні види доступу
                           |""ПравоЧтения"", ""ПравоИзменения"", якщо вони використовуються.'"),
				Строка.ВидДоступа);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка в функции ЕстьРоль модуля УправлениеДоступом.
                           ...
                           |;uk='Помилка у функції ЕстьРоль модуля УправлениеДоступом.
                           |Набір значень доступу містить невідомий вид доступу ""%1"".'"),
				Строка.ВидДоступа);
		КонецЕсли;
...
Функция ЕстьПраво(Право, СсылкаНаОбъект, Пользователь = Неопределено) Экспорт
	...
	Если ОписаниеПрав = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найдено описание возможных прав для таблицы ""%1""';uk='Не знайдений опис можливих прав для таблиці ""%1""'"),
	...
	Если ОписаниеПрава = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найдено описание права ""%1"" для таблицы ""%2""';uk='Не знайдений опис права ""%1"" для таблиці ""%2""'"),
...
Процедура ПриСозданииФормыЗначенияДоступа(Форма,
	...
	ЗаголовокОшибки =
		НСтр("ru='Ошибка в процедуре ПриСозданииФормыЗначенияДоступа
                   ...
                   |;uk='Помилка в процедурі ПриСозданииФормыЗначенияДоступа
                   |загального модуля УправлениеДоступом.'");
		...
		Исключение
			...
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Параметр СозданиеНового не указан, а автоматическое заполнение
                           ...
                           |%1'
                           |;uk='Параметр СозданиеНового не вказаний, а автоматичне заповнення
                           |з реквізиту форми ""Объект.Ссылка"" недоступне через:
		...
		Исключение
			...
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Параметр ТипЗначения не указан, а автоматическое заполнение
                           |из реквизита формы ""Объект.Ссылка"" недоступно по причине:
                           |%1'
                           |;uk='Параметр ТипЗначения не вказаний, а автоматичне заповнення
                           |з реквізиту форми ""Объект.Ссылка"" недоступне через:
                           |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
	Иначе
		...
		Исключение
			...
			ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Параметр Реквизит не указан, а автоматическое заполнение
                           ...
                           |%1'
                           |;uk='Параметр Реквізит не вказаний, а автоматичне заповнення
                           |з реквізиту форми ""Объект.ГруппаДоступа"" недоступне через:
                           |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецПопытки;
	Иначе
			...
			Исключение
				...
				ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось получить значение реквизита формы ""%1"", указанного параметре Реквизит по причине:
                               |%2'
                               |;uk='Не вдалося отримати значення реквізиту форми ""%1"", зазначеного параметрі Реквізит з причини:
	...
	Если ТипЗнч(ГруппаЗначенийДоступа) <> СвойстваГрупп.Тип Тогда
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для значений доступа типа ""%1""
                       ...
                       |;uk='Для значень доступу типу ""%1""
                       |використовуються вид доступу ""%2"" з типом значень ""%3"",
                       |що заданий у переобумовленому модулі.
                       |Але цей тип не збігається з типом ""%4"" в формі значення
                       |доступу у реквізита ГруппаДоступа.'"),
	...
	Если ГруппыЗначенийДляИзменения.Количество() = 0
	   И СозданиеНового Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для добавления требуются разрешенные ""%1"".';uk='Для додавання потрібні дозволені ""%1"".'"),
...
Функция ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа(ТипЗначенийДоступа, ВозвращатьВсе = Ложь) Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ru='Ошибка в процедуре ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа
                   |общего модуля УправлениеДоступом.'
                   |;uk='Помилка в процедурі ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа
                   |загального модуля УправлениеДоступом.'");
	...
	Если ТипЗнч(Список) <> Тип("ДинамическийСписок") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом.
                       |Значение параметра Список ""%1"" не является динамическим списком.'
                       |;uk='Помилка виклику процедури НастроитьОтборыДинамическогоСписка загального модуля УправлениеДоступом.
                       |Значення параметра Список ""%1"" не є динамічним списком.'"), Строка(Список));
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Список.ОсновнаяТаблица) Тогда
		ВызватьИсключение
			НСтр("ru='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом.
                       ...
                       |;uk='Помилка виклику процедури НастроитьОтборыДинамическогоСписка загального модуля УправлениеДоступом.
                       |У переданого динамічного списку не вказана основна таблиця.'");
	КонецЕсли;
	...
	Если Не Список.ПроизвольныйЗапрос Тогда
		ВызватьИсключение
			НСтр("ru='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом.
                       ...
                       |;uk='Помилка виклику процедури НастроитьОтборыДинамическогоСписка загального модуля УправлениеДоступом.
                       |У переданого динамічного списку не встановлений прапорець ""Довільний запит"".'");
	КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(Псевдоним) Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом.
                           ...
                           |""%1"".'
                           |;uk='Помилка виклику процедури НастроитьОтборыДинамическогоСписка загального модуля УправлениеДоступом.
                           |У переданого динамічного списку не вдалося знайти псевдонім основної таблиці
...
Функция РазрешенныеЗначенияДляДинамическогоСписка(Таблица, ТипЗначений) Экспорт
	...
	Для Каждого ТекущийТип Из ТипыЗначений Цикл
		...
		Если Свойства = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Тип ""%1"" не является типом значений доступа';uk='Тип ""%1"" не є типом значень доступу'"), Строка(ТекущийТип));
		КонецЕсли;
	...
	Если НЕ НаборыЗаполняются Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверные параметры.
                       ...
                       |Тип об''єкта ""%1""
                       |не знайдений у жодній з підписок на події
	...
	Исключение
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 ""%2""
                       ...
                       |%3'
                       |;uk='%1 ""%2""
                       |не сформував набір значень доступу через:
                       |%3'"),
	...
	Если Таблица.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 ""%2""
                       ...
                       |;uk='%1 ""%2""
                       |сформував порожній набір значень доступу.'"),
			ТипЗнч(СсылкаНаОбъект),
			Объект);
	КонецЕсли;
...
//                       Чтобы получить идентификатор нужно создать профиль в режиме клиента и
		...
		Если ПовторяющиесяНовыеИмена.Количество() > 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка в параметрах процедуры ПереименоватьПравоВНастройкахПравОбъектов
                           ...
                           |%1.'
                           |;uk='Помилка в параметрах процедури ПереименоватьПравоВНастройкахПравобъектов
                           |загального модуля УправлениеДоступом.
                           |
                           |Після оновлення будуть повторюватися настройки наступних нових імен прав:
                           |%1.'"),
				ПовторяющиесяНовыеИменаПрав);
		КонецЕсли;
		...
		ИначеЕсли ИменаВидовДоступа.Получить(Строка.ВидДоступа) <> Неопределено
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Объект ""%1"" сформировал набор значений доступа,
                           ...
                           |Указывать требуется только специальные виды доступа
                           |""ПравоЧтения"", ""ПравоИзменения"", если они используются.'
                           |;uk='Об''єкт ""%1"" сформував набір значень доступу,
                           |містить відомий вид доступу ""%2"", який не потрібно вказувати.
                           |
                           |Вказувати потрібно тільки спеціальні види доступу
                           |""ПравоЧтения"", ""ПравоИзменения"", якщо вони використовуються.'"),
				ТипЗнч(СсылкаНаОбъект),
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Объект ""%1"" сформировал набор значений доступа,
                           ...
                           |;uk='Об''єкт ""%1"" сформував набір значень доступу,
                           |який містить невідомий вид доступу ""%2"".'"),
				ТипЗнч(СсылкаНаОбъект),
				Строка.ВидДоступа);
		КонецЕсли;
	...
	Если СвойстваВидаДоступа = Неопределено Тогда
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для значений доступа типа ""%1""
                       ...
                       |;uk='Для значень доступу типу ""%1""
                       |не використовуються групи значень доступу.'"),
			Строка(ТипЗначенияДоступа));
	КонецЕсли;
