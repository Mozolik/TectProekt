...
Процедура ОбновлениеИндексаППД() Экспорт
	...
	ОбновитьИндекс(НСтр("ru='Обновление индекса ППД';uk='Оновлення індексу ППД'"), Ложь, Истина);
...
Процедура СлияниеИндексаППД() Экспорт
	...
	ОбновитьИндекс(НСтр("ru='Слияние индекса ППД';uk='Злиття індексу ППД'"), Истина);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если ДатаАктуальностиИндекса > ТекущаяДата() Тогда
		Интервал = НСтр("ru='менее одного дня';uk='менше одного дня'");
	Иначе
		// Исключение, должна использоваться ТекущаяДата().
	...
	Дело.Представление  = НСтр("ru='Индекс полнотекстового поиска устарел';uk='Індекс повнотекстового пошуку застарів'");
	...
	Дело.Подсказка      = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Последнее обновление %1 назад';uk='Останнє оновлення %1 назад'"), Интервал);
...
Процедура ОбновитьИндекс(ПредставлениеПроцедуры, РазрешитьСлияние = Ложь, Порциями = Ложь)
	...
	ЗаписьЖурнала(Неопределено, НСтр("ru='Запуск процедуры ""%1"".';uk='Запуск процедури ""%1"".'"), , ПредставлениеПроцедуры);
	
	Попытка
		ПолнотекстовыйПоиск.ОбновитьИндекс(РазрешитьСлияние, Порциями);
		ЗаписьЖурнала(Неопределено, НСтр("ru='Успешное завершение процедуры ""%1"".';uk='Успішне завершення процедури ""%1"".'"), , ПредставлениеПроцедуры);
	Исключение
		ЗаписьЖурнала(Неопределено, НСтр("ru='Ошибка выполнения процедуры ""%1"":';uk='Помилка виконання процедури ""%1"":'"), ИнформацияОбОшибке(), ПредставлениеПроцедуры);
	КонецПопытки;
...
Процедура ЗаписьЖурнала(УровеньЖурнала, КомментарийСПараметрами, ИнформацияОбОшибке = Неопределено, Параметр = Неопределено)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Полнотекстовое индексирование';uk='Повнотекстове індексування'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		УровеньЖурнала, , , 
		ТекстДляЖурнала);
