...
Процедура ПриДобавленииСлужебныхСобытий(КлиентскиеСобытия, СерверныеСобытия) Экспорт
	...
	//
	СерверныеСобытия.Добавить("ТехнологияСервиса.РаботаВМоделиСервиса.ОбменДаннымиВМоделиСервиса\ПриСозданииАвтономногоРабочегоМеста");
...
Функция ПродолжитьНастройкуАвтономногоРабочегоМеста(Параметры) Экспорт
	...
	Если Не НеобходимоВыполнитьНастройкуАвтономногоРабочегоМестаПриПервомЗапуске() Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ФлагНастройкиВопросаОДолгойСинхронизации(ЗначениеФлага = Неопределено, ОписаниеНастройки = Неопределено) Экспорт
	...
	ОписаниеНастройки.Вставить("Представление", НСтр("ru='Показывать предупреждение о длительной синхронизации';uk='Показувати попередження про тривалу синхронізацію'"));
...
Функция ИмяФайлаПакетаУстановки() Экспорт
	...
	Возврат НСтр("ru='Автономная работа.zip';uk='Автономна робота.zip'");
...
Функция МониторАвтономныхРабочихМест() Экспорт
	...
	Для Каждого НастройкаСинхронизации Из НастройкиСинхронизации Цикл
		...
		Иначе
			НастройкаСинхронизации.ПредставлениеДатыСинхронизации = НСтр("ru='не выполнялась';uk='не виконувалася'");
		КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииСозданиеАвтономногоРабочегоМеста() Экспорт
	...
	Возврат НСтр("ru='Автономная работа.Создание автономного рабочего места';uk='Автономна робота.Створення автономного робочого місця'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдалениеАвтономногоРабочегоМеста() Экспорт
	...
	Возврат НСтр("ru='Автономная работа.Удаление автономного рабочего места';uk='Автономна робота.Вилучення автономного робочого місця'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция НаименованиеАвтономногоРабочегоМестаПоУмолчанию()
	...
	Результат = НСтр("ru='Автономная работа - %1';uk='Автономна робота - %1'");
...
Функция ДопустимыеСимволыПрефиксаАвтономногоРабочегоМеста()
	...
	Возврат НСтр("ru='АБВГДЕЖЗИКЛМНОПРСТУФХЦЧШЭЮЯабвгдежзиклмнопрстуфхцчшэюя';uk='АБВГДЕЖЗИКЛМНОПРСТУФХЦЧШЭЮЯабвгдежзиклмнопрстуфхцчшэюя'"); // 54 символа
...
Процедура СинхронизироватьДанныеСПриложениемВИнтернете() Экспорт
	...
	Если Не ЭтоАвтономноеРабочееМесто() Тогда
		...
		ПодробноеПредставлениеОшибкиДляЖурналаРегистрации =
			НСтр("ru='Эта информационная база не является автономным рабочим местом. Синхронизация данных отменена.';uk='Ця інформаційна база не є автономним робочим місцем. Синхронізація даних скасована.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())
		;
		ПодробноеПредставлениеОшибки =
			НСтр("ru='Эта информационная база не является автономным рабочим местом. Синхронизация данных отменена.';uk='Ця інформаційна база не є автономним робочим місцем. Синхронізація даних скасована.'")
	...
	Если Отказ Тогда
		ВызватьИсключение НСтр("ru='В процессе синхронизации данных с приложением в Интернете возникли ошибки (см. журнал регистрации).';uk='У процесі синхронізації даних з прикладною програмою в Інтернеті виникли помилки (див. журнал реєстрації).'");
	КонецЕсли;
...
Процедура ВыполнитьНастройкуАвтономногоРабочегоМестаПриПервомЗапуске() Экспорт
	...
	Если Не ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		ВызватьИсключение НСтр("ru='Первый запуск автономного рабочего места должен выполняться
                                ...
                                |;uk='Перший запуск автономного робочого місця повинен виконуватися
                                |у файловому режимі роботи інформаційної бази.'");
	КонецЕсли;
...
Процедура ЗагрузитьПараметрыИзНачальногоОбраза()
	...
	Исключение
        ...
        |;uk='Можливо, інформаційна база відкрита в режимі конфігуратора.
        |Завершіть роботу конфігуратора і повторіть запуск програми.'");
	КонецПопытки;
	...
	Попытка
		...
		Если Пользователь = Неопределено Тогда
			ВызватьИсключение НСтр("ru='Идентификация пользователя не выполнена.
                ...
                |;uk='Ідентифікація користувача не виконана.
                |Можливо, довідник користувачів не включений до складу плану обміну автономної роботи.'");
		КонецЕсли;
		...
		Пользователь.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
			НСтр("ru='Начальная настройка автономного рабочего места.';uk='Початкова настройка автономного робочого місця.'"));
...
Функция ПолучитьПараметрыИзНачальногоОбраза()
	...
	Если ПустаяСтрока(СтрокаXML) Тогда
		ВызватьИсключение НСтр("ru='В автономное рабочее место не были переданы настройки.
                                    ...
                                    |;uk='В автономне робоче місце не були передані настройки.
                                    |Робота з автономним робочим місцем неможлива.'");
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииСинхронизацияДанных()
	...
	Возврат НСтр("ru='Автономная работа.Синхронизация данных';uk='Автономна робота.Синхронізація даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
