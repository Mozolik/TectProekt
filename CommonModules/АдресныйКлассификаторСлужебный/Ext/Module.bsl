...
Процедура ЗаполнитьВерсиюПоставщикаДанныхВнутр(Результат)
	...
	Результат.Данные = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='БСП %1';uk='БСП %1'"), СтандартныеПодсистемыСервер.ВерсияБиблиотеки());
...
Функция АдресаДляИнтерактивногоВыбора(Родитель, Уровень, ДополнительныеПараметры) Экспорт
	...
	Если Уровень = 1 Тогда
		Результат.Заголовок = НСтр("ru='Выберите регион';uk='Виберіть регіон'");
	ИначеЕсли Уровень = 2 Тогда
		Результат.Заголовок = НСтр("ru='Выберите округ';uk='Оберіть округ'");
	...
	ИначеЕсли Уровень = 5 Тогда
		Результат.Заголовок = НСтр("ru='Выберите внутригородской район';uk='Виберіть внутрішньоміський район'");
	...
	ИначеЕсли Уровень = 90 Тогда
		Результат.Заголовок = НСтр("ru='Выберите дополнительный элемент';uk='Виберіть додатковий елемент'");
	ИначеЕсли Уровень = 91 Тогда
		Результат.Заголовок = НСтр("ru='Выберите подчиненный элемент';uk='Виберіть підпорядкований елемент'");
	ИначеЕсли Уровень = -1 Тогда
		Результат.Заголовок = НСтр("ru='Выберите ориентир';uk='Оберіть орієнтир'");
	КонецЕсли;
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес, Уровни)
	...
	Если ПустаяСтрока(СубъектРФ.Наименование) Тогда
		// Нет субъекта
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("ru='Не указан субъект РФ адреса';uk='Не зазначений суб''єкт РФ адреси'") );
		Возврат Результат;
	ИначеЕсли Уровни.Найти(1) = Неопределено Тогда
		// В уровнях должно быть указано всегда.
		ВызватьИсключение НСтр("ru='В уровнях для проверки адреса не указан уровень субъекта РФ.';uk='У рівнях для перевірки адреси не вказано рівень суб''єкта РФ.'");
	КонецЕсли;
	...
	Если РегионЗагруженВАдресныеСведения = Неопределено Тогда 
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Регион ""%1"" не существует';uk='Регіон ""%1"" не існує'"), Адрес.СубъектРФ));
		Результат.АдресПроверен = Истина;
		Возврат Результат;
	ИначеЕсли НЕ РегионЗагруженВАдресныеСведения Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", НСтр("ru='Отсутствуют адресные сведения по региону';uk='Відсутні адресні відомості по регіону'") + " " + Адрес.СубъектРФ);
	...
	Если РезультатЗапроса.Пустой() Тогда
		// Нет региона
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Субъект РФ (регион) ""%1"" не найден в адресном классификаторе';uk='Суб''єкт РФ (регіон) ""%1"" не знайдений в адресному класифікаторі'"), СубъектРФ.Наименование + " " + СубъектРФ.Сокращение));
		Возврат Результат;
	...
	Для Каждого Запись Из ЗаписиДомов Цикл;
		...
		Если ПроверятьОкруг И Не Запись.ОкругНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Округ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Округ ""%1"" отсутствует в адресном классификаторе';uk='Округ ""%1"" відсутен у класифікаторі адресному'"), Округ.Наименование + " " + Округ.Сокращение));
		КонецЕсли;
		...
		Если ПроверятьРайон И Не Запись.РайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СвРайМО/Район", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Район ""%1"" отсутствует в адресном классификаторе';uk='Район ""%1"" відсутен у адресному класифікаторі'"), Район.Наименование + " " + Район.Сокращение));
		КонецЕсли;
		
		Если ПроверятьГород И Не Запись.ГородНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Город", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Город ""%1"" отсутствует в адресном классификаторе';uk='Місто ""%1"" відсутнє у класифікаторі адресному'"), Город.Наименование + " " + Город.Сокращение));
		КонецЕсли;
		...
		Если ПроверятьВнутригородскойРайон И Не Запись.ВнутригородскойРайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "ВнутригРайон", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Внутригородской район ""%1"" отсутствует в адресном классификаторе';uk='Внутрішньоміський район ""%1"" відсутня у класифікаторі адресному'"),
		...
		Если ПроверятьНаселенныйПункт И Не Запись.НаселенныйПунктНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "НаселПункт", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Населенный пункт ""%1"" отсутствует в адресном классификаторе';uk='Населений пункт ""%1"" відсутен у адресному класифікаторі'"),
		...
		Если ПроверятьДополнительный И Не Запись.ДополнительныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Дополнительный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Дополнительный элемент ""%1""отсутствует в адресном классификаторе';uk='Додатковий елемент ""%1""відсутій у адресному класифікаторі '"),
		...
		Если ПроверятьПодчиненный И Не Запись.ПодчиненныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Подчиненный элемент ""%1"" отсутствует в адресном классификаторе';uk='Підлеглий елемент ""%1"" відсутен у класифікаторі адресному'"),
		...
		Если ИсторияОбъекта.Количество() > 0 Тогда 
			Для каждого СтрокаОбъект Из ИсторияОбъекта Цикл
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Улица ""%1"" была переименована в ""%2""';uk='Вулиця ""%1"" була перейменована в ""%2""'"), СтрокаОбъект.Значение, СтрокаОбъект.Представление));
			КонецЦикла;
		Иначе
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Улица ""%1"" отсутствует в адресном классификаторе';uk='Вулиця ""%1"" відсутня у класифікаторі адресному'"), Улица.Наименование + " " + Улица.Сокращение));
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ ИндексНайден Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Индекс", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Индекс ""%1"" в адресном классификаторе не соответствует введенному адресу';uk='Індекс ""%1"" в адресному класифікаторі не відповідає введеної адресі'"), Индекс));
		Возврат Результат;
	КонецЕсли;
	...
	Если ЕстьЗаписиДомов И Не ЕстьВарианты Тогда
		...
		Если ЗначениеЗаполнено(ОписаниеЗданияПоСоответствиюФИАС) Тогда
			...
			Если НеточныеСовпадения.Количество() = 1 Тогда
				Подсказка = ", " + НСтр("ru='но есть';uk='але є'") + """" + НеточныеСовпадения[0]+ """";
			ИначеЕсли НеточныеСовпадения.Количество() > 1 Тогда
				...
				Если НеточныеСовпадения.Количество() = 2 Тогда
					Подсказка = НСтр("ru='но есть ""%1"" или ""%2""';uk='але є ""%1"" або ""%2""'");
					ВтороеЗначение =  НеточныеСовпадения[1];
				Иначе
					Подсказка = НСтр("ru='но есть ""%1"" и другие варианты(%2)';uk='але є ""%1"" і інші варіанти(%2)'");
			...
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='""%1"" отсутствует в адресном классификаторе';uk='""%1"" відсутня у класифікаторі адресному'"), ОписаниеЗданияПоСоответствиюФИАС(ЗданияИПомещения)) + Подсказка);
		КонецЕсли;
...
Функция СформироватьОписаниеЗдания(НазваниеВладения, НомерВладения, НомерКорпуса, НазваниеСтроения, НомерСтроения, ДополнительныеПараметры = Неопределено)
	...
	Если ЗначениеЗаполнено(НомерКорпуса) Тогда
		КорпусИСтроение = ", "+ НСтр("ru='корпус';uk='корпус'") + " " + НомерКорпуса;
	КонецЕсли;
...
Функция ТипыОбъектовАдресацииАдресаРФ()
	...
	Результат.Добавить(СтрокаОбъектаАдресации("2060", НСтр("ru='Этаж';uk='Поверх'"),   3, 6));
...
Функция ИдентификаторДома(СтрокаСоСпискомДомов, СведенияОДоме)
	...
	Пока Позиция <= ЧислоСтрок Цикл
		...
		Если Строка = "H" Тогда
			...
			ИначеЕсли НЕ ЗначениеЗаполнено(ОписаниеДома.Дом) ИЛИ ОписаниеДома.Дом = НСтр("ru='НЕТ';uk='НІ'") Тогда
...
Процедура ЗагрузитьКлассификаторАдресов(КодыСубъектовРФ, ОписаниеФайлов, ОповещатьОПрогрессе = Истина) Экспорт
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		Если ОповещатьОПрогрессе Тогда
			ДлительныеОперации.СообщитьПрогресс(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка региона ""%1 - %2"" (осталось %3) ...';uk='Завантаження регіону ""%1 - %2"" (залишилося %3) ...'"),
...
Процедура УдалитьВременныйФайл(ПолноеИмяФайла) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
            ...
            |%1: %2'"), ПолноеИмяФайла, КраткоеПредставлениеОшибки(ИнформацияОбОшибке())));
	КонецПопытки
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Описание = НСтр("ru='Загрузка обновлений адресного классификатора.';uk='Завантаження оновлень адресного класифікатора.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
			...
			Дело.Представление  = НСтр("ru='Адресный классификатор устарел';uk='Адресний класифікатор застарів'");
			Дело.Количество     = 0;
			Дело.Подсказка      = НСтр("ru='Автоподбор и проверка корректности адресов временно недоступны.';uk='Автоматичний вибір та перевірка коректності адрес тимчасово недоступні.'");
	...
	Подсказка = Подсказка + Символы.ПС + НСтр("ru='Необходимо проверить наличие обновлений.';uk='Необхідно перевірити наявність оновлень.'");
	Для Каждого Раздел Из Разделы Цикл
		Дело = ТекущиеДела.Добавить();
...
Процедура ПеренестиДанныеУстаревшегоКлассификатор(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Актуализация сведений о регионе ""%1 - %2"" (осталось %3) ...';uk='Актуалізація відомостей про регіоні ""%1 - %2"" (залишилося %3) ...'"),
			СубъектРФ, АдресныйКлассификатор.НаименованиеРегионаПоКоду(СубъектРФ),
			Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=")));
...
Функция ОписаниеДиапазонаКЛАДР(Описание, ТаблицыДанных)
	...
	Если ВариантыТипов.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка поиска типа диапазона для %1';uk='Помилка пошуку діапазону типу %1'"), Описание);
	КонецЕсли;
...
Функция ПредставлениеРегиона(Наименование, Сокращение)
	Если Сокращение = НСтр("ru='Респ';uk='Респ'") Тогда
		Возврат НСтр("ru='Республика';uk='Республіка'") + " " + Наименование;
	ИначеЕсли Сокращение = НСтр("ru='обл';uk='обл'") Тогда
		Возврат Наименование + " " + НСтр("ru='область';uk='область'");
	ИначеЕсли Сокращение = НСтр("ru='г';uk='г'") Тогда
		Возврат НСтр("ru='г.';uk='р.'") + " " + Наименование;
	КонецЕсли;
...
Процедура ДобавитьИнформациюОДоме(ВариантыДомов, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска = "")
	...
	Если ЗначениеЗаполнено(ОписаниеДома.Корпус) Тогда
		ТекстПроДом = ТекстПроДом + ", "+ НСтр("ru='корпус';uk='корпус'") + " " + ОписаниеДома.Корпус;
...
Функция НаименованиеВладенийИСтроений()
	...
	Результат.Владения.Вставить(0, НСтр("ru='Дом';uk='Будинок'"));
	Результат.Строения.Вставить(0, НСтр("ru='Строение';uk='Будівля'"));
		...
		Если Не ЗагруженныйФайл.Статус Тогда
			...
			ВызватьИсключение ЗагруженныйФайл.СообщениеОбОшибке + Символы.ПС
				+ НСтр("ru='Возможные причины:
                    ...
                    |;uk='Можливі причини:
                    |• Введено некоректно або не введений логін і пароль;
                    |• Немає підключення до Інтернету;
                    |• На веб-вузлі виникли неполадки;
                    |• Брандмауер або інше проміжне (антивіруси і т. п.) блокують спроби програми підключитися до Інтернету;
                    |• Підключення до Інтернету через проксі-сервер, але його параметри не зазначені в програмі.'");
		КонецЕсли;
...
Процедура ОчиститьКлассификаторАдресов(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Очистка региона ""%1 - %2"" (осталось %3) ...';uk='Очищення регіону ""%1 - %2"" (залишилося %3) ...'"),
			СубъектРФ, АдресныйКлассификатор.НаименованиеРегионаПоКоду(СубъектРФ),
			Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=")));
...
Функция РазрешенияБезопасностиОбновления() Экспорт
	...
	Разрешения.Добавить( РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		ВРег(Адрес.Схема), Адрес.Хост, Адрес.Порт, НСтр("ru='Проверка обновления адресного классификатора.';uk='Перевірка оновлення адресного класифікатора.'")));
	...
	Разрешения.Добавить( РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		ВРег(Адрес.Схема), Адрес.Хост, Адрес.Порт, НСтр("ru='Загрузка данных адресного классификатора.';uk='Завантаження даних адресного класифікатора.'")));
...
Функция ОписаниеПоследнейЗагрузки()
	...
	Если ДатаПоследнейЗагрузки = '00000000' Тогда
		...
		Результат.Представление        = НСтр("ru='Адресный классификатор еще не загружался.';uk='Адресний класифікатор ще не завантажувався.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ДнейРазницы = 0 Тогда
		Представление = НСтр("ru='Адресный классификатор был загружен сегодня.';uk='Адресний класифікатор був завантажений сьогодні.'");
		
	ИначеЕсли ДнейРазницы = 1 Тогда
		Представление = НСтр("ru='Адресный классификатор был загружен вчера.';uk='Адресний класифікатор був завантажений вчора.'");
			
	ИначеЕсли ДнейРазницы = 2 Тогда
		Представление = НСтр("ru='Адресный классификатор был загружен позавчера.';uk='Адресний класифікатор був завантажений позавчора.'");
			
	Иначе
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Адресный классификатор был загружен %1 назад.';uk='Адресний класифікатор був завантажений %1 назад.'"),
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	Если ТипXDTOЗаписи = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В файле ""%1"" не найден список записей в типе  %2';uk='У файлі ""%1"" не знайдено список записів типу %2'"), Результат.ПолноеИмя, Результат.ТипXDTO);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В файле ""%1"" не найден корневой узел %2';uk='У файлі ""%1"" не знайдено кореневий вузол %2'"), Результат.ПолноеИмя, ИмяОписывающегоУзла);
	КонецЕсли;
