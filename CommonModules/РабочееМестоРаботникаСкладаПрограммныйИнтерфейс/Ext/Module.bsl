...
Функция ТаблицаЗаданий(Склад, Помещение, ТипЗадания, Исполнитель) Экспорт
	...
	ИначеЕсли ТипЗадания = "Приемка" Тогда
		...
		|	ВЫБОР
		|		КОГДА ПриходныйОрдерНаТовары.Ответственный <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
		|			ТОГДА 0
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК НомерИсполнителя,
		|	ПриходныйОрдерНаТовары.Ответственный КАК Исполнитель,
		|	ЕСТЬNULL(ПриходныйОрдерНаТовары.Ответственный.Представление, """") КАК НаименованиеИсполнителя
		|ИЗ
		|	Документ.ПриходныйОрдерНаТовары КАК ПриходныйОрдерНаТовары
		|ГДЕ
		|	ПриходныйОрдерНаТовары.Проведен
		...
		|	И ПриходныйОрдерНаТовары.Склад = &Склад
		|	И ПриходныйОрдерНаТовары.Помещение = &Помещение
		|	И ПриходныйОрдерНаТовары.Ответственный В(&Исполнитель)
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерИсполнителя";
...
Функция ПараметрыЗаданияПоШтрихкоду(Штрихкод, Склад, Помещение, Пользователь, ТипЗадания = "",
	...
	Если МассивСсылок.Количество() = 0 Тогда
		...
		ПараметрыЗадания.ТекстСообщения = НСтр("ru='Не удалось найти складское задание по штрихкоду.';uk='Не вдалося знайти складське завдання за штрихкодом.'");
...
Функция ПараметрыЗаданияПриПринятииВРаботу(Задание, Пользователь, ПараметрыЗадания = Неопределено) Экспорт
	...
	Исключение
		...
		ПараметрыЗадания.ТекстСообщения = НСтр("ru='Не удалось взять складское задание в работу. 
            ...
            |;uk='Не вдалося взяти складське завдання в роботу. 
            |Відбулася помилка при спробі блокування завдання.'");
	...
	Если ПараметрыЗадания.КодВозврата = 0 Тогда
		...
		Если ТипЗадания = "Пересчет" Тогда
			ТекстСообщения = НСтр("ru='При взятии складского задания в работу произошла ошибка. 
                ...
                |;uk='При взятті складського завдання в роботу сталася помилка. 
                |Швидше за все, у завданні присутні вже заблоковані комірки, комірки, за якими ведеться відбір/розміщення.
                |Спробуйте взяти завдання в роботу пізніше або зверніться до адміністратора.'");
		Иначе
			ТекстСообщения = НСтр("ru='При взятии складского задания в работу произошла ошибка. 
...
//		* Ячейка - СправочникСсылка.Ячейка,
//		* КодЯчейки - Строка,
//		* ТипЯчейки - ПеречислениеСсылка.ТипыСкладскихЯчеек,
...
Функция НайтиТоварИлиУпаковочныйЛист(Штрихкод, ИскатьУпаковочныеЛисты) Экспорт
	...
	Если ТоварНеНайден(Штрихкод, КешированныеЗначения.Штрихкоды) Тогда
		...
		ПараметрыТовара.ТекстСообщения = "Неизвестный штрихкод";
	...
	Если Не ЗначениеЗаполнено(ДанныеПоШтрихкодуТовара.Упаковка) Тогда
		...
		Если ДанныеПоШтрихкодуТовара.НесколькоЕдиничныхУпаковок Тогда
			...
			ПараметрыТовара.ТекстСообщения = "У товара не указана упаковка.";
...
//			ОстаткиТовара - ТаблицаЗначений - данные об остатках по товаров в ячейках на складе.
//
		...
		Если ЭтоСканирование Тогда
			...
			Если ЗначениеЗаполнено(СрокГодностиСерии) Тогда
				...
				Если Не СрокГодностиСерииКорректный(ПараметрыУказанияСерии, СрокГодностиСерии, СрокГодности) Тогда
					...
					ПараметрыСерии.ТекстСообщения = НСтр("ru='Ошибка: отсканирована не та серия.';uk='Помилка: відсканована не та серія.'");
...
Функция Назнчения(Склад, Распоряжение, Ячейка, Товар, ТоварыДокумента, ПараметрыРежима, ДляАвтоподбора = Ложь) Экспорт
	...
	Иначе
		ПересчитатьОстаткиПоНазначениям(Результат, Товар);
	КонецЕсли;
...
Функция ОбработатьОтсканированныеТоварыОтбораПеремещения(Задание, ТоварыДляСканирования, ТЗОтсканированныеТовары) Экспорт
	...
	ОбщегоНазначения.ЗаполнитьМассивУникальнымиЗначениями(МассивТоваров, МассивТоваровЗадания);
	...
	Для каждого СтрокаТоварыДляСканирования Из ТоварыДляСканирования Цикл
		...
		Если СерииУказываютсяПоФактуОтбора(СтрокаТоварыДляСканирования.СтатусУказанияСерий)
			...
			Если СтрокаТоварыОтбор.КоличествоУпаковок = СтрокаТоварыДляСканирования.КоличествоУпаковокОтсканировано Тогда
...
Функция ЗавершитьВыполнениеЗаданияНовоеРазмещение(Склад, Помещение, ЗонаПриемки, Пользователь, ОтсканированныеТовары,
	...
	Исключение
		...
		ПараметрыЗадания.ТекстСообщения = НСтр("ru='При завершении выполнения подбора товаров произошла ошибка. Не удалось 
            ...
            |;uk='При завершенні виконання підбору товарів сталася помилка. Не вдалося 
            |виконати розміщення товарів. Зверніться до адміністратора.'");
		
		// Добавить запись об ошибке в журнал регистрации.
		КомментарийЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При создании нового задания на размещение произошла ошибка. Описание ошибки: ""%1""';uk='При створенні нового завдання на розміщення сталася помилка. Опис помилки: ""%1""'"), 
			ИнформацияОбОшибке().Описание);
		
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Ошибка при создании нового задания на размещение.';uk='Помилка при створенні нового завдання на розміщення.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		...
		Если РазмещениеВыполнено Тогда
			...
			Исключение
				...
				ПараметрыЗадания.ТекстСообщения = НСтр("ru='При завершении выполнения подбора товаров произошла ошибка. 
                    ...
                    |;uk='При завершенні виконання підбору товарів сталася помилка. 
                    |Не вдалося виконати розміщення товарів. Зверніться до адміністратора.'");
			КонецПопытки;
			
		Иначе
			ПараметрыЗадания.КодВозврата = 0;
			ПараметрыЗадания.ТекстСообщения = НСтр("ru='Не удалось разместить товары по ячейкам.';uk='Не вдалося розмістити товари по комірках.'");
		КонецЕсли;
...
Функция ЗавершитьВыполнениеСкладскогоЗадания(Задание, ИсходныйИсполнитель, ТЗОтсканированныеТовары, ТоварыДляСканирования,
	...
	Если ТипЗадания = "Отбор" Тогда
		...
		Отбор = Новый Структура("Количество", 0);
		МассивПустыхСтрок = ПараметрыЗадания.ТоварыОтбор.НайтиСтроки(Отбор);
		...
		Если Не (ПараметрыЗадания.ТоварыОтбор.Итог("КоличествоУпаковокОтобрано") = ПараметрыЗадания.ТоварыОтбор.Итог("КоличествоУпаковок")) Тогда
			ДокументОбъект.Статус = Перечисления.СтатусыОтборовРазмещенийТоваров.ВыполненоСОшибками;
		Иначе
	...
	Если ТипЗадания = "Перемещение" Тогда
		...
		Если Не ОтборОтсканирован
			И ТоварыДляСканирования.Итог("КоличествоУпаковокОтсканировано") = 0 Тогда
			
			ОтменитьВыполнениеЗадания(Задание, ИсходныйИсполнитель, ТипЗадания, ПараметрыЗадания);
			Возврат ПараметрыЗадания;
		КонецЕсли;
		...
		Если Не ОтборОтсканирован Тогда
			...
			Отбор = Новый Структура("Количество", 0);
			МассивПустыхСтрок = ПараметрыЗадания.ТоварыРазмещение.НайтиСтроки(Отбор);
		...
		Иначе
			...
			Иначе
				...
				Для каждого СтрокаСканирования Из ТоварыДляСканирования Цикл
					...
					Если СерииУказываютсяПоФактуОтбора(СтрокаСканирования.СтатусУказанияСерий)
						...
						Если СтрокаТоварыРазмещение.КоличествоУпаковок = СтрокаСканирования.КоличествоУпаковокОтсканировано Тогда
			...
			ЕстьНеОтсканированныеТовары = (ПараметрыЗадания.ТоварыОтбор.Итог("КоличествоУпаковок") <> 
					ПараметрыЗадания.ТоварыОтбор.Итог("КоличествоУпаковокОтобрано"))
				Или (ПараметрыЗадания.ТоварыРазмещение.Итог("КоличествоУпаковок") <>
					ПараметрыЗадания.ТоварыРазмещение.Итог("КоличествоУпаковокРазмещено"));
	...
	Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(ДокументОбъект, "ДатаОкончанияВыполнения") Тогда
		ДокументОбъект.ДатаОкончанияВыполнения = ТекущаяДата();
	КонецЕсли;
	...
	Исключение
		...
		КомментарийЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При записи и проведении складского задания произошла ошибка. Описание ошибки: ""%1""';uk='При запису та проведенні складського завдання сталася помилка. Опис помилки: ""%1""'"),
			ИнформацияОбОшибке().Описание);
		
		ПараметрыЗадания.ТекстСообщения = КомментарийЖурнала;
		
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Ошибка при записи и проведении складского задания из рабочего места работника склада';uk='Помилка при запису та проведенні складського завдання з робочого місця працівника складу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Задание.Метаданные(),
			Задание,
			КомментарийЖурнала,
...
//	Помещение - СправочникСсылка.СкладскиеПомещения - помещение склада,
//	Задание - ДокументСсылка - ссылка складского задания,
//	ТЗОтсканированныеТовары - ТаблицаЗначений - таблица товаров, содержащая информацию об отсканированных товарах,
...
Функция ЗавершитьВыполнениеЗаданияПроверкаОтбора(Склад, Помещение, Задание, ОтсканированныеТовары, ТоварыДляСканирования,
	...
	ВсеХорошо = ОбработкаПроверки.ЗавершитьПроверку(УчитываемыеПоля);
	...
	Если ВсеХорошо
		...
		ВсеХорошо = ОбработкаПроверки.ОтразитьРасхожденияВУчете(УчитываемыеПоля);
		
	КонецЕсли;
	
	Если Не ВсеХорошо Тогда
		
		ПараметрыЗадания.КодВозврата = 1;
		ПараметрыЗадания.ТекстСообщения = НСтр("ru='При завершении задания произошла ошибка. Не удалось провести ордер. 
            |Обратитесь к администратору.'
            |;uk='При завершенні завдання сталася помилка. Не вдалося провести ордер. 
            |Зверніться до адміністратора.'");
		
		ИмяСобытия = НСтр("ru='Ошибка при записи и проведении складского ордера из рабочего места работника склада';uk='Помилка при запису та проведенні складського ордера з робочого місця працівника складу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Иначе
		...
		Если ЕстьНеотгружаемые Тогда
			...
			ПараметрыЗадания.Вставить("НеотгружаемыеТовары", ОбработкаПроверки.Неотгружаемые);
		Иначе
			ПараметрыЗадания.ТекстСообщения = НСтр("ru='Выполнение проверки отбора завершено.';uk='Виконання перевірки відбору завершено.'");
		КонецЕсли;
...
Функция ПринятиеСкладскогоЗаданияВРаботуДоступно(Склад, Помещение, Пользователь, ПараметрыЗадания)
	...
	Если Не ПараметрыЗадания.Проведен Тогда
		ПараметрыЗадания.КодВозврата = 0;
		ПараметрыЗадания.ТекстСообщения = НСтр("ru='Документ не проведен.';uk='Документ не проведено.'");
	...
	Если ПараметрыЗадания.Статус <> ИсходныйСтатусСкладскогоЗадания(ПараметрыЗадания.ТипЗадания) Тогда
		ПараметрыЗадания.КодВозврата = 0;
		ПараметрыЗадания.ТекстСообщения = НСтр("ru='Неверный статус документа.';uk='Невірний статус документа.'");
	...
	Если Не (ПараметрыЗадания.Склад = Склад)
		...
		ПараметрыЗадания.ТекстСообщения = НСтр("ru='Склад и помещение в документе не соответствуют настройкам 
            |пользователя.'
            |;uk='Склад і приміщення в документі не відповідають настройкам 
            |користувача.'");
	...
	Если ЗначениеЗаполнено(Исполнитель) Тогда
		Если Не (Исполнитель = Пользователь) Тогда
			ПараметрыЗадания.КодВозврата = 0;
			ПараметрыЗадания.ТекстСообщения = НСтр("ru='Складское задание должен выполнять другой работник склада.';uk='Складське завдання повинен виконувати інший працівник складу.'");
...
//
Функция ПорядокОбработкиТоваровСкладскогоЗадания(Задание, ТипЗадания) Экспорт
...
Процедура УстановитьТоварыЗадания(Задание, ПараметрыЗадания)
	...
	Если Не ЗначениеЗаполнено(ТипЗадания) Тогда
		...
		ПараметрыЗадания.ТекстСообщения = НСтр("ru='Не удалось взять складское задание в работу. 
            ...
            |;uk='Не вдалося взяти складське завдання в роботу. 
            |Сталася помилка при отриманні товарів завдання. Не вдалося отримати тип завдання.'");
	...
	Если ДокументОбъект = Неопределено Тогда
		...
		ПараметрыЗадания.ТекстСообщения = НСтр("ru='Не удалось взять складское задание в работу. 
            ...
            |;uk='Не вдалося взяти складське завдання в роботу. 
            |Сталася помилка при отриманні товарів завдання. Не вдалося отримати об''єкт завдання.'");
	...
	Если ТипЗадания = "Отбор" Тогда
		...
		Если ДокументОбъект.ТоварыОтбор.Количество() = 0 Тогда
			...
			ПараметрыЗадания.ТекстСообщения = НСтр("ru='Не удалось взять складское задание в работу. 
                ...
                |;uk='Не вдалося взяти складське завдання в роботу. 
                |Сталася помилка при отриманні товарів завдання. Не заповнена таблична частина ""Товари 
                |(відбір)"" складського завдання.'");
	...
	ИначеЕсли ТипЗадания = "Размещение" Тогда
		...
		Если ДокументОбъект.ТоварыРазмещение.Количество() = 0 Тогда
			...
			ПараметрыЗадания.ТекстСообщения = НСтр("ru='Не удалось взять складское задание в работу. 
                ...
                |;uk='Не вдалося взяти складське завдання в роботу. 
                |Сталася помилка при отриманні товарів завдання. Не заповнена таблична частина ""Товари 
                |(розміщення)"" складського завдання.'");
	...
	ИначеЕсли ТипЗадания = "Пересчет" Тогда
		...
		Если ДокументОбъект.Товары.Количество() = 0 Тогда
			...
			ПараметрыЗадания.ТекстСообщения = НСтр("ru='Не удалось взять складское задание в работу. 
                ...
                |;uk='Не вдалося взяти складське завдання в роботу. 
                |Сталася помилка при отриманні товарів завдання. Не заповнена таблична частина складського завдання.'");
...
Процедура УстановитьСтатусЗаданияВРаботе(Задание, ПараметрыЗадания, Пользователь) Экспорт
	...
	Если ДокументОбъект = Неопределено Тогда
		...
		КомментарийЖурнала = НСтр("ru='Не удалось взять складское задание в работу. Произошла ошибка при установке статуса 
            ...
            |;uk='Не вдалося взяти складське завдання в роботу. Сталася помилка при встановленні статусу 
            |завдання. Опис помилки: Не вдалося отримати об''єкт завдання.'");
		
		ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка при постановке складского задания в работу.';uk='Помилка при постановці складського завдання в роботу.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Задание.Метаданные(),
			Задание,
			КомментарийЖурнала,
	...
	Исключение
		...
		КомментарийЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |Описание ошибки: ""%1"".'
            |;uk='Не вдалося взяти складське завдання в роботу. Сталася помилка при встановленні статусу завдання. 
            |Опис помилки: ""%1"".'"),
			ОписаниеОшибки);
...
//
Процедура ПересчитатьОстаткиПоНазначениям(Назначения, Товар)
...
Процедура УстановитьИсходныйСтатусСкладскогоЗадания(Задание, ТипЗадания, ПараметрыЗадания)
	...
	Исключение
		...
		ПараметрыЗадания.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При установке исходного статуса складского задания произошла ошибка. Описание ошибки: ""%1""';uk='При встановленні вихідного статусу складського завдання сталася помилка. Опис помилки: ""%1""'"),
			ИнформацияОбОшибке().Описание);
		
		// Добавить запись об ошибке в журнал регистрации.
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Ошибка при установке исходного статуса складского задания.';uk='Помилка при встановленні вихідного статусу складського завдання.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Задание.Метаданные(),
			Задание,
...
Процедура УстановитьИсходногоИсполнителяСкладскогоЗадания(Задание, ИсходныйИсполнитель, ПараметрыЗадания)
	...
	Исключение
		ПараметрыЗадания.КодВозврата = 0;
		ПараметрыЗадания.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При установке исходного исполнителя складского задания произошла ошибка. Описание ошибки: ""%1""';uk='При встановленні вихідного виконавця складського завдання сталася помилка. Опис помилки: ""%1""'"),
			ИнформацияОбОшибке().Описание);
		
		// Добавить запись об ошибке в журнал регистрации.
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Ошибка при установке исходного исполнителя складского задания.';uk='Помилка при встановленні вихідного виконавця складського завдання.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Задание.Метаданные(),
			Задание,
			ПараметрыЗадания.ТекстСообщения,
...
Процедура УстановитьЗначениеБлокировкиЯчеекВЗаданииНаПересчет(Задание, ПараметрыЗадания)
	...
	Исключение
		ПараметрыЗадания.КодВозврата = 0;
		ПараметрыЗадания.ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При снятии блокировок ячеек произошла ошибка. Описание ошибки: ""%1""';uk='При знятті блокування комірок сталася помилка. Опис помилки: ""%1""'"),
			ИнформацияОбОшибке().Описание);
		
		// Добавить запись об ошибке в журнал регистрации.
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Ошибка при снятии блокировок ячеек.';uk='Помилка при знятті блокування комірок.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			Задание.Метаданные(),
			Задание,
			ПараметрыЗадания.ТекстСообщения,
...
Функция ОчередьТоваровДляСканирования(Склад, Помещение, ТЗТоварыДляСканирования, ТекЯчейка) Экспорт
	...
	ОбщегоНазначения.ЗаполнитьМассивУникальнымиЗначениями(Товары, НоменклатураТЗ);
