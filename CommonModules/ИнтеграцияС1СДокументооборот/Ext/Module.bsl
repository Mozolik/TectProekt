...
Функция ВыполнитьЗапрос(Прокси, Запрос) Экспорт
	...
	Исключение
		...
		Если Найти(КраткоеПредставлениеОшибки, НСтр("ru='Аутентификация пользователя не выполнена';uk='Аутентифікація користувача не виконана'")) <> 0 Тогда
			...
			КраткоеПредставлениеОшибки = НСтр("ru='Неверное имя пользователя или пароль. Проверьте имя
                |и пароль в настройках авторизации в Документообороте.'
                |;uk='Невірне ім''я користувача або пароль. Перевірте ім''я
                |і пароль в настройках авторизації у Документообігу.'");
...
// Устанавливает параметры сеанса работы с подсистемой "Интеграция с Документооборотом".
//
	...
	Если ИмяПараметра = "ИнтеграцияС1СДокументооборотВерсияСервиса" Тогда
		...
		Иначе
			...
			Иначе // получим версию сервиса асинхронно, в фоновом задании
				ЗаписьЖурналаРегистрации(НСтр("ru='Интеграция с Документооборотом';uk='Інтеграція з Документообігом'"), 
					УровеньЖурналаРегистрации.Информация,,,НСтр("ru='Начато получение версии сервиса';uk='Розпочато отримання версії сервісу'"));
...
//
Функция ПолучитьВерсиюВызовомСервиса(ИмяПользователя, Пароль)
	
	Прокси = ПолучитьПрокси(Ложь, ИмяПользователя, Пароль);
	Если Прокси = Неопределено Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Интеграция с Документооборотом';uk='Інтеграція з Документообігом'"), 
			УровеньЖурналаРегистрации.Ошибка,,,НСтр("ru='Сервис недоступен';uk='Сервіс недоступний'"));
		Возврат "0.0.0.0";
	Иначе
		...
		Иначе
			ЗаписьЖурналаРегистрации(НСтр("ru='Интеграция с Документооборотом';uk='Інтеграція з Документообігом'"), 
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Подключен сервис версии %1';uk='Підключений сервіс версії %1'"), Ответ.versionNumber));
...
//
// Параметры:
//   АдресВХранилище - Строка - адрес во временном хранилище для помещения версии сервиса.
...
//
Процедура УстановитьВерсиюСервиса(АдресВХранилище, ИмяПользователя, Пароль) Экспорт
	
	ВерсияСервиса = ПолучитьВерсиюВызовомСервиса(ИмяПользователя, Пароль);
	...
	Иначе
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Интеграция с Документооборотом';uk='Інтеграція з Документообігом'"), 
			УровеньЖурналаРегистрации.Информация,,,НСтр("ru='Завершено получение версии сервиса';uk='Завершено отримання версії сервісу'"));
	КонецЕсли;
...
Процедура ПроверитьВозвратВебСервиса(Прокси, Ответ) Экспорт
	...
	Если ПроверитьТип(Прокси, Ответ, "DMError") Тогда 
		...
		КонецЦикла;
		ПозицияТекстаПоПричине = Найти(ТекстОшибки, Символы.ПС + НСтр("ru='по причине:';uk='через:'"));
...
Функция НовоеИсходящееПисьмо(Прокси, Предмет = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Эта версия Документооборота не поддерживает создание исходящих писем';uk='Ця версія Документообігу не підтримує створення вихідних листів'");
	КонецЕсли;
...
Функция ПолучитьПрокси(ВызыватьИсключение = Истина, ИмяПользователя = Неопределено, Пароль = Неопределено) Экспорт
	...
	Если Определения = Неопределено Тогда
		...
		Если ВызыватьИсключение Тогда
			ВызватьИсключение НСтр("ru='Ошибка подключения к Документообороту.
                |
                |Возможно, не прошла авторизация, указан неверный адрес веб-сервиса 
                |или база Документооборота не опубликована на веб-сервере.
                |
                |Подробности в журнале регистрации. Обратитесь к администратору системы.'
                |;uk='Помилка підключення до Документообігу.
                |
                |Можливо, не пройшла авторизація, вказано неправильну адресу веб-сервісу 
                |або база Документообігу не опублікована на веб-сервері.
                |
                |Подробиці в журналі реєстрації. Зверніться до адміністратора системи.'");
		Иначе
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
...
//      * id - идентификатор объекта по данным Документооборота;
//      * type - тип объекта по данным Документооборота.
//
// Возвращаемое значение:
...
// Устарела. Рекомендуется использовать ЗначениеРеквизитаПоОбъектуXDTO.
// 
// Параметры:
//   ОбъектXDTO - ОбъектXDTO - исходный объект, соответствие которому нужно найти.
//   ТипОбъекта - Тип - тип искомого объекта ИС.
...
//
Функция СсылкаПоОбъектуXDTO(ОбъектXDTO, ТипОбъекта, ЭтоДополнительноеСвойство = Ложь, Свойство = Неопределено) Экспорт
	
	Результат = Неопределено;
	
	ИнтеграцияС1СДокументооборотПереопределяемый.СсылкаПоОбъектуXDTO(Результат, ОбъектXDTO, ТипОбъекта);
	...
	КонецЕсли;
	Если Результат = Неопределено Тогда
		Результат = РегистрыСведений.ИнтегрированныеОбъекты.СсылкаНаОбъектПоДаннымДокументооборота(
			ОбъектXDTO.objectId.id, ОбъектXDTO.objectId.type, ТипОбъекта);
	КонецЕсли;
	...
	Если Результат = Неопределено Тогда
		...
		Если ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта) Тогда
			...
			Иначе // продолжим поиск по другим реквизитам
				...
				КонецЕсли;
				Если Не ЗначениеЗаполнено(Результат) и ПравоДоступа("Добавление", МетаданныеОбъекта) Тогда
					...
					Если ЭтоДополнительноеСвойство
						И (ТипОбъекта = Тип("СправочникСсылка.ЗначенияСвойствОбъектов")
						 Или ТипОбъекта = Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) Тогда
						НовыйОбъект.Владелец = Свойство;
					КонецЕсли;
					ИнтеграцияС1СДокументооборотПереопределяемый.ПриСозданииПоСсылке(НовыйОбъект, ОбъектXDTO);
					НовыйОбъект.Записать();
					Результат = НовыйОбъект.Ссылка;
					РегистрыСведений.ИнтегрированныеОбъекты.ДобавитьСвязь(
...
//   ЛюбаяСсылка - ссылка на найденный или созданный объект.
//
Функция ЗначениеРеквизитаПоОбъектуXDTO(ОбъектXDTO, ТипРеквизита, ИмяРеквизита, ТипОбъекта,
	...
	Если Результат = Неопределено Тогда
		Результат = РегистрыСведений.ИнтегрированныеОбъекты.СсылкаНаОбъектПоДаннымДокументооборота(
			ОбъектXDTO.objectId.id, ОбъектXDTO.objectId.type, ТипРеквизита);
	КонецЕсли;
...
//
Функция ПолучитьXDTOИзмененийИзОбъекта(ДанныеОбъекта) Экспорт
...
Процедура ИнтеграцияС1СДокументооборотПередЗаписьюДокумента(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	УстановитьСсылкуНовогоПередЗаписьюОбъекта(Источник, "Документ");
...
Процедура ИнтеграцияС1СДокументооборотПередЗаписьюСправочника(Источник, Отказ) Экспорт
	...
	УстановитьСсылкуНовогоПередЗаписьюОбъекта(Источник, "Справочник");
...
Функция СокращенноеНаименованиеКонфигурации() Экспорт
	...
	СокращенноеНаименование = НСтр("ru='БИД (демо)';uk='БІД (демо)'");
...
Функция СоздатьОбъектДОПоПравилу(Прокси, ОбъектИС, Правило) Экспорт
	...
	Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.0.8.1") Тогда
		...
		РеквизитыПравила = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			Правило, "Шаблон, ШаблонID, ШаблонТип");
...
Процедура ДобавитьКомандуПереключитьХронометраж(Форма, Знач МестоРазмещенияКоманд)
	...
	Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
		...
		КомандаФормы.Заголовок = НСтр("ru='Переключить хронометраж';uk='Переключити хронометраж'");
		КомандаФормы.Подсказка = НСтр("ru='Переключить хронометраж';uk='Переключити хронометраж'");
...
Процедура ДобавитьКомандуУказатьТрудозатраты(Форма, Знач МестоРазмещенияКоманд)
	...
	Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
		КомандаФормы = Форма.Команды.Добавить(ИмяКоманды);
		КомандаФормы.Действие = "Подключаемый_ВыполнитьКомандуИнтеграции";
		КомандаФормы.Заголовок = НСтр("ru='Указать трудозатраты';uk='Вказати трудовитрати'");
		КомандаФормы.Подсказка = НСтр("ru='Указать трудозатраты';uk='Вказати трудовитрати'");
		КомандаФормы.ИзменяетСохраняемыеДанные = Ложь;
		КомандаФормы.Отображение = ОтображениеКнопки.Авто;
...
//
Функция СтруктураРеквизитовЗаполняемогоОбъектаДО(ТипОбъектаДО)
	...
	Если ТипОбъектаДО = "DMCorrespondent" Тогда
		...
		СтруктураРеквизитов.Вставить("КодПоЕДРПОУ");
	...
	Если ТипОбъектаДО = "DMInternalDocument" Тогда 
		...
		РеквизитыСсылочногоТипа.Вставить("Корреспондент");
...
Функция ВерсияНеМеньше(ОписаниеВерсииСервиса, ТребуемаяВерсия)
	...
	НомерНеМеньше = Ложь;
	Попытка
		Если Число(ОписаниеВерсииСервиса[0]) > Число(ОписаниеТребуемойВерсии[0]) Тогда
			НомерНеМеньше = Истина;
		ИначеЕсли Число(ОписаниеВерсииСервиса[0]) = Число(ОписаниеТребуемойВерсии[0]) Тогда
			Если Число(ОписаниеВерсииСервиса[1]) > Число(ОписаниеТребуемойВерсии[1]) Тогда
				НомерНеМеньше = Истина;
			ИначеЕсли Число(ОписаниеВерсииСервиса[1]) = Число(ОписаниеТребуемойВерсии[1]) Тогда
				Если Число(ОписаниеВерсииСервиса[2]) > Число(ОписаниеТребуемойВерсии[2]) Тогда
					НомерНеМеньше = Истина;
				ИначеЕсли Число(ОписаниеВерсииСервиса[2]) = Число(ОписаниеТребуемойВерсии[2]) Тогда
					Если Число(ОписаниеВерсииСервиса[3]) >= Число(ОписаниеТребуемойВерсии[3]) Тогда
						НомерНеМеньше = Истина;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	Исключение // номер версии содержит не числа, считаем, что требуемый функционал отсутствует
		Возврат Ложь;
	КонецПопытки;
...
//
Процедура УстановитьСсылкуНовогоПередЗаписьюОбъекта(Объект, ТипОбъекта)
	...
	ИдентификаторОбъектаДокументооборота = 
		Объект.ДополнительныеСвойства.ИдентификаторОбъектаДокументооборота;
	УникальныйИдентификатор = Новый УникальныйИдентификатор(ИдентификаторОбъектаДокументооборота);
	ИмяВидаДокумента = Объект.Метаданные().Имя;
	СсылкаПоИдентификатору = Документы[ИмяВидаДокумента].ПолучитьСсылку(УникальныйИдентификатор);
	Запрос = Новый Запрос(
		"ВЫБРАТЬ ПЕРВЫЕ 1 
		|	Ссылка
		|ИЗ 
		|	" + ТипОбъекта + "." + ИмяВидаДокумента + "
		|ГДЕ
		|	Ссылка = &СсылкаПоИдентификатору");
	Запрос.УстановитьПараметр("СсылкаПоИдентификатору", СсылкаПоИдентификатору);
	УстановитьПривилегированныйРежим(Истина);
