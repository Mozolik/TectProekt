...
Функция ПолучитьДвоичныеДанныеФайла(Знач ПрисоединенныйФайл) Экспорт
	...
	Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		...
		ПредставлениеВладельцаФайла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Присоединен к %1 : %2';uk='Приєднаний до %1 : %2'"),
...
Функция ПолучитьДанныеФайла(Знач ПрисоединенныйФайл,
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ФайлОбъект <> Неопределено, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найден присоединенный файл ""%1"" (%2)';uk='Не знайдено приєднаний файл ""%1"" (%2)'"),
...
Функция ДобавитьПрисоединенныйФайл(ПараметрыФайла,
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка при добавлении присоединенного файла.';uk='Помилка при додаванні приєднаного файлу.'");
	...
	Если НоваяСсылкаНаФайл = Неопределено Тогда
		ИмяСправочника = ПрисоединенныеФайлыСлужебный.ИмяСправочникаХраненияФайлов(ВладелецФайлов, "", ЗаголовокОшибки,
			НСтр("ru='В этом случае параметр ""НоваяСсылкаНаФайл"" должен быть указан.';uk='В цьому випадку параметр ""НоваяСсылкаНаФайл"" повинен бути вказаний.'"));
	...
	Иначе
		Если Не Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(НоваяСсылкаНаФайл))
			...
			ВызватьИсключение НСтр("ru='Ошибка при добавлении присоединенного файла.
                ...
                |;uk='Помилка при додаванні приєднаного файлу.
                |Посилання на новий файл не заповнене.'");
		КонецЕсли;
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ru='Ошибка при добавлении присоединенного файла ""%1"":
                                     |%2'
                                     |;uk='Помилка при додаванні приєднаного файлу ""%1"":
                                     |%2'");
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Файлы.Добавление присоединенного файла';uk='Файли.Додавання приєднаного файлу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
...
Функция НоваяСсылкаНаФайл(ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка при получении новой ссылки на присоединенный файл.';uk='Помилка при отриманні нового посилання на приєднаний файл.'");
...
Функция ПолучитьИмяФормыОбъектаПрисоединенныхФайловПоВладельцу(Знач ВладелецФайлов) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка при получении имени формы присоединенного файла.';uk='Помилка при отриманні імені форми приєднаного файлу.'");
	ОкончаниеОшибки = НСтр("ru='В этом случае получение формы невозможно.';uk='У цьому випадку одержання форми неможливо.'");
...
Процедура СконвертироватьФайлыВПрисоединенные(Знач ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	ЗаголовокОшибки = НСтр("ru='Ошибка при конвертации присоединенных файлов подсистемы Работа с файлами
                                 ...
                                 |;uk='Помилка при конвертації приєднаних файлів підсистеми Робота з файлами
                                 |у приєднані файли підсистеми Приєднані файли.'");
	...
	Если Не Источник.ЭтоНовый() Тогда
		...
		Если ИзмененаПометкаУдаления И ЗначениеЗаполнено(Источник.Редактирует) Тогда
			...
			Если Источник.Редактирует = Пользователи.АвторизованныйПользователь() Тогда
				...
				ТекстОшибки = НСтр("ru='Действие недоступно, так как файл ""%1"" занят для редактирования.';uk='Дія недоступна, так як файл ""%1"" зайнятий для редагування.'");
			...
			Иначе
				ТекстОшибки = НСтр("ru='Действие недоступно, так как файл ""%1"" занят для редактирования
                    |пользователем %2.'
                    |;uk='Дія недоступна, так як файл ""%1"" зайнятий для редагування
                    |користувачем %2.'");
...
Процедура ПометитьНаУдалениеПрисоединенныеФайлы(Знач Источник, ИмяСправочника = Неопределено)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru='Ошибка при пометке на удаление присоединенных файлов.';uk='Помилка при відмітці для вилучення приєднаних файлів.'")
			+ Символы.ПС
			+ ПредставлениеОшибки;
	КонецПопытки;
			...
			Если Источник.ПометкаУдаления И ЗначениеЗаполнено(Выборка.Редактирует) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='""%1"" не может быть удален,
                               ...
                               |;uk='""%1"" не може бути вилучений,
                               |тому що містить приєднаний файл ""%2"",
