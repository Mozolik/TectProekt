...
Функция ОтправкаПакетовЭД(Знач МассивПакетовЭД, Знач СоотвСоглашенийИПараметровСертификатов, ТекстСообщения = "") Экспорт
	...
	Для Каждого ПакетЭД Из МассивПакетовЭД Цикл
		...
		Исключение
			...
			ШаблонВидаОперации = НСтр("ru='отправка пакета ЭД по настройке: %1, способ обмена %2';uk='відправка пакету ЕД по налаштуванню: %1, спосіб обміну %2'");
...
Функция ПередатьПисьмоСВложениями(ПакетЭД, СоответствиеВложений)
	...
	Исключение
		...
		ВидОперации = НСтр("ru='Отправка эл.документов';uk='Відправка ел.документів'");
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
							ВидОперации,
							ТекстОшибки,
							ТекстСообщения);
