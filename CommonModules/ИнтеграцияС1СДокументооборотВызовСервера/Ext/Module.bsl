...
//Возвращает:
//	Строка - Имя типа XDTO объекта Документооборота
//
...
//Возвращает:
//	Строку представления типа объекта Документооборота
Функция ПредставлениеТипа(ОбъектТип)

	ПредставлениеТипа = "";
	
	Если ОбъектТип = "DMFile" Тогда
		ПредставлениеТипа = НСтр("ru='Файл';uk='Файл'");
		
	ИначеЕсли ОбъектТип = "DMInternalDocument" Тогда
		ПредставлениеТипа =  НСтр("ru='Внутренний документ';uk='Внутрішній документ'");
		
	ИначеЕсли ОбъектТип = "DMIncomingDocument" Тогда
		ПредставлениеТипа =  НСтр("ru='Входящий документ';uk='Вхідний документ'");
		
	ИначеЕсли ОбъектТип = "DMOutgoingDocument" Тогда
		ПредставлениеТипа =  НСтр("ru='Исходящий документ';uk='Вихідний документ'");
...
//Добавляет связ объекта с объектом Документооборота.
//Параметры:
//	ID - идентификатор объекта Документооборота
//	Тип - тип XDTO объекта Документооборота
//	ИнтегрированныйОбъектт - ссылка на объект системы
//
Процедура ДобавитьСвязь(ID, Тип, ИнтегрированныйОбъект) Экспорт

	НачатьТранзакцию();
	
	Попытка
	
		РегистрыСведений.ИнтегрированныеОбъекты.ДобавитьСвязь(ID, Тип, ИнтегрированныйОбъект);
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Не удалось добавить связь';uk='Не вдалося додати зв''язок'"), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура УдалитьСвязь(ID, Тип, ИнтегрированныйОбъект) Экспорт
	...
	Попытка
		...
		РегистрыСведений.ИнтегрированныеОбъекты.УдалитьСвязь(ID, Тип, ИнтегрированныйОбъект);
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Не удалось удалить связь';uk='Не вдалося вилучити зв''язок'"), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура СохранитьНастройкиАвторизации(Знач ИмяПользователя, Знач Пароль,
	Знач ИмяКомпьютера = Неопределено, Знач ЧастьПароляВИБ = "", Знач ВременныйФайлЧастиПароля = "") Экспорт
	...
	Если ИмяКомпьютера = Неопределено Тогда
		...
		Запись.ХешИмениКомпьютера = 0;
	Иначе
		Запись.ПарольСохранен = Истина;
		Запись.ХешИмениКомпьютера = ХешСуммаCRC32(ИмяКомпьютера);
		Запись.ЧастьПароляВИнформационнойБазе = ЧастьПароляВИБ;
		Запись.ВременныйФайлЧастиПароля = ВременныйФайлЧастиПароля;
	КонецЕсли;
	Запись.Записать();
	
	УстановитьНастройкиАвторизацииВПараметрыСеанса(ИмяПользователя, Пароль);
...
// Получает имя пользователя ДО и пароль из ИБ (в веб-клиенте - только пароль).
//
//   ИмяПользователя - Строка - имя пользователя ДО.
...
//   ВременныйФайлЧастиПароля - Строка - путь ко временному файлу с другой частью пароля.
//
Процедура ПрочитатьНастройкиАвторизации(ИмяПользователя, Знач ИмяКомпьютера = Неопределено, ПарольСохранен = Ложь,
		ЧастьПароляВИБ = "", ВременныйФайлЧастиПароля = "") Экспорт
	...
	// Выберем имя пользователя и данные для восстановления пароля, или хотя бы имя пользователя.
	Запрос = Новый Запрос("
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
		|	ПарольСохранен,
		|	ИмяПользователя,
		|	ЧастьПароляВИнформационнойБазе,
		|	ВременныйФайлЧастиПароля,
		|	ВЫБОР
		...
		|	Приоритет
		|");
	Если ИмяКомпьютера = Неопределено Тогда // веб-клиент, восстановим хотя бы имя
...
Функция ЭтоПользовательЗаданияОбмена() Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	ПользовательИБ = ПользователиИнформационнойБазы.ТекущийПользователь();
	ЗаданиеОбмена = РегламентныеЗадания.НайтиПредопределенное(
...
//   Пароль - Строка - пароль пользователя ДО.
//
Процедура УстановитьНастройкиАвторизацииВПараметрыСеанса(Знач ИмяПользователя, Знач Пароль) Экспорт
...
//                 - Неопределено - признак необходимости получить версию сервиса.
//
...
//
// Параметры:
//   АдресСервиса - Строка - адрес веб-сервиса ДО.
//   ИмяПользователя - Строка - имя пользователя ДО.
//   Пароль - Строка - пароль пользователя ДО.
...
//
Функция ПроверитьПодключение(Знач АдресСервиса, Знач ИмяПользователя, Знач Пароль, ТекстСообщенияОбОшибке = "") Экспорт

	Если Не ЗначениеЗаполнено(АдресСервиса) Тогда
		ТекстСообщенияОбОшибке = НСтр("ru='Не указан адрес веб-сервиса Документооборота.';uk='Не зазначена адреса веб-сервісу Документообігу.'");
		Возврат Ложь;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ИмяПользователя) Тогда
		ТекстСообщенияОбОшибке = НСтр("ru='Не указано имя пользователя Документооборота.';uk='Не вказано ім''я користувача Документообігу.'");
		Возврат Ложь;
	КонецЕсли;
	...
	// Window, если это имеет смысл.
	ЭтоСоединениеSSL = СтрНачинаетсяС(МестоположениеWSDL, "https");
	Если ЭтоСоединениеSSL Тогда
		Если ОбщегоНазначенияПовтИсп.ТипПлатформыСервера() = ТипПлатформы.Windows_x86
			Или ОбщегоНазначенияПовтИсп.ТипПлатформыСервера() = ТипПлатформы.Windows_x86_64 Тогда
	...
	Попытка
		Определения = Новый WSОпределения(
			...
			Пароль,,
			Таймаут,
			ЗащищенноеСоединение); // ws/dm.1cws?wsdl
	Исключение
	...
	Если Определения = Неопределено Тогда
		Попытка
			Определения = Новый WSОпределения(
				МестоположениеWSDL + "ws/DMService?wsdl", 
				ИмяПользователя,
				Пароль,,
				Таймаут,
				ЗащищенноеСоединение); // ws/DMService?wsdl
		Исключение
			Определения = Неопределено;
...
// Реализует вызов переданной функции асинхронно, в фоновом задании.
//
// Параметры:
//   Адрес - Строка - адрес для результата выполнения во временном хранилище.
//   Имя - Строка - имя вызываемой функции.
//   Параметры - Произвольный - передаваемые параметры.
...
//     ТипОбъектаИС - Строка - тип объекта ИО.
//
	...
	Пока Не Достаточно Цикл
		...
		Пока ВыборкаПравила.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(ОбъектИС)
				...
				Выполнить(ВыборкаПравила.УсловиеПрименимостиПриВыгрузке);
			...
			Если ТипЗнч(ОбъектXDTO) = Тип("ОбъектXDTO")
				...
				УстановитьБезопасныйРежим(Истина);
				Выполнить(ВыборкаПравила.УсловиеПрименимостиПриЗагрузке);
				УстановитьБезопасныйРежим(Ложь);
...
// Переключает хронометраж и возвращает параметры хронометража по объектам Документооброта.
//
// Параметры:
...
Процедура ДанныеДляАвтоПодбора(ТипыЗначений, ДанныеВыбора, Текст, СтандартнаяОбработка) Экспорт
	...
	Для Каждого ТипXDTO Из МассивТипов Цикл
		...
		НайденныеЗначения = ОбъектыДокументооборотаПоНаименованию(ТипXDTO, Текст);
	...
	Если Результат = Неопределено Тогда
		...
		Если Правила.Количество() = 0 Тогда
			Результат = НСтр("ru='Необходимо настроить
                ...
                |;uk='Необхідно настроїти
                |правила інтеграції. Зверніться до адміністратора.'");
...
//      - РеквизитID - идентификатор шаблона Документооборота.
...
Функция ВыполнитьСогласование(Знач Задачи, Знач Результат, Знач Комментарий, ТекстСообщенияОбОшибке) Экспорт
	...
	Для каждого ИдентификаторЗадачи из Задачи Цикл
		...
		Если Результат = "Согласовано" Тогда
			Ответ.name = НСтр("ru='Согласовано';uk='Погоджено'");
		ИначеЕсли Результат = "СогласованоСЗамечаниями" Тогда
			Ответ.name = НСтр("ru='Согласовано с замечаниями';uk='Погоджено із зауваженнями'");
		ИначеЕсли Результат = "НеСогласовано" Тогда
			Ответ.name = НСтр("ru='Не согласовано';uk='Не погоджено'");
		КонецЕсли;
...
//
Процедура ПриИзмененииСостоянияСогласования(ПредметСогласования = Неопределено, Знач Идентификатор, Знач Тип, 
		Знач Состояние, Знач ВызовИзФормыОбъекта, Знач Установил = Неопределено, Знач ДатаУстановки = Неопределено) Экспорт
	...
	Если Состояние <> Неопределено Тогда // Неопределено - удаление
		...
		Если Установил = Неопределено Тогда
			Запись.Установил = Строка(ПараметрыСеанса.ТекущийПользователь);
		Иначе
...
Функция ПолучитьСостояниеСогласования(Знач ПредметСогласования, Пояснение = "") Экспорт
	...
	Запрос = Новый Запрос(
		...
		|ГДЕ 
		|	ИнтегрированныеОбъекты.Объект = &ПредметСогласования
		|");
...
Функция СтруктураИсполненияЗадачи(Знач ТипЗадачи, Знач ИдентификаторЗадачи, Знач ТипПроцесса, 
	...
	Если ТипПроцесса = "DMBusinessProcessApproval" Тогда
		Если ТочкаМаршрута = "Согласовать" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Согласовано';uk='Погоджено'"));
			Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Согласовано с замечаниями';uk='Погоджено із зауваженнями'"));
			Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи);
			Результат.Вставить("ВыполнитьЗадачуТретья", НСтр("ru='Не согласовано';uk='Не погоджено'"));
		ИначеЕсли ТочкаМаршрута = "Ознакомиться" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Ознакомился';uk='Ознайомився'"));
			Если РезультатВыполнения = "Не согласовано" Тогда
				Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Повторить согласование...';uk='Повторити погодження...'"));
	...
	ИначеЕсли ТипПроцесса = "DMBusinessProcessConfirmation" Тогда
		Если ТочкаМаршрута = "Утвердить" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Утверждено';uk='Затверджено'"));
			Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Не утверждено';uk='Не затверджено'"));
			Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
		ИначеЕсли ТочкаМаршрута = "Ознакомиться" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Ознакомился';uk='Ознайомився'"));
			Если РезультатВыполнения = "Не утверждено" Тогда
				Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Повторить утверждение...';uk='Повторити затвердження...'"));
				Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
			КонецЕсли;
		КонецЕсли;
	// Рассмотрение.
	ИначеЕсли ТипПроцесса = "DMBusinessProcessConsideration" Тогда
		Если ТочкаМаршрута = "Рассмотреть" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Рассмотрено';uk='Розглянутий'"));
		ИначеЕсли ТочкаМаршрута = "Ознакомиться" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Обработано';uk='Оброблено'"));
		КонецЕсли;
	// Поручение.
	ИначеЕсли ТипПроцесса = "DMBusinessProcessOrder" Тогда
		Если ТочкаМаршрута = "Выполнить" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Выполнено';uk='Виконано'"));
		ИначеЕсли ТочкаМаршрута = "Проверить" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Завершить поручение';uk='Завершити доручення'"));
			Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Вернуть на доработку';uk='Повернути на доопрацювання'"));
			Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
		ИначеЕсли ТочкаМаршрута = "Контролировать" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Снять с контроля';uk='Зняти з контролю'"));
		КонецЕсли;
	// Исполнение.
	ИначеЕсли ТипПроцесса = "DMBusinessProcessPerformance" Тогда
		Если ТочкаМаршрута = "Исполнить" или ТочкаМаршрута = "ОтветственноеИсполнение" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Исполнено';uk='Виконано'"));
		ИначеЕсли ТочкаМаршрута = "Контролировать" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Снять с контроля';uk='Зняти з контролю'"));
		ИначеЕсли ТочкаМаршрута = "Проверить" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Проверено';uk='Перевірено'"));
		КонецЕсли;
	ИначеЕсли ТипПроцесса = "DMComplexBusinessProcess" Тогда
		Если ТочкаМаршрута = "Контролировать" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Проконтролировано';uk='Проконтрольовано'"));
		КонецЕсли;
	// Регистрация.
	ИначеЕсли ТипПроцесса = "DMBusinessProcessRegistration" Тогда
		Если ТочкаМаршрута = "Зарегистрировать" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Зарегистрировано';uk='Зареєстровано'"));
		ИначеЕсли ТочкаМаршрута = "Ознакомиться" Тогда
			Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Ознакомился';uk='Ознайомився'"));
		КонецЕсли;
	// Ознакомление.
	...
	ИначеЕсли ТипПроцесса = "DMBusinessProcessIssuesSolution" Тогда
		Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.2.7.3") Тогда
			...
			ИначеЕсли ТочкаМаршрута = "ОзнакомлениеСРезультатомРассмотрения" Тогда
				Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Закрыть вопрос';uk='Закрити питання'"));
				Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Уточнить';uk='Уточнити'"));
				Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
			КонецЕсли;
		КонецЕсли;
	// Приглашение.
	ИначеЕсли ТипПроцесса = "DMBusinessProcessInvitation" Тогда
		Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.2.7.3") Тогда
			Если ТочкаМаршрута = "Пригласить" Тогда
				Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Принять приглашение';uk='Прийняти запрошення'"));
				Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Отклонить приглашение';uk='Відхилити запрошення'"));
				Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
			ИначеЕсли ТочкаМаршрута = "Ознакомиться" Тогда
				...
				Если РезультатID = "ПринятоВсемиУчастниками" Тогда 
					Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Подтвердить приглашения';uk='Підтвердити запрошення'"));
				ИначеЕсли РезультатID = "ПринятоОбязательнымиУчастниками" Тогда 	
					Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Подтвердить приглашения';uk='Підтвердити запрошення'"));
					Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Повторить приглашение...';uk='Повторити запрошення...'"));
					Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаОтрицательногоВыполненияЗадачи);
				ИначеЕсли РезультатID = "НеПринятоОбязательнымиУчастниками"
					Или РезультатID = "НеПринятоВсемиУчастниками" Тогда 
					Результат.Вставить("ВыполнитьЗадачуПервая", НСтр("ru='Подтвердить приглашения';uk='Підтвердити запрошення'"));
					Результат.Вставить("ВыполнитьЗадачуВторая", НСтр("ru='Повторить приглашение...';uk='Повторити запрошення...'"));
					Результат.Вставить("ВыполнитьЗадачуТретья", НСтр("ru='Отменить приглашение';uk='Скасувати запрошення'"));
					Результат.Вставить("ЦветТекстаВторая", ЦветаСтиля.ОтметкаПоложительногоВыполненияЗадачи);
				КонецЕсли;
...
// Создает файлы печатных форм объектов по имени команды формы документа Документооборот и присоединяет их к документам.
//
// Параметры:
//   ВнешнийОбъект - ЛюбаяСсылка - ссылка на объект текущей базы данных.
...
//
Процедура ДобавитьСвязьДокументов(ИсходныйДокумент, СвязанныйДокумент, ТипСвязи) Экспорт
...
Процедура ОбновитьПредставлениеРезолюций(РезолюцииXDTO, ПредставлениеHTML, ЭлементФормы) Экспорт
	...
	Если РезолюцииXDTO.Количество() = 0 Тогда
		ПредставлениеHTML = "";
		ЭлементФормы.Заголовок = НСтр("ru='Резолюции';uk='Резолюції'");
		Возврат;
	КонецЕсли;
	...
	ЭлементФормы.Заголовок = СтрШаблон(
		НСтр("ru='Резолюции (%1)';uk='Резолюції (%1)'"),
...
Процедура ОбновитьВизыСогласования(ВизыXDTO, ВизыСогласования, ЭлементФормы) Экспорт
	...
	Если ВизыXDTO.Количество() = 0 Тогда
		ЭлементФормы.Заголовок = НСтр("ru='Визы';uk='Візи'");
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьСписокФайлов(ФайлыXDTO, Файлы, ЭлементДляОбновленияЗаголовка = Неопределено) Экспорт
	...
	Для каждого СведенияОФайле Из ФайлыXDTO Цикл
		...
		НоваяСтрока.ИндексКартинки = ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьИндексПиктограммыФайла(
...
Функция ПолучитьФайлИПоместитьВХранилище(ID, УникальныйИдентификатор, ДатаЗаема = Неопределено,
	...
	Иначе // получение с захватом
		Если Не ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("1.4.9.1") Тогда
			ВызватьИсключение НСтр("ru='Заем файлов не поддерживается используемой версией Документооборота';uk='Позика файлів не підтримується вашою версією Документообігу'");
		КонецЕсли;
...
//
Функция ОписаниеФайла(ID, Форма = Неопределено) Экспорт
...
Функция СделатьВерсиюАктивной(ИдентификаторФайла, ИдентификаторВерсии, ТекстСообщения = "") Экспорт
	...
	Если Не ДоступенФункционалВерсииСервиса("1.4.6.1") Тогда
		ТекстСообщения = 
			НСтр("ru='Для работы с версиями нужен Документооборот версии не ниже 1.4.6.1';uk='Для роботи з версіями потрібен Документообіг версії не нижче 1.4.6.1'");
		Возврат Ложь;
	КонецЕсли;
	...
	Если Размер > МаксРазмерФайла Тогда
		ВызватьИсключение
			   СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				 НСтр("ru='Размер файла ""%1"" (%2 Мб) превышает максимально допустимый размер файла для передачи (%3 Мб).';uk='Розмір файлу ""%1"" (%2 Мб) перевищує максимально допустимий розмір файлу для передачі (%3 Мб).'"),
...
Процедура ЗаполнитьСписокПодписейСервер(ПодписиXDTO, ФайлыXDTO, Форма, 
	...
	Если ЭлементДляИзмененияЗаголовка <> Неопределено Тогда
		ТекстЗаголовка = НСтр("ru='ЭП';uk='ЕП'");
...
Процедура ПроверитьОднуПодписьНаСервере(ДанныеСтроки, МенеджерКриптографии, УникальныйИдентификатор, 
	...
	Попытка
		...
		ДанныеСтроки.Статус = ?(ДанныеСтроки.Неверна,
			ДанныеСтроки.Статус = НСтр("ru='Не верна';uk='Не вірна'") + ": " + ОписаниеОшибки,
			НСтр("ru='Верна';uk='Вірна'"));
	Исключение
		ДанныеСтроки.Статус = НСтр("ru='Не верна: не указан менеджер криптографии';uk='Не вірна: не вказаний менеджер криптографії'");
...
// Возвращает:
//	Список объектов XDTO, соответствующих найденным объектам в Документообороте
Функция ОбъектыДокументооборотаПоНаименованию(Тип, Наименование)
