...
#КонецЕсли
	...
	Исключение
		...
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),,Истина);
		ВызватьИсключение(НСтр("ru='Ошибка при открытии файла шаблона.';uk='Помилка при відкритті файлу шаблона.'") + Символы.ПС 
...
Функция СобытиеЖурналаРегистрации()
	Возврат НСтр("ru='Печать';uk='Друк'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
КонецФункции

Процедура НеУдалосьСформироватьПечатнуюФорму(ИнформацияОбОшибке)
#Если ВебКлиент Тогда
	ТекстУточнения = НСтр("ru='При работе через веб требуется браузер Internet Explorer под управлением операционной системы Windows. См. также главу документации ""Настройка веб-браузеров для работы в веб-клиенте""';uk='При роботі через веб потрібен браузер Internet Explorer під управлінням операційної системи Windows. Див. також главу документації ""Настройка веб-браузерів для роботи у веб-клиєнті""'");
#Иначе		
	ТекстУточнения = "";	
#КонецЕсли
	ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось сформировать печатную форму: %1. 
            ...
            |Для виводу друкованих форм у форматі Microsoft Word потрібно, щоб на комп''ютері був установлений пакет Microsoft Office. %2'"),
