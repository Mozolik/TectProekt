	...
	Если ДоступноИзменениеПользователей() Тогда
		...
		Иначе
			...
			Если ЕстьПравоДобавленияПользователей() Тогда
				Возврат ДействияСНовымПользователемСервиса();
			Иначе
				ВызватьИсключение НСтр("ru='Недостаточно прав доступа для добавления новых пользователей';uk='Недостатньо прав доступу для додавання нових користувачів'");
			КонецЕсли;
...
Процедура ПередНачаломОбработкиПользователяИБ(ПользовательОбъект, ПараметрыОбработки) Экспорт
	...
	Если ТипЗнч(ПользовательОбъект) = Тип("СправочникОбъект.ВнешниеПользователи")
	   И ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		ВызватьИсключение НСтр("ru='Внешние пользователи не поддерживаются в модели сервиса.';uk='Зовнішні користувачі не підтримуються в моделі сервісу.'");
	КонецЕсли;
		...
		Если НЕ ОбщегоНазначенияПовтИсп.СеансЗапущенБезРазделителей() Тогда
			ВызватьИсключение
				НСтр("ru='Обновление пользователя по сообщению
                           ...
                           |;uk='Оновлення користувача за повідомленням
                           |каналу вилученого адміністрування
                           |доступно тільки нерозділеним користувачам.'");
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СтарыйПользователь.ИдентификаторПользователяСервиса) Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при записи пользователя ""%1"".
                           ...
                           |Не можна змінювати вже встановлений ідентифікатор
                           |користувача сервісу в елементі довідника.'"),
		...
		Если ПользователиСлужебный.ПользовательПоИдентификаторуСуществует(
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при записи пользователя ""%1"".
                           ...
                           |;uk='Помилка при записі користувача ""%1"".
                           |Не можна встановлювати ідентифікатор
                           |користувача сервісу ""%2""
                           |у цей елемент довідника, тому що він
                           |уже використається в елементі ""%3"".'"),
				ПользовательОбъект.Наименование,
				АвтоРеквизиты.ИдентификаторПользователяСервиса,
				НайденныйПользователь);
		КонецЕсли;
	...
	Если АвтоРеквизиты.ИдентификаторПользователяСервиса <> ПользовательОбъект.ИдентификаторПользователяСервиса Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |;uk='Помилка при записі користувача ""%1"".
                       |Реквізит ИдентификаторПользователяСервиса не допускається змінювати.
	...
	Для каждого ПраваДоступаОбъектаXDTO Из ПраваДоступаОбъектовXDTO.Item Цикл
		...
		ИначеЕсли ПраваДоступаОбъектаXDTO.Object.Тип() = ТипКИПользователя Тогда
			...
			Если ВидКИ = Неопределено Тогда
				ШаблонСообщения = НСтр("ru='Получен неизвестный вид контактной информации: %1';uk='Отриманий невідомий вид контактної інформації: %1'");
			...
			КонецЦикла;
		Иначе
			ШаблонСообщения = НСтр("ru='Получен неизвестный тип объектов доступа: %1';uk='Отриманий невідомий тип об''єктів доступу: %1'");
