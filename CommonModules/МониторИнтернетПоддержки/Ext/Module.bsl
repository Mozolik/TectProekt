...
Процедура ПараметрыСозданияКонтекста(Параметры, ПрерватьОбработку) Экспорт
	...
	Если НЕ ПрерватьОбработку Тогда
		// Заполнение дополнительных стартовых параметров
		// Признак возможности использования механизма автоматического обновления платформы
		Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеОбновленийПрограммы") Тогда
			...
			Если ОбщийМодуль.ДоступноИспользованиеПолученияОбновленийПлатформы() Тогда
				Параметры.ДополнительныеСтартовыеПараметры.Добавить(
					Новый Структура("Имя, Значение", "usePlatformUpdate", "true"));
				Параметры.ДополнительныеСтартовыеПараметры.Добавить(
					Новый Структура("Имя, Значение", "ClientPlatformType", ОбщийМодуль.ИмяТипПлатформы()));
				Параметры.ДополнительныеСтартовыеПараметры.Добавить(
					Новый Структура("Имя, Значение", "ClientOSVersion", ОбщийМодуль.ВерсияОС()));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриСозданииКонтекстаВзаимодействия(Контекст) Экспорт
	...
	Контекст.Вставить("СообщениеОНедоступностиДействия",
		НСтр("ru='Отображение монитора Интернет-поддержки недоступно для этой конфигурации.';uk='Відображення монітора Інтернет-підтримки недоступне для цієї конфігурації.'"));
...
//
Функция ПоказатьМониторПриНачалеРаботыСПрограммой() Экспорт
