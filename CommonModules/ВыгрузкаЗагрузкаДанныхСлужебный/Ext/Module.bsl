...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	ВыгрузкаЗагрузкаНеразделенныхДанных.ЗарегистрироватьОбработчикиОбновления(Обработчики);
...
Процедура ЗагрузитьДанныеИзКаталога(Знач КаталогЗагрузки, Знач ПараметрыЗагрузки) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.2.19.0") < 0
		...
		ВызватьИсключение
			НСтр("ru='Для выполнения загрузки данных требуется обновить технологическую платформу.
                  ...
                  |;uk='Для виконання завантаження даних потрібно оновити технологічну платформу.
                  |Для версії 8.2 необхідно використовувати реліз 8.2.19 (або новіший).
                  |Для версії 8.3 необхідно використовувати реліз 8.3.4 (або новіший).'");
	...
	Если Не ВыгрузкаВАрхивеСовместимаСТекущейКонфигурацией(ИнформацияОВыгрузке) Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Невозможно загрузить данные из файла, т.к. файл был выгружен из другой конфигурации (файл выгружен из конфигурации %1 и не может быть загружен в конфигурацию %2)';uk='Неможливо завантажити дані з файлу, оскільки файл був вивантажений з іншої конфігурації (файл вивантажений з конфігурації %1 і не може бути завантажений в конфігурацію %2)'"),
	...
	Если Не ВыгрузкаВАрхивеСовместимаСТекущейВерсиейКонфигурации(ИнформацияОВыгрузке) Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Невозможно загрузить данные из файла, т.к. файл был выгружен из другой версии конфигурации (файл выгружен из конфигурации версии %1 и не может быть загружен в конфигурацию версии %2)';uk='Неможливо завантажити дані з файлу, т. я. файл був вивантажений з іншої версії конфігурації (файл вивантажений з конфігурації версії %1 і не може бути завантажений в конфігурацію версії %2)'"),
...
// Сравнивает совместима ли версия конфигурациии с выгруженной.
//
// Параметры:
//	ИнформацияОВыгрузке - ОбъектXDTO - см. процедуру "СохранитьОписаниеВыгрузки"
//
...
// Возвращает типы файлов, которые пооддерживают замену ссылок.
//
// Возвращаемое значение:
...
Функция XMLТипСсылки(Знач Значение) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Ошибка при определении XMLТипа ссылки для объекта %1: объект не является ссылочным!';uk='Помилка при визначенні XMLТипа посилання для об''єкту %1: об''єкт не є посиланнями!'"),
...
// Возвращает префиксы для часто используемых пространст имен.
//
// Возвращаемое знечение:
//	Соответствие:
...
// Возвращает имя элемента в потоке записи/четния, в котором хранится XDTOОбъект.
//
// Возвращаемое значение:
//	Строка - имя элемента.
//
...
// Возвращает имя элемента в потоке записи/четния, в котором хранится объект.
//
// Возвращаемое значение:
//	Строка - имя элемента.
//
...
// Возвращаемое значение:
//	Соответствие:
//		Ключ - Тип - тип ссылки точки бизнец процесса.
