...
Функция УстановитьСоединениеССерверомМеханизмаОнлайнСервисовРО(ОписаниеОшибки = Неопределено) Экспорт
	...
	URLСервера = "downloads.bas-soft.eu";
...
Функция ЗапуститьФоновоеЗаданиеАктуализироватьСлужебнуюИнформациюМеханизмаОнлайнСервисовРО(УникальныйИдентификаторФормы, МожноЗагружатьВнешниеФормыОтчетов = Ложь) Экспорт
	...
	Возврат ДлительныеОперации.ЗапуститьВыполнениеВФоне(
				...
				НСтр("ru='Производится обновление служебной информации из Интернет...';uk='Проводиться оновлення службової інформації з Інтернет...'"));
	...
	Если НЕ ПустаяСтрока(СтруктураURI.ПутьНаСервере) Тогда
		...
		Если ПозицияПараметры > 0 Тогда
			...
			Если ПозицияПараметраПуть > 0 Тогда
				...
				АдресСайта = "http://downloads.bas-soft.eu/ftp//" + ПараметрПутьНаСервере;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьИЗаписатьВнешнийОтчетВСправочникРО(ПараметрыВнешнейФормы, ПутьКФайлу)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru='Не удалось распаковать архив с внешней формой отчета.';uk='Не вдалося розпакувати архів з зовнішньою формою звіту.'");
		ЗаписьЖурналаРегистрации(НСтр("ru='Загрузка внешних регламентированных отчетов';uk='Завантаження зовнішніх регламентованих звітів'"), УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Получение объекта отчета по ссылке.
            |Описание:
            |""%1""'
            |;uk='Отримання об''єкта звіту за посиланням.
            |Опис:
	...
	Если НЕ ВнешнийОтчетИспользовать Тогда 
		...
		ТекстДляЖурнала = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Внешний отчет ""%1"" вер. ""%2"" отключен.';uk='Зовнішній звіт ""%1"" вер. ""%2"" відключений.'"), СвойстваМанифеста["Имя"], СвойстваМанифеста["КраткаяВерсияВнешнегоОтчета"]);
	...
	Если СвойстваКонфигурации["ИДКонфигурации"] <> СвойстваОтчета["ИДКонфигурации"] Тогда
		ТекстОшибки = НСтр("ru='Не пройдена проверка на соответствие идентификатора конфигурации идентификатору конфигурации в текущем подключаемом отчете';uk='Не пройдена перевірка на відповідність ідентифікатора конфігурації ідентифікатору конфігурації в поточному звіті що підключається'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Если ИспользуетсяВнешнийОтчет Тогда
		Если СвойстваОтчета["ВерсияКонфигурации"] <> СвойстваКонфигурации["ВерсияКонфигурации"] Тогда
			ТекстОшибки = НСтр("ru='Не пройдена проверка на соответствие версий текущего и подключаемого отчетов';uk='Не пройдена перевірка на відповідність версій поточного і підключеного звітів'");
			ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ВРег(СвойстваОтчета["ИсточникОтчета"]) <> ВРег(СвойстваКонфигурации["ИсточникОтчета"]) Тогда
		ТекстОшибки = НСтр("ru='Не пройдена проверка на соответствие имен метаданных текущего и подключаемого отчетов';uk='Не пройдена перевірка на відповідність імен метаданих поточного і підключеного звітів'");
		ЗаписатьВЖурналРегистрации(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки);
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьВЖурналРегистрации(Уровень, ТекстОшибки)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Загрузка внешних регламентированных отчетов';uk='Завантаження зовнішніх регламентованих звітів'"), Уровень,,, ТекстОшибки);
