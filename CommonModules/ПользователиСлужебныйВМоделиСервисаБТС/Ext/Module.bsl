...
//  СтандартнаяОбработчка - Булево, значение может быть установлено внутри обработчика, в этом случае
...
Функция ПолноеИмяСлужебногоПользователя(Знач Идентификатор = Неопределено) Экспорт
	...
	Результат = НСтр("ru='<Служебный пользователь %1>';uk='<Службовий користувач %1>'");
	...
	Если ПользователиИнформационнойБазы.ТекущийПользователь().РазделениеДанных.Количество() = 0 Тогда
		...
		Если ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
			...
			Если Не ПользовательЗарегистрированКакНеразделенный(ИдентификаторПользователя) Тогда
				...
				ВызватьИсключение СтрШаблон(НСтр("ru='Пользователь с идентификатором %1 не зарегистрирован в качестве неразделенного!';uk='Користувач з ідентифікатором %1 не зареєстрований в якості нерозподіленого!'"),
...
Функция ТекстИсключенияЗаписьНеразделенныхПользователейЗапрещена()
	...
	Возврат НСтр("ru='Запись неразделенных пользователей
                  ...
                  |;uk='Запис нерозділених користувачів
                  |при включеному використанні роздільників заборонена.'");
