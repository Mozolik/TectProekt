...
Функция ПроверитьИспользованиеИнтернетПоддержкаПользователей() Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
		Попытка
			МодульИнтернетПоддержкиВызовСервера = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтернетПоддержкаПользователейВызовСервера");
			Если МодульИнтернетПоддержкиВызовСервера.ДоступноПодключениеИнтернетПоддержки() Тогда
				МодульИнтернетПоддержки = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтернетПоддержкаПользователейКлиент");
				ОшибкаОбращенияКВебСервису = Ложь;
				Использование = МодульИнтернетПоддержки.КонфигурацияЗарегистрированаВСервисеИПП(ОшибкаОбращенияКВебСервису);
...
Процедура ПолучитьСоответствиеСоглашенийИПараметровСертификатов(ОбработчикОповещения, МассивСоглашений = Неопределено, МассивЭД = Неопределено) Экспорт
...
Процедура ТестНастроекСПроверкойСертификата(
	...
	ЗаголовокФормы = НСтр("ru='Проверка сертификата %1';uk='Перевірка сертифіката %1'");
...
Процедура ОбработатьОтклонениеАннулированиеЭД(СсылкаНаЭД, ПараметрыЭД, ОтклонитьАннулирование = Ложь) Экспорт
	...
	Если ПараметрыЭД.Отклонить Тогда
		...
		Если ОтклонитьАннулирование Тогда
			Заголовок = НСтр("ru='Укажите причины отклонения предложения об аннулировании';uk='Вкажіть причини відхилення пропозиції про анулювання'");
		Иначе
			
			Заголовок = НСтр("ru='Укажите причины отклонения документа';uk='Вкажіть причини відхилення документа'");
			
		КонецЕсли;
	Иначе
		ФормироватьЭД = Истина;
		ПродолжитьОбработку = ОбменСКонтрагентамиСлужебныйВызовСервера.МожноАннулироватьЭтотЭД(СсылкаНаЭД);
		Заголовок = НСтр("ru='Укажите причины аннулирования документа';uk='Вкажіть причини анулювання документа'");
	КонецЕсли;
...
// Только для внутреннего использования
Процедура ОтправитьПодтверждениеЭД(ПараметрКоманды, ЭД = Неопределено, ДопПараметры = Неопределено) Экспорт
...
Процедура ПодготовитьИОтправитьПЭД(
	...
	Если (СоответствиеПрофилейИПараметровСертификатов = Неопределено
		...
		ПолучитьСоответствиеСоглашенийИПараметровСертификатов(ОписаниеОповещения, , ДобавленныеФайлы);
	Иначе
...
Процедура ОтправитьМассивПакетовЭД(Знач МассивПЭД, ОбработкаОповещения = Неопределено) Экспорт
	...
	ПолучитьСоответствиеСоглашенийИПараметровСертификатов(Оповещение, , МассивПЭД);
		...
		Для Каждого Элемент Из СтруктСтруктурМассивовЭДИСертификатов Цикл
			...
			Если Структура.Свойство("СоответствиеЭдИДд", СоответствиеЭдИДд)
				...
				Если СоответствиеЭдИДд.Количество() = 1 Тогда
					Операция = НСтр("ru='Подписание электронного документа';uk='Підписання електронного документа'");
				Иначе
					Операция = НСтр("ru='Подписание электронных документов';uk='Підписання електронних документів'");
				КонецЕсли;
				...
				Иначе
					ПредставлениеЭД = НСтр("ru='Электронные документы (%1)';uk='Електронні документи (%1)'");
...
Процедура ОтправитьПолучитьОбменСКонтрагентами(ТребуетсяПовторитьПолучение = Ложь) Экспорт
	...
	Если ИспользоватьЭП Тогда
		ПолучитьСоответствиеСоглашенийИПараметровСертификатов(ОбработкаОповещения);
	Иначе
...
Процедура ОтправитьПолучитьЭДВыполнить(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекстСообщения = НСтр("ru='Выполняется отправка и получение пакетов электронных документов. Подождите...';uk='Надсилання та отримання пакетів електронних документів. Почекайте...'");
	Состояние(НСтр("ru='Отправка и получение.';uk='Відправка і отримання.'"), , ТекстСообщения);
	...
	ШаблонОповещения = НСтр("ru='Отправлено пакетов: (%1), получено пакетов: (%2).';uk='Відправлено пакетів: (%1), отримано пакетів: (%2).'");
	...
	ТекстСообщения = НСтр("ru='Выполняется распаковка пакетов электронных документов. Подождите...';uk='Виконується розпакування пакетів електронних документів. Почекайте...'");
	Состояние(НСтр("ru='Распаковка.';uk='Розпакування.'"), , ТекстСообщения);
...
Процедура ОбработатьПакетыЭДЦиклНачало(Контекст) Экспорт
...
Процедура РасшифроватьЭлектронныеДокументыВнутреннийЦиклНачало(Контекст)
	...
	ОписаниеДанных = Новый Структура;
	ОписаниеДанных.Вставить("Операция",            НСтр("ru='Расшифровка электронного документа';uk='Розшифрування електронного документа'"));
...
Процедура РасшифроватьЭлектронныеДокументыВнутреннийЦиклПослеРасшифровкиЭД(РезультатВыполнения, Контекст) Экспорт
	...
	Если Не РезультатВыполнения.Успех Тогда
		Контекст.СтруктураПакетаЭД.Вставить("ОписаниеОшибки", НСтр("ru='Не удалось расшифровать электронный документ из пакета.';uk='Не вдалося розшифрувати електронний документ з пакета.'"));
		ОбработатьПакетыЭДЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
Процедура РасшифроватьЭлектронныеДокументыПослеВнутреннегоЦикла(Контекст) Экспорт
...
Процедура ПрочитатьСертификатыПодписейПослеСозданияМенеджераКриптографии(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("МенеджерКриптографии") Тогда
		Контекст.СтруктураПакетаЭД.Вставить("ОписаниеОшибки", НСтр("ru='Не удалось получить МенеджерКриптографии.';uk='Не вдалося отримати МенеджерКриптографии.'"));
		ОбработатьПакетыЭДЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ПрочитатьСертификатыПодписейВнутреннийЦиклПослеОшибкиВыгрузкиСертификата(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	ШаблонСообщения = НСтр("ru='Выгрузка сертификата: %1';uk='Вивантаження сертифіката: %1'");
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ИнформацияОбОшибке);
...
Процедура ПрочитатьСертификатыПодписейВнутреннийЦиклПослеВыгрузкиСертификата(ДанныеСертификата, Контекст) Экспорт
	...
	Контекст.Подпись.Вставить("АдресСертификата", ПоместитьВоВременноеХранилище(ДанныеСертификата));
	...
	Если НЕ ЗначениеЗаполнено(Контекст.Подпись.АдресЭД) Тогда
		Контекст.СтруктураПакетаЭД.Вставить("ОписаниеОшибки", НСтр("ru='Не удалось найти исходные данные для проверки ответной подписи.';uk='Не вдалося знайти вихідні дані для перевірки відповідної підпису.'"));
		ОбработатьПакетыЭДЦиклНачало(Контекст);
		Возврат;
	КонецЕсли;
...
// Продолжение процедуры ОбработатьПакетыЭДЦиклНачало.
Процедура ОбработатьПакетыЭДПослеЦикла(Контекст) Экспорт
	
	// Перебор массива ДанныеПакетовЭД. Отобразить ошибки и удалить ошибочные пакеты из массива.
	КолвоЭлементовКоллекции = Контекст.ДанныеПакетовЭД.Количество();
	...
	ОбменСКонтрагентамиСлужебныйВызовСервера.ЗаписатьДанныеПакетовЭД(Контекст.ДанныеПакетовЭД, МассивСлужебныхЭД, КолРаспакованныхЭД);
	...
	ШаблонОповещения = НСтр("ru='Распаковано пакетов: %1.';uk='Распаковано пакунків: %1.'");
		...
		Если ДополнительныеПараметры.Свойство("СоответствиеСоглашенийИСертификатов", СоответствиеСоглашенийИСертификатов)
			...
			Для Каждого Элемент Из СоответствиеСоглашенийИСертификатов Цикл
				...
				Если МассивСертификатов.Количество() > 0 Тогда
					...
					ОписаниеДанных.Вставить("Операция", НСтр("ru='Аутентификация в сервисе ЭДО';uk='Аутентифікація в сервісі ЕДО'"));
					ОписаниеДанных.Вставить("РазрешитьЗапоминатьПароль", Истина);
...
Процедура ОтправитьМассивПакетовОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Результат = ОбменСКонтрагентамиСлужебныйВызовСервера.ОтправкаПакетовЭД(МассивПЭД, СоотвСоглашенийИСтруктурСертификатов);
...
Процедура ОбработатьОтклонениеАннулированиеЭДЗавершить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат <> Неопределено Тогда
		ТекстСообщения = НСтр("ru='Причина не указана, действие отменено.';uk='Причина не вказана, дію скасовано.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Процедура ПослеПолученияОтпечатковСертификатов(Отпечатки, Параметры) Экспорт
	...
	ПараметрыАутентификации = Неопределено;
	СтСоответствий = ОбменСКонтрагентамиСлужебныйВызовСервера.СтруктураСоответствийСоглашенийИСертификатовАвторизации(
...
Процедура ОпределитьСтатусОчереднойПодписи(Результат, Параметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Результат) = Тип("Строка") Тогда
		ВидОперации = НСтр("ru='Проверка подписи';uk='Перевірка підпису'");
		ТекстСообщения = НСтр("ru='При проверке подписи электронного документа:
                                    |%1
                                    |произошла ошибка: %2'
                                    |;uk='При перевірці підпису електронного документа:
                                    |%1
                                    |сталася помилка: %2'");
...
Процедура ОткрытьЭДДляПросмотраПослеПолученияОтпечатков(Отпечатки, Параметры) Экспорт
	...
	ПараметрыЭД = ОбменСКонтрагентамиСлужебныйВызовСервера.ВидИВладелецЭД(СсылкаНаЭД);
	...
	Если ТипЗнч(СоответствиеДанныхПакетов) = Тип("Соответствие") Тогда
		Если СоответствиеДанныхПакетов.Количество() > Параметры.ИндексПеребораПакетов Тогда
			...
			Если Параметры.ИндексПеребораДанных + 1 < МассивДанныхЭД.Количество() Тогда
				...
				Исключение
					ВидОперации = НСтр("ru='Шифрование данных';uk='Шифрування даних'");
					ТекстСообщения = НСтр("ru='При шифровании данных произошла ошибка: %1';uk='При шифруванні даних сталася помилка: %1'");
...
// Функция определяет сертификат и сохраненным паролем из набора. Если в СоотвСертификатовИИхСтруктур передано несколько
...
//
// Возвращаемое значение:
//  Булево - Истина - если сертификат с паролем найден, иначе - Ложь.
//
Функция ЕстьСертификатССохраненнымПаролем(СоотвСертификатовИИхСтруктур, ОбъектыДляОбработки = Неопределено) Экспорт
...
Процедура ПолучитьПарольКСертификату(СоотвСертификатовИИхСтруктур, ВидОперации, Параметры, ОбъектыДляОбработки = Неопределено, ДляЗаписиВИБ = Ложь) Экспорт
	...
	Если ТипЗнч(СоотвСертификатовИИхСтруктур) = Тип("Соответствие")
		...
		Если СоотвСертификатовИИхСтруктур.Количество() = 1 Тогда
			Для Каждого Элемент Из СоотвСертификатовИИхСтруктур Цикл
				Если Элемент.Значение.ПарольПолучен = Истина Тогда
...
Процедура СформироватьЭД(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	Если ПараметрКоманды.Количество() > 1 Тогда
		...
		ТекстВопроса = НСтр("ru='Создать электронные документы для выделенных элементов?';uk='Створити електронні документи для виділених елементів?'");
...
Процедура СформироватьПодписатьОтправитьЭД(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	Если ПараметрКоманды.Количество() > 1 Тогда
		...
		ТекстВопроса = НСтр("ru='Отправить электронные документы выделенных элементов?
            ...
            |;uk='Надіслати електронні документи виділених елементів?
            |Якщо електронних документів немає, вони будуть створені автоматично.'");
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
...
// Конанда открытия формы просмотра электронных документов
...
Процедура СоздатьПроизвольныйЭДНаОсновании(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	ОткрытьФорму("Документ.ПроизвольныйЭД.ФормаОбъекта", ПараметрыФормы, ПараметрыВыполненияКоманды.Источник);
