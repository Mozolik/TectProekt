...
Процедура СоздатьКонтакт(Описание, Адрес, Основание,ТипыКонтактов) Экспорт
	...
	ТипыКонтактов.ПоказатьВыборЭлемента(ОбработчикОповещения, НСтр("ru='Выбор типа контакта';uk='Вибір типу контакту'"));
...
Процедура ВопросПриИзмененииФорматаСообщенияНаОбычныйТекст(Форма, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ТекстСообщения = НСтр("ru='При преобразовании этого сообщения в обычный текст будут утеряны все элементы оформления, картинки и прочие вставленные элементы. Продолжить?';uk='При перетворенні цього повідомлення в звичайний текст, будуть втрачені всі елементи оформлення, картинки та інші вставлені елементи. Продовжити?'");
	ПоказатьВопрос(ОбработчикОповещенияОЗакрытии, ТекстСообщения, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет, НСтр("ru='Изменение формата письма';uk='Зміна формату листа'"));
...
Процедура ПроверкаЗаполненностиРеквизитовОтложеннойОтправки(Объект, Отказ) Экспорт
	...
	Если Объект.ДатаКогдаОтправить > Объект.ДатаАктуальностиОтправки И (Не Объект.ДатаАктуальностиОтправки = Дата(1,1,1)) Тогда
		...
		Отказ = Истина;
		ТекстСообщения= НСтр("ru='Дата актуальности отправки меньше чем дата отправки.';uk='Дата актуальності відправленяя менше ніж дата відправлення.'");
	...
	Если НЕ Объект.ДатаАктуальностиОтправки = Дата(1,1,1)
		...
		Отказ = Истина;
		ТекстСообщения= НСтр("ru='Указанная дата актуальности меньше текущей даты, такое сообщение никогда не будет отправлено';uk='Зазначена дата актуальності менше поточної дати, таке повідомлення ніколи не буде відправлене'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.ДатаАктуальностиОтправки");
