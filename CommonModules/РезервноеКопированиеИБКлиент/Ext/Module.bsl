...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	Если ФиксированныеПараметрыРезервногоКопированияИБ.ПроведеноВосстановление Тогда
		ТекстОповещения = НСтр("ru='Восстановление данных проведено успешно.';uk='Відновлення даних проведене успішно.'");
		ПоказатьОповещениеПользователя(НСтр("ru='Данные восстановлены.';uk='Дані відновлені.'"), , ТекстОповещения);
	КонецЕсли;
...
Процедура ПриПолученииСпискаПредупрежденийЗавершенияРаботы(Предупреждения) Экспорт
	...
	ПараметрыПредупреждения.ТекстФлажка = НСтр("ru='Выполнить резервное копирование';uk='Виконати резервне копіювання'");
...
Процедура ПровестиРезервноеКопирование()
	...
	Кнопки.Добавить("Отложить", НСтр("ru='Отложить на 15 минут';uk='Відкласти на 15 хвилин'"));
	...
	ПоказатьВопрос(ОписаниеОповещение, НСтр("ru='Все готово для выполнения резервного копирования по расписанию.
        ...
        |;uk='Все готово для виконання резервного копіювання за розкладом.
        |Виконати резервне копіювання зараз?'"),
		Кнопки, 30, "Да", НСтр("ru='Резервное копирование по расписанию';uk='Резервне копіювання за розкладом'"), "Да");
	...
	Если ПараметрыУдаления.ТипОграничения <> "ХранитьВсе" И КаталогХранения <> Неопределено Тогда
		...
		Исключение
			...
			ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
				НСтр("ru='Не удалось провести очистку каталога с резервными копиями.';uk='Не вдалося провести очищення каталогу з резервними копіями.'") + Символы.ПС 
...
Процедура ПроверитьРезервноеКопированиеИБ(Параметры)
	...
	Иначе
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Резервное копирование';uk='Резервне копіювання'"),
			"e1cib/command/ОбщаяКоманда.ПоказатьРезультатРезервногоКопирования",
			НСтр("ru='Резервное копирование проведено успешно';uk='Резервне копіювання проведене успішно'"), БиблиотекаКартинок.Информация32);
...
Процедура ОповеститьПользователяОРезервномКопировании(ВариантОповещения)
	...
	Если ВариантОповещения = "Просрочено" Тогда
		...
		ТекстПояснения = НСтр("ru='Автоматическое резервное копирование не было выполнено.';uk='Автоматичне резервне копіювання не було виконано.'"); 
		ПоказатьОповещениеПользователя(НСтр("ru='Резервное копирование';uk='Резервне копіювання'"),
	...
	ИначеЕсли ВариантОповещения = "ЕщеНеНастроено" Тогда
		...
		ТекстПояснения = НСтр("ru='Рекомендуется настроить резервное копирование информационной базы.';uk='Рекомендується настроїти резервне копіювання інформаційної бази.'"); 
		ПоказатьОповещениеПользователя(НСтр("ru='Резервное копирование';uk='Резервне копіювання'"),
...
Функция СобытиеЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Резервное копирование информационной базы';uk='Резервне копіювання інформаційної бази'",СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиентаПриЗапуске().КодОсновногоЯзыка);
...
Функция КлиентскиеПараметрыРезервногоКопирования() Экспорт
	#Если НЕ ВебКлиент Тогда
		...
		СтруктураПараметров.Вставить("СобытиеЖурналаРегистрации", НСтр("ru='Резервное копирование ИБ';uk='Резервне копіювання ІБ'"));
