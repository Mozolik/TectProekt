...
Функция АдресаЭлектроннойПочты(Контрагент) Экспорт
	...
	Для каждого АдресПолучателя Из АдресаПолучателя Цикл
		...
		ПараметрыПолучателя.Представление 					= Контрагент.Наименование;
...
Функция УстановитьСоединениеССервером(УчетнаяЗапись, ДляПолучения = Ложь, Протокол = Неопределено) Экспорт
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru='Установка соединения с сервером электронной почты';uk='Встановлення з''єднання з сервером електронної пошти'",КодОсновногоЯзыка);
...
Функция ОписьВложенийПисьма(Вложения) Экспорт
	...
	ТекстПисьма = НСтр("ru='К письму приложены файлы:';uk='До листа додано файли:'");
	
	Если ТипЗнч(Вложения) = Тип("СписокЗначений") Тогда
		Для Каждого Вложение Из Вложения Цикл
			ТекстПисьма = ТекстПисьма + Символы.ПС + СтрШаблон(НСтр("ru='- %1';uk='- %1'"), Вложение.Представление);
		КонецЦикла;
	КонецЕсли;
	
	Если ТипЗнч(Вложения) = Тип("Структура") ИЛИ ТипЗнч(Вложения) = Тип("Соответствие") Тогда
		Для Каждого Вложение Из Вложения Цикл
			ТекстПисьма = ТекстПисьма + Символы.ПС + СтрШаблон(НСтр("ru='- %1';uk='- %1'"), Вложение.Ключ);
		КонецЦикла;
	КонецЕсли;
