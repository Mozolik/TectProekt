...
Процедура НастроитьОбменШаг1(Сообщение, Отправитель)
	...
	Попытка
		...
		Если Не ПустаяСтрока(КодЭтогоУзла)
			...
			СтрокаСообщения = НСтр("ru='Код предопределенного узла в этом приложении ""%1"" не соответствует ожидаемому ""%2"". План обмена: %3';uk='Код наперед визначеного вузла в цьому додатку ""%1"" не відповідає очікуваному ""%2"". План обміну: %3'");
...
Функция КорреспондентОбмена(Знач ИмяПланаОбмена, Знач Код)
	...
	Если Не ЗначениеЗаполнено(Результат) Тогда
		СтрокаСообщения = НСтр("ru='Не найден узел плана обмена; имя плана обмена %1; код узла %2';uk='Не знайдений вузол плану обміну; ім''я плану обміну %1; код вузла %2'");
