...
Процедура ПроверитьСписокПередОперацией(ОбработчикРезультата, СписокРеквизит)
	...
	Если Не СортировкаВСпискеУстановленаПравильно(СписокРеквизит) Тогда
		ТекстВопроса = НСтр("ru='Для изменения порядка элементов необходимо настроить сортировку списка
                                ...
                                |;uk='Для зміни порядку елементів необхідно настроїти сортування списку
                                |по полю ""Порядок"". Настроїти необхідне сортування?'");
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Настроить';uk='Настроїти'"));
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Не настраивать';uk='Не настроювати'"));
...
Процедура ПроверитьСписокПередОперациейОтветПоСортировкеПолучен(РезультатОтвета, ДополнительныеПараметры) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ПользовательскиеНастройкиПорядка <> Неопределено, НСтр("ru='Пользовательская настройка порядка не найдена.';uk='Користувацька настройка порядку не знайдена.'"));
