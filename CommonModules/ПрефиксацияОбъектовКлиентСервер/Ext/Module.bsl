...
Функция УдалитьПрефиксыИзНомераОбъекта(Знач НомерОбъекта, УдалитьПрефиксОрганизации = Ложь, УдалитьПрефиксИнформационнойБазы = Ложь) Экспорт
	...
	НомерСодержитПрефиксНалоговогоДокумента = НомерСодержитПрефиксНалоговогоДокумента(НомерОбъекта);
	
	Если НомерСодержитПятизначныйПрефикс
	 ИЛИ НомерСодержитПрефиксНалоговогоДокумента Тогда
	...
	Возврат ПрефиксОбъекта + Сред(НомерОбъекта, ?(НомерСодержитПятизначныйПрефикс, 6, ?(НомерСодержитПрефиксНалоговогоДокумента, 13, 4)));
КонецФункции
...
Функция УдалитьЛидирующиеНулиИзНомераОбъекта(Знач НомерОбъекта) Экспорт
	...
	Если НомерСодержитСтандартныйПрефикс(НомерОбъекта) Тогда
		...
		ИначеЕсли НомерСодержитПрефиксНалоговогоДокумента(НомерОбъекта) Тогда
			Префикс = Лев(НомерОбъекта, 12);
			Номер = Сред(НомерОбъекта, 13 + СтрДлина(ПользовательскийПрефикс));
		Иначе
...
Функция УдалитьПользовательскиеПрефиксыИзНомераОбъекта(Знач НомерОбъекта) Экспорт
	...
	Если НомерСодержитСтандартныйПрефикс(НомерОбъекта) Тогда
		...
		ИначеЕсли НомерСодержитПрефиксНалоговогоДокумента(НомерОбъекта) Тогда
			Префикс     = Лев(НомерОбъекта, 12);
			НомерПолный = Сред(НомерОбъекта, 13);
		Иначе
			Префикс     = Лев(НомерОбъекта, 3);
...
Функция ПолучитьПользовательскийПрефикс(Знач НомерОбъекта) Экспорт
	...
	Если НомерСодержитСтандартныйПрефикс(НомерОбъекта) Тогда
		...
		ИначеЕсли НомерСодержитПрефиксНалоговогоДокумента(НомерОбъекта) Тогда
			НомерОбъекта = Сред(НомерОбъекта, 13);
		Иначе
...
Функция НомерСодержитСтандартныйПрефикс(Знач НомерОбъекта)
	...
	Возврат (ПозицияРазделителя = 3 Или ПозицияРазделителя = 5 Или ПозицияРазделителя = 12);
...
Функция НомерСодержитПрефиксНалоговогоДокумента(Знач НомерОбъекта)
	
	Возврат СтрНайти(НомерОбъекта, "-") = 12;
