			...
			Иначе // Перечисления.ТипыХраненияФайлов.ВИнформационнойБаза
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации(
						НСтр("ru='Файлы.Не удалось отправить файл при обмене данными';uk='Файли.Не вдалося відправити файл при обміні даними'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Попытка
		...
		Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			...
			Если ФайлНаДиске.Существует() Тогда
				...
				ТекстОписания = НСтр("ru='Файл удален при очистке ненужных файлов.';uk='Файл вилучено при очищенні непотрібних файлів.'")
...
Процедура ОбновитьДвоичныеДанныеФайлаНаСервере(Знач ПрисоединенныйФайл,
	...
	Попытка
		...
		Иначе
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Файлы.Запись файла на диск';uk='Файли.Запис файлу на диск'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
	...
	Исключение
		...
		КонецЕсли;
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Файлы.Обновление данных присоединенного файла в хранилище файлов';uk='Файли.Оновлення даних приєднаного файлу в сховище файлів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Если Метаданные.Справочники.Найти(КлючИЗначение.Ключ) = Неопределено Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при определении имен справочников для хранения файлов.
                           ...
                           |;uk='Помилка при визначенні імен довідників для зберігання файлів.
                           |У власника файлів типу ""%1""
                           |вказаний неіснуючий довідник ""%2"".'"),
		...
		ИначеЕсли Не СтрЗаканчиваетсяНа(КлючИЗначение.Ключ, "ПрисоединенныеФайлы") Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при определении имен справочников для хранения файлов.
                           ...
                           |;uk='Помилка при визначенні імен довідників для зберігання файлів.
                           |У власника файлів типу ""%1""
                           |зазначено ім''я довідника ""%2""
                           |без закінчення ""ПрисоединенныеФайлы"".'"),
				Строка(ТипВладельцаФайлов),
				Строка(КлючИЗначение.Ключ));
			...
			Если ОсновнойСправочникУказан Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка при определении имен справочников для хранения файлов.
                               ...
                               |;uk='Помилка при визначенні імен довідників для зберігання файлів.
                               |У власника файлів типу ""%1""
                               |основний довідник вказано більше одного разу.'"),
					Строка(ТипВладельцаФайлов),
					Строка(КлючИЗначение.Ключ));
			КонецЕсли;
	...
	Если ИменаСправочников.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при определении имен справочников для хранения файлов.
                       ...
                       |;uk='Помилка при визначенні імен довідників для зберігання файлів.
                       |У власника файлів типу ""%1""
                       |не має довідників для зберігання файлів.'"),
			Строка(ТипВладельцаФайлов));
	КонецЕсли;
...
Функция ИмяСправочникаХраненияФайлов(ВладелецФайлов, ИмяСправочника = "",
	...
	Если ИменаСправочников.Количество() = 0 Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			+ НСтр("ru='У владельца файлов ""%1"" типа ""%2""
                         ...
                         |;uk='У власника файлів ""%1"" типу ""%2""
                         |немає довідників для зберігання файлів.'"),
	...
	Если ЗначениеЗаполнено(ИмяСправочника) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			+ НСтр("ru='У владельца файлов ""%1"" типа ""%2""
                         ...
                         |;uk='У власника файлів ""%1"" типу ""%2""
                         |немає довідника ""%3"" для зберігання файлів.'"),
			Строка(ВладелецФайлов),
			Строка(ТипЗнч(ВладелецФайлов)),
	...
	ШаблонПричиныОшибки = 
		НСтр("ru='У владельца файлов ""%1"" типа ""%2""
            ...
            |;uk='У власника файлів ""%1"" типу ""%2""
            |не вказано основний довідник для зберігання файлів.'") + Символы.ПС;
...
Функция ТекстОшибкиПриСохраненииФайлаВТоме(Знач СообщениеОбОшибке, Знач Файл)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ошибка, при сохранении файла в томе:
                   ...
                   |
                   |Ссылка на файл: ""%2"".'
                   |;uk='Помилка при збереженні файлу в томі:
                   |""%1"".
                   |
                   |Посилання на файл: ""%2"".'"),
		СообщениеОбОшибке,
