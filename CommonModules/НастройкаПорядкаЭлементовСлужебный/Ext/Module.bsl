...
Функция СоседнийЭлемент(ПеремещаемыйЭлемент, Список, Знач Направление) Экспорт
	...
	Если Не ПоляОтбораВСпискеДоступных(КомпоновщикНастроекКомпоновкиДанных.ПолучитьНастройки().Отбор.Элементы, КомпоновщикНастроекКомпоновкиДанных.ПолучитьНастройки().Отбор.ДоступныеПоляОтбора.Элементы) Тогда
		ВызватьИсключение НСтр("ru='Для перемещения элемента необходимо отключить все отборы в списке.';uk='Для переміщення елемента необхідно відключити всі відбори в списку.'");
	КонецЕсли;
...
Функция ПроверитьВозможностьПеремещения(Ссылка, Список, ОтображениеСписком)
	...
	Если Не ПараметрыДоступа.Доступность Тогда
		Возврат НСтр("ru='Недостаточно прав для изменения порядка элементов.';uk='Недостатньо прав для зміни порядку елементів.'");
	КонецЕсли;
	...
	Для Каждого ЭлементГруппировки Из Список.КомпоновщикНастроек.ПолучитьНастройки().Структура Цикл
		Если ЭлементГруппировки.Использование Тогда
			Возврат НСтр("ru='Для изменения порядка элементов необходимо отключить все группировки.';uk='Для зміни порядку елементів необхідно відключити всі групування.'");
		КонецЕсли;
	КонецЦикла;
	...
	Если Информация.ЕстьРодитель И ОтображениеСписком И Не СписокСодержитОтборПоРодителю(Список) Тогда
		Возврат НСтр("ru='Для изменения порядка элементов необходимо установить режим просмотра ""Дерево"" или ""Иерархический список"".';uk='Для зміни порядку елементів необхідно встановити режим перегляду ""Дерево"" або ""Ієрархічний список"".'");
	КонецЕсли;
	...
	Если Информация.ЕстьВладелец И Не СписокСодержитОтборПоВладельцу(Список) Тогда
		Возврат НСтр("ru='Для изменения порядка элементов необходимо установить отбор по полю ""Владелец"".';uk='Для зміни порядку елементів необхідно встановити відбір по полю ""Власник"".'");
	КонецЕсли;
	...
	Если Информация.ЕстьГруппы Тогда
		...
		Если ЭтоГруппа И Не Информация.ДляГрупп Или Не ЭтоГруппа И Не Информация.ДляЭлементов Тогда
			Возврат НСтр("ru='Выбранный элемент нельзя перемещать.';uk='Обраний елемент не можна переміщати.'");
		КонецЕсли;
	КонецЕсли;
