	...
	Для Каждого ЭлементСценария Из СценарийПримененияРазрешений Цикл
		...
		Если ЭлементСценария.Операция = ВидыОпераций.Создание Тогда
			...
			Если АдминистрированиеКластераКлиентСервер.ПрофильБезопасностиСуществует(ПараметрыАдминистрированияКластера, ЭлементСценария.Профиль) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Профиль безопасности %1 уже присутствует в кластере серверов. Настройки в профиле безопасности будут замещены...';uk='Профіль безпеки %1 вже присутній у кластері серверів. Настройки в профілі безпеки будуть заміщені...'"), ЭлементСценария.Профиль));
		...
		ИначеЕсли ЭлементСценария.Операция = ВидыОпераций.Удаление Тогда
			...
			Иначе
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Профиль безопасности %1 отсутствует в кластере серверов. Возможно, профиль безопасности был удален ранее...';uk='Профіль безпеки %1 відсутній в кластері серверів. Можливо, профіль безпеки був вилучений раніше...'"), ЭлементСценария.Профиль));
...
Процедура ПослеЗавершенияЗапросаРазрешенийНаИспользованиеВнешнихРесурсов(Результат, Состояние) Экспорт
	...
	Если Результат = КодВозвратаДиалога.ОК Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Настройка разрешений';uk='Настройка дозволів'"),,
			НСтр("ru='Внесены изменения в настройки профилей безопасности в кластере серверов.';uk='Внесено зміни в настройки профілів безпеки в кластері серверів.'"));
...
//  внешних (относительно кластера серверов платформы) ресурсов.
//
