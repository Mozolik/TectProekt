...
Процедура СформироватьДокументыПриложение2ПоУсловнымПродажамПропорциональныйНДС(ПараметрыОтбора, НалоговыеНакладные = Неопределено) Экспорт
	...
	Если НалоговыеНакладные = Неопределено Тогда
	    ...
	    Если РезультатЗапроса.Пустой() Тогда
			ТекстСообщения = НСтр("ru='Нет налоговых накладных удовлетворяющих условию. Формирование документов ""Приложение 2 к налоговой накладной"" не выполнено!';uk='Немає податкових накладних задовольняють умові. Формування документів ""Додаток 2 до податкової накладної"" не виконано!'");
