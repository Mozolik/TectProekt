...
Процедура СоздатьЗаписьТипа(Знач СловарьТипов, Знач Описание) Экспорт
	...
	Если ТипЗнч(СловарьТипов)<>Тип("Соответствие") Тогда		
		ВызватьИсключение НСтр("ru='Параметр СловарьТипов должен иметь тип Соответствие.';uk='Параметр СловарьТипов повинен мати тип Соответствие.'");
	КонецЕсли;
	
	Если ТипЗнч(Описание)<>Тип("Структура") Тогда		
		ВызватьИсключение НСтр("ru='Параметр Описание должен иметь тип Структура.';uk='Параметр Описание повинен мати тип Структура.'");
	КонецЕсли;
	...
	Если ПустаяСтрока(Описание.ТипИнцидента) Тогда
		ВызватьИсключение НСтр("ru='ТипИнцидента не указан';uk='ТипИнцидента не вказаний'");
	КонецЕсли;
	...
	Если УровеньИнцидента<>"Информация" И 
		...
		ВызватьИсключение НСтр("ru='Неверное значение уровня инцидента';uk='Невірне значення рівня інциденту'");
	КонецЕсли;
...
Процедура ОткрытьИнцидент(Знач ТипИнцидента, Знач КодИнцидента, Знач ТекстСообщения, Знач Асинхронно = Ложь) Экспорт
	...
	Если ПустаяСтрока(ТипИнцидента) Тогда	
		ВызватьИсключение НСтр("ru='Не указан тип инцидента';uk='Не вказано тип інциденту'"); 
	КонецЕсли;
	Если ПустаяСтрока(КодИнцидента) Тогда	
		ВызватьИсключение НСтр("ru='Не указан код инцидента';uk='Не вказано код інциденту'"); 
	КонецЕсли;
	Если ПустаяСтрока(ТекстСообщения) Тогда	
		ВызватьИсключение НСтр("ru='Не указано сообщение инцидента';uk='Не зазначено повідомлення інциденту'"); 
	КонецЕсли;
	...
	ВызватьИсключение СтрШаблон(
		НСтр("ru='Тип инцидента %1 не зарегистрирован';uk='Тип інциденту %1 не зареєстрований'"), 
		ТипИнцидента
	);
...
Процедура ОтослатьСтатистику(Знач ИдентификаторСчетчика, Знач ЗначениеСчетчика, Знач Данные = неопределено, Знач ДатаСчетчика = неопределено) Экспорт
	...
	Прокси = ТехнологияСервисаИнтеграцияСБСП.WSПрокси(
		АдресWS, 
		"http://1c.ru", 
		?(ДатаСчетчика = неопределено, "InputStatistics", 		"InputStatisticsDate"),
		?(ДатаСчетчика = неопределено, "InputStatisticsSoap",	"InputStatisticsDateSoap"),
		ЛогинWS, 
		ПарольWS
	);
	...
	Если Результат <> "OK" Тогда
		ЗаписьЖурналаРегистрации(
			...
			НСтр("ru='Ошибка при передаче счетчика в ЦКК';uk='Помилка при передачі лічильника до ЦКЯ'")
		);
	КонецЕсли;
...
Процедура ВыполнитьЗапрос(КомандаЗапроса, Данные)
	...
	Если Ответ.КодСостояния<>200 Тогда
		ЗаписьЖурналаРегистрации(
			...
			УровеньЖурналаРегистрации.Ошибка,,, 
			СтрШаблон(
			НСтр("ru='Ошибка (код %3) команды %1 сервиса инцидентов ЦКК: %2';uk='Помилка (код %3) команди %1 сервісу інцидентів ЦКЯ: %2'"), 
...
Процедура ОтослатьПоказателиДлиныОчередейЗаданий() 
	...
	Иначе
		ВызватьИсключение НСтр("ru='Невозможно определить имя кластера и информационной базы из строки подключения';uk='Неможливо визначити ім''я кластера та інформаційної бази з рядка підключення'");
	КонецЕсли;
	...
	ИмяСчетчика = Cluster + "\" + InfoBase + ".ЗаданияСОшибками";	
	Результат = 0.001;
