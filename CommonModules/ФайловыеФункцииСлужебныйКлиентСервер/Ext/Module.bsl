	...
	Если Файл.Размер() > ОбщиеНастройки.МаксимальныйРазмерФайла Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Размер файла ""%1"" (%2 Мб)
                       ...
                       |;uk='Розмір файлу ""%1"" (%2 Мб)
                       |перевищує максимально припустимий розмір файлу (%3 Мб).'"),
			Файл.Имя,
	...
	Если Не ПроверитьРасширениеФайлаДляЗагрузки(Файл.Расширение, Ложь) Тогда
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Загрузка файлов с расширением ""%1"" запрещена.
                       ...
                       |;uk='Завантаження файлів з розширенням ""%1"" заборонена.
...
Функция СтрокаСообщенияОНедопустимостиПодписанияЗанятогоФайла(ФайлСсылка = Неопределено) Экспорт
	...
	Если ФайлСсылка = Неопределено Тогда
		Возврат НСтр("ru='Нельзя подписать занятый файл.';uk='Не можна підписати зайнятий файл.'");
	Иначе
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Нельзя подписать занятый файл: %1.';uk='Не можна підписати зайнятий файл: %1.'"),
			Строка(ФайлСсылка) );
	КонецЕсли;
...
Функция СтрокаСообщенияОНедопустимостиПодписанияЗашифрованногоФайла(ФайлСсылка = Неопределено) Экспорт
	...
	Если ФайлСсылка = Неопределено Тогда
		Возврат НСтр("ru='Нельзя подписать зашифрованный файл.';uk='Не можна підписати зашифрований файл.'");
	Иначе
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Нельзя подписать зашифрованный файл: %1.';uk='Не можна підписати зашифрований файл: %1.'"),
						Строка(ФайлСсылка) );
	КонецЕсли;
...
Функция ОшибкаСозданияНовогоФайла(ИнформацияОбОшибке) Экспорт
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ошибка создания нового файла.
                   |
                   |%1'
                   |;uk='Помилка створення нового файлу.
                   |
                   |   %1'"),
		КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
	...
	Если ПоискВТоме Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка открытия файла:
                       ...
                       |Обратитесь к администратору.'
                       |;uk='Помилка відкриття файлу:
                       |""%1"".
                       |%2
                       |Файл не знайдений в сховище файлів.
                       |Можливо файл вилучено антивірусною програмою.
                       |Зверніться до адміністратора.'"),
			ИмяФайла,
			ВладелецФайла);
	Иначе
...
Процедура ЗаполнитьСтатусПодписи(СтрокаПодписи, ТекстОшибки = "") Экспорт
	...
	Если СтрокаПодписи.ПодписьВерна Тогда
		СтрокаПодписи.Статус = НСтр("ru='Верна';uk='Вірна'");
		
	ИначеЕсли ЗначениеЗаполнено(ТекстОшибки) Тогда
		СтрокаПодписи.Статус = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неверна. %1';uk='Невірна. %1'"), ТекстОшибки);
		Если Прав(СтрокаПодписи.Статус, 1) = "." Тогда
			СтрокаПодписи.Статус = Лев(СтрокаПодписи.Статус, СтрДлина(СтрокаПодписи.Статус)-1);
		КонецЕсли;
	Иначе
		СтрокаПодписи.Статус = НСтр("ru='Неверна';uk='Невірна'");
	КонецЕсли;
...
Функция АдресВОблачномСервисе(Сервис, Href) Экспорт
	...
	Если Не ПустаяСтрока(Сервис) Тогда
		Если Сервис = "https://webdav.4shared.com" Тогда
