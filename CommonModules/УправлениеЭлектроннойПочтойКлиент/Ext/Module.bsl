...
Процедура ОтправитьЗагрузитьПочтуПользователя(ЭлементСписок = Неопределено) Экспорт
	...
	Состояние(НСтр("ru='Идет отправка и получение электронной почты ...';uk='Йде надсилання та отримання електронної пошти ...'"));
	ВзаимодействияВызовСервера.ОтправитьЗагрузитьПочтуПользователя(ПолученоПисем, ДоступноУчетныхЗаписей, ЕстьОшибки);
	Состояние();
	
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Получено писем: %1';uk='Отримане листів: %1'"), ПолученоПисем);
	Если ДоступноУчетныхЗаписей > 1 Тогда
		ТекстСообщения = ТекстСообщения + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='(учетных записей: %1)';uk='(облікових записів: %1)'"), ДоступноУчетныхЗаписей);
	КонецЕсли;
	...
	Если ЕстьОшибки Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='При получении почты были ошибки. Подробности см. в журнале регистрации';uk='При отриманні пошти були помилки. Подробиці див. у журналі реєстрації'"));
