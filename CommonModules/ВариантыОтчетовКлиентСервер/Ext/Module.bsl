...
Функция НаименованиеПодсистемы(КодЯзыка) Экспорт
	Возврат НСтр("ru='Варианты отчетов';uk='Варіанти звітів'",?(КодЯзыка=Неопределено,ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(),КодЯзыка));
КонецФункции

// Представление группы важности.
Функция ПредставлениеСмТакже() Экспорт
	Возврат НСтр("ru='См. также';uk='Див. також'");
КонецФункции 

// Представление группы важности.
Функция ПредставлениеВажный() Экспорт
	Возврат НСтр("ru='Важный';uk='Важливий'");
КонецФункции
...
Функция ТекстОшибки(Номер, Реквизит1 = Неопределено, Реквизит2 = Неопределено, Реквизит3 = Неопределено) Экспорт
	Если Номер = 1 Или Номер = 2 Тогда
		Если Номер = 1 Тогда
			ТекстОшибки = НСтр("ru='Отчет ""%1"" подключен к основной форме, но не подключен к форме настроек.';uk='Звіт ""%1"" підключений до основної форми, але не підключений до форми настройки.'");
		Иначе
			ТекстОшибки = НСтр("ru='Отчет ""%1"" не подключен к основной форме, но подключен к форме настроек.';uk='Звіт ""%1"" не підключений до основної формі, але підключений до форми настройки.'");
		КонецЕсли;
		ТекстОшибки = ТекстОшибки + Символы.ПС + НСтр("ru='Подробнее см. в документации по внедрению подсистемы.';uk='Докладніше див. у документації по впровадженню підсистеми.'");
