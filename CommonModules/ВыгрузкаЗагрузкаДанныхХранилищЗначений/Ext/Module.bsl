...
Процедура ПередВыгрузкойОбъекта(Контейнер, МенеджерВыгрузкиОбъекта, Сериализатор, Объект, Артефакты, Отказ) Экспорт
	...
	Если РеквизитыСХранилищемЗначений = Неопределено Тогда
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Объект метаданных %1 не может быть обработан обработчиком ВыгрузкаЗагрузкаДанныхХранилищЗначений.ПередВыгрузкойОбъекта()!';uk='Об''єкт метаданих %1 не може бути оброблений обробником ВыгрузкаЗагрузкаДанныхХранилищЗначений.ПередВыгрузкойОбъекта()!'"),
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Неожиданный объект метаданных: %1!';uk='Несподіваний об''єкт метаданих: %1!'"),
			ОбъектМетаданных.ПолноеИмя);
...
// Вызывается перед выгрузкой ссылочного объкета с хранилищем значения.
//
// Параметры:
//	Контейнер - ОбработкаОбъект.ВыгрузкаЗагрузкаДанныхМенеджерКонтейнера - менеджер
//		контейнера, используемый в процессе выгрузи данных. Подробнее см. комментарий
...
// Вызывается перед выгрузкой набора записей объкета с хранилищем значения.
//
// Параметры:
//	Контейнер - ОбработкаОбъект.ВыгрузкаЗагрузкаДанныхМенеджерКонтейнера - менеджер
//		контейнера, используемый в процессе выгрузи данных. Подробнее см. комментарий
...
Функция ВыгрузитьХранилищеЗначения(Контейнер, ХранилищеЗначения, Артефакт)
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("ДвоичныеДанные") Тогда
		...
		// В двоичных данных не поддерживается сопоставление и пересоздание ссылок.
		// Двоичные данные остаются в объекте, чтобы уменьшить количество файлов
		// и сократить время выгрузки и загрузки.
		Возврат Ложь;
...
// Записывает хранилище значкений в артефакт.
//
// Параметры:
//	Контейнер - ОбработкаОбъект.ВыгрузкаЗагрузкаДанныхМенеджерКонтейнера - менеджер
//		контейнера, используемый в процессе выгрузи данных. Подробнее см. комментарий
...
Процедура ПередЗагрузкойКонстанты(Контейнер, Объект, Артефакт)
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Тип владельца {%1}%2 не должен использоваться для объекта метаданных %3!';uk='Тип власника {%1}%2 не повинен використовуватися для об''єкта метаданих %3!'"),
...
//Загружает занчение хранилище значения из артефакта.
//
// Параметры:
//	Контейнер - ОбработкаОбъект.ВыгрузкаЗагрузкаДанныхМенеджерКонтейнера - менеджер
//		контейнера, используемый в процессе выгрузи данных. Подробнее см. комментарий
...
Процедура ЗагрузитьХранилищеЗначения(Контейнер, ХранилищеЗначения, Артефакт)
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Неожиданный тип размещения данных хранилища значений в контейнере выгрузки: {%1}%2!';uk='Неочікуваний тип розміщення даних сховища значень в контейнері вивантаження: {%1}%2!'"),
...
// Возвращает пространство имен XDTO-пакета для хранилищ значеий.
//
// Возвращаемое значение:
//	Строка - пространство имен XDTO-пакета для хранилищ значеий.
//
Функция Пакет()
