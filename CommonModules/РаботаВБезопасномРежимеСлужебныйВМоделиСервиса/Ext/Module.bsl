...
Функция РежимИсполненияВнешнегоМодуля(Знач ВнешнийМодуль) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Ключ = КлючРегистраПоСсылке(ВнешнийМодуль);
...
Процедура ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики) Экспорт
	...
	СерверныеОбработчики["СтандартныеПодсистемы.БазоваяФункциональность\ПриЗапросеСозданияПрофиляБезопасности"].Добавить(
		"РаботаВБезопасномРежимеСлужебныйВМоделиСервиса");
	
	СерверныеОбработчики["СтандартныеПодсистемы.БазоваяФункциональность\ПриЗапросеУдаленияПрофиляБезопасности"].Добавить(
		"РаботаВБезопасномРежимеСлужебныйВМоделиСервиса");
	
	СерверныеОбработчики["СтандартныеПодсистемы.БазоваяФункциональность\ПриПодключенииВнешнегоМодуля"].Добавить(
		"РаботаВБезопасномРежимеСлужебныйВМоделиСервиса");
	...
	КлиентскиеОбработчики["СтандартныеПодсистемы.БазоваяФункциональность\ПриПодтвержденииЗапросовНаИспользованиеВнешнихРесурсов"].Добавить(
...
Процедура ПриПодключенииВнешнегоМодуля(Знач ВнешнийМодуль, БезопасныйРежим) Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		БезопасныйРежим = РежимИсполненияВнешнегоМодуля(ВнешнийМодуль);
...
Функция КлючРегистраПоСсылке(Знач Ссылка)
...
Функция КлючРазрешения(Знач Разрешение) Экспорт
	...
	Если СтрДлина(Ключ) > 32 Тогда
		ВызватьИсключение НСтр("ru='Превышение длины ключа';uk='Перевищення довжини ключа'");
	КонецЕсли;
...
Функция СтрокаТаблицыРазрешений(Знач ТаблицаРазрешений, Знач Отбор, Знач ДобавитьПриОтсутствии = Истина) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Нарушение уникальности строк в таблице разрешений по отбору %1';uk='Порушення унікальності рядків у таблиці дозволів за відбором %1'"),
...
//   * Ключ - Строка - ключ разрешения (см. функцию КлючРазрешения в модуеле менеджера регистра
//      РазрешенияНаИспользованиеВнешнихРесурсов),
//   * Значение - Структура - см. фукнцию ДополнениеРазрешения в модуле менеджера регистра
//      РазрешенияНаИспользованиеВнешнихРесурсов).
//
	...
	Если Менеджер.Выбран() Тогда
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru='Дублирование разрешений по ключевым полям:
                  ...
                  |- КлючРазрешения: %5.'
                  |;uk='Дублювання дозволів по ключових полях:
                  |- ТипПрограммногоМодуля: %1
                  |- ИдентификаторПрограммногоМодуля: %2
                  |- ТипВладельца: %3
                  |- ИдентификаторВладельца: %4
...
Процедура УдалитьРазрешение(Знач ТипПрограммногоМодуля, Знач ИдентификаторПрограммногоМодуля, Знач ТипВладельца, Знач ИдентификаторВладельца, Знач КлючРазрешения, Знач Разрешение) Экспорт
	...
	Если Менеджер.Выбран() Тогда
		...
		Если Менеджер.ТелоРазрешения <> ОбщегоНазначения.ОбъектXDTOВСтрокуXML(Разрешение) Тогда
			...
			ВызватьИсключение СтрШаблон(
				НСтр("ru='Коллзиция разрешений по ключам:
                      |- ТипПрограммногоМодуля: %1
                      |- ИдентификаторПрограммногоМодуля: %2
                      |- ТипВладельца: %3
                      |- ИдентификаторВладельца: %4
                      |- КлючРазрешения: %5.'
                      |;uk='Колізія дозволів по ключам:
                      |- ТипПрограммногоМодуля: %1
                      |- ИдентификаторПрограммногоМодуля: %2
                      |- ТипВладельца: %3
                      |- ИдентификаторВладельца: %4
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru='Попытка удаления несуществующего разрешения:
                  ...
                  |- ИдентификаторВладельца: %4
                  |- КлючРазрешения: %5.'
                  |;uk='Спроба вилучення неіснуючого дозволу:
                  |- ТипПрограммногоМодуля: %1
                  |- ИдентификаторПрограммногоМодуля: %2
                  |- ТипВладельца: %3
                  |- ИдентификаторВладельца: %4
...
Функция РезультатОбработкиПакета(Знач ИдентификаторПакета) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru='Не найден пакет запросов %1';uk='Не знайдено пакет запитів %1'"), ИдентификаторПакета);
		
	КонецЕсли;
	
КонецФункции

Процедура УстановитьРезультатОбработкиПакета(Знач ИдентификаторПакета, Знач Результат) Экспорт
...
Функция СостояниеОбработкиПакета(Знач ИдентификаторПакета)
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Не найден пакет запросов %1';uk='Не знайдено пакет запитів %1'"), ИдентификаторПакета);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТехнологияСервисаИнтеграцияСБСП.ПодсистемаСуществует("ТехнологияСервиса.РаботаВМоделиСервиса.ДополнительныеОтчетыИОбработкиВМоделиСервиса") Тогда
			...
			МодульДополнительныеОтчетыИОбработкиВМоделиСервиса.ПриСериализацииВладельцаРазрешенийНаИспользованиеВнешнихРесурсов(
				Владелец, СтандартнаяОбработка, ПредставлениеПрограммногоМодуля, ПредставлениеВладельца);
			...
			Иначе
				...
				ВызватьИсключение СтрШаблон(
					НСтр("ru='Не сериализован программный модуль по ключу:
                          |- Тип: %1
                          |- Идентификатор: %2'
                          |;uk='Не серіалізувати програмний модуль по ключу:
                          |- Тип: %1
                          |- Идентификатор: %2'"),
					Выборка.ТипПрограммногоМодуля,
					Выборка.ИдентификаторПрограммногоМодуля);
