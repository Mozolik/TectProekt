...
Функция ИнициализироватьСостояниеДляВыборкиПорцийНезависимогоНабораЗаписей(Знач МетаданныеОбъекта, Знач Отбор, Знач ДополнениеКИмениТаблицы = "")
	...
	ПсведонимТаблицы = "_ТаблицаНабораЗаписей"; // Псевдоним таблицы в тексте запроса
	...
	Для каждого ПолеВыборки Из ПоляВыборки Цикл
		...
		СтрокаПолейВыборки = СтрокаПолейВыборки + Символы.Таб + ПсведонимТаблицы + "." + ПолеВыборки + " КАК " + ПолеВыборки;
	КонецЦикла;
	...
	Если ТипЗнч(Отбор) = Тип("Массив") Тогда
		Отбор = СформироватьУсловиеОтбора(ПсведонимТаблицы, Отбор);
	КонецЕсли;
	...
	Если ПоляКлюча.Количество() = 0 Тогда 
		...
		|" + СтрокаПолейВыборки + "
		|ИЗ
		|	" + МетаданныеОбъекта.ПолноеИмя() + ДополнениеКИмениТаблицы + " КАК " + ПсведонимТаблицы + "
		|[Условие]";
	Иначе
		ШаблонЗапроса = // Общая часть запроса
	...
	Для СчетчикЗапросов = 0 По ПоляКлюча.ВГраница() Цикл
		...
		Для ИндексПоля = 0 По КоличествоПолейУсловий - 1 Цикл
			...
			СтрокаПолейУсловия = СтрокаПолейУсловия + ПсведонимТаблицы + "." + ПолеКлюча + " " 
...
Функция СформироватьУсловиеОтбора(Знач ПсведонимТаблицы, Знач Отбор)
	...
	Если Отбор.Количество() > 0 Тогда
		Для каждого ОписаниеОтбора Из Отбор Цикл
			...
			ИначеЕсли ОписаниеОтбора.ВидСравнения = ВидСравнения.МеньшеИлиРавно Тогда
				ЛогическийОператор = "<=";
			Иначе
				ШаблонСообщения = НСтр("ru='Вид сравнения %1 не поддерживается.';uk='Вид порівняння %1 не підтримується.'");
				ТекстСообщения = СтрШаблон(ШаблонСообщения, ОписаниеОтбора.ВидСравнения);
			КонецЕсли;
			
			СтрокаОтбора = СтрокаОтбора + ПсведонимТаблицы + "." + ОписаниеОтбора.Поле + " " + ЛогическийОператор + " " + Операнд;
		КонецЦикла;
	КонецЕсли;
