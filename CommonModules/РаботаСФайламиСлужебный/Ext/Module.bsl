	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ПутьКФайлу) И ЗначениеЗаполнено(Выборка.Том) Тогда
			...
			Если Не Файл.Существует() Тогда
				...
				НесуществующийФайл.СтатусПроверки = НСтр("ru='Отсутствуют данные в томе на диске';uk='Відсутні дані в томі на диску'");
			...
			Иначе
				...
				СуществующийФайл[0].СтатусПроверки = НСтр("ru='Целостные данные';uk='Цілісні дані'");
...
Процедура ВыполнитьОчисткуДанныхОФайле(ФайлСсылка) Экспорт
	...
	Попытка
		...
		Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			...
			Если ФайлНаДиске.Существует() Тогда
				...
				ТекстКомментария = НСтр("ru='Файл удален при очистке ненужных файлов.';uk='Файл вилучено при очищенні непотрібних файлів.'")
	...
	Исключение
		...
		Если Не ПустаяСтрока(ИмяФайлаСПутем) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Очистка ненужных файлов';uk='Очищення непотрібних файлів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов(ВозможныеПрава) Экспорт
	...
	Право.Имя           = "Чтение";
	Право.Заголовок     = НСтр("ru='Чтение';uk='Читання'");
	Право.Подсказка     = НСтр("ru='Чтение папок и файлов';uk='Читання папок і файлів'");
	...
	Право.Подсказка     = НСтр("ru='Добавление, изменение и
                                     ...
                                     |;uk='Додавання, зміна та
                                     |позначка вилучення папок файлів'");
	...
	Право.Заголовок     = НСтр("ru='Изменение
                                     ...
                                     |;uk='Зміна
                                     |файлів'");
	Право.Подсказка     = НСтр("ru='Изменение файлов в папке';uk='Зміна файлів у папці'");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("Чтение");
	// Права для стандартных шаблонов ограничений доступа.
	...
	Право.Заголовок     = НСтр("ru='Добавление
                                     ...
                                     |;uk='Додавання
                                     |файлів'");
	Право.Подсказка     = НСтр("ru='Добавление файлов в папку';uk='Додавання файлів у папку'");
	// Права, требуемые для этого права.
	...
	Право.Заголовок     = НСтр("ru='Пометка
                                     ...
                                     |;uk='Позначка
                                     |вилучення'");
	Право.Подсказка     = НСтр("ru='Пометка удаления файлов в папке';uk='Позначка вилучення файлів у папці'");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("ИзменениеФайлов");
	...
	Право.Заголовок     = НСтр("ru='Управление
                                     ...
                                     |;uk='Управління
                                     |правами'");
	Право.Подсказка     = НСтр("ru='Управление правами папки';uk='Управління правами папки'");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("Чтение");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru='Редактируемые файлы';uk='Файли, що редагуються'");
...
Функция СобытиеЖурналаРегистрацииДляОбмена() 
	...
	Возврат НСтр("ru='Файлы.Не удалось отправить файл при обмене данными';uk='Файли.Не вдалося відправити файл при обміні даними'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
