...
Функция ФормаВыполненияЗадачи(Знач ЗадачаСсылка) Экспорт
	...
	Если ТипЗнч(ЗадачаСсылка) <> Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неправильный тип параметра ЗадачаСсылка (передан: %1; ожидается: %2)';uk='Неправильний тип параметра ЗадачаСсылка (переданий: %1; очікується: %2)'"),
...
Процедура ВыполнитьЗадачу(ЗадачаСсылка, ДействиеПоУмолчанию = Ложь) Экспорт
	...
	Попытка
		...
		Если ЗадачаОбъект.Выполнена Тогда
			ВызватьИсключение НСтр("ru='Задача уже была выполнена ранее.';uk='Задача вже була виконана раніше.'");
		КонецЕсли;
...
Процедура СделатьАктивнымБизнесПроцесс(БизнесПроцесс) Экспорт
	...
	Попытка
		...
		Если Объект.Состояние = Перечисления.СостоянияБизнесПроцессов.Активен Тогда
			...
			ВызватьИсключение НСтр("ru='Бизнес-процесс уже активен.';uk='Бізнес-процес вже активний.'");
		КонецЕсли;
...
Процедура ОстановитьБизнесПроцесс(БизнесПроцесс) Экспорт
	...
	Попытка
		...
		Если Объект.Состояние = Перечисления.СостоянияБизнесПроцессов.Остановлен Тогда
			...
			ВызватьИсключение НСтр("ru='Бизнес-процесс уже остановлен.';uk='Бізнес-процес вже зупинено.'");
		КонецЕсли;
