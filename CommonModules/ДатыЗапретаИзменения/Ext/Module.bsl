...
Функция НайтиДатуЗапрета(Адресаты, РазделЗапрета, ОбъектЗапрета, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		Если ПоискЗапретовЗагрузки Тогда
			Если НоваяВерсия Тогда
				Шаблон = НСтр("ru='%1 невозможно загрузить в запрещенный период.';uk='%1 неможливо завантажити в заборонений період.'");
			Иначе
				Шаблон = НСтр("ru='%1 в запрещенном периоде невозможно заменить загружаемыми данными.';uk='%1 в забороненому періоді неможливо замінити завантажуваними даними.'");
			КонецЕсли;
		Иначе
			Если НоваяВерсия Тогда
				Шаблон = НСтр("ru='%1 невозможно поместить в запрещенный период.';uk='%1 неможливо помістити в заборонений період.'");
			Иначе
				Шаблон = НСтр("ru='%1 в запрещенном периоде невозможно изменить.';uk='%1 в забороненому періоді неможливо змінити.'");
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого Запрет Из Запреты Цикл
		...
		Если Запрет.Раздел = Запрет.Объект Тогда
			Если Запрет.Раздел = ДатыЗапретаИзмененияСлужебныйПовтИсп.ПустойРаздел() Тогда
				Текст = Текст + НСтр("ru='Дате %1';uk='Даті %1'");
			Иначе
				Текст = Текст + НСтр("ru='Дате %1 по разделу ""%2""';uk='Даті %1 по розділу ""%2""'");
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(СвойстваВстраивания.ЕдинственныйРаздел) Тогда
			Текст = Текст + НСтр("ru='Дате %1 по объекту ""%3""';uk='Даті %1 по об''єкту ""%3""'");
		Иначе
			Текст = Текст + НСтр("ru='Дате %1 по объекту ""%3"" раздела ""%2""';uk='Даті %1 по об''єкту ""%3"" розділу ""%2""'");
		КонецЕсли;
		Если ПоискЗапретовЗагрузки Тогда
			Текст = Текст + " " + НСтр("ru='соответствует запрет загрузки данных';uk='відповідає заборона завантаження даних'") + " ";
		Иначе
			Текст = Текст + " " + НСтр("ru='соответствует запрет изменения данных';uk='відповідає заборона зміни даних'") + " ";
		КонецЕсли;
		Если Запрет.Адресат = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехПользователей Тогда
			Текст = Текст + НСтр("ru='для всех пользователей';uk='для всіх користувачів'");
			
		ИначеЕсли Запрет.Адресат = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехИнформационныхБаз Тогда
			Текст = Текст + НСтр("ru='для всех информационных баз';uk='для всіх інформаційних баз'");
		...
		ИначеЕсли ТипЗнч(Запрет.Адресат) = Тип("СправочникСсылка.ГруппыПользователей")
			...
			Текст = Текст + НСтр("ru='для группы пользователей ""%4""';uk='для групи користувачів ""%4""'");
		...
		ИначеЕсли ТипЗнч(Запрет.Адресат) = Тип("СправочникСсылка.Пользователи")
			...
			Текст = Текст + НСтр("ru='для пользователя ""%4""';uk='для користувача ""%4""'");
			
		ИначеЕсли ЗначениеЗаполнено(Запрет.Адресат) Тогда
			Текст = Текст + НСтр("ru='для информационной базы ""%4""';uk='для інформаційної бази ""%4""'");
		Иначе
			Текст = Текст + НСтр("ru='для всех информационных баз ""%6""';uk='для всіх інформаційних баз ""%6""'");
		КонецЕсли;
		Текст = Текст + " " + НСтр("ru='по %5';uk='по %5'");
		Если НЕ СвойстваВстраивания.БезРазделовИОбъектов Тогда
			Если ЗначениеЗаполнено(Запрет.Раздел) Тогда
				Если Запрет.Объект = Запрет.Раздел Тогда
					Текст = Текст + " " + НСтр("ru='(запрет установлен на раздел ""%2"")';uk='(заборона встановлена на розділ ""%2"")'");
				ИначеЕсли ЗначениеЗаполнено(СвойстваВстраивания.ЕдинственныйРаздел) Тогда
					Текст = Текст + " " + НСтр("ru='(запрет установлен на объект ""%3"")';uk='(заборона встановлена на об''єкт ""%3"")'");
				Иначе
					Текст = Текст + " " + НСтр("ru='(запрет установлен на объект ""%3"" раздела ""%2"")';uk='(заборона встановлена на об''єкт ""%3"" розділу ""%2"")'");
				КонецЕсли;
			Иначе
				Текст = Текст + " " + НСтр("ru='(установлена общая дата запрета)';uk='(встановлена загальна дата заборони)'");
			КонецЕсли;
		КонецЕсли;
	...
	Если ЭтоРегистр Тогда
		...
		Если КоличествоПолей = 1 Тогда
			ПредставлениеДанных = ПредставлениеДанных
				+ " " + НСтр("ru='с полем';uk='з полем'")  + " " + Строка(Данные.Отбор);
			
		ИначеЕсли КоличествоПолей > 1 Тогда
			ПредставлениеДанных = ПредставлениеДанных
				+ " " + НСтр("ru='с полями';uk='з полями'") + " " + Строка(Данные.Отбор);
		КонецЕсли;
