		...
		Если Строка.ЗаполнятьОбязательно И НЕ Строка.Удалено Тогда
			...
			Если НЕ ЗначениеЗаполнено(Форма[Строка.ИмяРеквизитаЗначение]) Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Поле ""%1"" не заполнено.';uk='Поле ""%1"" не заповнене.'"), Строка.Наименование),
					"");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
		...
		Если НаборыСвойствОбъекта.Количество() > 1 Тогда
			...
			Иначе
				...
				Если ОписаниеНабора = Неопределено Тогда
					...
					ОписаниеНабора.Заголовок = НСтр("ru='Удаленные реквизиты';uk='Вилучені реквізити'")
				КонецЕсли;
...
Процедура СоздатьОсновныеОбъектыФормы(Форма, Контекст, СоздатьОписаниеДополнительныхРеквизитов)
	...
	Если ОпцияИспользоватьСвойства Тогда
		...
		Если СоздатьОписаниеДополнительныхРеквизитов Тогда
			...
			Если Пользователи.РолиДоступны("ДобавлениеИзменениеДополнительныхРеквизитовИСведений") Тогда
				...
				Команда.Заголовок = НСтр("ru='Изменить состав дополнительных реквизитов';uk='Змінити склад додаткових реквізитів'");
				Команда.Действие = "Подключаемый_РедактироватьСоставСвойств";
				Команда.Подсказка = НСтр("ru='Изменить состав дополнительных реквизитов';uk='Змінити склад додаткових реквізитів'");
	...
	Если ОпцияИспользоватьСвойства
		...
		Если Родитель <> Неопределено Тогда
			...
			ПредупреждениеОРазмещении = НСтр("ru='Для отображения дополнительных реквизитов необходимо разместить группу ""%1"" не первым элементом (после любой другой группы) в группе ""%2"" (меню Еще - Изменить форму).';uk='Для відображення додаткових реквізитів необхідно розмістити групу ""%1"" не першим елементом (після будь-якої іншої групи) в групі ""%2"" (меню Ще - Змінити форму).'");
			ТекстПодсказки = НСтр("ru='Также можно установить стандартные настройки формы:
                ...
                |;uk='Також можна встановити стандартні форми:
