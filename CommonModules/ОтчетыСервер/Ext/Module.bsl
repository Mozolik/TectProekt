...
Процедура ЗаполнитьПредставлениеНастройки(СвойстваНастройки, НастройкаВариантаКД, ПользовательскаяНастройкаКД, ДоступнаяНастройкаКД)
	...
	ИначеЕсли СвойстваНастройки.Тип = "Порядок" Тогда
		ПредставлениеПоУмолчанию = НСтр("ru='Сортировка';uk='Сортування'");
	ИначеЕсли СвойстваНастройки.Тип = "ВыбранныеПоля" Тогда
		ПредставлениеПоУмолчанию = НСтр("ru='Поля';uk='Поля'");
	ИначеЕсли СвойстваНастройки.Тип = "УсловноеОформление" Тогда
		ПредставлениеПоУмолчанию = НСтр("ru='Оформление';uk='Оформлення'");
	...
	ИначеЕсли СвойстваНастройки.Тип = "Группировка"
		...
		Если ПустаяСтрока(ПредставлениеПоУмолчанию) Тогда
			ПредставлениеПоУмолчанию = НСтр("ru='<Детальные записи>';uk='<Детальні записи>'");
		КонецЕсли;
	ИначеЕсли СвойстваНастройки.Тип = "Таблица" Тогда
		ПредставлениеПоУмолчанию = НСтр("ru='Таблица';uk='Таблиця'");
	ИначеЕсли СвойстваНастройки.Тип = "Диаграмма" Тогда
		ПредставлениеПоУмолчанию = НСтр("ru='Диаграмма';uk='Діаграма'");
	ИначеЕсли СвойстваНастройки.Тип = "НастройкиВложенногоОбъекта" Тогда
		...
		Если ПустаяСтрока(ПредставлениеПоУмолчанию) Тогда
			ПредставлениеПоУмолчанию = НСтр("ru='Вложенная группировка';uk='Вкладене групування'");
		КонецЕсли;
	ИначеЕсли СвойстваНастройки.Тип = "СтруктураНастроек" Тогда
		ПредставлениеПоУмолчанию = НСтр("ru='Структура';uk='Структура'");
	Иначе
...
Процедура ВывестиПоПорядку(Форма, ГруппаВывода, Родитель, КоличествоКолонок, ГибкаяБалансировка = Истина) Экспорт
	...
	ТребуетсяФиксированнаяШиринаБыстрыхНастроек = (Не ИнформацияЭкрановКлиента = Неопределено 
		И ИнформацияЭкрановКлиента[0].Ширина >= 1280);
	...
	Для Каждого ЭлементВывода Из ГруппаВывода.Порядок Цикл
		...
		Иначе
			Если ТекущаяГруппа2 = Неопределено Тогда
				...
				// Переопределение поведения БСП
				Если ТребуетсяФиксированнаяШиринаБыстрыхНастроек Тогда
					ТекущаяГруппа2.РастягиватьПоГоризонтали = Истина;
				КонецЕсли;
			КонецЕсли;
			...
			// Переопределение поведения БСП
			Если ТребуетсяФиксированнаяШиринаБыстрыхНастроек 
				И ТипЗнч(Элементы[ЭлементВывода.ИмяЭлемента2]) = Тип("ПолеФормы") Тогда
				Элементы[ЭлементВывода.ИмяЭлемента2].Ширина = 36 - ?(НомерКолонки = 1, 0, 8);
				Элементы[ЭлементВывода.ИмяЭлемента2].РастягиватьПоГоризонтали = Ложь;
			КонецЕсли;
		КонецЕсли;
