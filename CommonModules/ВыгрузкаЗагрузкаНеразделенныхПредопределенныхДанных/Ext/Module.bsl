		...
		Если ПараметрыВыгрузки.ВыгружаемыеТипы.Найти(ОбъектМетаданных) <> Неопределено Тогда
			...
			Для Каждого ИмяПоля Из ПравилоКонтроля.Значение Цикл
				...
				Иначе
					...
					ВызватьИсключение СтрШаблон(НСтр("ru='Объект метаданных %1 не поддерживается!';uk='Об''єкт метаданих %1 не підтримується!'"), ОбъектМетаданных.ПолноеИмя());
...
Процедура ВызватьИсключениеНеУдалосьОпределитьПоле(Знач ИмяПоля, Знач ИмяОбъекта)
	...
	ВызватьИсключение СтрШаблон(НСтр("ru='Не удалось построить запрос получения значения поля %1 объекта мтеданных %2!';uk='Не вдалося побудувати запит отримання значення поля %1 об''єкта метаданих %2!'"),
		ИмяПоля, ИмяОбъекта
	);
