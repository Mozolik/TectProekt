...
Процедура ПодключитьКорреспондента(Сообщение, Отправитель)
	...
	Если ЭтаКонечнаяТочка.Пустая()
		...
		ПредставлениеОшибки = НСтр("ru='Конечная точка не соответствует ожидаемой. Код ожидаемой конечной точки %1. Код текущей конечной точки %2.';uk='Кінцева точка не відповідає очікуваній. Код очікуваної кінцевої точки %1. Код поточної кінцевої точки %2.'");
	...
	Если Корреспондент.Пустая() Тогда // Подключаем конечную точку корреспондента
		...
		Если Отказ Тогда // Отправляем сообщение в менеджер сервиса об ошибке
			...
			ПредставлениеОшибки = НСтр("ru='Ошибка подключения конечной точки корреспондента обмена. Код конечной точки корреспондента обмена %1.';uk='Помилка підключення кінцевої точки кореспондента обміну. Код кінцевої точки кореспондента обміну %1.'");
			ПредставлениеОшибки = СтрШаблон(ПредставлениеОшибки,
				Тело.RecipientId);
		...
		Если ПодключенныйКорреспондентКод <> Тело.RecipientId Тогда
			...
			ПредставлениеОшибки = НСтр("ru='Ошибка при подключении конечной точки корреспондента обмена.
                ...
                |;uk='Помилка при підключенні кінцевої точки кореспондента обміну.
                |Настройки підключення веб-сервісу не відповідають очікуваним.
                |Код очікуваної кінцевої точки кореспондента обміну %1.
                |Код підключеної кінцевої точки кореспондента обміну %2.'");
			ПредставлениеОшибки = СтрШаблон(ПредставлениеОшибки,
				Тело.RecipientId,
	...
	Иначе // Обновляем настройки подключения этой конечной точки и корреспондента
		...
		Если Отказ Тогда // Отправляем сообщение в менеджер сервиса об ошибке
			...
			ПредставлениеОшибки = НСтр("ru='Ошибка обновления параметров подключения этой конечной точки и конечной точки корреспондента обмена.
                ...
                |;uk='Помилка оновлення параметрів підключення цієї кінцевої точки і кінцевої точки кореспондента обміну.
                |Код цієї кінцевої точки %1
                |Код кінцевої точки кореспондента обміну %2'");
			ПредставлениеОшибки = СтрШаблон(ПредставлениеОшибки,
				ОбменСообщениямиВнутренний.КодЭтогоУзла(),
				Тело.RecipientId);
...
Процедура УстановитьНастройкиТранспорта(Сообщение, Отправитель)
	...
	Если Корреспондент.Пустая() Тогда
		СтрокаСообщения = НСтр("ru='Не найдена конечная точка корреспондента с кодом ""%1"".';uk='Не знайдена кінцева точка кореспондента з кодом ""%1"".'");
...
Функция СобытиеЖурналаРегистрацииПодключениеКорреспондента()
	...
	Возврат НСтр("ru='Обмен данными.Подключение корреспондента обмена';uk='Обмін даними.Підключення кореспондента обміну'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
