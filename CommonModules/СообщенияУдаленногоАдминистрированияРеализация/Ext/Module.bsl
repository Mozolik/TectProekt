...
Процедура УдалитьОбластьДанных(Знач КодОбластиДанных) Экспорт
	...
	Попытка
		...
		Если НЕ МенеджерЗаписи.Выбран() Тогда
			ШаблонСообщения = НСтр("ru='Область данных %1 не существует.';uk='Область даних %1 не існує.'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, КодОбластиДанных);
			ВызватьИсключение(ТекстСообщения);
		КонецЕсли;
	...
	Попытка
		...
		Для каждого ОписаниеПользователя Из Параметры.UsersList.Item Цикл
			...
			ПользовательОбъект.ДополнительныеСвойства.Вставить("СозданиеАдминистратора",
				НСтр("ru='Создание администратора области данных из менеджера сервиса.';uk='Створення адміністратора області даних менеджера сервісу.'"));
...
Функция ПолучитьЯзыкПоКоду(Знач КодЯзыка)
	...
	Если ЗначениеЗаполнено(КодЯзыка) Тогда
		...
		ШаблонСообщения = НСтр("ru='Неподдерживаемый код языка: %1';uk='Код мови, що не підтримується: %1'");
	...
	Если ЗначениеЗаполнено(АдресЭлектроннойПочты) Тогда
		...
		Исключение
			ШаблонСообщения = НСтр("ru='Указан некорректный адрес электронной почты: %1
                |Ошибка: %2'
                |;uk='Вказано неправильну адресу електронної пошти: %1
                |Помилка: %2'");
	...
	Если ПолучитьЧасовойПоясИнформационнойБазы() <> ЧасовойПояс Тогда
		...
		Иначе
			...
			Исключение
				...
				ШаблонСообщения = НСтр("ru='Не удалось заблокировать область данных для установки часового пояса ""%1""';uk='Не вдалося заблокувати область даних для встановлення часового поясу ""%1""'");
