...
Функция Параметры() Экспорт
	...
	Если НЕ СохраненныеПараметры.Свойство("ВозможныеПраваДляНастройкиПравОбъектов") Тогда
		ПредставлениеПараметра = НСтр("ru='Возможные права для настройки прав объектов';uk='Можливі права для настройки прав об''єктів'");
		
	ИначеЕсли НЕ СохраненныеПараметры.Свойство("ПоставляемыеПрофилиГруппДоступа") Тогда
		ПредставлениеПараметра = НСтр("ru='Поставляемые профили групп доступа';uk='Профілі груп доступу, що поставляються'");
		
	ИначеЕсли НЕ СохраненныеПараметры.Свойство("ПредопределенныеПрофилиГруппДоступа") Тогда
		ПредставлениеПараметра = НСтр("ru='Предопределенные профили групп доступа';uk='Напередвизначені профілі груп доступу'");
		
	ИначеЕсли НЕ СохраненныеПараметры.Свойство("СвойстваВидовДоступа") Тогда
		ПредставлениеПараметра = НСтр("ru='Свойства видов доступа';uk='Властивості видів доступу'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПредставлениеПараметра) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка обновления информационной базы.
                           ...
                           |Не заповнений параметр обмеження доступу:
                           |""%1"".'"),
				ПредставлениеПараметра)
	...
	Для НомерСтроки = 1 По СтрЧислоСтрок(ОграниченияПрав) Цикл
		...
		Если ЗначениеЗаполнено(ТекущаяСтрока) Тогда
			...
			Если СтрЧислоВхождений(ТекущаяСтрока, ".") <> 3 И СтрЧислоВхождений(ТекущаяСтрока, ".") <> 5 Тогда
				ПояснениеОшибки = НСтр("ru='Строка должна быть в формате ""<Полное имя таблицы>.<Имя права>.<Имя вида доступа>[.Таблица объекта]"".';uk='Рядок повинна бути у форматі ""<Повне ім''я таблиці>.<Ім''я права>.<Ім''я виду доступу>[.Таблиця об''єкта]"".'");
			Иначе
				...
				Если Метаданные.НайтиПоПолномуИмени(Таблица) = Неопределено Тогда
					ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не найдена таблица ""%1"".';uk='Не знайдена таблиця ""%1"".'"), Таблица);
				
				ИначеЕсли Право <> "Чтение" И Право <> "Изменение" Тогда
					ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не найдено право ""%1"".';uk='Не знайдене право ""%1"".'"), Право);
				
				ИначеЕсли ВРег(ВидДоступа) = ВРег("Объект") Тогда
					Если Метаданные.НайтиПоПолномуИмени(ТаблицаОбъекта) = Неопределено Тогда
						ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Не найдена таблица объекта ""%1"".';uk='Не знайдена таблиця об''єкта ""%1"".'"),
							ТаблицаОбъекта);
					Иначе
				...
				ИначеЕсли ВРег(ВидДоступа) = ВРег("НастройкиПрав") Тогда
					Если Метаданные.НайтиПоПолномуИмени(ТаблицаОбъекта) = Неопределено Тогда
						ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Не найдена таблица владельца настроек прав ""%1"".';uk='Не знайдена таблиця власника настройок прав ""%1"".'"),
							ТаблицаОбъекта);
					Иначе
				...
				ИначеЕсли ВидыДоступаПоИменам.Получить(ВидДоступа) = Неопределено Тогда
					ПояснениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Не найден вид доступа ""%1"".';uk='Не знайдений вид доступу ""%1"".'"), ВидДоступа);
				Иначе
			...
			Если ЗначениеЗаполнено(ПояснениеОшибки) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Ошибка в строке описания вида ограничений права объекта метаданных:
                                   ...
                                   |
                                   |'
                                   |;uk='Помилка в рядку опису виду обмежень права об''єкта метаданих:
                                   |""%1"".
                                   |
                                   |'"),
						ТекущаяСтрока)
