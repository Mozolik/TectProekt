...
//  Нет.
//
Процедура ПодключитьОбработчикОповещенияОВажныхИОченьВажныхНовостях_ПервыйЗапуск() Экспорт

	ТипЧисло = Тип("Число");

	ОтключитьОбработчикОжидания("ПроверитьВажныеНовостиСВключеннымиНапоминаниями_ПервыйЗапуск");

	Если ОбработкаНовостейКлиентПовтИсп.РазрешенаРаботаСНовостямиТекущемуПользователю() = Истина Тогда
		ИнтервалСекунд = 2;
...
//  Нет.
//
Процедура ПодключитьОбработчикОптимизацииКонтекстныхНовостей() Экспорт
...
Процедура ИзменитьПризнакПрочтенностиНовостей(Знач МассивНовостей, ПризнакПрочтенности = Истина) Экспорт
	...
	Оповестить(
		"Новости. Изменена прочтенность списка новостей",
		ПризнакПрочтенности,
		МассивНовостей);
...
Процедура ИзменитьПризнакУдаленияИзСпискаНовостей(Знач МассивНовостей, Знач ПризнакУдаленияИзСписка = Истина) Экспорт
	...
	Оповестить(
		"Новости. Изменен признак удаления из списка новостей",
...
Процедура ПоказатьКонтекстныеНовости(
	...
	Если ТипЗнч(ПараметрыОткрытияФормы) = ТипСтруктура Тогда
		Если ПараметрыОткрытияФормы.Свойство("ЗаголовокФормы") = Истина
		...
		КонецЕсли;
		Если ПараметрыОткрытияФормы.Свойство("СкрыватьКолонкуДатаПубликации") = Истина
		...
		КонецЕсли;
		Если ПараметрыОткрытияФормы.Свойство("СкрыватьКолонкуПодзаголовок") = Истина
		...
		КонецЕсли;
		Если ПараметрыОткрытияФормы.Свойство("СкрыватьКолонкуЛентаНовостей") = Истина
		...
		КонецЕсли;
		Если ПараметрыОткрытияФормы.Свойство("ПоказыватьПанельНавигации") = Истина
		...
		КонецЕсли;
		Если ПараметрыОткрытияФормы.Свойство("РежимОткрытияОкна") = Истина Тогда
...
Функция ПоказатьНовость(
	...
	Если ПараметрыОткрытияФормы.Свойство("Ключ") = Ложь Тогда
...
Процедура ОбработкаНавигационнойСсылки(НовостьСсылка, Форма, Действие, СписокПараметров) Экспорт
		...
		Для каждого ТекущийПараметр Из СписокПараметров Цикл
			Если ВРег(СокрЛП(ТекущийПараметр.Представление))=ВРег("Навигационная ссылка") Тогда
				...
				Исключение
					...
					лкКомментарий = НСтр("ru='Произошла ошибка при обработке действия %Действие% с параметрами %Параметры%. Информация об ошибке: %ИнформацияОбОшибке%';uk='Сталася помилка при обробці дії %Действие% з параметрами %Параметры%. Інформація про помилку: %ИнформацияОбОшибке%'");
					лкКомментарий = СтрЗаменить(лкКомментарий, "%Действие%", Действие);
					лкКомментарий = СтрЗаменить(лкКомментарий, "%Параметры%", СписокПараметров);
					лкКомментарий = СтрЗаменить(лкКомментарий, "%ИнформацияОбОшибке%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
					ОбработкаНовостейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru='БИП:Новости.Клиент';uk='БІП:Новини.Клієнт'"), // ИмяСобытия
						НСтр("ru='Новости. Клиент. ОбработкаНовостейКлиент. ОбработкаНавигационнойСсылки';uk='Новини. Клієнт. ОбработкаНовостейКлиент. ОбработкаНавигационнойСсылки'"), // ИдентификаторШага
	...
	Иначе
		лкКомментарий = НСтр("ru='Действие %Действие% с параметрами %Параметры% не обрабатывается.';uk='Дія %Действие% з параметрами %Параметры% не обробляється.'");
		лкКомментарий = СтрЗаменить(лкКомментарий, "%Действие%", Действие);
		лкКомментарий = СтрЗаменить(лкКомментарий, "%Параметры%", СписокПараметров);
		ОбработкаНовостейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Клиент';uk='БІП:Новини.Клієнт'"), // ИмяСобытия
...
Процедура ОбработкаНажатияВТекстеНовости(лкНовости, ДанныеСобытия, СтандартнаяОбработка, Форма, ЭлементФормы) Экспорт
	...
	Если (ДанныеСобытия.Anchor <> Неопределено) Тогда
		...
		Если (ТипЗнч(НавигационнаяСсылка) = ТипСтрока) Тогда
			...
			Если (ПустаяСтрока(НавигационнаяСсылка) = Истина) Тогда
...
Процедура ПерейтиПоИнтернетСсылке(ИнтернетСсылка) Экспорт
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		лкКомментарий = НСтр("ru='Произошла ошибка при переходе по ссылке %ИнтернетСсылка%. Информация об ошибке: %ИнформацияОбОшибке%';uk='Сталася помилка при переході по посиланню %ИнтернетСсылка%. Інформація про помилку: %ИнформацияОбОшибке%'");
		лкКомментарий = СтрЗаменить(лкКомментарий, "%ИнтернетСсылка%", ИнтернетСсылка);
		лкКомментарий = СтрЗаменить(лкКомментарий, "%ИнформацияОбОшибке%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		ОбработкаНовостейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Клиент';uk='БІП:Новини.Клієнт'"), // ИмяСобытия
			НСтр("ru='Новости. Клиент. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке';uk='Новини. Клієнт. ОбработкаНовостейКлиент. ПерейтиПоИнтернетСсылке'"), // ИдентификаторШага
			"Ошибка", // УровеньЖурналаРегистрации строкой
			"", // ОбъектМетаданных строкой
...
Процедура КонтекстныеНовости_ОбработкаОповещения(
	...
	ИначеЕсли ИмяСобытия = "Новости. Новость прочтена" Тогда
		...
		Если (ТипЗнч(Форма.Новости) = ТипСтруктура) Тогда
			...
			Если (Форма.Новости.Свойство("Новости") = Истина)
			...
			Если (Форма.Новости.Свойство("Метаданные") = Истина)
					И (Форма.Новости.Свойство("Форма") = Истина) Тогда // Есть идентификаторы метаданных и формы
				ИдентификаторМетаданных = Форма.Новости.Метаданные;
				ИдентификаторФормы      = Форма.Новости.Форма;
				Если (Форма.Новости.Свойство("ПолучатьНовостиНаСервере") = Истина)
						И (Форма.Новости.ПолучатьНовостиНаСервере = Ложь) Тогда // Новости рассчитываются в ПриОткрытии
					// ПараметрыПриложения - глобальная переременная.
			...
			Если (Форма.Новости.Свойство("ХранитьМассивНовостейТолькоНаСервере") = Истина)
					И (Форма.Новости.ХранитьМассивНовостейТолькоНаСервере = Истина) Тогда // Новости хранятся на сервере
				Если (Форма.Новости.Свойство("АдресМассиваНовостей") = Истина)
					...
					Если (Форма.Новости.Свойство("СписокНовостейИзменениеПризнака_Прочтена") = Истина)
		...
		Если (ТипЗнч(Форма.Новости) = ТипСтруктура) Тогда
			...
			Если (Форма.Новости.Свойство("ХранитьМассивНовостейТолькоНаСервере") = Истина)
				...
				Если (Форма.Новости.Свойство("АдресМассиваНовостей") = Истина)
						И (ТипЗнч(Форма.Новости.АдресМассиваНовостей) = ТипСтрока)
						И (НЕ (ПустаяСтрока(Форма.Новости.АдресМассиваНовостей))) Тогда // Новости хранятся на сервере, заполнен адрес хранилища.
					Если (Форма.Новости.Свойство("СписокНовостейИзменениеПризнака_ОповещениеВключено") = Истина)
...
Процедура КонтекстныеНовости_ПриОткрытии(Форма) Экспорт
	...
	Если (ТипЗнч(Форма.Новости) = ТипСтруктура)
			И (
				((Форма.Новости.Свойство("КоличествоОченьВажныхНовостейПриОткрытии") = Истина)
					И (Форма.Новости.КоличествоОченьВажныхНовостейПриОткрытии > 0))
				ИЛИ ((Форма.Новости.Свойство("КоличествоВажныхНовостейПриОткрытии") = Истина)
					И (Форма.Новости.КоличествоВажныхНовостейПриОткрытии > 0))
				ИЛИ ((Форма.Новости.Свойство("ПолучатьНовостиНаСервере") = Истина)
					И (Форма.Новости.ПолучатьНовостиНаСервере = Ложь))
			) Тогда
			...
			Иначе
				...
				Если НайденноеЗначениеДляФормы.Свойство("ДатаЗаполнения")
					...
					Если НайденноеЗначениеДляФормы.Свойство("Новости")
							...
							И НайденноеЗначениеДляФормы.Свойство("МассивСтруктурПостоянныхНовостей")
							Тогда
						// Ничего не делать.
					Иначе
						РассчитатьДляФормы = Истина;
				...
				Для Каждого лкИмяФормы Из лкИмяМетаданных.Значение Цикл
					Если ТипЗнч(лкИмяФормы.Значение) = ТипСтруктура Тогда
						Если лкИмяФормы.Значение.Свойство("ДатаЗаполнения")
								...
								И лкИмяФормы.Значение.ДатаЗаполнения >= (ТекущаяДата() - 1*60*60)
								Тогда
		...
		Если НайденноеДействие <> Неопределено Тогда
			...
			ИначеЕсли НайденноеДействие.Значение = "Предупреждение" Тогда
				...
				Если ТипЗнч(Текст) = Тип("ЭлементСпискаЗначений") Тогда
					...
					Заголовок = ОбработкаНовостейКлиентСервер.НайтиПоПредставлению(СписокПараметров, "Заголовок");
					Если ТипЗнч(Заголовок) = Тип("ЭлементСпискаЗначений") Тогда
						Заголовок = Заголовок.Значение;
					Иначе
						Заголовок = НСтр("ru='Информация';uk='Інформація'");
					КонецЕсли;
					ПоказатьПредупреждение(
						,
						Текст,
