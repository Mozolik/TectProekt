...
Функция ИнициализироватьПечатнуюФормуOOWriter(Знач Шаблон = Неопределено) Экспорт
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
			НСтр("ru='Ошибка при связи с сервис менеджером (com.sun.star.ServiceManager).';uk='Помилка при зв''язку із сервіс менеджером (com.sun.star.Servicemanager).'")
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(), "Ошибка",
			НСтр("ru='Ошибка при запуске сервиса Desktop (com.sun.star.frame.Desktop).';uk='Помилка при запуску сервісу Desktop (com.sun.star.frame.Desktop).'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),,Истина);
		НеУдалосьСформироватьПечатнуюФорму(ИнформацияОбОшибке());
	КонецПопытки;
...
Функция ПолучитьПозициюНачалаОбласти(Знач xDocument, Знач ИмяОбласти)
	...
	Если xFound = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Не найдено начало области макета:';uk='Не знайдений початок області макета:'") + " " + ИмяОбласти;	
	КонецЕсли;
	Возврат xFound.End;
...
Функция ПолучитьПозициюОкончанияОбласти(Знач xDocument, Знач ИмяОбласти)
	...
	xFound = xDocument.findFirst(xSearchDescr);
	Если xFound = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Не найден конец области макета:';uk='Не знайдений кінець області макета:'") + " " + ИмяОбласти;	
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрации()
	Возврат НСтр("ru='Печать';uk='Друк'");
КонецФункции

Процедура НеУдалосьСформироватьПечатнуюФорму(ИнформацияОбОшибке)
#Если ВебКлиент Тогда
	ТекстУточнения = НСтр("ru='При работе через веб, требуется браузер Internet Explorer под управлением операционной системы Windows.';uk='При роботі через веб, потрібен браузер Internet Explorer під управлінням операційної системи Windows.'");
#Иначе		
	ТекстУточнения = "";	
#КонецЕсли
	ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось сформировать печатную форму: %1. 
            ...
            |;uk='Не вдалося сформувати друковану форму: %1. 
            |Для виводу друкованих форм у форматі Openoffice.org Writer потрібно, щоб на комп''ютері був установлений пакет Openoffice.org. %2'"),
