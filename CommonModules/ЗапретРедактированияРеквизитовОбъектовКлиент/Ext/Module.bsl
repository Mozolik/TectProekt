...
Процедура ПроверитьСсылкиНаОбъект(Знач ОбработкаПродолжения, Знач МассивСсылок, Знач СинонимыРеквизитов) Экспорт
	...
	ЗаголовокДиалога = НСтр("ru='Разрешение редактирования реквизитов';uk='Дозвіл редагування реквізитів'");
	...
	Если СинонимыРеквизитов.Количество() > 1 Тогда
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для того чтобы не допустить рассогласования данных в программе,
                       ...
                       |;uk='Для того щоб не допустити неузгодженості даних в програмі,
                       |наступні реквізити не доступні для редагування:
                       |%1.
                       |
                       |Перед тим, як дозволити їх редагування, рекомендується оцінити наслідки,
                       |перевіривши всі місця використання цього елемента в програмі.
                       |Пошук місць використання може зайняти тривалий час.'"),
			РеквизитыПредставление);
	Иначе
		Если МассивСсылок.Количество() = 1 Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для того чтобы не допустить рассогласования данных в программе,
                           ...
                           |Поиск мест использования может занять длительное время.'
                           |;uk='Для того щоб не допустити неузгодженості даних в програмі,
                           |реквізит %1 не доступний для редагування.
                           |
                           |Перед тим, як дозволити його редагування, рекомендується оцінити наслідки,
                           |перевіривши всі місця використання ""%2"" в програмі.
                           |Пошук місць використання може зайняти тривалий час.'"),
		...
		Иначе
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для того чтобы не допустить рассогласования данных в программе,
                           ...
                           |
                           |Перед тим, як дозволити його редагування, рекомендується оцінити наслідки,
                           |перевіривши всі місця використання вибраних елементів (%2) у програмі.
                           |Пошук місць використання може зайняти тривалий час.'"),
	...
	Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Проверить и разрешить';uk='Перевірити й дозволити'"));
...
Процедура ПоказатьПредупреждениеВсеВидимыеРеквизитыРазблокированы(ОбработкаПродолжения = Неопределено) Экспорт
	...
	ПоказатьПредупреждение(ОбработкаПродолжения,
		НСтр("ru='Редактирование всех видимых реквизитов объекта уже разрешено.';uk='Редагування всіх видимих реквізитів об''єкта вже дозволено.'"));
