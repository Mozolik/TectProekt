...
Функция ОбработатьПараметрыЗапуска(Знач ЗначениеПараметраЗапуска, Знач ПараметрыЗапуска)
	...
	Если ЗначениеПараметраЗапуска = ВРег("РазрешитьРаботуПользователей") Тогда
		...
		Если НЕ СоединенияИБВызовСервера.РазрешитьРаботуПользователей() Тогда
			ТекстСообщения = НСтр("ru='Параметр запуска РазрешитьРаботуПользователей не отработан. Нет прав на администрирование информационной базы.';uk='Параметр запуску РазрешитьРаботуПользователей не відпрацьований. Немає прав на адміністрування інформаційної бази.'");
			ПоказатьПредупреждение(,ТекстСообщения);
			Возврат Ложь;
		КонецЕсли;
		
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СоединенияИБКлиентСервер.СобытиеЖурналаРегистрации(),,
			НСтр("ru='Выполнен запуск с параметром ""РазрешитьРаботуПользователей"". Работа программы будет завершена.';uk='Виконано запуск із параметром ""РазрешитьРаботуПользователей"". Робота програми буде завершена.'"), ,Истина);
	...
	ИначеЕсли ЗначениеПараметраЗапуска = ВРег("ЗавершитьРаботуПользователей") Тогда
		...
		Если Не СоединенияИБВызовСервера.УстановитьБлокировкуСоединений() Тогда
			ТекстСообщения = НСтр("ru='Параметр запуска ЗавершитьРаботуПользователей не отработан. Нет прав на администрирование информационной базы.';uk='Параметр запуску ЗавершитьРаботуПользователей не відпрацьований. Немає прав на адміністрування інформаційної бази.'");
			ПоказатьПредупреждение(,ТекстСообщения);
			Возврат Ложь;
		КонецЕсли;
...
Процедура ЗавершитьРаботуЭтогоСеанса(ВыводитьВопрос = Истина) Экспорт
	...
	Заголовок = НСтр("ru='Завершение работы текущего сеанса';uk='Завершення роботи поточного сеансу'");
...
Процедура ИнтерактивнаяОбработкаПередНачаломРаботыСистемы(Параметры, Неопределен) Экспорт
	...
	Если Не ПустаяСтрока(ТекстВопроса) Тогда
		...
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Войти';uk='Увійти'"));
		Если ПараметрыКлиента.ВозможноСнятьБлокировку Тогда
			Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ru='Снять блокировку и войти';uk='Зняти блокування й увійти'"));
		КонецЕсли;
