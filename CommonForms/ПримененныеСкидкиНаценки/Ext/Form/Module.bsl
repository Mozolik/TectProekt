...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Скидка предоставлется на %1", НоменклатураКлиентСервер.ПредставлениеНоменклатуры(Параметры.ТекущиеДанные.Номенклатура, Параметры.ТекущиеДанные.Характеристика)));
		...
		Иначе
			...
			Для каждого СтрокаУсловие Из СтрокаДерева.ПараметрыУсловий.ТаблицаУсловий Цикл
				...
				Если СтрокаУсловие.ЗначениеПоказателя <> Неопределено Тогда
					НоваяСтрокаУсловие.Представление = СтрШаблон(НСтр("ru='%1 (Текущее значение: %2)';uk='%1 (Поточне значення: %2)'"), Строка(СтрокаУсловие.УсловиеПредоставления), Формат(СтрокаУсловие.ЗначениеПоказателя, "ЧН=0"));
				Иначе
