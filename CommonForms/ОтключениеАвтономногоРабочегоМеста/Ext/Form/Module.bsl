...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(АвтономноеРабочееМесто) Тогда
		ВызватьИсключение НСтр("ru='Не задано автономное рабочее место.';uk='Не встановлене автономне робоче місце.'");
	КонецЕсли;
...
Процедура ОбработчикОжиданияДлительнойОперации()
	...
	Исключение
		...
		ДлительнаяОперация = Ложь;
		ПерейтиНазад();
		ПоказатьПредупреждение(,НСтр("ru='В процессе работы возникли ошибки.';uk='У процесі роботи виникли помилки.'"));
...
Процедура УдалитьАвтономноеРабочееМесто(Отказ)
	...
	Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
					...
					НСтр("ru='Удаление автономного рабочего места';uk='Вилучення автономного робочого місця'"));
