...
Процедура ОтобразитьТекущееСообщение()
	...
	Если ЗначениеЗаполнено(СтрокаТЧ.ВидКарты) Тогда
		Если ЗначениеЗаполнено(КартаЛояльности) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Заменить карту лояльности ""%1"" на карту ""%2""';uk='Замінити картку лояльності ""%1"" на карту ""%2""'"), ВидЗаменяемойКартыЛояльности, СтрокаТЧ.ВидКарты);
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выдать карту лояльности ""%1""';uk='Видати карту лояльності ""%1""'"), СтрокаТЧ.ВидКарты);
		КонецЕсли;
	Иначе
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сообщение %1 из %2';uk='Повідомлення %1 з %2'"), НомерСообщения, ВсегоСообщений);
