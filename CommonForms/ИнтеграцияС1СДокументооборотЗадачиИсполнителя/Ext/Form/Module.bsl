...
Процедура ОбработатьФормуСогласноВерсииСервиса()
	...
	Если ПустаяСтрока(ВерсияСервиса) Тогда // идет подключение
		...
		Заголовок = НСтр("ru='Документооборот: Мои задачи (ожидается подключение...)';uk='Документообіг: Мої задачі (очікується підключення...)'");
		
	Иначе
		
		Заголовок = НСтр("ru='Документооборот: Мои задачи';uk='Документообіг: Мої задачі'");
		
		Если ВерсияСервиса = "0.0.0.0" Тогда
		
			Элементы.ГруппаФункционалНеПоддерживается.Видимость = Истина;
			Элементы.ГруппаПроверкаАвторизации.Видимость = Истина;
			Элементы.ДекорацияФункционалНеПоддерживается.Заголовок = НСтр("ru='Нет доступа к Документообороту.';uk='Немає доступу до Документообігу.'");
