...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СписокПлановОбмена.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='Возможность настройки синхронизации данных не предусмотрена.';uk='Можливість настройки синхронізації даних не передбачена.'");
...
Процедура СписокСостоянияУзловВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru='Выполнить синхронизацию данных?';uk='Виконати синхронізацію даних?'"), РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
Процедура ОбновитьКоличествоСинхронизаций(КоличествоНастроенныхОбменов)
	...
	Если КоличествоСинхронизаций = 0 Тогда
		...
		Заголовок = НСтр("ru='Синхронизация данных';uk='Синхронізація даних'");
	...
	Иначе
		...
		Заголовок = НСтр("ru='Список настроенных синхронизаций данных';uk='Список налаштованих синхронізацій даних'");
...
Процедура УстановитьЭлементыОднойСинхронизации()
	...
	Если НастроеннаяСинхронизация.УзелИнформационнойБазы = Неопределено Тогда
		...
		ВызватьИсключение НСтр("ru='Работа монитора синхронизации данных в неразделенном сеансе не поддерживается';uk='Робота монітора синхронізації даних у нерозділеному сеансі не підтримується'");
	...
	Если НастроеннаяСинхронизация.ДатаПоследнейУспешнойВыгрузки <> НастроеннаяСинхронизация.ДатаПоследнейВыгрузки
		...
		ПодсказкаВыгрузки = НСтр("ru='Данные отправлены: %ДатаОтправки%
                                        ...
                                        |;uk='Дані відправлені: %ДатаОтправки%
                                        |Остання спроба: %ДатаПопытки%'");
	...
	Если НастроеннаяСинхронизация.ДатаПоследнейУспешнойЗагрузки <> НастроеннаяСинхронизация.ДатаПоследнейЗагрузки
		...
		ПодсказкаЗагрузки = НСтр("ru='Данные получены: %ДатаПолучения%
                                        |Последняя попытка: %ДатаПопытки%'
                                        |;uk='Дані отримані: %ДатаПолучения%
                                        |Остання спроба: %ДатаПопытки%'");
	...
	Иначе
		...
		РасписаниеСинхронизацииДанных = НСтр("ru='Расписание синхронизации не настроено';uk='Розклад синхронізації не настроєне'");;
			...
			Если СписокВозможныхНастроекОбмена.Количество() > 0 Тогда
				...
				Если МенеджерПланаОбмена.КорреспондентВМоделиСервиса() Тогда
					...
					Для Каждого ПредопределеннаяНастройка Из СписокВозможныхНастроекОбмена Цикл
						...
						СоздатьКомандуИЭлементФормы(НаименованиеКоманды, ЗаголовокКоманды + " " + НСтр("ru='(в сервисе)';uk='(в сервісі)'"), СуммирующаяГруппа);
			...
			Иначе
				...
				Если МенеджерПланаОбмена.КорреспондентВМоделиСервиса() Тогда
					СоздатьКомандуИЭлементФормы(НаименованиеКоманды + "КорреспондентВМоделиСервиса", 
					                            ЗаголовокКоманды + " " + НСтр("ru='(в сервисе)';uk='(в сервісі)'"),
					                            РодительскаяГруппа);
				КонецЕсли;
...
Процедура ПроверитьСовместимостьПравилКонвертации(Знач ИмяПланаОбмена, ОбработкаПродолжения)
	...
	Иначе
		...
		Кнопки.Добавить("ПерейтиКЗагрузкеПравил", НСтр("ru='Загрузить правила';uk='Завантажити правила'"));
		...
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			ПараметрыФормы.Заголовок = НСтр("ru='Синхронизация данных не может быть выполнена';uk='Синхронізація даних не може бути виконана'");
		Иначе
			ПараметрыФормы.Заголовок = НСтр("ru='Синхронизация данных может быть выполнена некорректно';uk='Синхронізація даних може бути виконана некоректно'");
		КонецЕсли;
...
Функция ПравилаКонвертацииСовместимыСТекущейВерсией(ИмяПланаОбмена, ОписаниеОшибки)
	...
	Если ИмяКонфигурацииИзПравил <> ИмяКонфигурацииИнформационнойБазы Тогда
			...
			ОписаниеОшибки.Вставить("ТекстОшибки", НСтр("ru='Синхронизация данных не может быть выполнена, так как используются правила, предназначенные для программы ""%1"". Следует использовать правила из конфигурации или загрузить корректный комплект правил из файла.';uk='Синхронізація даних не може бути виконана, так як використовуються правила, призначені для програми ""%1"". Слід використовувати правила з конфігурації або завантажити коректний комплект правил з файлу.'"));
		...
		Если РезультатСравнения <> 0 Тогда
			...
			Если РезультатСравнения < 0 Тогда
				...
				ТекстОшибки = НСтр("ru='Синхронизация данных может быть выполнена некорректно, так как используются правила, предназначенные для предыдущей версии программы ""%1"" (%2). Рекомендуется использовать правила из конфигурации или загрузить комплект правил, предназначенный для текущей версии программы (%3).';uk='Синхронізація даних може бути виконана некоректно, так як використовуються правила, призначені для попередньої версії програми ""%1"" (%2). Рекомендується використовувати правила з конфігурації або завантажити комплект правил, призначений для поточної версії програми (%3).'");
				ВидОшибки = "УстаревшаяВерсияКонфигурации";
				
			Иначе
				
				ТекстОшибки = НСтр("ru='Синхронизация данных может быть выполнена некорректно, так как используются правила, предназначенные для более новой версии программы ""%1"" (%2). Рекомендуется обновить версию программы или использовать комплект правил, предназначенный для текущей версии программы (%3).';uk='Синхронізація даних може бути виконана некоректно, так як використовуються правила, призначені для більш нової версії програми ""%1"" (%2). Рекомендується оновити версію програми або використовувати комплект правил, призначений для поточної версії програми (%3).'");
