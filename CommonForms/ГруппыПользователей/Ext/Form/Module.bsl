...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Пользователь <> Неопределено Тогда
		...
		Элементы.ФормаЗаписатьИЗакрыть.Заголовок = НСтр("ru='Записать';uk='Записати'");
	...
	Если КоличествоПользователей = 0 Тогда
		ВызватьИсключение НСтр("ru='Не выбрано ни одного пользователя.';uk='Не обрано жодного користувача.'");
	КонецЕсли;
	...
	Для Каждого ПользовательИзМассива Из МассивПользователей Цикл
		...
		Если ТипПользователей <> ТипПользователяИзМассива Тогда
			ВызватьИсключение НСтр("ru='Команда не может быть выполнена сразу для двух разных видов пользователей.';uk='Команда не може бути виконана відразу для двох різних видів користувачів.'");
		КонецЕсли;
	КонецЦикла;
	...
	Если КоличествоПользователей > 1
	   И Параметры.Пользователь = Неопределено Тогда
		...
		Заголовок = НСтр("ru='Группы пользователей';uk='Групи користувачів'");
	...
	Если УведомлениеПользователя.ЕстьОшибки = Ложь Тогда
		Если УведомлениеПользователя.Сообщение <> Неопределено Тогда
			ПоказатьОповещениеПользователя(
				НСтр("ru='Перемещение пользователей';uk='Переміщення користувачів'"), , УведомлениеПользователя.Сообщение, БиблиотекаКартинок.Информация32);
		КонецЕсли;
	Иначе
...
Процедура СформироватьТекстСообщения(МассивПеремещенныхПользователей, УведомлениеПользователя, НеПеремещенныеПользователи)
		...
		Если КоличествоНеПеремещенныхПользователей = 1 Тогда
			...
			СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Пользователя ""%1"" не удалось включить в выбранные группы,
                           ...
                           |;uk='Користувача ""%1"" не вдалося включити в обрані групи,
                           |так як у них розрізняється тип або у груп встановлена ознака ""Всі користувачі заданого типу"".'"),
				Предмет);
		Иначе
			Предмет = Формат(КоличествоНеПеремещенныхПользователей, "ЧДЦ=0") + " "
				+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоНеПеремещенныхПользователей,
					"Л = ru_RU", НСтр("ru='пользователю,пользователям,пользователям,,,,,,0';uk='користувачеві,користувачам,користувачам,,,,,,0'"));
			СообщениеПользователю =
				НСтр("ru='Не всех пользователей удалось включить в выбранные группы,
                           |т.к. у них различается тип или у групп установлен признак ""Все пользователи заданного типа"".'
                           |;uk='Не всіх користувачів удалося включити в обрані групи,
                           |т.к. у них розрізняється тип або в груп установлена ознака ""Всі користувачі заданого типу"".'");
			Для Каждого НеПеремещенныйПользователь Из НеПеремещенныеПользователи Цикл
			...
			УведомлениеПользователя.ПолныйТекстСообщения =
				НСтр("ru='Следующие пользователи не были включены в группы:';uk='Наступні користувачі не були включені в групи:'")
	...
	ИначеЕсли КоличествоПользователей = 1 Тогда
		...
		УведомлениеПользователя.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Изменен состав групп у пользователя ""%1""';uk='Змінено склад груп у користувача ""%1""'"),
	...
	ИначеЕсли КоличествоПользователей > 1 Тогда
		СтрокаОбъект = Формат(КоличествоПользователей, "ЧДЦ=0") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоПользователей,
				"Л = ru_RU", НСтр("ru='пользователя,пользователей,пользователей,,,,,,0';uk='користувача,користувачів,користувачів,,,,,,0'"));
		
		УведомлениеПользователя.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Изменен состав групп у %1';uk='Змінено склад груп в %1'"), СтрокаОбъект);
	КонецЕсли;
...
Процедура ЗаписатьИЗакрытьОбработкаВопроса(Ответ, Отчет) Экспорт
	...
	Если Ответ = "Ок" Тогда
		Возврат;
	Иначе
		Отчет.Показать(НСтр("ru='Пользователи, не включенные в группы';uk='Користувачі, не включені в групи'"));
		Возврат;
	КонецЕсли;
