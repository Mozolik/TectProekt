...
Процедура ОтменитьЗаданиеПодготовки(Знач ИдентификаторЗадания)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Отмена выполнения задания подготовки выгрузки области данных';uk='Скасування виконання завдання підготовки вивантаження області даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбработатьОшибку(Знач КраткоеПредставление, Знач ПодробноеПредставление)
	...
	ШаблонИсключения = НСтр("ru='При выгрузке данных произошла ошибка: %1.
                             ...
                             |
                             |Розширена інформація для служби підтримки записана в журнал реєстрації. Якщо Вам невідома причина помилки - рекомендується звернутися в службу технічної підтримки, надавши для розслідування інформаційну базу і вивантаження журналу реєстрації.'");
