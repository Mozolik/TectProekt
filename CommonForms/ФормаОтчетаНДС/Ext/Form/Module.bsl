...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//В отчетах (некоторые отчеты по НДС), которые используют эту общую форму ФормаОтчетаНДС, могут быть определены дополнительные действия 
	//для обработчиков форм отчета
	Если Параметры.Свойство("События") Тогда
		СобытияПолученные = Параметры.События;
		//Проверим какие события будут обрабатываться для конкретного отчета
		СобытияПоУмолчанию = НастройкиОтчета.События;
		Для каждого Событие Из СобытияПоУмолчанию Цикл
			Если СобытияПолученные.Свойство(Событие.Ключ) Тогда
				СобытияПоУмолчанию[Событие.Ключ] = СобытияПолученные[Событие.Ключ];
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	//А действие ПриСозданииНаСервере определенно для всех отчетов, которые используют эту общую форму
...
Процедура АктуализироватьРасчеты(Команда)
	...
	ПоказатьВопрос(Оповещение, ТекстВопросаАктуализацииРасчетов, РежимДиалогаВопрос.ДаНет);
...
Процедура АктуализироватьРасчетыПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(
		...
		, // НавигационнаяСсылка
		ТекстПоясненияПоОкончаниюАктуализацииРасчетов,
...
Процедура АктуализироватьРасчетыСервер()
	...
	Если АктуализироватьРасчетыСКлиентами Тогда
