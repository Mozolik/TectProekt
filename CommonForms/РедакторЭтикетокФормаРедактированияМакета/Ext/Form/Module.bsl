...
&НаКлиенте
Перем ОдинМиллиметрШирины; // Колличество у.е. измерения ширины, которое содержится в одном миллиметре при текущем разрешении.
&НаКлиенте
Перем ОдинМиллиметрВысоты; // Колличество у.е. измерения высоты, которое содержится в одном миллиметре при текущем разрешении.
&НаКлиенте
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗакрытьФорму Тогда
		...
		Если МакетИзменен Тогда
			Отказ = Истина;
		    ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект), НСтр("ru='Закрыть без сохранения?';uk='Закрити без збереження?'"), РежимДиалогаВопрос.ДаНет);
		КонецЕсли;
...
Процедура ШагСеткиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение<>ШагСетки Тогда
		...
		Если УпраздняемыеПоля.Количество()>0 Тогда
			...
			СтрокаВопрос = НСтр("ru='Поля:%Поля%имеют слишком маленькие размеры.%ПереносСтроки%Продолжить изменение шага сетки, удалив указанные поля?';uk='Поля:%Поля%мають дуже маленькі розміри.%ПереносСтроки%Продовжити зміну кроку сітки, вилучивши зазначені поля?'");
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	Если Поля.Количество()>0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗагрузитьИзФайлаЗавершение", ЭтотОбъект), НСтр("ru='Загрузить макет этикетки из файла?';uk='Завантажити макет етикетки з файлу?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
		ВыборФайла();
	КонецЕсли;
...
Процедура СохранитьВФайл(Команда)
	...
	ДОФ.Фильтр = НСтр("ru='XML-файл';uk='XML-файл'") + "(*.xml)|*.xml";
	ДОФ.МножественныйВыбор = Ложь;
	ДОФ.Заголовок = НСтр("ru='Сохранение файла-макета';uk='Збереження файлу-макету'");
...
Процедура ОчиститьМакет(Команда)
	...
	Если Поля.Количество()>0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОчиститьМакетЗавершение", ЭтотОбъект), НСтр("ru='Очистить макет?';uk='Очистити макет?'"), РежимДиалогаВопрос.ДаНет);
	Иначе
		ОчисткаМакета();
		МакетИзменен = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьПоДаннымМакета(ЧтениеИзФайла = Ложь)
	...
	Если МенеджерОборудованияВызовСервера.ПолучитьСтруктуруСОписаниемМакета(Источник, СообщениеОбОшибке, СодержимоеXMLФайла) Тогда
				...
				Если ТекПоле.Тип<>"UserData" Тогда
					...
					Если ГоризонтальноеПересечение Тогда
						...
						Если ВертикальноеПересечение Тогда
							...
							ПересекаемыеПоля.Добавить(Новый Структура("Верх, Лево, Низ, Право, Наименование, Представление", ТекПоле.Верх, ТекПоле.Лево, ТекПоле.Низ, ТекПоле.Право, ТекПоле.Наименование, ПредставлениеПоля));
...
Процедура ВыборФайла()
	...
	ДОФ.Фильтр = НСтр("ru='XML-файл';uk='XML-файл'") + "(*.xml)|*.xml";
	ДОФ.МножественныйВыбор = Ложь;
	ДОФ.Заголовок = НСтр("ru='Выберите файл макета';uk='Виберіть файл макету'");
...
Процедура СохранитьОформлениеПоля(ОписаниеПоля, ОткрытиеМакета=Ложь, ТабличныйДокумент=Неопределено)
	...
	#Если ВебКлиент Тогда
		...
		Если ТипЗнч(ОписаниеПоля)<>Тип("Структура") Тогда
			ОписаниеПоля = ПолучитьСтруктуруСоЗначениямиПолей(ОписаниеПоля);
		КонецЕсли;
...
&НаКлиенте
Функция ПолучитьСтруктуруСоЗначениямиПолей(ОписаниеПоля)
...
Процедура ПредложитьПользователюПродолжитьРедактирование(ОписаниеПоля, РедактируемоеПоле)
	...
	ПоказатьВопрос(Новый ОписаниеОповещения("ПредложитьПользователюПродолжитьРедактированиеЗавершение", ЭтотОбъект, ПередаваемыеПараметры),
					НСтр("ru='Запись невозможна! Текущее значение реквизита ""Имя поля"" уже используется. Продолжить редактирование?';uk='Запис неможливий! Поточне значення реквізиту ""Ім''я поля"" вже використовується. Продовжити редагування?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОпроситьПользователяОбУдаленииПересекаемыхПолей(ПередаваемыеПараметры)
	...
	Если ПередаваемыеПараметры.Свойство("РедактируемоеПоле") Тогда
		ТекстВопроса = НСтр("ru='Новые границы редактируемого поля пересекаются с полями:%СписокПолей%Отменить внесенные изменения?';uk='Нові межі редагованого поля перетинаються з полями:%СписокПолей%Скасувати внесені зміни?'");
	Иначе
		ТекстВопроса = НСтр("ru='Создаваемое поле пересекается с уже существующими полями:%СписокПолей%Отменить внесенные изменения?';uk='Поле, яке створюється, перетинається з вже існуючими полями:%СписокПолей%Скасувати внесені зміни?'");
	КонецЕсли;
...
// ИндексГраницы	- Число: 0 - нет границ, 1 - левая граница, 2 - верхняя граница, 3 - правая граница, 4 - нижняя граница, 5 - все границы
//
&НаКлиенте
