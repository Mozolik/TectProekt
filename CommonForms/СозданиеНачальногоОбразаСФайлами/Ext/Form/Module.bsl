...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Узел = Менеджер.ЭтотУзел() Тогда
		ВызватьИсключение
			НСтр("ru='Создание начального образа для данного узла невозможно.';uk='Створення початкового образа для даного вузла неможливо.'");
	Иначе
	...
	Если Не СтандартныеПодсистемыВызовСервера.ПараметрыРаботыКлиента().ИнформационнаяБазаФайловая Тогда
		Элементы.ПутьКАрхивуСФайламиТомов.ПодсказкаВвода = НСтр("ru='\\имя сервера\resource\files.zip';uk='\\ім''я сервера\resource\files.zip'");
	...
	Если ВидБазы = 0 И НЕ МожноСоздатьФайловуюБазу Тогда
		ВызватьИсключение
			НСтр("ru='Создание начального образа файловой информационной базы
                       ...
                       |;uk='Створення початкового образу файлової інформаційної бази
                       |на цій платформі не підтримується.'");
	Иначе
		...
		Если ВидБазы = 0 Тогда
			...
			ПараметрыЗадания.Вставить("НаименованиеЗадания", НСтр("ru='Создание файлового начального образа';uk='Створення файлового початкового образу'"));
		...
		Иначе
			...
			ПараметрыЗадания.Вставить("НаименованиеЗадания", НСтр("ru='Создание серверного начального образа';uk='Створення серверного початкового образу'"));
...
Процедура ОбработчикСохраненияФайлаПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	Диалог.Заголовок                = НСтр("ru='Выберите файл для сохранения';uk='Виберіть файл для збереження'");
