...
Процедура КомандаНайтиВыполнить()
	...
	Если ПустаяСтрока(СтрокаПоиска) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не задана строка поиска!';uk='Не заданий рядок пошуку!'"),,"СтрокаПоиска");
		Возврат;
	КонецЕсли;
...
Процедура УстановитьГруппуПоУмолчанию()
	...
	Если ПустаяСтрока(ГруппаПоУмолчанию) Тогда
		ГруппаПоУмолчанию = НСтр("ru='Кому';uk='Кому'");
	КонецЕсли;
