...
Процедура СохранитьИЗакрыть(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Для каждого СтрокаТаблицыКонтактов Из ТаблицаКонтактов Цикл
		...
		Если (НЕ ЗначениеЗаполнено(СтрокаТаблицыКонтактов.Адрес) ИЛИ Не ЗначениеЗаполнено(СтрокаТаблицыКонтактов.Контакт) 
			ИЛИ ВРег(СтрокаТаблицыКонтактов.Адрес) = ВРег(СтрокаТаблицыКонтактов.ТекущийАдресКонтакта)) Тогда
			СтрокаТаблицыКонтактов.Изменять = Ложь;
		КонецЕсли;
		...
		Исключение
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Во время обновления контактной информации %1 произошла ошибка
                    |%2'
                    |;uk='Під час оновлення контактної інформації %1 сталася помилка
