...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ИспользоватьОграниченияРучныхСкидок Тогда
		СкидкаНаценкаНеОграничена = НСтр("ru='Не ограничена';uk='Не обмежена'");
	КонецЕсли;
	...
	Если ИспользоватьОграниченияРучныхСкидок Тогда
		...
		Если ВариантПредоставления = 1 Тогда
			Если СуммаРучнойСкидкиНаценки > МаксимальнаяСуммаСкидкиДляКонтроля Тогда
				ТекстСообщения = НСтр("ru='Сумма скидки превышает максимальное возможное значение:';uk='Сума знижки перевищує максимальне можливе значення:'") + " " + МаксимальнаяСуммаСкидкиДляКонтроля + " " + Валюта;
			КонецЕсли;
		КонецЕсли;
		
		Если ВариантПредоставления = 2 Тогда
			Если СуммаРучнойСкидкиНаценки > МаксимальнаяСуммаНаценкиДляКонтроля Тогда
				ТекстСообщения = НСтр("ru='Сумма наценки превышает максимальное возможное значение:';uk='Сума націнки перевищує максимальне можливе значення:'") + " " + МаксимальнаяСуммаНаценкиДляКонтроля + " " + Валюта;
			КонецЕсли;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ОКЗавершение", ЭтотОбъект), ТекстСообщения + ". " + НСтр("ru='Продолжить?';uk='Продовжити?'"), РежимДиалогаВопрос.ОКОтмена);
            Возврат;
		КонецЕсли;
...
Процедура СуммаДокументаСУчетомСкидкиПодготовитьСписокВыбора()
	...
	Пока ТочностьОкругления <= МаксимальноеТочностьОкругления Цикл
		...
		Если Варианты.Найти(Значение) = Неопределено Тогда
			...
			СоответствиеТочностей.Вставить(Значение, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='(+окр.до %1)';uk='(+окр.до %1)'"), ТочностьОкругления));
		КонецЕсли;
...
Процедура СуммаСкидкиПодготовитьСписокВыбора()
	...
	Пока ТочностьОкругления <= МаксимальноеТочностьОкругления Цикл
		...
		Если Варианты.Найти(Значение) = Неопределено Тогда
			Варианты.Добавить(Значение);
			СоответствиеТочностей.Вставить(Значение, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='(окр.до %1)';uk='(окр.до %1)'"), Значение));
		КонецЕсли;
