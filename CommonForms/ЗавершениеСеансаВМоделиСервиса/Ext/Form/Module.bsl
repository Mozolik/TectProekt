...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Попытка
		...
		Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
			ВызватьИсключение НСтр("ru='Недостаточно прав для завершения сеанса!';uk='Недостатньо прав для завершення сеансу!'");
		КонецЕсли;
		
		НомерСеанса = Параметры.НомерСеанса;
...
&НаКлиенте
Функция НачатьЗавершениеСеанса() Экспорт
	
	Если ПустаяСтрока(Пароль) Тогда
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не указан пароль для доступа к сервису!';uk='Не вказаний пароль для доступу до сервісу!'"), ,
			"Пароль"
		);
	...
	Иначе
		...
		Попытка
			...
			НачатьЗавершениеСеансаНаСервере();
...
&НаСервере
Процедура НачатьЗавершениеСеансаНаСервере()
	...
	Задание = ФоновыеЗадания.Выполнить(
		"УдаленноеАдминистрированиеБТССлужебный.ЗавершитьСеансОбластиДанных",
		ПараметрыЗадания,
		,
		НСтр("ru='Завершение активного сеанса';uk='Завершення активного сеансу'")
	);
...
Функция ФоновоеЗаданиеВыполнено(ИдентификаторЗадания)
	...
	Если Задание = Неопределено Тогда
		...
		ВызватьИсключение НСтр("ru='Фоновое задание не найдено!';uk='Фонове завдання не знайдено!'");
	...
	Иначе
		...
		ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
			...
			ВызватьИсключение НСтр("ru='Фоновое задание отменено администратором!';uk='Фонове завдання скасовано адміністратором!'");
...
Функция ВыполнитьОбработчикПереходаМеждуШагами(Знач Обработчик)
	...
	Если ЗначениеЗаполнено(Обработчик) Тогда
		...
		Попытка
			...
			Если Обработчик = "НачатьЗавершениеСеанса" Тогда
				Результат = НачатьЗавершениеСеанса();
			КонецЕсли;
...
Функция СценарийМастера()
	...
	НоваяСтрока.Обработчик = "НачатьЗавершениеСеанса";
