	...
	Если УсиленнаяЗащитаЗакрытогоКлюча Тогда
		ПоказатьПредупреждение(,
			НСтр("ru='У сертификата установлена усиленная защита закрытого ключа.
                       ...
                       |а платформа должна передать пустой пароль, чтобы не было ошибки.
                       |
                       |Запоминание и запись пароля невозможны.'
                       |;uk='У сертифікату встановлено посилений захист закритого ключа.
                       |У такому разі пароль запитує програма електронного підпису та шифрування,
                       |а платформа повинна передати порожній пароль, щоб не було помилки.
                       |
                       |Запам''ятовування і запис пароля неможливі.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если Не (ЗначениеЗаполнено(Программа) ИЛИ ЗначениеЗаполнено(ПрограммаБанка)) Тогда
		ПоказатьПредупреждение(,
			НСтр("ru='У сертификата не указана программа для закрытого ключа.
                       ...
                       |;uk='У сертифікату не вказана програма для закритого ключа.
                       |Неможливо перевірити пароль перед записом.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
		...
		Иначе
			...
			Если ТипЗнч(Свойства) <> Тип("Структура")
			 ...
			 Или Не ТипЗнч(Свойства.Пароль) = Тип("Строка") Тогда
				...
				ВызватьИсключение НСтр("ru='Введите пароль повторно.';uk='Введіть пароль повторно.'");
			КонецЕсли;
		КонецЕсли;
...
Функция ОписаниеОшибкиПраваДоступа()
	...
	Возврат
		НСтр("ru='Недостаточно прав для записи пароля.
                   ...
                   |;uk='Недостатньо прав для запису пароля.
                   |
                   |Запис пароля може зробити користувач, зазначений в полях
                   |сертифіката ""Користувач"" і ""Додав"" або адміністратор.'");
...
Процедура УстановитьЗаголовокНадписиИПометкуКомандыДоступенВсем()
	...
	Если ЗначениеЗаполнено(Пользователь) Тогда
		Элементы.НадписьДоступенПользователю.Заголовок = НСтр("ru='Пароль доступен пользователю:';uk='Пароль доступний користувачеві:'")
			+ " " + Пользователь;
	Иначе
		Элементы.НадписьДоступенПользователю.Заголовок = НСтр("ru='Пароль доступен всем пользователям.';uk='Пароль доступний всім користувачам.'")
	КонецЕсли;
