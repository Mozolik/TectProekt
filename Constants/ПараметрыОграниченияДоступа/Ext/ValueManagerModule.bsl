...
Функция СвойстваВидовДоступа()
	...
	ЗаголовокОшибки =
		НСтр("ru='Ошибка в процедуре ЗаполнитьСвойстваВидаДоступа
                   ...
                   |
                   |'
                   |;uk='Помилка в процедурі ЗаполнитьСвойстваВидаДоступа
	...
	Для каждого ВидДоступа Из ВидыДоступа Цикл
		...
		Если ВсеИменаВидовДоступа[ВРег(ВидДоступа.Имя)] <> Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Имя вида доступа ""%1"" уже определено.';uk='Ім''я види доступу ""%1"" вже визначене.'"),
				ВидДоступа.Имя);
		КонецЕсли;
	...
	Если Параметры.ОпределяемыеТипыЗначенийДоступа.Получить(
		...
		ОписаниеОшибки =
			НСтр("ru='Тип ПланВидовХарактеристикСсылка.УдалитьВидыДоступа,
                       ...
                       |;uk='Тип ПланВидовХарактеристикСсылка.УдалитьВидыДоступа,
                       |необхідний для переходу на нові версії програми
                       |не вказано в типі ""Значення доступу"", що визначається.'");
	   ...
	   И НЕ ОбщегоНазначения.ЭтоПеречисление(МетаданныеТипаЗначений) Тогда
		...
		ВызватьИсключение Параметры.ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Тип значения доступа ""%1"", использующий группы значений, не указан
                       ...
                       |;uk='Тип значення доступу ""%1"", що використовує групи значень, не вказано
                       |в аналізованому типі ""Значення доступу з групами значень доступу об''єкт"".'"),
			Строка(ТипОбъекта));
	КонецЕсли;
...
Процедура ПроверитьТип(ВидДоступа, Тип, ВсеТипы, Параметры, ПроверкаТиповГрупп = Ложь, ПроверкаПересечения = Ложь)
	...
	Если Тип = Тип("Неопределено") Тогда
		...
		КонецЕсли;
		ВызватьИсключение Параметры.ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для вида доступа ""%1"" не указан тип значений доступа.';uk='Для виду доступу ""%1"" не вказаний тип значень доступу.'"),
			ВидДоступа.Имя);
	КонецЕсли;
		...
		Если ПроверкаТиповГрупп Тогда
			ОписаниеОшибки =
				НСтр("ru='Тип ""%1"" указан, как тип групп значений, для вида доступа ""%2"".
                           ...
                           |;uk='Тип ""%1"" вказано, як тип груп значень, для виду доступу ""%2"".
                           |Однак це не тип посилання.'");
		Иначе
			ОписаниеОшибки =
				НСтр("ru='Тип ""%1"" указан, как тип значений, для вида доступа ""%2"".
                           |Однако это не тип ссылки.'
                           |;uk='Тип ""%1"" вказано, як тип значень, для виду доступу ""%2"".
                           |Однак це не тип посилання.'");
		КонецЕсли;
		ВызватьИсключение Параметры.ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		Если ПроверкаПересечения Тогда
			ОписаниеОшибки =
				НСтр("ru='Тип ""%1"" указан, как тип значений, для вида доступа ""%2"".
                           ...
                           |;uk='Тип ""%1"" вказано, як тип значень, для виду доступу ""%2"".
                           |Для виду доступу ""%3"" його не можна вказати, як тип груп значень.'");
		Иначе
			...
			ОписаниеОшибки =
				НСтр("ru='Тип групп значений ""%1"" уже указан для вида доступа ""%2"".
                           ...
                           |;uk='Тип груп значень ""%1"" вже вказаний для виду доступу ""%2"".
                           |Для виду доступу ""%3"" його не можна вказати.'");
		КонецЕсли;
	Иначе
		Если ПроверкаПересечения Тогда
			ОписаниеОшибки =
				НСтр("ru='Тип ""%1"" указан, как тип групп значений, для вида доступа ""%2"".
                           ...
                           |;uk='Тип ""%1"" вказано, як тип груп значень, для виду доступу ""%2"".
                           |Для виду доступу ""%3"" його не можна вказати, як тип значень.'");
		Иначе
			ОписаниеОшибки =
				НСтр("ru='Тип значений ""%1"" уже указан для вида доступа ""%2"".
                           |Для вида доступа ""%3"" его нельзя указать.'
                           |;uk='Тип значення ""%1"" вже вказаний для виду доступу ""%2"".
                           |Для виду доступу ""%3"" його не можна вказати.'");
		КонецЕсли;
	КонецЕсли;
		...
		Если ПроверкаТиповГрупп Тогда
			ОписаниеОшибки =
				НСтр("ru='Тип групп значений доступа ""%1"" вида доступа ""%2""
                           |не указан в определяемом типе ""Значение доступа"".'
                           |;uk='Тип груп значень доступу ""%1"" види доступу ""%2""
                           |не вказано в типі ""Значення доступу"", що визначається.'");
		Иначе
			ОписаниеОшибки =
				НСтр("ru='Тип значений доступа ""%1"" вида доступа ""%2""
                           |не указан в определяемом типе ""Значение доступа"".'
                           |;uk='Тип значень доступу ""%1"" види доступу ""%2""
                           |не вказано в типі ""Значення доступу"", що визначається.'");
		КонецЕсли;
	КонецЕсли;
