...
Процедура ОткрытьПодбор(Команда)
	...
	Иначе
		ПараметрЗаголовок = СтрЗаменить(ПараметрЗаголовок, "%Документ%", НСтр("ru='изменение ассортимента';uk='зміна асортименту'"));
	КонецЕсли;
...
Процедура УстановитьРольДляВсехТоваров(Команда)
	Если Объект.Товары.Количество() = 0 Тогда
		...
		ТекстПредупреждения = НСтр("ru='В документе не заполнена таблица %ПредставлениеТабличнойЧасти%. Роль не может быть заполнена';uk='В документі не заповнена таблиця %ПредставлениеТабличнойЧасти%. Роль не може бути заповнена'");
...
Процедура УстановитьВидЦенДляВсехТоваров(Команда)
	Если Объект.Товары.Количество() = 0 Тогда
		...
		ТекстПредупреждения = НСтр("ru='В документе не заполнена таблица %ПредставлениеТабличнойЧасти%. Вид цен не может быть заполнен';uk='В документі не заповнена таблиця %ПредставлениеТабличнойЧасти%. Вид цін не може бути заповнений'");
		ТекстПредупреждения = СтрЗаменить(ТекстПредупреждения, "%ПредставлениеТабличнойЧасти%", "Товары");
		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
		
	КонецЕсли;
	ПараметрыФормы = Новый Структура("Отбор", Новый Структура("ИспользоватьПриПродаже",Истина));
...
Процедура ЗагрузитьТоварыИзФайла(Команда)
	...
	ПараметрыЗагрузки.Заголовок = НСтр("ru='Загрузка списка товаров из файла';uk='Завантаження списку товарів з файлу'");
...
Процедура ЗаполнитьТоварыПоЗапросуНаКлиенте(ПараметрыЗаполнения)
	...
	Иначе
		ТекстВопроса = НСтр("ru='Табличная часть содержит строки.
            ...
            |;uk='Таблична частина містить рядки.
...
Функция СформироватьТекстДляКоманды(ИмяКомандыЗаполнения,РежимЗаполнения)
	...
	Иначе //такой команды не предусмотрено - надо вызвать исключение
		ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Неизвестный вариант заполнения: %1';uk='Невідомий варіант заповнення: %1'"),
...
Функция ВидЦенМожетИспользоваться(ВидЦены)
	...
	Если ЗначениеЗаполнено(ВидЦены) И НЕ ПараметрыВидаЦены.ИспользоватьПриПродаже Тогда
		ТекстСообщения = НСтр("ru='Можно назначить только виды цен, используемые при продаже.';uk='Можна призначити тільки види цін, які використовуються при продажі.'");
	...
	Если (ТипЗнч(Объект.ОбъектПланирования) = Тип("СправочникСсылка.Склады")
		...
		ИЛИ (ТипЗнч(Объект.ОбъектПланирования) <> Тип("СправочникСсылка.Склады")
			И ПараметрыВидаЦены.ЦенаВключаетНДС <> ПараметрыОбъектаПланироавния.РозничныйВидЦеныЦенаВключаетНДС)
		 Тогда
		ТекстСообщения = НСтр("ru='Метод учета НДС выбранного вида цен должен соответствовать методу учета НДС вида цен формата.';uk='Метод обліку ПДВ обраного виду цін повинен відповідати методу обліку ПДВ виду цін формату.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ТекстСообщения,
			Объект.Ссылка,
			,
...
Функция НоменклатураУжеДобавлена(ИДТекущейСтроки)
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		...
		КонецЕсли;
		Если Найдено Тогда
			ТекстСообщения = НСтр("ru='Товар ""%1"" уже содержится в строке %2';uk='Товар ""%1"" вже міститься в рядку %2'");
...
Функция ПроверитьВозможностьПодбора()
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДатаНачалаДействия) Тогда
		ТекстСообщения = НСтр("ru='Поле ""Действует с"" не заполнено';uk='Поле ""Діє з"" не заповнено'");
	...
	Если Объект.Операция = ПредопределенноеЗначение("Перечисление.ОперацииИзмененияАссортимента.ИзменениеВАссортименте") 
		...
		ТекстСообщения = НСтр("ru='Поле ""Этап"" не заполнено';uk='Поле ""Етап"" не заповнено'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Объект.Ссылка, "Объект.Этап",, ЕстьОшибки);
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.ОбъектПланирования) Тогда
		ТекстСообщения = НСтр("ru='Поле ""Формат магазина"" не заполнено';uk='Полі ""Формат магазину"" не заповнено'");
...
Процедура ЭтапПриИзмененииНаСервере()
	...
	Иначе
		Объект.Стадия = Перечисления.СтадииАссортимента.ПустаяСсылка();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Некорректный этап';uk='Некоректний етап'"),
			Объект.Ссылка,
			"Объект.Этап",
			,
			Истина);
