...
Процедура ПоОрдеруПриИзменении(Элемент)
	...
	ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени(
		"Документ.ПеремещениеТоваров.ФормаПодбораТоваровИзЗаказа.Элемент.ПоОрдеру.ПриИзменении");
...
Процедура ПоЗаказуПриИзменении(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени(
		"Документ.ПеремещениеТоваров.ФормаПодбораТоваровИзЗаказа.Элемент.ПоЗаказу.ПриИзменении");
...
Процедура ПоЗаказамПриИзменении(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени(
		"Документ.ПеремещениеТоваров.ФормаПодбораТоваровИзЗаказа.Элемент.ПоЗаказам.ПриИзменении");
...
Процедура НадписьЗаголовокЗаказыПриИзменении(Элемент)
	...
	ЗаголовокСпискаЗаказов = НСтр("ru='Заказы на перемещение (%КоличествоДокументов%)';uk='Замовлення на переміщення (%КоличествоДокументов%)'");
...
Процедура ТаблицаТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элементы.ТаблицаТовары.ТекущиеДанные <> Неопределено Тогда
		...
		Если Поле.Имя = "ТаблицаТоварыЗаказНаПеремещение"
			И ЗначениеЗаполнено(Элементы.ТаблицаТовары.ТекущиеДанные.ЗаказНаПеремещение) Тогда
			ПоказатьЗначение(Неопределено, Элементы.ТаблицаТовары.ТекущиеДанные.ЗаказНаПеремещение);
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТаблицаТоварыЗаказНаПеремещение.Имя);

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТаблицаТовары.ЗаказНаПеремещение");
...
Процедура ЗаполнитьРеквизитыПриСоздании(ТоварыНакладной)
	...
	ИспользоватьНакладныеПоНесколькимЗаказам = ПолучитьФункциональнуюОпцию("ИспользоватьПеремещениеПоНесколькимЗаказам");
	ЭтоРаспоряжениеНакладная = НакладныеСервер.ЕстьРасходныйОрдерДляЗаказовНаОтгрузку(Параметры.РеквизитыШапки.Ссылка);
	ИспользоватьЗаказы                       = ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыНаПеремещение")
...
Процедура ЗаполнитьТаблицуТоваров(ТоварыНакладной)
	...
	// Если в накладной уже были строки без заказа, нельзя подбирать из заказов, так как строки сверх заказа не поддерживаются.
	...
	РезультатЗапроса = Документы.ПеремещениеТоваров.РезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, "Подбор");
...
Процедура ОбойтиТаблицуУдалитьСтрокиБезОтклонений(Таблица)
	...
	Для Счетчик = 1 По КоличествоСтрок Цикл
		...
		РаспоряжениеНакладная = ТипЗнч(СтрокаТаблицы.ЗаказНаПеремещение) = Тип("ДокументСсылка.ПеремещениеТоваров");
		...
		Если РаспоряжениеНакладная Тогда
			СтрокаТаблицы.ЗаказНаПеремещение = Неопределено;
		КонецЕсли;
		
		Если СписокЗаказов.НайтиПоЗначению(СтрокаТаблицы.ЗаказНаПеремещение) <> Неопределено
			Или СтрокаТаблицы.ЗаказНаПеремещение = Заказ Тогда
...
Процедура НастроитьЭлементыФормыПриСоздании()
	...
	МенеджерНакладной = Документы.ПеремещениеТоваров;
	// Элементы формы.
	...
	Элементы.ТаблицаТоварыЗаказНаПеремещение.Видимость        = ИспользоватьЗаказы;
...
Функция ПроверитьВыборНесколькихЗаказов()
	...
	Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
		...
		Если СтрокаТовары.СтрокаВыбрана И СтрокаТовары.Количество <> 0 Тогда
			...
			Если ПервыйЗаказ <> Неопределено И СтрокаТовары.ЗаказНаПеремещение <> ПервыйЗаказ
				И Не ИспользоватьНакладныеПоНесколькимЗаказам Тогда
					Возврат ШаблонБолееОдногоЗаказа;
			ИначеЕсли ЗначениеЗаполнено(СтрокаТовары.ЗаказНаПеремещение) Тогда
				ПервыйЗаказ = СтрокаТовары.ЗаказНаПеремещение;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(СтрокаТовары.ЗаказНаПеремещение) Тогда
