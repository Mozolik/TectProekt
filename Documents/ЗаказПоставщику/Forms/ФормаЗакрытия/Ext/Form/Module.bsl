...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Не Готово И Модифицированность Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru='Закрытие заказов не выполнено. Закрыть форму?';uk='Закриття замовлень не виконано. Закрити форму?'"),
			СписокКнопок);
...
Процедура Далее(Команда)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаТоварыКОтмене Тогда
		...
		Если ИспользоватьПричиныОтменыЗаказовПоставщиками
			...
			ТекстОшибки = НСтр("ru='Поле ""Причина отмены"" не заполнено';uk='Поле ""Причина відміни"" не заповнено'");
...
Процедура Готово(Команда)
	...
	ИначеЕсли ГотовоСервер() Тогда
		...
		ТекстСообщения = НСтр("ru='Для заказов поставщикам (%КоличествоОбработанных%) установлен статус ""Закрыт""';uk='Для замовлень постачальникам (%КоличествоОбработанных%) встановлено статус ""Закритий""'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%КоличествоОбработанных%", ЗаказыПоставщикам.Количество());
		ТекстЗаголовка = НСтр("ru='Статус ""Закрыт"" установлен';uk='Статус ""Закритий"" встановлено'");
...
Процедура ЗаполнитьОтклоненияПоЗаказамПоставщикам()
	...
	|ГДЕ
	|	ПоступлениеТоваровУслугТовары.ЗаказПоставщику В (&ЗаказыПоставщикам)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ПоступлениеТоваровУслугТовары.Ссылка.Валюта КАК ВалютаПоступления,
	...
	|	* ВЫБОР
	|		КОГДА ТоварыЗаказа.Ссылка.Валюта <> ПоступлениеТоваровУслугТовары.Ссылка.Валюта
	|			ТОГДА ВЫБОР
	...
	|			ПО ТоварыЗаказа.КодСтроки = ПоступлениеТоваровУслугТовары.КодСтроки
	|				И ТоварыЗаказа.Ссылка = ПоступлениеТоваровУслугТовары.ЗаказПоставщику
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтКурсыВалют КАК КурсыВалютыЗаказ
	|		ПО ПоступлениеТоваровУслугТовары.ЗаказПоставщику.Ссылка.Валюта = КурсыВалютыЗаказ.Валюта
	|			И ПоступлениеТоваровУслугТовары.ЗаказПоставщику = КурсыВалютыЗаказ.Ссылка 
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтКурсыВалют КАК КурсыВалюты
	|		ПО ПоступлениеТоваровУслугТовары.Ссылка.Валюта = КурсыВалюты.Валюта
	|			И ПоступлениеТоваровУслугТовары.Ссылка = КурсыВалюты.Ссылка 
	|ГДЕ
