...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.АдресТаблицыШаблонов) Или
		...
		ТекстСообщения = НСтр("ru='Непосредственное открытие этой формы не предусмотрено. 
            ...
            |;uk='Безпосереднє відкриття цієї форми не передбачено. 
            |Для відкриття форми можна скористатися командою ""Заповнити етапи за попереднім замовленням"" у формі документа ""Замовлення постачальнику""'");
...
Процедура ТаблицаГрафиковПриАктивизацииСтроки(Элемент)
	...
	СтрокаИнформации = НСтр("ru='Используется в %1 из %2 %3, дата последнего использования %4';uk='Використовується в %1 з %2 %3, дата останнього використання %4'");
	СтрокаИнформации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		СтрокаИнформации, 
		ТекущиеДанные.ЧастотаИспользования,
		КоличествоДокументов,
		?(КоличествоДокументов%10 = 1, НСтр("ru='заказа';uk='замовлення'"),НСтр("ru='заказов';uk='замовлень'")),
