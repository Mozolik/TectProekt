...
Процедура ПолучитьДанныеДляВыгрузки()
	...
	Если ЭтаФорма.ВладелецФормы = Неопределено ИЛИ НЕ ТипЗнч(ЭтаФорма.ВладелецФормы) = Тип("УправляемаяФорма") Тогда
		...
		Исключение
			ПоказатьПредупреждение(,НСтр("ru='Внимание! Устаревшая редакция формы отчета не поддерживается текущей версией конфигурации.';uk='Увага! Застаріла редакція форми звіту не підтримується "
"поточною версією конфігурації.'"));
			Возврат;
		КонецПопытки;
...
Процедура СформироватьТекстВыгрузкиСервер_ГНСУ_XML(Параметры)
	...
	ТекстОтчета.ЗаписатьНачалоЭлемента("DECLAR");
		...
		ЗаполнитьПоляРазделаВыгрузкиXML(ТекстОтчета, "DHead", НазваниеСтраницыПанели, ИмяСхемы, Префикс, , СлужебныеДанные, ДанныеДляОтчета,  СхемаОтчета);
			...
			Если НЕ ПустаяСтрока(ТекСтр.ИмяФайла) Тогда
				...
				Если ТекСтр.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиГНСУ_XML  Тогда
					...
					Исключение
						...
						Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось записать файл ""%1""! Возможно, недостаточно места на диске или диск защищен от записи.';uk='Не вдалося записати файл ""%1""! Можливо, недостатньо місця на "
"диску або диск захищений від запису.'"), ИмяКонечногоФайла);
...
Процедура ПоказатьТекстВыгрузки(Команда)
	...
	Текст.Показать(Элементы.Выгрузки.ТекущиеДанные.ИмяСхемы + " - электронное представление");
...
Процедура ПроставитьФлажкиДекларацияПриложение()
	...
	Для Каждого НомерВИерархии ИЗ Иерархия Цикл
		...
		Для каждого Строка Из СтрокиОднойДекларации Цикл
			Строка.Декларация =    (Объект.Выгрузки.Индекс(Строка) = ПервыйИндекс) И НомерВИерархии.НомерВИерархии = 0 И Строка.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиГНСУ_XML;
			Строка.Приложение = НЕ (Объект.Выгрузки.Индекс(Строка) = ПервыйИндекс) ИЛИ НЕ (НомерВИерархии.НомерВИерархии = 0) И Строка.Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиГНСУ_XML И Строка.фВыгрузка;
		КонецЦикла;
