...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<зона доставки не указана>';uk='<зона доставки не зазначена>'"));
...
Процедура УстановитьЗаголовкиКолонокВесОбъемСервер()
	...
	ТекстВес = НСтр("ru='Вес брутто,';uk='Вага брутто,'") + " " + ЕдиницаИзмеренияВеса;
...
Функция ПроверитьВыбраныГруппировки()
	...
	Для Каждого Идентификатор Из Элементы.РаспоряженияНаДоставку.ВыделенныеСтроки Цикл
		Если РаспоряженияНаДоставку.НайтиПоИдентификатору(Идентификатор).ПолучитьРодителя() = Неопределено Тогда
			ТекстСообщения = НСтр("ru='Выберите строки с распоряжениями';uk='Виберіть рядки з розпорядженнями'");
