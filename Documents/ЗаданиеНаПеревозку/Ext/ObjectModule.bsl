...
Функция УстановитьСтатус(НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если ЭтотОбъект.Операция = Перечисления.ВидыДоставки.СоСклада
		...
		И СкладыСервер.ЕстьОрдерныйНаОтгрузкуСклад(Распоряжения.ВыгрузитьКолонку("Склад"),ДатаВремяРейсаПланС)
		...
		Если ДоставкаТоваров.ПерезаполнитьЗаданиеНаПеревозкуПоРасходнымОрдерам(ЭтотОбъект) Тогда // есть изменения после перезаполнения
			...
			Если Распоряжения.Количество() = 0 Тогда
				Текст = НСтр("ru='Статус документа %Задание не установлен, т.к. после перезаполнения по расходным ордерам в нем не остается распоряжений.';uk='Статус документа %Задание не встановлений, так як після перезаповнення за видатковими ордерами в ньому не залишається розпоряджень.'");
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ДатаВремяРейсаПланС  = Макс(ОбъектКопирования.ДатаВремяРейсаПланС, ТекущаяДата());
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Статус = Перечисления.СтатусыЗаданийНаПеревозку.КПогрузке Тогда
		МассивНепроверяемыхРеквизитов.Добавить("ДатаВремяРейсаФактС");
		МассивНепроверяемыхРеквизитов.Добавить("ДатаВремяРейсаФактПо");
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ИначеЕсли Статус = Перечисления.СтатусыЗаданийНаПеревозку.Отправлено Тогда
	...
	ИначеЕсли Статус = Перечисления.СтатусыЗаданийНаПеревозку.Закрыто Тогда
	...
	Если ЗначениеЗаполнено(ДатаВремяРейсаПланПо) Тогда
		ДатаВремяРейсаПланПо = Макс(ДатаВремяРейсаПланС, ДатаВремяРейсаПланПо);
	КонецЕсли;
	Если ЗначениеЗаполнено(ДатаВремяРейсаФактПо) Тогда
		ДатаВремяРейсаФактПо = Макс(ДатаВремяРейсаФактС, ДатаВремяРейсаФактПо);
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если Операция = Перечисления.ВидыДоставки.СоСклада
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстСообщения = НСтр("ru='Расходный ордер ""%Ссылка%"" по заданию на перевозку не готов к отгрузке.';uk='Видатковий ордер ""%Ссылка%"" за завданням на перевезення не готовий до відвантаження.'");
...
Процедура КонтрольОформленныхОрдеровПоЗаданиюНаПеревозку(Ссылка, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = НСтр("ru='Не оформлены все расходные ордера по заданию на перевозку ""%ЗаданиеНаПеревозку%"" для распоряжения ""%ДокументОтгрузки%"" на  складе ""%Склад%"". Оформите ордера и(или) перезаполните задание на перевозку по расходным ордерам.';uk='Не оформлені всі видаткові ордери за завданням на перевезення ""%ЗаданиеНаПеревозку%"" для розпорядження ""%ДокументОтгрузки%"" на складі ""%Склад%"". Оформіть ордера і(або) перезаповніть завдання на перевезення за видатковими ордерами.'");
...
Процедура ПроверитьЗаполнитьТоварыКДоставке(ЗаданиеНаПеревозку) Экспорт
	...
	|				И Склады.ДатаНачалаОрдернойСхемыПриОтгрузке < ЗаданиеНаПеревозкуРаспоряжения.Ссылка.ДатаВремяРейсаПланС
