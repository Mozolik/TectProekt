...
Процедура ЗаполнитьСписокЯчеекПоОтбору(Команда)
	Если Ячейки.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьСписокЯчеекПоОтборуЗавершение", ЭтотОбъект), НСтр("ru='Перед заполнением список ячееек будет очищен. Продолжить?';uk='Перед заповненням список комірок буде очищений. Продовжити?'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
...
Функция АдресЯчеекВХранилище(УникальныйИдентификаторФормы)
	...
	Если ТаблицаЯчеек.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='Не указано ни одной ячейки';uk='Не зазначено жодної комірки'");
