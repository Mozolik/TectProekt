...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Объект.Опрос.Пустая() Тогда
		...
		Если Не ШаблонАнкеты.Пустая() Тогда
			...
			Анкетирование.УстановитьЭлементДереваРазделовАнкетыВступлениеЗаключение(ДеревоРазделов,"Вступление",НСтр("ru='Вступление';uk='Вступ'"));
			Анкетирование.ЗаполнитьДеревоРазделов(ЭтотОбъект,ДеревоРазделов);
			Анкетирование.УстановитьЭлементДереваРазделовАнкетыВступлениеЗаключение(ДеревоРазделов,"Заключение",НСтр("ru='Заключение';uk='Висновок'"));
	...
	ЗаголовокКомандаРазделы = НСтр("ru='Показать разделы';uk='Показати розділи'");
...
Процедура ФормаЗаполненияПровестиЗакрыть(Команда)
	...
	ПоказатьВопрос(ОбработчикОповещенияОЗакрытии,
                     ...
                     |Продолжить?'
                     |;uk='Ваша анкета буде прийнята. 
                     |Подальше заповнення анкети буде неможливе
                     |Продовжити?'")
...
Функция ПроверитьЗаполнениеАнкеты()
	...
	Пока Выборка.Следующий() Цикл
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не указан ответ на вопрос';uk='Не вказано відповідь на питання'") + "- "+ СтрЗаменить(Выборка.Ссылка.ПолныйКод(),"/",".") + " " + Выборка.Формулировка);
	КонецЦикла;
...
Процедура УстановитьЗначенияРеквизитовСогласноОпросу(РеквизитыОпрос)
	...
	Вступление                            = ?(ПустаяСтрока(РеквизитыОпрос.Вступление),
	                                         НСтр("ru='Нажмите далее для заполнения анкеты.';uk='Натисніть далі для заповнення анкети.'"),
	                                         РеквизитыОпрос.Вступление);
	Заключение                            = ?(ПустаяСтрока(РеквизитыОпрос.Заключение),
	                                         НСтр("ru='Спасибо за то что заполнили анкету.';uk='Дякую за те, що заповнили анкету.'"),
...
Процедура ИзменитьВидимостьДеревоРазделов()
	...
	ЗаголовокКомандаРазделы = ?(Элементы.ГруппаДеревоРазделов.Видимость,НСтр("ru='Скрыть разделы';uk='Приховати розділи'"),НСтр("ru='Показать разделы';uk='Показати розділи'"));
	Элементы.СкрытьПоказатьДеревоРазделов.Заголовок         = ЗаголовокКомандаРазделы;
	Элементы.СкрытьПоказатьДеревоРазделовДокумент.Заголовок = ЗаголовокКомандаРазделы;
