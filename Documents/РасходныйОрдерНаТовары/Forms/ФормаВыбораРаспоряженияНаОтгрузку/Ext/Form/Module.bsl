...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	|					ТОГДА ВЫРАЗИТЬ(ДокументыОтгрузки.ДокументОтгрузки КАК Документ.ЗаказНаСборку).ТипОперации
	|				КОГДА ДокументыОтгрузки.ДокументОтгрузки ССЫЛКА Документ.СборкаТоваров
	|					ТОГДА ВЫРАЗИТЬ(ДокументыОтгрузки.ДокументОтгрузки КАК Документ.СборкаТоваров).ТипОперации
	|				ИНАЧЕ НЕОПРЕДЕЛЕНО
...
Процедура Выбрать(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ТекстПредупрежедения = НСтр("ru='Выберите распоряжение.';uk='Виберіть розпорядження.'");
