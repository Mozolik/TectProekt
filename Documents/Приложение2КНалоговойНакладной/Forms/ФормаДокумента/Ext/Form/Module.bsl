...
Процедура ЗаполнитьИзНалоговойНакладной(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.НалоговаяНакладная) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Поле ""Налоговая накладная"" не заполнено';uk='Поле ""Податкова накладна"" не заповнено'"), , "Объект.НалоговаяНакладная");
		Возврат;	
	КонецЕсли;
