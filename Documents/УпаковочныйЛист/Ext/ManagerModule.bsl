...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСоздатьНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.УпаковочныйЛист) Тогда
		...
		КомандаСоздатьНаОсновании.Идентификатор = Метаданные.Документы.УпаковочныйЛист.ПолноеИмя();
		КомандаСоздатьНаОсновании.Представление = ВводНаОсновании.ПредставлениеОбъекта(Метаданные.Документы.УпаковочныйЛист);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru='Упаковочный лист';uk='Пакувальний лист'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "УпаковочныйЛистСРасшифровкой";
	КомандаПечати.Представление = НСтр("ru='Упаковочный лист (с расшифровкой состава)';uk='Пакувальний лист (з розшифровкою складу)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	...
	КомандаПечати.Представление = НСтр("ru='Этикетка упаковочного листа';uk='Етикетка пакувального листа'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "УпаковочныйЛист") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"УпаковочныйЛист",
			НСтр("ru='Упаковочный лист';uk='Пакувальний лист'"),
			ПечатьУпаковочныйЛист(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),
			,
			"Документ.УпаковочныйЛист.ПФ_MXL_УпаковочныйЛист",
			,
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "УпаковочныйЛистСРасшифровкой") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм, 
			"УпаковочныйЛистСРасшифровкой",
			НСтр("ru='Упаковочный лист (с расшифровкой состава)';uk='Пакувальний лист (з розшифровкою складу)'"),
			ПечатьУпаковочныйЛистСРасшифровкой(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),
			,
			"Документ.УпаковочныйЛист.ПФ_MXL_УпаковочныйЛист",
			,
			Истина // ЭтоМногоязычнаяПечатнаяФорма
...
Функция ПечатьУпаковочныйЛист(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.УпаковочныйЛист.ПФ_MXL_УпаковочныйЛист", КодЯзыкаПечать);
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если НЕ ВыборкаПоТабличнымЧастям.НайтиСледующий(Новый Структура("Ссылка",ВыборкаПоДокументам.Ссылка)) Тогда
			ТекстСообщения = НСтр("ru='Невозможно напечатать ""%Документ%"", т.к. не заполнена табличная часть.';uk='Неможливо надрукувати ""%Документ%"", т. я. не заповнена таблична частина.'");
		...
		ТекстЗаголовка = НСтр("ru='Упаковочный лист';uk='Пакувальний лист'",КодЯзыкаПечать) + " " + ВыборкаПоДокументам.Код;
		Если ВыборкаПоДокументам.Вид = Перечисления.ВидыУпаковочныхЛистов.Входящий Тогда
			ТекстЗаголовка = ТекстЗаголовка  + " " + НСтр("ru='(входящий)';uk='(вхідний)'",КодЯзыкаПечать);
			ВыводитьСкладИУпаковщика = Ложь;
		Иначе
			ТекстЗаголовка = ТекстЗаголовка + " " + НСтр("ru='(исходящий)';uk='(вихідний)'",КодЯзыкаПечать);
...
Функция ПечатьУпаковочныйЛистСРасшифровкой(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если ВыборкаПоДокументам.ЕстьВложенностьУпаковочныхЛистов Тогда
			...
			РекурсивноВывестиИерархиюУпаковочныхЛистов(ТабличныйДокумент, ОбластьСтрокаТаблицыУпаковочныеЛисты, УпаковочныеЛистыДокумента, ВыборкаПоДокументам.Ссылка,, КодЯзыкаПечать);
...
Процедура РекурсивноВывестиИерархиюУпаковочныхЛистов(ТабличныйДокумент, ОбластьСтрокаТаблицыУпаковочныеЛисты, УпаковочныеЛистыДокумента, УпаковочныйЛистРодитель, Уровень = 0, КодЯзыка="ru") Экспорт
	...
	Для Каждого Строка Из НайденныеСтроки Цикл
		...
		ПредставлениеУпаковочногоЛиста = НСтр("ru='%Отступ% %УпаковочныйЛист% от %Дата%';uk='%Отступ% %УпаковочныйЛист% від %Дата%'",КодЯзыка);
		...
		РекурсивноВывестиИерархиюУпаковочныхЛистов(ТабличныйДокумент, ОбластьСтрокаТаблицыУпаковочныеЛисты, УпаковочныеЛистыДокумента, Строка.УпаковочныйЛист, Уровень, КодЯзыка);
