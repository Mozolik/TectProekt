...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ДатаАктуальности <> Дата(1, 1, 1) 
		...
		Если ДатаАктуальности < ТекущаяДатаСеанса() Тогда
			...
			ТекстСообщения= НСтр("ru='Указанная дата актуальности меньше текущей даты, сообщения никогда не будут отправлены.';uk='Зазначена дата актуальності менше поточної дати, повідомлення не буде надіслане.'");
		...
		Если ДатаАктуальности < ДатаРассылки Тогда
			...
			ТекстСообщения= НСтр("ru='Указанная дата актуальности меньше даты рассылки, сообщения никогда не будут отправлены.';uk='Зазначена дата актуальності менше дати розсилки, повідомлення не буде надіслане.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "ДатаАктуальности",,Отказ);
	...
	Для каждого ДополнительныйПолучатель Из ДополнительныеПолучатели Цикл
		...
		Если ПредназначенаДляSMS Тогда
			...
			Если СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ДополнительныйПолучатель.КонтактнаяИнформация, ";", Истина).Количество() > 1 Тогда
				...
				ТекстСообщения = НСтр("ru='В строке %1 указан более чем один номер телефона.';uk='У рядку %1 вказано більше ніж один номер телефону.'");
			...
			ТекстСообщения = НСтр("ru='Номер телефона в строке %1 должен быть указан в формате ""+xx (xxx) xxxxxxx"".';uk='Номер телефону в рядку %1 повинен бути вказаний у форматі ""+xx (xxx) xxxxxxx"".'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, НомерСтроки);
			
			СтруктураПолей = ВзаимодействияКлиентСервер.СтруктураПолейПоПредставлениюТелефона(ДополнительныйПолучатель.КонтактнаяИнформация);
		...
		Иначе
			...
			Если МассивАдресов.Количество() > 1 Тогда
				...
				ТекстСообщения = НСтр("ru='В строке %1 указан более чем один адрес электронной почты.';uk='У рядку %1 вказано більше ніж один адресу електронної пошти.'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, НомерСтроки);
			...
			Если МассивАдресов.Количество() = 1 И Не ПустаяСтрока(МассивАдресов[0].ОписаниеОшибки) Тогда
				...
				ТекстСообщения = НСтр("ru='Ошибка в строке %1. %2';uk='Помилка в рядку %1. %2'");
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.НазначениеОпросов") Тогда
		...
		Тема      = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Рассылка по опросу ""%1"" от %2.';uk='Розсилка за опитуванням ""%1"" від %2.'"),
