...
Процедура Вставить(Команда)
	...
	Если ПустаяСтрока(АдресПубликацииИнформационнойБазыВИнтернете) Тогда
		...
		ТекстСообщения = НСтр("ru='Не указан адрес публикации информационной базы в интернете.';uk='Не зазначена адреса публікації інформаційної бази в інтернеті.'");
	...
	Если ПустаяСтрока(СсылкаНаОбъект) Тогда
		...
		ТекстСообщения = НСтр("ru='Не указана внутренняя ссылка на объект.';uk='Не вказана внутрішнє посилання на об''єкт.'");
