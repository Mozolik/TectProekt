...
#Область ОбработчикиСобытийЭлементовТаблицыФормы_ИмяТабличнойЧасти
#КонецОбласти
		...
		Для каждого КлючЗначение Из СтруктураДействий Цикл
			...
			ИначеЕсли КлючЗначение.Ключ = "ОбновляетсяССервера" Тогда
				...
				Если Объект.ОбновляетсяССервера = Истина Тогда
					...
					Элементы.ЗначенияКатегорийНовостейСправочник.Заголовок = НСтр("ru='Список доступных значений (обновляется с сервера, редактирование запрещено)';uk='Список доступних значень (оновлюється з сервера, редагування заборонено)'");
				Иначе
					...
					Элементы.ЗначенияКатегорийНовостейСправочник.Заголовок = НСтр("ru='Список доступных значений';uk='Список доступних значень'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция УправлениеФормойСервер(Знач СтруктураПараметров)
	...
	Если СтруктураПараметров.ЗаполняетсяАвтоматически = Истина Тогда
		...
		Если ПустаяСтрока(Значение) Тогда
			СтруктураДействий.Вставить("ФиксированноеЗначение", НСтр("ru='Невозможно определить значение для этой категории';uk='Неможливо визначити значення для цієї категорії'"));
		...
		Иначе
			СтруктураДействий.Вставить(
				...
				СтрШаблон(
					НСтр("ru='Для этой конфигурации значение этой категории всегда равно %1';uk='Для цієї конфігурації значення цієї категорії завжди дорівнює %1'"),
					Значение
				)
			);
		КонецЕсли;
...
Процедура ОбновитьИнформационныеСтроки()
	...
	Иначе
		Если Объект.ОбновляетсяССервера Тогда
			...
			Если Запись.Выбран() Тогда
				Если Запись.ТекущаяВерсияНаКлиенте >= Запись.ТекущаяВерсияНаСервере Тогда
					ТекстНадписи = НСтр("ru='Данные актуальны и соответствуют данным с сервера от %ТекущаяВерсияНаСервере%.';uk='Дані актуальні та відповідають даним з сервера від %ТекущаяВерсияНаСервере%.'");
					ТекстНадписи = СтрЗаменить(ТекстНадписи, "%ТекущаяВерсияНаСервере%", Формат(Запись.ТекущаяВерсияНаСервере, "ДФ='dd.MM.yyyy HH:mm:ss'"));
					ТребуетсяОбновление = Ложь;
				Иначе // Устарели
					Если Запись.ТекущаяВерсияНаКлиенте = '00010101' Тогда
						ТекстНадписи = НСтр("ru='Данные никогда не обновлялись с сервера,
                            ...
                            |;uk='Дані ніколи не оновлювалися з сервера,
                            |а на сервері вже версія від %ТекущаяВерсияНаСервере%.'");
					Иначе
						ТекстНадписи = НСтр("ru='Последний раз данные обновлялись с сервера %ТекущаяВерсияНаКлиенте%,
                            |а на сервере уже версия от %ТекущаяВерсияНаСервере%.'
                            |;uk='Останній раз дані оновлювалися з сервера %ТекущаяВерсияНаКлиенте%,
                            |а на сервері вже версія від %ТекущаяВерсияНаСервере%.'");
					КонецЕсли;
					ТекстНадписи = СтрЗаменить(ТекстНадписи, "%ТекущаяВерсияНаКлиенте%", Формат(Запись.ТекущаяВерсияНаКлиенте, "ДФ='dd.MM.yyyy HH:mm:ss'"));
					ТекстНадписи = СтрЗаменить(ТекстНадписи, "%ТекущаяВерсияНаСервере%", Формат(Запись.ТекущаяВерсияНаСервере, "ДФ='dd.MM.yyyy HH:mm:ss'"));
					ТребуетсяОбновление = Истина;
				КонецЕсли;
			Иначе
				ТекстНадписи = НСтр("ru='Данные никогда не обновлялись с сервера.';uk='Дані ніколи не оновлювалися з сервера.'");
				ТребуетсяОбновление = Истина;
			КонецЕсли;
		...
		Иначе
			Элементы.ДекорацияТребуетсяОбновлениеССервера.Заголовок  = НСтр("ru='Данные вводятся вручную и обновление с сервера не требуется.';uk='Дані вводяться вручну і оновлення з сервера не потрібно.'");
			Элементы.ДекорацияТребуетсяОбновлениеССервера.ЦветТекста = ЦветаСтиля.ЦветМикротекста;
		КонецЕсли;
